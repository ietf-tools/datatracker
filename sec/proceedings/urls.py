...
urlpatterns = patterns('sec.proceedings.views',
    ...
    url(r'^(?P<meeting_id>\d{1,6})/$', 'select', name='proceedings_select'),
    url(r'^(?P<meeting_id>\d{1,6})/(?P<acronym>[A-Za-z0-9_\-\+]+)/$',
        'upload_unified', name='proceedings_upload_unified'),
    url(r'^(?P<meeting_id>\d{1,6})/(?P<acronym>[A-Za-z0-9_\-\+]+)/(?P<name>[A-Za-z0-9._\-\+]+)/delete/$',
    ...
    url(r'^(?P<meeting_id>\d{1,6})/(?P<acronym>[A-Za-z0-9_\-\+]+)/(?P<slide_id>[A-Za-z0-9._\-\+]+)/$',
        'edit_slide', name='proceedings_edit_slide'),
    url(r'^(?P<meeting_id>\d{1,6})/(?P<acronym>[A-Za-z0-9_\-\+]+)/(?P<slide_id>[A-Za-z0-9._\-\+]+)/replace/$',
        'replace_slide', name='proceedings_replace_slide'),
    url(r'^(?P<meeting_id>\d{1,6})/(?P<acronym>[A-Za-z0-9_\-\+]+)/(?P<slide_id>[A-Za-z0-9._\-\+]+)/(?P<direction>(up|down))/$',
