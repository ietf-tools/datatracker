...
class BaseSubmitTestCase(TestCase):
    ...
    def post_to_upload_submission(self, *args, **kwargs):
        ...
        # ever being made.
        with mock.patch(
            "ietf.submit.views.process_uploaded_submission_task"
        ) as mocked_task:
            with mock.patch(
                "ietf.submit.views.transaction.on_commit", side_effect=lambda x: x()
            ):
...
class ManualSubmissionTests(TestCase):
    def test_manualpost_view(self):
        ...
        self.assertIn(submission.name, q("#manual.submissions td a").text())
...
class SubmitTests(BaseSubmitTestCase):
    ...
    def create_and_post_submission(
        self,
        name,
        rev,
        author,
        ...
        for format in formats:
            ...
            files[format], __ = submission_file(
                f"{name}-{rev}",
                f"{name}-{rev}.{format}",
                group,
                fn,
        ...
        if process_submission:
            ...
            for format in formats:
                self.assertTrue(
                    os.path.exists(
                        os.path.join(self.staging_dir, "%s-%s.%s" % (name, rev, format))
                    )
                )
                if format == "xml":
                    self.assertTrue(
                        os.path.exists(
                            os.path.join(
                                self.staging_dir, "%s-%s.%s" % (name, rev, "html")
                            )
                        )
                    )
        return r
    ...
    def supply_extra_metadata(
        ...
        action = (
            post_button.parents("form").find('input[type=hidden][name="action"]').val()
        )
        ...
        post_data = {
            ...
            "replaces": replaces,
            "resources": (
                "\n".join(r.to_form_entry_str() for r in extresources)
                if extresources
                else ""
            ),
        }
        ...
        if r.status_code == 302:
            ...
            self.assertEqual(
                ...
                email.utils.formataddr((submitter_name, submitter_email)),
            )
            self.assertEqual(
                [] if submission.replaces == "" else submission.replaces.split(","),
    ...
    def verify_bibxml_ids_creation(self, draft):
        ...
        ref_rev_file_name = os.path.join(
            os.path.join(settings.BIBXML_BASE_PATH, "bibxml-ids"),
            "reference.I-D.%s-%s.xml" % (draft.name, draft.rev),
        )
        self.assertTrue(os.path.exists(ref_rev_file_name))
    ...
    def submit_new_wg(self, formats):
        ...
        sug_replaced_draft = Document.objects.create(
            ...
            + datetime.timedelta(days=settings.INTERNET_DRAFT_DAYS_TO_EXPIRE),
            notify="aliens@example.mars",
        )
        sug_replaced_draft.set_state(
        ...
        r = self.supply_extra_metadata(
            name,
            status_url,
            author.ascii,
            author.email().address.lower(),
        ...
        self.assertTrue(
            "New Internet-Draft waiting for approval" in outbox[-1]["Subject"]
        )
        ...
        action = (
            approve_button.parents("form")
            .find('input[type=hidden][name="action"]')
            .val()
        )
        ...
        self.assertTrue(
            not os.path.exists(
                os.path.join(self.staging_dir, "%s-%s.txt" % (name, rev))
            )
        )
        self.assertTrue(
            os.path.exists(os.path.join(self.repository_dir, "%s-%s.txt" % (name, rev)))
        )
        ...
        self.assertTrue(
            draft.expires
            >= timezone.now()
            + datetime.timedelta(days=settings.INTERNET_DRAFT_DAYS_TO_EXPIRE - 1)
        )
        self.assertEqual(
            draft.get_state("draft-stream-%s" % draft.stream_id).slug, "wg-doc"
        )
        authors = draft.documentauthor_set.all()
        self.assertEqual(len(authors), 1)
        self.assertEqual(authors[0].person, author)
        self.assertEqual(
            set(draft.formal_languages.all()),
            set(FormalLanguageName.objects.filter(slug="json")),
        )
        ...
        self.assertTrue(
            draft.relations_that_doc("possibly-replaces").first().target,
            sug_replaced_draft,
        )
        ...
        self.assertEqual(
            draft.submission().xml_version, "3" if "xml" in formats else None
        )
    ...
    def test_submit_new_wg_as_author(self):
        ...
        r = self.supply_extra_metadata(
            name, status_url, author.ascii, author.email().address.lower(), replaces=[]
        )
        self.assertEqual(r.status_code, 302)
        status_url = r["Location"]
        ...
        self.assertEqual(r.status_code, 200)
        self.assertContains(
            r, "The submission is pending approval by the group chairs."
        )
    ...
    def test_submit_new_wg_as_author_bad_submitter(self):
        ...
        r = self.supply_extra_metadata(
            name,
            status_url,
            "=?utf-8?q?Peter_Christen_Asbj=C3=B8rnsen?=",
            author.email().address.lower(),
            replaces=[],
        )
        self.assertEqual(r.status_code, 200)
    ...
    def submit_new_concluded_wg_as_author(self, group_state_id="conclude"):
        ...
        self.assertEqual(
            len(post_button), 0
        )  # no UI option to post the submission in this state
    ...
    def submit_existing(
        self, formats, change_authors=True, group_type="wg", stream_type="ietf"
    ):
        ...
        if stream_type == "ietf":
            ...
            draft = DocumentFactory(
                ...
                ad=ad,
                authors=PersonFactory.create_batch(1),
            )
            ...
            not_ok_state = State.objects.get(
                used=True, type="draft-iana-review", slug="not-ok"
            )
            draft.set_state(not_ok_state)
            update_action_holders(
                draft,
                prev_state=State.objects.get(
                    used=True, type="draft-iana-review", slug="changed"
                ),
            ...
            ballot_position.balloter = ballot_position.by = Person.objects.get(
                user__username="ad2"
            )
            ...
            group = GroupFactory(
                type_id="rg", parent=Group.objects.get(acronym="irtf"), acronym="mars"
            )
            draft = DocumentFactory(
                type_id="draft",
                group=group,
            ...
            draft = IndividualDraftFactory(
                stream_id=stream_type, authors=PersonFactory.create_batch(1)
            )
        ...
        if change_authors:
            ...
            DocumentAuthor.objects.create(
                document=draft,
                person=nomail_author,
                email=email,
                order=draft.documentauthor_set.latest("order").order + 1,
            )
        ...
        old_rev = draft.rev
        with io.open(
            os.path.join(self.repository_dir, "%s-%s.txt" % (name, old_rev)), "w"
        ) as f:
            f.write("a" * 2000)

        old_docevents = list(draft.docevent_set.all())
        _assert_authors_are_action_holders(
            draft, True
        )  # authors should be action holders prior to the test

        status_url, author = self.do_submission(
            name, rev, group, formats, author=prev_author.person
        )

        _assert_authors_are_action_holders(
            draft, True
        )  # still waiting for author confirmation

        # supply submitter info, then previous authors get a confirmation email
        mailbox_before = len(outbox)
        r = self.supply_extra_metadata(
            name, status_url, "Submitter Name", "submitter@example.com", replaces=[]
        )
        self.assertEqual(r.status_code, 302)
        status_url = r["Location"]
        r = self.client.get(status_url)
        ...
        new_docevents = draft.docevent_set.exclude(
            pk__in=[event.pk for event in old_docevents]
        )
        _assert_authors_are_action_holders(
            draft, False
        )  # confirmed and posted, authors no longer hold action
        ...
        for event_delta, (event_type, be_in_desc, by_name) in enumerate(
            expected_docevents[::-1]
        ):
            inspect_docevents(docevents, event_delta, event_type, be_in_desc, by_name)

        self.assertTrue(
            not os.path.exists(
                os.path.join(self.repository_dir, "%s-%s.txt" % (name, old_rev))
            )
        )
        self.assertTrue(
            os.path.exists(
                os.path.join(self.archive_dir, "%s-%s.txt" % (name, old_rev))
            )
        )
        self.assertTrue(
            not os.path.exists(
        ...
        if stream_type == "ietf":
            self.assertEqual(draft.stream_id, "ietf")
            self.assertEqual(
                draft.get_state_slug("draft-stream-%s" % draft.stream_id), "wg-doc"
            )
        ...
        self.assertTrue(
            ("I-D Action: %s" % name) in draft.message_set.order_by("-time")[0].subject
        )
        ...
        if draft.stream_id == "ietf":
            self.assertTrue(
                draft.ad.role_email("ad").address in force_str(outbox[-2].as_string())
            )
            self.assertTrue(
                ballot_position.balloter.role_email("ad").address
                in force_str(outbox[-2].as_string())
            )
    ...
    def do_submit_existing_concluded_wg_test(
        self, group_state_id="conclude", submit_as_author=False
    ):
        ...
        RoleFactory(name_id="ad", group=area, person=ad)
        group = GroupFactory(
            type_id="wg", state_id=group_state_id, parent=area, acronym="mars"
        )
        author = PersonFactory()
        ...
        self.assertEqual(r.status_code, 200)
        self.assertContains(
            r, "The submission is pending approval by the area director."
        )
    ...
    def test_submit_existing_bofconc_wg_as_author(self):
        self.do_submit_existing_concluded_wg_test(
            group_state_id="bof-conc", submit_as_author=True
        )
    ...
    def test_submit_existing_replaced_wg_as_author(self):
        self.do_submit_existing_concluded_wg_test(
            group_state_id="replaced", submit_as_author=True
        )
    ...
    def submit_new_individual(self, formats):
        ...
        )  # Note that this doesn't check that it has the right _content_
        ftp_path = Path(settings.FTP_DIR) / "internet-drafts" / repository_path.name
        self.assertTrue(repository_path.samefile(ftp_path))
        all_archive_path = (
            Path(settings.INTERNET_ALL_DRAFTS_ARCHIVE_DIR) / repository_path.name
        )
        ...
        for ext in settings.IDSUBMIT_FILE_TYPES:
            ...
            if variant_path.exists():
                variant_ftp_path = (
                    Path(settings.FTP_DIR) / "internet-drafts" / variant_path.name
                )
                self.assertTrue(variant_path.samefile(variant_ftp_path))
                variant_all_archive_path = (
                    Path(settings.INTERNET_ALL_DRAFTS_ARCHIVE_DIR) / variant_path.name
                )
    ...
    def submit_new_draft_no_org_or_address(self, formats):
        ...
        r = self.create_and_post_submission(
            ...
            author,
            formats=formats,
            base_filename="test_submission_no_org_or_address",
        )
        status_url = r["Location"]
        r = self.supply_extra_metadata(
            name, status_url, "Submitter name", "submitter@example.com", replaces=[]
        ...
        action = (
            force_post_button.parents("form")
            .find('input[type=hidden][name="action"]')
            .val()
        )
        r = self.client.post(status_url, dict(action=action))
    ...
    def _assert_extresources_in_table(self, response, extresources, th_label=None):
        ...
        if extresources:
            for res in extresources:
                ...
                self.assertIn(
                    res.value, td_html, "Value of resource %s not found" % (res)
                )
        else:
    ...
    def _assert_extresource_change_event(self, doc, is_present=True):
        ...
        event = doc.latest_event(
            ...
            desc__contains="Changed document external resources",
        )
        if is_present:
            self.assertIsNotNone(
                event, "External resource change event was not created properly"
            )
        else:
            self.assertIsNone(
                event, "External resource change event was unexpectedly created"
            )
    ...
    def submit_new_individual_logged_in(self, formats):
        ...
        status_url, author = self.do_submission(
            name, rev, group, formats, author=author
        )

        # supply submitter info, then draft should be be ready for email auth
        mailbox_before = len(outbox)
        r = self.supply_extra_metadata(
            name, status_url, author.name, username, replaces=[]
        )
    ...
    def test_submit_new_logged_in_with_extresources(self):
        ...
        r = self.supply_extra_metadata(
            name, status_url, author.name, username, replaces=[], extresources=resources
        )
        self.assertEqual(r.status_code, 302)
        status_url = r["Location"]
    ...
    def test_submit_update_individual(self):
        IndividualDraftFactory(
            name="draft-ietf-random-thing",
            states=[("draft", "active"), ("draft-iesg", "approved")],
            pages=5,
        )
        ad = Person.objects.get(user__username="ad")
        ...
        replaces_count = draft.relateddocument_set.filter(
        ...
        r = self.supply_extra_metadata(
            name, status_url, "Submitter Name", "author@example.com", replaces=[]
        )
        self.assertEqual(r.status_code, 302)
        status_url = r["Location"]
        r = self.client.get(status_url)
        ...
        self.assertEqual(draft.rev, rev)
        self.assertEqual(
            draft.relateddocument_set.filter(relationship_id="replaces").count(),
            replaces_count,
        )
        self.assertEqual(draft.docextresource_set.count(), 0)
        #
    ...
    def submit_existing_with_extresources(self, group_type, stream_type="ietf"):
        ...
        orig_draft.docextresource_set.create(
            name_id="faq", value="https://faq.example.com/"
        )
        orig_draft.docextresource_set.create(
            name_id="wiki", value="https://wiki.example.com", display_name="Test Wiki"
        )
        orig_extresources = list(orig_draft.docextresource_set.all())

        status_url, _ = self.do_submission(
            name=name, rev=new_rev, author=author, group=group
        )
        ...
        self.assertEqual(r.status_code, 200)
        self._assert_extresources_in_table(r, [])
        self._assert_extresources_in_table(
            r, orig_extresources, "Current document additional resources"
        )
        self._assert_extresources_form_not_present(r)
    ...
    def submit_new_individual_replacing_wg(
        self, logged_in=False, group_state_id="active", notify_ad=False
    ):
        ...
        self.assertIn(
            (
                ad.user.email
                if notify_ad
                else "%s-chairs@ietf.org" % replaced_draft.group.acronym
            ),
            notice["To"],
        )
    ...
    def test_submit_new_individual_replacing_concluded_wg(self):
        self.submit_new_individual_replacing_wg(
            group_state_id="conclude", notify_ad=True
        )

    def test_submit_new_individual_replacing_concluded_wg_logged_in(self):
        self.submit_new_individual_replacing_wg(
            group_state_id="conclude", notify_ad=True, logged_in=True
        )
    ...
    def test_submit_new_wg_with_dash(self):
        group = Group.objects.create(
            acronym="mars-special", name="Mars Special", type_id="wg", state_id="active"
        )
    ...
    def test_submit_new_wg_v2_country_only(self):
        ...
        submission = Submission.objects.get(name=name)
        self.assertEqual(
            submission.xml_version, "2"
        )  # should reflect the submitted version

    def test_submit_new_irtf(self):
        group = Group.objects.create(
            acronym="saturnrg", name="Saturn", type_id="rg", state_id="active"
        )
    ...
    def test_edit_submission_and_force_post(self):
        ...
        self.assertEqual(
            submission.submitter, "Some Random Test Person <random@example.com>"
        )
    ...
    def test_search_for_submission_and_edit_as_secretariat(self):
        ...
        # search
        r = self.client.post(
            urlreverse("ietf.submit.views.search_submission"), dict(name=name)
        )
        self.assertEqual(r.status_code, 302)
        ...
        # search with rev
        r = self.client.post(
            urlreverse("ietf.submit.views.search_submission"),
            dict(name=name + "-" + rev),
        )
        self.assertEqual(r.status_code, 302)
        unprivileged_status_url = r["Location"]
    ...
    def test_submit_all_file_types(self):
        ...
        self.assertTrue(
            os.path.exists(os.path.join(self.staging_dir, "%s-%s.txt" % (name, rev)))
        )
        ...
        self.assertTrue(
            os.path.exists(os.path.join(self.staging_dir, "%s-%s.xml" % (name, rev)))
        )
        self.assertTrue(exists_in_storage("staging", f"{name}-{rev}.txt"))
    ...
    def test_blackout_access(self):
        ...
        meeting.importantdate_set.create(
            name_id="idcutoff", date=date_today() - datetime.timedelta(days=2)
        )
    ...
    def test_no_blackout_at_all(self):
        ...
        meeting.importantdate_set.create(
            name_id="idcutoff", date=date_today() + datetime.timedelta(days=7)
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
        ...
        meeting.importantdate_set.filter(name_id="idcutoff").delete()
        meeting.importantdate_set.create(
            name_id="idcutoff", date=date_today() - datetime.timedelta(days=1)
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
    ...
    def submit_bad_file(self, name, formats):
        ...
        files = {}
        for format in formats:
            files[format], author = submission_file(
                f"{name}-{rev}", f"{name}-{rev}.bad", group, "test_submission.bad"
            )
    ...
    def submit_bad_doc_name_with_ext(self, name_in_doc, name_in_post, formats):
        ...
        for format in formats:
            files[format], author = submission_file(
                name_in_doc, name_in_post, group, "test_submission.%s" % format
            )
    ...
    def test_submit_bad_doc_name(self):
        r = self.submit_bad_doc_name_with_ext(
            name_in_doc="draft-foo.dot-bar",
            name_in_post="draft-foo.dot-bar",
            formats=["txt"],
        )
        ...
        r = self.submit_bad_doc_name_with_ext(
            name_in_doc="draft-foo-bar-00.xml",
            name_in_post="draft-foo-bar-00.xml",
            formats=["xml"],
        )
        self.assertContains(
            r, "Could not extract a valid Internet-Draft revision from the XML"
        )
        r = self.submit_bad_doc_name_with_ext(
            name_in_doc="../malicious-name-in-content-00",
            name_in_post="../malicious-name-in-post-00.xml",
            formats=["xml"],
        )
        self.assertContains(
            r, "Did you include a filename extension in the name by mistake?"
        )
    ...
    def test_submit_nonascii_name(self):
        ...
        status_url, _ = self.do_submission(
            ...
            rev=rev,
            author=author,
            base_filename="test_submission.nonascii",
            ascii=False,
        )
        r = self.client.get(status_url)
        q = PyQuery(r.content)
    ...
    def test_submit_bad_author_email(self):
        ...
        status_url, _ = self.do_submission(
            name=name, rev=rev, author=author, formats=("xml",)
        )
        r = self.client.get(status_url)
        q = PyQuery(r.content)
        m = q("p.text-danger").text()
    ...
    def test_submit_invalid_yang(self):
        ...
        status_url, _ = self.do_submission(
            name=name, rev=rev, base_filename="test_submission_invalid_yang"
        )
        r = self.client.get(status_url)
        q = PyQuery(r.content)
        #
        ...
        for command in ["xym", "pyang", "yanglint"]:
            ...
            if command != "yanglint" or (
                settings.SUBMIT_YANGLINT_COMMAND
                and os.path.exists(settings.YANGLINT_BINARY)
            ):
        ...
        if settings.SUBMIT_YANGLINT_COMMAND and os.path.exists(
            ...
            self.assertIn(
                'libyang err : Parsing module "ietf-yang-metadata" failed.', m
            )

    def submit_conflicting_submissiondocevent_rev(
        self, new_rev="01", existing_rev="01"
    ):
        ...
        sub = Submission.objects.exclude(pk=existing_sub.pk).get(
            name=draft.name, rev=new_rev
        )
        ...
        expected_error = "Rev %s conflicts with existing submission (%s)" % (
            new_rev,
            existing_rev,
        )
    ...
    def do_wg_approval_auth_test(self, state, chair_can_approve=False):
        ...
        class _SubmissionFactory:
            ...
            def next(self):
                self.index += 1
                sub = Submission.objects.create(
                    name="draft-ietf-mars-bar-%d" % self.index,
                ...
                status_url = urlreverse(
                    "ietf.submit.views.submission_status",
                    kwargs=dict(submission_id=sub.pk, access_token=sub.access_token()),
                )
        ...
        def _assert_approval_refused(username, submission_factory, user_description):
            ...
            self.assertEqual(
                r.status_code,
                403,
                "%s should not be able to approve" % user_description.capitalize(),
            )
            submission = Submission.objects.get(pk=submission.pk)  # refresh from DB
            self.assertEqual(
                submission.state_id,
                state,
                "Submission should still be awaiting approval after %s approval attempt fails"
                % user_description,
            )
        ...
        def _assert_approval_allowed(username, submission_factory, user_description):
            ...
            self.assertEqual(
                r.status_code,
                302,
                "%s should be able to approve" % user_description.capitalize(),
            )
            submission = Submission.objects.get(pk=submission.pk)  # refresh from DB
            self.assertEqual(
                submission.state_id,
        ...
        mars = GroupFactory(
            type_id="wg", acronym="mars", parent=area
        )  # WG for submission
    ...
    def do_approval_with_extresources_test(self, submission, url, action, permitted):
        ...
        mailbox_before = len(outbox)
        with mock.patch(
            "ietf.submit.utils.can_edit_docextresources",
            return_value=permitted,
        ) as mocked_permission_check:
            r = self.client.post(url, dict(action=action))
        self.assertEqual(r.status_code, 302)
        ...
        if permitted:
            ...
            suggestion_email = [m for m in new_mail if m["Subject"] == subject]
    ...
    def test_can_edit_docextresources(self):
        ...
        with mock.patch(
            "ietf.doc.utils.is_authorized_in_doc_stream", return_value=True
        ):
...
class ApprovalsTestCase(BaseSubmitTestCase):
    def test_approvals(self):
        ...
        Preapproval.objects.create(
            name="draft-ietf-mars-foo",
            by=Person.objects.get(user__username="marschairman"),
        )
        Preapproval.objects.create(
            name="draft-ietf-mars-baz",
            by=Person.objects.get(user__username="marschairman"),
        )
        ...
        self.assertEqual(
            len(q('.approvals a:contains("draft-ietf-mars-quux")')), 0
        )  # wg chair does not see ad-appr
        ...
        self.assertEqual(
            len(q('.recently-approved a:contains("draft-ietf-mars-baz")')), 0
        )
        self.assertEqual(
            len(q('.recently-approved a:contains("draft-ietf-mars-bar")')), 0
        )
        self.assertEqual(
            len(q('.recently-approved a:contains("draft-ietf-mars-quux")')), 0
        )
        ...
        self.assertEqual(len(q('.approvals a:contains("draft-ietf-mars-foo")')), 0)
        self.assertEqual(
            len(q('.approvals a:contains("draft-ietf-mars-bar")')), 1
        )  # AD sees grp-appr in their area
        self.assertEqual(
            len(q('.approvals a:contains("draft-ietf-mars-quux")')), 1
        )  # AD does see ad-appr
        self.assertEqual(len(q('.preapprovals td:contains("draft-ietf-mars-foo")')), 0)
        self.assertEqual(len(q('.preapprovals td:contains("draft-ietf-mars-baz")')), 1)
        self.assertEqual(len(q('.preapprovals td:contains("draft-ietf-mars-bar")')), 0)
    ...
    def test_cancel_preapproval(self):
        ...
        url = urlreverse(
            ...
            kwargs=dict(preapproval_id=preapproval.pk),
        )
        login_testing_unauthorized(self, "marschairman", url)
...
class ApiSubmissionTests(BaseSubmitTestCase):
    ...
    def test_upload_draft(self):
        ...
        self.assertIn(
            "Uploaded submission through API",
            submission.submissionevent_set.last().desc,
        )
    ...
    def test_rejects_broken_upload(self):
        ...
        xml, _ = submission_file(
            "", "draft-somebody-test-00.xml", None, "test_submission.xml", author=author
        )
        data = {
            "xml": xml,
            "user": author.user.username,
        ...
        SubmissionFactory(
            name="draft-somebody-test", rev="00"
        )  # create an already-in-progress submission
    ...
    def test_submission_status(self):
        ...
        url = urlreverse(
            "ietf.submit.views.api_submission_status", kwargs={"submission_id": s.pk}
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        self.assertEqual(
    ...
    def test_upload_blackout(self):
        ...
        with mock.patch("ietf.submit.views.process_uploaded_submission_task"):
            r = self.client.post(url, data)
        self.assertContains(
            r, "The last submission time for the I-D submission was", status_code=400
        )
...
class SubmissionUploadFormTests(BaseSubmitTestCase):
    ...
    def test_replaces_field(self):
        ...
        xml, auth = submission_file(
            "draft-somebody-test-01",
            "draft-somebody-test-01.xml",
            None,
            "test_submission.xml",
        )
        files_dict = {
        ...
        form = SubmissionAutoUploadForm(
            ...
            data={
                ...
                "replaces": f"{existing_drafts[0].name},{existing_drafts[1].name}",
            },
            files=files_dict,
        )
        self.assertTrue(form.is_valid())
        self.assertEqual(
            form.cleaned_data["replaces"],
            f"{existing_drafts[0].name},{existing_drafts[1].name}",
        )
        ...
        self.assertIn(
            "An Internet-Draft cannot replace itself", form.errors["replaces"]
        )
        ...
        self.assertFalse(form.is_valid())
        self.assertIn(
            "An Internet-Draft can only replace another Internet-Draft",
        ...
        self.assertFalse(form.is_valid())
        self.assertIn(
            "An Internet-Draft cannot replace another Internet-Draft that has become an RFC",
            form.errors["replaces"],
        )

        # can't replace draft approved by iesg
        existing_drafts[0].set_state(
            State.objects.get(type="draft-iesg", slug="approved")
        )
        form = SubmissionAutoUploadForm(
            request_factory.get("/some/url"),
            data={"user": auth.user.username, "replaces": existing_drafts[0].name},
        ...
        self.assertFalse(form.is_valid())
        self.assertIn(
            f"{existing_drafts[0].name} is approved by the IESG and cannot be replaced",
            form.errors["replaces"],
        )
    ...
    def test_invalid_xml(self):
        ...
        form = SubmissionBaseUploadForm(
            RequestFactory().post("/some/url"), files={"xml": not_xml}
        )
        self.assertFalse(form.is_valid())
...
class AsyncSubmissionTests(BaseSubmitTestCase):
    ...
    def test_process_and_accept_uploaded_submission(self):
        ...
        for ext in ["txt", "html"]:
            self.assertFalse(
                exists_in_storage("staging", f"draft-somebody-test-00.{ext}")
            )
        process_and_accept_uploaded_submission(submission)

        submission = Submission.objects.get(pk=submission.pk)  # refresh
        self.assertEqual(
            submission.state_id, "auth", "accepted submission should be in auth state"
        )
        ...
        self.assertEqual(
            submission.abstract.strip(), "This document describes how to test tests."
        )
    ...
    def test_process_and_accept_uploaded_submission_invalid(self):
        ...
        self.assertIn(
            "not one of the document authors",
            submission.submissionevent_set.last().desc,
        )
        ...
        store_str(
            "staging",
            "draft-somebody-test-00.xml",
            re.sub(r"<email>.*</email>", "", xml_data),
        )
        process_and_accept_uploaded_submission(submission)
        submission = Submission.objects.get(pk=submission.pk)  # refresh
        self.assertEqual(submission.state_id, "cancel")
        self.assertIn(
            "Email address not found for all authors",
            submission.submissionevent_set.last().desc,
        )
        ...
        store_str(
            "staging",
            "draft-somebody-test-00.xml",
            re.sub(r"<title>.*</title>", "<title></title>", xml_data),
        )
        process_and_accept_uploaded_submission(submission)
        submission = Submission.objects.get(pk=submission.pk)  # refresh
        self.assertEqual(submission.state_id, "cancel")
        self.assertIn(
            "Could not extract a valid title",
            submission.submissionevent_set.last().desc,
        )
        ...
        self.assertEqual(submission.state_id, "cancel")
        self.assertIn(
            "Submission rejected: XML Internet-Draft filename",
            submission.submissionevent_set.last().desc,
        )
        ...
        self.assertEqual(submission.state_id, "cancel")
        self.assertIn(
            "Submission rejected: XML Internet-Draft revision",
            submission.submissionevent_set.last().desc,
        )
        ...
        self.assertEqual(submission.state_id, "cancel")
        self.assertIn(
            "Only XML Internet-Draft submissions",
            submission.submissionevent_set.last().desc,
        )
        ...
        with xml_path.open(
            "w"
        ) as f:  # Why is this state being written if the thing that uses it is mocked out?
            f.write(xml_data)
        store_str("staging", "draft-somebody-test-00.xml", xml_data)
        ...
        submission = Submission.objects.get(pk=submission.pk)  # refresh
        self.assertFalse(
            mock_proc_xml.called,
            'Should not process submission not in "validating" state',
        )
    ...
    @mock.patch("ietf.submit.tasks.process_and_accept_uploaded_submission")
    def test_process_and_accept_uploaded_submission_task_ignores_invalid_id(
        self, mock_method
    ):
    ...
    def test_process_submission_xml(self):
        ...
        self.assertEqual(
            len(output["authors"]), 1
        )  # not checking in detail, parsing is unreliable
        ...
        TestBlobstoreManager().emptyTestBlobstores()
        xml_path.write_text(
            re.sub(r"<date year=.+ month", "<date month", xml_contents)
        )  # remove year
        store_str(
            "staging",
            "draft-somebody-test-00.xml",
        ...
        TestBlobstoreManager().emptyTestBlobstores()
        xml_path.write_text(
            re.sub(r"(<date.+) month=.+day=(.+>)", r"\1 day=\2", xml_contents)
        )  # remove month
        store_str(
            "staging",
            "draft-somebody-test-00.xml",
        ...
        TestBlobstoreManager().emptyTestBlobstores()
        xml_path.write_text(
            re.sub(r"<date(.+) day=.+>", r"<date\1>", xml_contents)
        )  # remove day
        store_str(
            "staging",
            "draft-somebody-test-00.xml",
        ...
        with self.assertRaisesMessage(
            SubmissionError, "disagrees with submission filename"
        ):
        ...
        store_str("staging", "draft-somebody-test-00.xml", xml.read())
        with self.assertRaisesMessage(
            SubmissionError, "disagrees with submission revision"
        ):
            process_submission_xml("draft-somebody-test", "00")
        ...
        store_str("staging", "draft-somebody-test-00.xml", xml.read())
        with self.assertRaisesMessage(
            SubmissionError, "Could not extract a valid title"
        ):
            process_submission_xml("draft-somebody-test", "00")
    ...
    def test_process_submission_text(self):
        ...
        self.assertEqual(output["title"], "Correct Draft Title")
        self.assertEqual(
            output["abstract"].strip(), "This document describes how to test tests."
        )
        self.assertEqual(
            len(output["authors"]), 1
        )  # not checking in detail, parsing is unreliable
        self.assertLessEqual(
            output["document_date"] - date_today(), datetime.timedelta(days=1)
        )
        self.assertEqual(output["pages"], 2)
        self.assertGreater(output["words"], 0)  # make sure it got something
        self.assertGreater(
            len(output["first_two_pages"]), 0
    ...
    def test_process_and_validate_submission(self):
        ...
        with mock.patch(
            "ietf.submit.utils.process_submission_xml", return_value=xml_data
        ):
            with mock.patch(
                "ietf.submit.utils.process_submission_text", return_value=text_data
            ):
                with mock.patch(
                    "ietf.submit.utils.render_missing_formats"
                ) as mock_render:
                    with mock.patch(
                        "ietf.submit.utils.apply_checkers"
                    ) as mock_checkers:
    ...
    def test_status_of_validating_submission(self):
        s = SubmissionFactory(state_id="validating")
        url = urlreverse(
            "ietf.submit.views.submission_status", kwargs={"submission_id": s.pk}
        )
        r = self.client.get(url)
        self.assertContains(r, s.name)
        self.assertContains(
            r, "This submission is being processed and validated.", status_code=200
        )
    ...
    def test_cancel_stale_submissions(self):
        ...
        for state in DraftSubmissionStateName.objects.filter(used=True).exclude(
            slug="validating"
        ):
            ...
            to_skip.submissionevent_set.create(
                ...
                - datetime.timedelta(
                    minutes=45
                ),  # would be canceled if it were "validating"
            )
            ...
            if state.pk in ["posted", "cancel"]:
                submissions_to_skip.append(
                    (to_expire, state.pk)
                )  # these ones should not be expired regardless of age
            else:
...
class RefsTests(BaseSubmitTestCase):
    ...
    def test_draft_refs_identification(self):
        ...
        group = None
        file, __ = submission_file(
            "draft-some-subject-00",
            "draft-some-subject-00.txt",
            group,
            "test_submission.txt",
        )
...
class PostSubmissionTests(BaseSubmitTestCase):
    ...
    def test_post_submission_rebuilds_ref_relations(
        self, mock_find_filenames, mock_rebuild_reference_relations
    ):
        ...
        """
        submission = SubmissionFactory()
        mock_find_filenames.return_value = {
            "xml": f"{self.staging_dir}/{submission.name}.xml"
        }
...
class TestOldNamesAreProtected(BaseSubmitTestCase):
    ...
    def test_submit_case_conflited_name_fails(self):
        ...
        with self.assertRaisesMessage(
            ValidationError, "Case-conflicting draft name found"
        ):
            SubmissionBaseUploadForm.check_for_old_uppercase_collisions(
                "draft-something-hascapitalletters"
            )
        url = urlreverse("ietf.submit.views.upload_submission")
        files = {}
        files["xml"], _ = submission_file(
            "draft-something-hascapitalletters-00",
            "draft-something-hascapitalletters-00.xml",
            None,
            "test_submission.xml",
        )
        r = self.post_to_upload_submission(url, files)
...
class YangCheckerTests(TestCase):
    ...
    def test_apply_yang_checker_to_draft(self):
        ...
        self.assertEqual(
            checker.check_file_txt.call_args, mock.call(draft.get_file_name())
        )


@override_settings(
    IDSUBMIT_REPOSITORY_PATH="/some/path/", IDSUBMIT_STAGING_PATH="/some/other/path"
)
