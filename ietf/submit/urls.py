...
urlpatterns = [
    ...
    url(
        r"^status/(?P<submission_id>\d+)/(?P<access_token>[a-f\d]*)/$",
        views.submission_status,
    ),
    url(
        r"^status/(?P<submission_id>\d+)/confirm/(?P<auth_token>[a-f\d]+)/$",
        views.confirm_submission,
    ),
    url(r"^status/(?P<submission_id>\d+)/edit/$", views.edit_submission),
    url(
        r"^status/(?P<submission_id>\d+)/(?P<access_token>[a-f\d]+)/edit/$",
    ...
    url(
        r"^approvals/cancelpreapproval/(?P<preapproval_id>[a-f\d]+)/$",
