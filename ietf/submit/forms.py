...
class SubmissionBaseUploadForm(forms.Form):
    ...
    def set_cutoff_warnings(self):
        ...
        if cutoff_00 == cutoff_01:
            if (
                now.date() >= (cutoff_00.date() - meeting.idsubmit_cutoff_warning_days)
                ...
                self.cutoff_warning = (
                    ...
                    "After that, you will not be able to submit Internet-Drafts until after %s (IETF-meeting local time)"
                    % (
                        cutoff_00_str,
                        reopen_str,
                    )
            ...
            if (
                ...
                self.cutoff_warning = "The last submission time for new documents (i.e., version -00 Internet-Drafts) before %s is %s.<br><br>" % (
                    meeting,
                    cutoff_00_str,
    ...
    def _clean_file(self, field_name, parser_class):
        ...
        if self.file_info[field_name].errors:
            raise forms.ValidationError(
                self.file_info[field_name].errors, code="critical_error"
            )
        return f
        ...
        if xml_file:
            ...
            with ExitStack() as stack:
                ...
                with tempfile.NamedTemporaryFile(
                    prefix=name + "-", suffix=".xml", mode="wb+", delete=False
                ) as tf:
                    tfn = tf.name
                ...
                try:
                    ...
                    raise forms.ValidationError(
                        f"Error parsing XML Internet-Draft: {e}", code="parse_exception"
                    )
                ...
                self._extracted_filenames_and_revisions["xml"] = (
                    xml_draft.filename,
                    xml_draft.revision,
                )
        return xml_file
    ...
    def clean(self):
        ...
        for fmt, (
            extracted_name,
            extracted_rev,
        ) in self._extracted_filenames_and_revisions.items():
        ...
        existing = Submission.objects.filter(
            name=self.filename, rev=self.revision
        ).exclude(state__in=("posted", "cancel", "waiting-for-draft"))
        if existing:
            raise forms.ValidationError(
                format_html(
    ...
    def check_submissions_thresholds(which, filter_kwargs, max_amount, max_size):
        ...
        if (
            sum(s.file_size for s in submissions if s.file_size)
            > max_size * 1024 * 1024
        ):
            raise forms.ValidationError(
    ...
    def deduce_group(name):
        ...
        if existing_draft:
            ...
            if len(name_parts) < 3:
                raise forms.ValidationError(
                    'The Internet-Draft name "%s" is missing a third part, please rename it'
                    % name
                )
            ...
            if name.startswith("draft-ietf-") or name.startswith("draft-irtf-"):
                ...
                try:
                    ...
                    raise forms.ValidationError(
                        "There is no active group with acronym '%s', please rename your Internet-Draft"
                        % name_parts[2]
                    )
                ...
                name.startswith("draft-rfc-editor-")
                or name.startswith("draft-rfced-")
                or name.startswith("draft-rfceditor-")
            ):
                ...
                if GroupTypeName.objects.filter(slug=ntype).exists():
                    ...
                    if group:
                        ...
                        raise forms.ValidationError(
                            "Internet-Draft names starting with draft-%s- are restricted, please pick a different name"
                            % ntype
                        )
            return None
    ...
    def clean_txt(self):
        ...
        if txt_file is not None:
            ...
            try:
                ...
                self._extracted_filenames_and_revisions["txt"] = (
                    parsed_draft.filename,
                    parsed_draft.revision,
                )
            except (UnicodeDecodeError, LookupError) as e:
                raise forms.ValidationError(
                    f'Failed decoding the uploaded file: "{str(e)}"',
                    code="decode_failed",
                )
...
class SubmissionAutoUploadForm(SubmissionBaseUploadForm):
    ...
    def clean(self):
        ...
        if self.cleaned_data["replaces"]:
            names_replaced = [
                s.strip() for s in self.cleaned_data["replaces"].split(",")
            ]
            ...
            for doc in documents_replaced:
                if doc.name == self.filename:
                    self.add_error(
                        "replaces",
                        forms.ValidationError(
                            "An Internet-Draft cannot replace itself"
                        ),
                    )
                ...
                elif doc.get_state_slug("draft-iesg") in (
                    "approved",
                    "ann",
                    "rfcqueue",
                ):
...
class SubmitterForm(NameEmailForm):
    ...
    approvals_received = forms.BooleanField(
        label="Approvals received", required=False, initial=False
    )
...
class EditSubmissionForm(forms.ModelForm):
    ...
    formal_languages = ModelMultipleChoiceField(
    ...
    note = forms.CharField(
        label=mark_safe("Comment to the Secretariat"),
    ...
    def clean_document_date(self):
        ...
        error = validate_submission_document_date(
            self.instance.submission_date, document_date
        )
        if error:
            raise forms.ValidationError(error)
...
class PreapprovalForm(forms.Form):
    name = forms.CharField(
        max_length=255, required=True, label="Pre-approved name", initial="draft-"
    )
    ...
    def clean_name(self):
        ...
        if components[-1] == "00":
            raise forms.ValidationError(
                "Name appears to end with a revision number -00 - do not include the revision."
            )
        if len(components) < 4:
            raise forms.ValidationError(
                "Name has less than four dash-delimited components - can't form a valid group Internet-Draft name."
            )
        acronym = components[2]
        if acronym not in [g.acronym for g in self.groups]:
            raise forms.ValidationError(
                "Group acronym not recognized as one you can approve Internet-Drafts for."
            )
        ...
        if Submission.objects.filter(state="posted", name=n):
            raise forms.ValidationError(
                "An Internet-Draft with this name has already been submitted and accepted. A pre-approval would not make any difference."
            )
...
class SubmissionEmailForm(forms.Form):
    ...
    def clean(self):
        ...
        if not match:
            self.add_error(
                "name",
                "Submission name {} must start with 'draft-' and only contain digits, lowercase letters and dash characters and end with revision.".format(
                    name
                ),
            )
        else:
