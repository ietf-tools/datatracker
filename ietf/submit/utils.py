    ...
    try:
        ...
        for dirname in [settings.INTERNET_DRAFT_PATH, settings.INTERNET_DRAFT_ARCHIVE_DIR, ]:
            ...
            if existing:
                ...
                return f'Unexpected file{plural} already in the archive: {files}'
...
def post_submission(request, submission, approved_doc_desc, approved_subm_desc):
    ...
    if submitter_parsed["name"] and submitter_parsed["email"]:
        ...
        submitter_info = '{} <{}>'.format(submitter_parsed["name"], submitter_parsed["email"])
    else:
        submitter = system
    ...
    e = NewRevisionDocEvent.objects.create(
        ...
        rev=draft.rev,
        by=submitter,
        desc=f"New version available: <b>{draft.name}-{draft.rev}.txt</b>",
    )
    events.append(e)
    ...
    with open(ref_rev_file_name, "w", encoding='utf-8') as f:
        f.write(ref_text)
...
def update_replaces_from_submission(request, submission, draft):
    ...
    set_replaces_for_document(request, draft, existing_replaces + approved, by,
                              email_subject="{} replacement status set during submit by {}".format(draft.name, submission.submitter_parsed()["name"]))
...
def rename_submission_files(submission, prev_rev, new_rev):
    for ext in settings.IDSUBMIT_FILE_TYPES:
        source = os.path.join(settings.IDSUBMIT_STAGING_PATH, f'{submission.name}-{prev_rev}.{ext}')
        dest = os.path.join(settings.IDSUBMIT_STAGING_PATH, f'{submission.name}-{new_rev}.{ext}')
...
def move_files_to_repository(submission):
    for ext in settings.IDSUBMIT_FILE_TYPES:
        source = os.path.join(settings.IDSUBMIT_STAGING_PATH, f'{submission.name}-{submission.rev}.{ext}')
        dest = os.path.join(settings.IDSUBMIT_REPOSITORY_PATH, f'{submission.name}-{submission.rev}.{ext}')
        if os.path.exists(source):
            os.rename(source, dest)
        else:
...
def save_files(form):
    ...
    for ext in list(form.fields.keys()):
        ...
        name = os.path.join(settings.IDSUBMIT_STAGING_PATH, f'{form.filename}-{form.revision}.{ext}')
        file_name[ext] = name
...
def get_draft_meta(form, saved_files):
    ...
    if form.cleaned_data['xml']:
        ...
        file_name['txt'] = os.path.join(settings.IDSUBMIT_STAGING_PATH, f'{form.filename}-{form.revision}.txt')
...
def accept_submission(request, submission, autopost=False):
    ...
    sub_event_desc = f'Set submitter to \"{parse_unicode(submission.submitter)}\", replaces to {pretty_replaces}'
    docevent_desc = None
    address_list = []
