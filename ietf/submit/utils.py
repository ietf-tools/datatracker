...
from ietf.doc.mails import (
    ...
    send_external_resource_change_request,
)
...
def validate_submission(submission):
    ...
    if submission.state_id not in ("cancel", "posted"):
        for ext in submission.file_types.split(","):
            source = os.path.join(
                ...
                "%s-%s%s" % (submission.name, submission.rev, ext),
            )
            if not os.path.exists(source):
                errors["files"] = (
                    '"%s" was not found in the staging area. We recommend you that you cancel this submission and upload your files again.'
    ...
    # draft date
    error = validate_submission_document_date(
    ...
    if author_error_count:
        errors["authors"] = (
            "Author email error (see below)"
            if author_error_count == 1
            else "Author email errors (see below)"
        )
...
def validate_submission_name(name):
    if not re.search(r"^draft-[a-z][-a-z0-9]{0,43}(-\d\d)?$", name):
        ...
        if len(name) > 50:
            return (
                "Expected the Internet-Draft name to be at most 50 ascii characters long; found %d."
                % len(name)
            )
        else:
            msg = (
                "Expected name 'draft-...' using lowercase ascii letters, digits, and hyphen; found '%s'."
                % name
            )
            if "." in name:
...
def validate_submission_rev(name, rev):
    ...
    try:
        ...
        existing_revs = [
            int(i.rev)
            for i in Document.objects.filter(name=name)
            if i.rev and i.rev.isdigit()
        ]
        unexpected_revs = [
            i.rev
            for i in Document.objects.filter(name=name)
            if i.rev and not i.rev.isdigit()
        ]  # pyflakes:ignore
    ...
    if replaced_by:
        return "This document has been replaced by %s" % ",".join(
            rd.name for rd in replaced_by
        )
...
def check_submission_revision_consistency(submission):
    ...
    """
    unexpected_submissions = Submission.objects.filter(
        name=submission.name, rev__gte=submission.rev, state_id="posted"
    ).order_by("rev")
    if len(unexpected_submissions) != 0:
        conflicts = [sub.rev for sub in unexpected_submissions]
        return (
            "Rev %s conflicts with existing %s (%s). This indicates a database inconsistency that requires investigation."
            % (
                submission.rev,
...
def docevent_from_submission(submission, desc, who=None):
    ...
    if who:
        ...
        if submitter_parsed["name"] and submitter_parsed["email"]:
            by, _ = ensure_person_email_info_exists(
                submitter_parsed["name"], submitter_parsed["email"], submission.name
            )
        else:
...
def post_rev00_submission_events(draft, submission, submitter):
    ...
    for subevent in submission.submissionevent_set.all().order_by("id"):
        ...
        if desc.startswith("Uploaded submission"):
            ...
            if pos > 0:
                desc = "Request for posting confirmation emailed %s" % (
                    subevent.desc[pos + 23 :]
                )
            else:
                pos = subevent.desc.find("sent appproval email")
                if pos > 0:
                    desc = "Request for posting approval emailed %s" % (
                        subevent.desc[pos + 19 :]
                    )
...
def find_submission_filenames(draft):
    ...
    return {
        ext: str(filename) for ext, filename in candidates.items() if filename.exists()
    }
...
def post_submission(request, submission, approved_doc_desc, approved_subm_desc):
    ...
    if not (
        group.type_id == "individ" and draft.group and draft.group.type_id == "area"
    ):
    ...
    draft.expires = timezone.now() + datetime.timedelta(
    ...
    if draft.stream_id == "ietf" and draft.group.type_id == "wg" and draft.rev == "00":
        ...
        draft.set_state(
            State.objects.get(
                used=True, type="draft-stream-%s" % draft.stream_id, slug="wg-doc"
            )
        )

    # automatic state changes for IANA review
    if draft.get_state_slug("draft-iana-review") in (
        "ok-act",
        "ok-noact",
        "not-ok",
    ) and not draft.get_state_slug("draft-iesg") in (
        "approved",
        "ann",
        "rfcqueue",
        "pub",
    ...
    if not was_rfc and draft.tags.filter(slug="need-rev"):
        ...
        e = update_action_holders(
            draft, prev_tags=tags_before, new_tags=draft.tags.all()
        )
        if e is not None:
            events.append(e)
    ...
    submission_resources = "\n".join(
        sorted(str(r) for r in submission.external_resources.all())
    )
    if draft_resources != submission_resources:
        if can_edit_docextresources(request.user, draft):
            ...
            send_external_resource_change_request(
                request, draft, submitter_info, submission.external_resources.all()
            )
    ...
    with io.open(ref_rev_file_name, "w", encoding="utf-8") as f:
        f.write(ref_text)
    store_str(
        "bibxml-ids", f"reference.I-D.{draft.name}-{draft.rev}.txt", ref_text
    )  # TODO-BLOBSTORE verify with test
...
def update_replaces_from_submission(request, submission, draft):
    ...
    if request.user.is_authenticated:
        is_chair_of = list(
            Group.objects.filter(role__person__user=request.user, role__name="chair")
        )

    replaces = Document.objects.filter(
        name__in=submission.replaces.split(",")
    ...
    for r in replaces:
        ...
        if (
            is_secretariat
            or (
                draft.group in is_chair_of
                and (r.group.type_id == "individ" or r.group in is_chair_of)
            )
            or (
                submitter_email
                and r.documentauthor_set.filter(
                    email__address__iexact=submitter_email
                ).exists()
            )
        ):
            approved.append(r)
    ...
    try:
        by = (
            request.user.person
            if request.user.is_authenticated
            else Person.objects.get(name="(System)")
        )
    except Person.DoesNotExist:
        by = Person.objects.get(name="(System)")
    set_replaces_for_document(
        request,
        draft,
        existing_replaces + approved,
        by,
        email_subject="%s replacement status set during submit by %s"
        % (draft.name, submission.submitter_parsed()["name"]),
    )
    ...
    if suggested:
        ...
        for r in suggested:
            RelatedDocument.objects.create(
                source=draft, target=r, relationship=possibly_replaces
            )
        ...
        DocEvent.objects.create(
            ...
            rev=draft.rev,
            by=by,
            type="added_suggested_replaces",
            desc="Added suggested replacement relationships: %s"
...
def update_docextresources_from_submission(request, submission, draft):
    doc_resources = [
        DocExtResource.from_sibling_class(res)
...
def update_authors(draft, submission):
    docauthors = []
    for author in submission.authors:
        person, email = ensure_person_email_info_exists(
            author["name"], author.get("email"), submission.name
        )
        docauthors.append(
            DocumentAuthor(
...
def move_files_to_repository(submission):
    for ext in settings.IDSUBMIT_FILE_TYPES:
        ...
        if source.exists():
            move(source, dest)
            all_archive_dest = (
                Path(settings.INTERNET_ALL_DRAFTS_ARCHIVE_DIR) / dest.name
            )
            ...
            log.log(
                "Intended to move '%s' to '%s', but found source missing while destination exists."
            )
        elif f".{ext}" in submission.file_types.split(","):
            raise ValueError(
                "Intended to move '%s' to '%s', but found source and destination missing."
            )
...
def remove_staging_files(name, rev):
    ...
    for ext in exts:
        ...
        remove_from_storage(
            "staging", basename.with_suffix(ext).name, warn_if_missing=False
        )
...
def approvable_submissions_for_user(user):
    ...
    return (
        group_approvable.filter(
            group__role__name="chair", group__role__person__user=user
        )
        | group_approvable.filter(
            group__parent__role__name="ad", group__parent__role__person__user=user
        )
        | ad_approvable.filter(
            group__parent__role__name="ad", group__parent__role__person__user=user
        )
...
def preapprovals_for_user(user):
    ...
    posted = (
        Submission.objects.distinct()
        .filter(state="posted")
        .values_list("name", flat=True)
    )
    res = (
        Preapproval.objects.exclude(name__in=posted)
        .order_by("-time")
        .select_related("by")
    )
    if has_role(user, "Secretariat"):
    ...
    accessible_groups = Group.objects.filter(
        role__person__user=user, type__features__req_subm_approval=True
    ) | Group.objects.filter(
        parent__role__name="ad",
        parent__role__person__user=user,
        type__features__req_subm_approval=True,
    )
...
def recently_approved_by_user(user, since):
    ...
    res = (
        Submission.objects.distinct()
        .filter(state="posted", submission_date__gte=since, rev="00")
        .order_by("-submission_date")
    )
    if has_role(user, "Secretariat"):
        return res
...
def expirable_submissions(older_than_days):
    ...
    return Submission.objects.exclude(state__in=("cancel", "posted")).filter(
        submission_date__lt=cutoff
    )
...
def expire_submission(submission, by):
    ...
    SubmissionEvent.objects.create(
        submission=submission, by=by, desc="Cancelled expired submission"
    )
...
def get_submission(form):
    ...
    submissions = Submission.objects.filter(
        name=form.filename, rev=form.revision, state_id="waiting-for-draft"
    ).distinct()
    if not submissions:
    ...
    if form.parsed_draft is not None:
        submission.formal_languages.set(
            FormalLanguageName.objects.filter(
                slug__in=form.parsed_draft.get_formal_languages()
            )
        )
...
def accept_submission_requires_group_approval(submission):
    ...
    return (
        ...
        and submission.group.features.req_subm_approval
...
class SubmissionError(Exception):
    ...
    def sanitize_message(msg):
        ...
        return re.sub(rf"({'|'.join(paths)})/*", "**/", msg)
...
def accept_submission(
    submission: Submission, request: Optional[HttpRequest] = None, autopost=False
):
    ...
    prev_authors = (
        [] if not doc else [author.person for author in doc.documentauthor_set.all()]
    )
    curr_authors = [
        get_person_from_name_email(author["name"], author.get("email"))
        for author in submission.authors
    ]
    ...
    if request is not None and request.user.is_authenticated:
        ...
        requester_is_author = requester in (
            prev_authors if submission.rev != "00" else curr_authors
        )
    ...
    pretty_replaces = (
        "(none)"
        if not replaces
        else (", ".join(prettify_std_name(r.name) for r in replaces))
    )
    ...
    requires_ad_approval = (
        submission.revises_wg_draft and not submission.group.is_active
    )
    ...
    sub_event_desc = 'Set submitter to "%s", replaces to %s' % (
        parse_unicode(submission.submitter),
        pretty_replaces,
    )
    ...
    if requires_ad_approval or requires_prev_ad_approval:
        ...
        docevent_desc = (
            "Request for posting approval emailed to group chairs: %s" % sent_to
        )
        ...
        create_event = (
            False  # do not create submission event below, post_submission() handled it
        )
    else:
        ...
        address_list = send_submission_confirmation(
            request, submission, chair_notice=group_authors_changed
        )
        sent_to = ", ".join(address_list)
        if submission.state_id == "aut-appr":
            sub_event_desc += (
                " and sent confirmation email to previous authors: %s" % sent_to
            )
            docevent_desc = (
                "Request for posting confirmation emailed to previous authors: %s"
                % sent_to
            )
        else:
            sub_event_desc += (
                " and sent confirmation email to submitter and authors: %s" % sent_to
            )
            docevent_desc = (
                "Request for posting confirmation emailed to submitter and authors: %s"
                % sent_to
            )
    ...
    if docevent_desc:
        docevent_from_submission(
            submission, docevent_desc, who=Person.objects.get(name="(System)")
        )
...
def process_and_validate_submission(submission):
    ...
    if len(set(submission.file_types.split(",")).intersection({".xml", ".txt"})) == 0:
        raise SubmissionError(
            "Require XML and/or text format to process an Internet-Draft submission."
        )
    ...
    try:
        ...
        if ".xml" in submission.file_types:
            ...
            try:
                render_missing_formats(
                    submission
                )  # makes HTML and text, unless text was uploaded
        ...
        errors = [
            c.message
            for c in submission.checks.filter(passed__isnull=False)
            if not c.passed
        ]
        if len(errors) > 0:
        ...
        raise SubmissionError(
            "A system error occurred while processing the submission."
        ) from err
...
def process_and_accept_uploaded_submission(submission):
    ...
    create_submission_event(
        None, submission, desc="Completed submission validation checks"
    )
...
def process_uploaded_submission(submission):
    ...
    try:
        ...
        submission.save()
        create_submission_event(
            None, submission, desc="Uploaded submission (diverted to manual process)"
        )
        send_manual_post_request(
            None, submission, errors=dict(consistency=str(consistency_error))
        )
    except SubmissionError as err:
...
def apply_yang_checker_to_draft(checker, draft):
    submission = (
        Submission.objects.filter(name=draft.name, rev=draft.rev)
        .order_by("-id")
        .first()
    )
    if submission:
        ...
        if check:
            ...
            old_res = (
                (check.passed, check.errors, check.warnings, check.message)
                if check
                else ()
            )
            if new_res != old_res:
                ...
                submission.checks.filter(checker=checker.name).exclude(
                    pk=qs.first().pk
                ).delete()
        ...
        log.log(
            f"Could not run yang checker for {draft.name}-{draft.rev}: missing submission object"
        )
...
def populate_yang_model_dirs():
    ...
    * As drafts expire, models should be removed in order to catch cases
    ...
    def extract_from(file, dir, strict=True):
        ...
        try:
            model_list = xym.xym(
                str(file), str(file.parent), str(dir), strict=strict, debug_level=-2
            )
    ...
    for item in rfcdir.iterdir():
        if (
            item.is_file()
            and item.name.startswith("rfc")
            and item.name.endswith(".txt")
            and item.name[3:-4].isdigit()
        ):
            if item.stat().st_mtime > latest:
    ...
    for item in draftdir.iterdir():
        try:
            if (
                item.is_file()
                and item.name.startswith("draft")
                and item.name.endswith(".txt")
                and active(item)
            ):
