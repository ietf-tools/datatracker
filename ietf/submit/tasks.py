...
def process_uploaded_submission_task(submission_id):
    try:
        ...
        log.log(
            f"process_uploaded_submission_task called for missing submission_id={submission_id}"
        )
    else:
...
def cancel_stale_submissions():
    ...
    for subm in stale_unvalidated_submissions:
        ...
        create_submission_event(
            None, subm, "Submission canceled: validation checks took too long"
        )
    ...
    for subm in expired_submissions:
        ...
        cancel_submission(subm)
        create_submission_event(
            None, subm, "Submission canceled: expired without being posted"
        )
