...
def all_id_txt():
    ...
    revision_time = dict(
        NewRevisionDocEvent.objects.filter(
            type="new_revision", doc__name__startswith="draft-"
        )
        .order_by("time")
        .values_list("doc__name", "time")
    )
    ...
    replacements = dict(
        RelatedDocument.objects.filter(
            target__states=State.objects.get(type="draft", slug="repl"),
            relationship="replaces",
        ).values_list("target__name", "source__name")
    )
    ...
    includes = list(
        State.objects.filter(type="draft-iesg").exclude(slug__in=inactive_states)
    )
    in_iesg_process = (
        all_ids.exclude(states__in=excludes)
        .filter(states__in=includes)
        .only("name", "rev")
    )
        ...
        for name, rev in not_in_process.filter(states=s).values_list("name", "rev"):
            ...
            if s.slug == "rfc":
                ...
                if rfc:
                    last_field = rfc[
                        3:
                    ]  # Rework this to take advantage of having the number at hand already.
            elif s.slug == "repl":
...
def all_id2_txt():
    ...
    revision_time = dict(
        DocEvent.objects.filter(type="new_revision", doc__name__startswith="draft-")
        .order_by("time")
        .values_list("doc__name", "time")
    )
    ...
    authors = {}
    for a in (
        DocumentAuthor.objects.filter(document__name__startswith="draft-")
        .order_by("order")
        .select_related("email", "person")
        .iterator()
    ):
        ...
        if a.email:
            l.append(
                "%s <%s>"
                % (
                    a.person.plain_name().replace("@", ""),
                    a.email.address.replace(",", ""),
                )
            )
        else:
    ...
    shepherds = dict(
        (e.pk, e.formatted_ascii_email().replace('"', ""))
        for e in Email.objects.filter(shepherd_document_set__type="draft")
        .select_related("person")
        .distinct()
    )
    ads = dict(
        (p.pk, p.formatted_ascii_email().replace('"', ""))
    ...
    for d in drafts:
        ...
        fields.append(
            "-1"
        )  # used to be internal numeric identifier, we don't have that anymore
        # 2
        ...
        if state == "active":
            ...
            if iesg_state:
                s = iesg_state.name
                tags = d.tags.filter(slug__in=IESG_SUBSTATE_TAGS).values_list(
                    "name", flat=True
                )
                if tags:
                    s += "::" + "::".join(tags)
        ...
        if d.group:
            if d.group.type_id == "area":
                ...
                d.group.type_id == "wg"
                and d.group.parent
                and d.group.parent.type_id == "area"
            ):
        ...
        # 13
        fields.append(
            clean_whitespace(d.title)
        )  # FIXME: we should make sure this is okay in the database and in submit
        # 14
...
def active_drafts_index_by_group(extra_values=()):
    ...
    for d in Document.objects.filter(states=active_state).filter(
        states__in=[wg_cand, wg_adopt]
    ):
        docs_dict[d.name]["group_id"] = individual.id

    # add initial and latest revision time
    for time, doc_name in (
        NewRevisionDocEvent.objects.filter(
            type="new_revision", doc__states=active_state
        )
        .order_by("-time")
        .values_list("time", "doc__name")
    ):
    ...
    # add authors
    for a in (
        DocumentAuthor.objects.filter(document__states=active_state)
        .order_by("order")
        .select_related("person")
    ):
        ...
        if d:
            if "authors" not in d:
                d["authors"] = []
            d["authors"].append(
                a.person.plain_ascii()
            )  # This should probably change to .plain_name() when non-ascii names are permitted
