...
class IndexTests(TestCase):
    ...
    def test_all_id_txt(self):
        ...
        RelatedDocument.objects.create(
            ...
            source=Document.objects.create(
                type_id="draft", rev="00", name="draft-test-replacement"
            ),
            target=draft,
        )
    ...
    def test_all_id2_txt(self):
        draft = WgDraftFactory(
            ...
            shepherd=EmailFactory(
                address="shepherd@example.com", person__name="Draft δραφτυ Shepherd"
            ),
        ...
        NewRevisionDocEvent.objects.create(
            doc=draft, rev=draft.rev, type="new_revision", by=draft.ad
        )
        ...
        self.assertEqual(
            t[6], draft.latest_event(type="new_revision").time.strftime("%Y-%m-%d")
        )
        ...
        self.assertEqual(
            t[14], "%s <%s>" % (author.person.plain_name(), author.email.address)
        )
        self.assertEqual(
            t[15],
        ...
        self.assertEqual(
            t[16], "%s <%s>" % (draft.ad.plain_ascii(), draft.ad.email_address())
        )
    ...
    def test_id_index_txt(self):
        draft = WgDraftFactory(
            states=[("draft", "active")], abstract="a" * 20, authors=[PersonFactory()]
        )
    ...
    def test_temp_file_manager(self):
        with TemporaryDirectory() as temp_dir:
            with TemporaryDirectory() as other_dir:
                ...
                self.assertEqual(retrieve_str("indexes", "yay.txt"), "yay")
