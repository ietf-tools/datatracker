...
from ietf.liaisons.factories import (
    LiaisonStatementFactory,
    LiaisonStatementEventFactory,
    LiaisonStatementAttachmentFactory,
    RelatedLiaisonStatementFactory,
)
...
class LiaisonTests(TestCase):
    ...
    def test_details(self):
        ...
        r = self.client.get(
            urlreverse(
                "ietf.liaisons.views.liaison_detail", kwargs={"object_id": liaison.pk}
            )
        )
        self.assertEqual(r.status_code, 200)
        self.assertContains(r, liaison.title)
    ...
    def test_feeds(self):
        ...
        r = self.client.get(
            "/feed/liaison/from/%s/" % liaison.from_groups.first().acronym
        )
        self.assertEqual(r.status_code, 200)
        self.assertContains(r, liaison.title)
...
class UnitTests(TestCase):
    def test_get_cc(self):
        ...
        cc = get_cc(sdo)
        self.assertTrue(
            contacts_from_roles([sdo.role_set.filter(name="liaiman").first()]) in cc
        )
...
class AjaxTests(TestCase):
    def test_ajax(self):
        ...
        url = (
            urlreverse("ietf.liaisons.views.ajax_get_liaison_info")
            + "?to_groups=&from_groups="
        )
    ...
    def test_ajax_to_contacts(self):
        ...
        contact_email = contact_email_from_role(
            RoleFactory(name_id="liaison_contact", group=group)
        )

        url = urlreverse(
            "ietf.liaisons.views.ajax_get_liaison_info"
        ) + "?to_groups={}&from_groups=".format(group.pk)
        self.client.login(username="secretary", password="secretary+password")
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        data = r.json()
    ...
    def test_ajax_select2_search_liaison_statements(self):
        ...
        # test text search
        url = (
            urlreverse("ietf.liaisons.views.ajax_select2_search_liaison_statements")
            + "?q=%s" % liaison.title[:5]
        )
        self.client.login(username="secretary", password="secretary+password")
        r = self.client.get(url)
...
class ManagementCommandTests(TestCase):
    def test_check_liaison_deadlines(self):
        ...
        LiaisonStatementFactory(
            deadline=date_today(DEADLINE_TZINFO) + datetime.timedelta(days=1)
        )
...
class LiaisonManagementTests(TestCase):
    ...
    def test_add_comment(self):
        ...
        # test unauthorized
        addurl = urlreverse(
            "ietf.liaisons.views.add_comment", kwargs=dict(object_id=liaison.pk)
        )
        url = urlreverse(
            "ietf.liaisons.views.liaison_history", kwargs=dict(object_id=liaison.pk)
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
        ...
        # private comment
        r = self.client.post(
            addurl, dict(comment="Private comment", private=True), follow=True
        )
        self.assertEqual(r.status_code, 200)
    ...
    def test_taken_care_of(self):
        ...
        url = urlreverse(
            "ietf.liaisons.views.liaison_detail", kwargs=dict(object_id=liaison.pk)
        )
        # normal get
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
    ...
    def test_approval_process(self):
        ...
        liaison = LiaisonStatementFactory(
            state_id="pending",
            from_groups=[
                GroupFactory(type_id="wg"),
            ],
            to_groups=[
                RoleFactory(
                    name_id="liaiman",
                    person__user__username="ulm-liaiman",
                    group__type_id="sdo",
                ).group
            ],
        )
        ...
        url = urlreverse(
            "ietf.liaisons.views.liaison_list", kwargs=dict(state="pending")
        )
    ...
    def test_edit_liaison(self):
        ...
        url = urlreverse(
            "ietf.liaisons.views.liaison_edit", kwargs=dict(object_id=liaison.pk)
        )
        ...
        r = self.client.post(
            ...
            dict(
                ...
                purpose="action",
                deadline=(liaison.deadline + datetime.timedelta(days=1)).strftime(
                    "%Y-%m-%d"
                ),
                title="title",
                submitted_date=(liaison.posted + datetime.timedelta(days=1)).strftime(
                    "%Y-%m-%d"
                ),
                body="body",
        ...
        self.assertEqual(
            new_liaison.technical_contacts, "technical_contact@example.com"
        )
        self.assertEqual(new_liaison.cc_contacts, "cc@example.com")
        self.assertEqual(
            new_liaison.purpose, LiaisonStatementPurposeName.objects.get(slug="action")
        )
        self.assertEqual(
            new_liaison.deadline, liaison.deadline + datetime.timedelta(days=1)
        ),
        ...
        with (
            Path(settings.LIAISON_ATTACH_PATH) / attachment.uploaded_filename
        ).open() as f:
    ...
    def test_incoming_access(self):
        ...
        addurl = urlreverse(
            "ietf.liaisons.views.liaison_add", kwargs={"type": "incoming"}
        )
    ...
    def test_outgoing_access(self):
        ...
        url = urlreverse("ietf.liaisons.views.liaison_list")
        addurl = urlreverse(
            "ietf.liaisons.views.liaison_add", kwargs={"type": "outgoing"}
        )
    ...
    def test_outgoing_options(self):
        ...
        person = Person.objects.filter(
            role__name="chair", role__group__acronym="mars"
        ).first()
        self.client.login(username="marschairman", password="marschairman+password")
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        ...
        groups = Group.objects.filter(
            role__person=person, role__name="chair", state="active", type="wg"
        )
    ...
    def test_add_incoming_liaison(self):
        ...
        GroupFactory(
            type_id="sdo"
        )  # This test assumes there are two sdo groups in the database
        wg = RoleFactory(
            name_id="chair",
            person__user__username="marschairman",
            group__acronym="mars",
        ...
        self.assertEqual(
            l.purpose, LiaisonStatementPurposeName.objects.get(slug="action")
        )
        ...
        self.assertEqual(written_content, test_content)
        self.assertEqual(
            retrieve_str(attachment.type_id, attachment.uploaded_filename), test_content
        )
    ...
    def test_add_outgoing_liaison(self):
        ...
        RoleFactory(
            name_id="ad", person=Person.objects.get(user__username="ad"), group=wg
        )
        liaison = LiaisonStatementFactory()
    ...
    def test_liaison_add_attachment(self):
        ...
        self.assertEqual(
            retrieve_str(attachment.type_id, attachment.uploaded_filename), test_data
        )
    ...
    def test_liaison_delete_attachment(self):
        ...
        url = urlreverse(
            ...
            kwargs=dict(object_id=liaison.pk, attach_id=attachment.pk),
        )
        login_testing_unauthorized(self, "secretary", url)
        r = self.client.get(url)
        self.assertEqual(r.status_code, 302)
        self.assertEqual(
            liaison.liaisonstatementattachment_set.filter(removed=False).count(), 0
        )
    ...
    def test_in_response(self):
        ...
        url = urlreverse(
            "ietf.liaisons.views.liaison_add", kwargs=dict(type="incoming")
        )
        login_testing_unauthorized(self, "secretary", url)
    ...
    def test_resend_liaison(self):
        ...
        url = urlreverse(
            "ietf.liaisons.views.liaison_resend", kwargs=dict(object_id=liaison.pk)
        )
        login_testing_unauthorized(self, "secretary", url)
    ...
    def test_liaison_reply(self):
        ...
        url = urlreverse(
            "ietf.liaisons.views.liaison_reply", kwargs=dict(object_id=liaison.pk)
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
        ...
        self.assertEqual(
            q("#id_from_groups").find("option:selected").val(), reply_from_group_id
        )
        self.assertEqual(
            q("#id_to_groups").find("option:selected").val(), reply_to_group_id
        )
    ...
    def test_search(self):
        ...
        LiaisonStatementEventFactory(
            type_id="posted",
            statement__body="Has recently in its body",
            statement__from_groups=[
                GroupFactory(type_id="sdo", acronym="ulm"),
            ],
        )
        # Statement 2
        s2 = LiaisonStatementEventFactory(
            type_id="posted",
            statement__body="That word does not occur here",
            statement__title="Nor does it occur here",
        )
        ...
        # test 0 results
        url = (
            urlreverse("ietf.liaisons.views.liaison_list")
            + "?text=gobbledygook&source=&destination=&start_date=&end_date="
        )
        r = self.client.get(url)
        q = PyQuery(r.content)
        self.assertEqual(r.status_code, 200)
        ...
        url = (
            urlreverse("ietf.liaisons.views.liaison_list")
            + "?text=recently&source=&destination=&start_date=&end_date="
        )
        r = self.client.get(url)
        q = PyQuery(r.content)
        self.assertEqual(r.status_code, 200)
        ...
        url = (
            urlreverse("ietf.liaisons.views.liaison_list")
            + "?text=&source=ulm&destination=&start_date=&end_date="
        )
        r = self.client.get(url)
        q = PyQuery(r.content)
        self.assertEqual(r.status_code, 200)
        ...
        url = (
            urlreverse("ietf.liaisons.views.liaison_list")
            + "?text=&source=&destination=&start_date=2015-01-01&end_date="
        )
        r = self.client.get(url)
        q = PyQuery(r.content)
        self.assertEqual(r.status_code, 200)
    ...
    def test_post_and_send_fail(self):
        ...
        self.assertEqual(r.status_code, 200)
        self.assertContains(
            r, "As an IETF Liaison Manager you can not send incoming liaison statements"
        )
    ...
    def test_send_sdo_reminder(self):
        RoleFactory(
            name_id="liaiman",
            person__user__username="ulm-liaiman",
            person__user__email="ulm-liaiman@somewhere.example",
            group__type_id="sdo",
            group__acronym="ulm",
        )
