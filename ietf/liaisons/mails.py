...
def send_sdo_reminder(sdo):
    ...
    authorized_list = (
        Role.objects.filter(group=sdo, name="auth").select_related("person").distinct()
    )
    ...
    send_mail_text(
        None, to_email, settings.LIAISON_UNIVERSAL_FROM, subject, body, cc=cc
    )
...
def possibly_send_deadline_reminder(liaison):
    ...
    if days_to_go < 0:
        ...
        subject = "[Liaison deadline %s] %s" % (
            PREVIOUS_DAYS[days_to_go],
            liaison.title,
        )
