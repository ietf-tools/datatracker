...
class LiaisonStatementsFeed(Feed):
    ...
    def get_object(self, request, kind, search=None):
        ...
        if kind == "from":
            ...
            try:
                ...
                statement = LiaisonStatement.objects.filter(
                    from_groups__name__iregex=search_string
                ).first()
                if not statement:
                    raise FeedDoesNotExist
        ...
        if kind == "subject":
            ...
            obj["q"] = [
                Q(title__icontains=search) | Q(attachments__title__icontains=search)
            ]
    ...
    def item_title(self, item):
        return render_to_string(
            "liaisons/liaison_title.html", {"liaison": item}
        ).strip()

    def item_description(self, item):
        return render_to_string(
            "liaisons/feed_item_description.html",
            {
                "liaison": item,
    ...
    def item_link(self, item):
        return urlreverse(
            "ietf.liaisons.views.liaison_detail", kwargs={"object_id": item.pk}
        )
