...
def liaison_manager_sdos(person):
    return Group.objects.filter(
        type="sdo", state="active", role__person=person, role__name="liaiman"
    ).distinct()
...
def get_internal_choices(user):
    ...
    main = [
        (g.pk, "The {}".format(g.acronym.upper()))
        for g in groups.filter(acronym__in=("ietf", "iesg", "iab"))
    ]
    areas = [
        (g.pk, "{} - {}".format(g.acronym, g.name)) for g in groups.filter(type="area")
    ]
    wgs = [
        (g.pk, "{} - {}".format(g.acronym, g.name)) for g in groups.filter(type="wg")
    ]
...
def get_groups_for_person(person):
    ...
    if (
        person == None
        or has_role(person.user, "Secretariat")
        or has_role(person.user, "Liaison Manager")
    ):
    ...
    return (
        Group.objects.filter(reduce(operator.or_, queries))
        .order_by("acronym")
        .distinct()
    )
    ...
    for email in emails:
        ...
        try:
            ...
            raise forms.ValidationError(
                "Invalid email address: %s (check character %d)" % (addr, e.start)
            )
...
class AddCommentForm(forms.Form):
    ...
    private = forms.BooleanField(
        ...
        required=False,
        help_text="If this box is checked the comment will not appear in the statement's public history view.",
    )
        ...
        if self.is_bound:
            ...
            if query:
                q = (
                    ...
                    | Q(
                        attachments__title__icontains=query,
                        liaisonstatementattachment__removed=False,
                    )
            ...
            if source:
                ...
                if len(source_list) > 1:
                    ...
                    results = results.filter(
                        Q(from_groups__name__icontains=source)
                        | Q(from_groups__acronym__iexact=source)
                    )
            ...
            if destination:
                ...
                if len(destination_list) > 1:
                    ...
                    results = results.filter(
                        Q(to_groups__name__icontains=destination)
                        | Q(to_groups__acronym__iexact=destination)
                    )
            ...
            if start_date:
                ...
                if end_date:
                    events = events.filter(
                        time__lte=datetime_from_date(end_date, DEADLINE_TZINFO)
                    )
            elif end_date:
                events = LiaisonStatementEvent.objects.filter(
                    type="posted", time__lte=end_date
                )
            if events:
...
class CustomModelMultipleChoiceField(ModelMultipleChoiceField):
    ...
    def prepare_value(self, value):
        ...
        if (
            ...
            return [
                super(CustomModelMultipleChoiceField, self).prepare_value(v)
                for v in value
            ]
...
class LiaisonModelForm(forms.ModelForm):
    ...
    from_groups = ModelMultipleChoiceField(
    ...
    from_contact = (
        forms.EmailField()
    )  # type: Union[forms.EmailField, SearchableEmailField]
    to_contacts = forms.CharField(
        label="Contacts",
    ...
    to_groups = ModelMultipleChoiceField(
        queryset=Group.objects, label="Groups", required=False
    )
    ...
    related_to = SearchableLiaisonStatementsField(
        label="Related Liaison Statement", required=False
    )
    submitted_date = DatepickerDateField(
        date_format="yyyy-mm-dd",
        picker_settings={"autoclose": "1"},
        label="Submission date",
        required=True,
        initial=lambda: date_today(DEADLINE_TZINFO),
    )
    attachments = CustomModelMultipleChoiceField(
        queryset=Document.objects,
        label="Attachments",
        widget=ShowAttachmentsWidget,
        required=False,
    )
    attach_title = forms.CharField(label="Title", required=False)
    attach_file = forms.FileField(label="File", required=False)
    attach_button = forms.CharField(
        label="",
        widget=ButtonWidget(
            label="Attach",
            show_on="id_attachments",
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        self.fields["from_groups"].widget.attrs[
            "data-placeholder"
        ] = "Type in name to search for group"
        self.fields["to_groups"].widget.attrs[
            "data-placeholder"
        ] = "Type in name to search for group"
    ...
    def clean_from_contact(self):
        ...
        try:
            ...
            if not email.origin:
                email.origin = "liaison: %s" % (
                    ",".join([g.acronym for g in from_groups.all()])
                )
                email.save()
    ...
    def clean(self):
        if not self.cleaned_data.get("body", None) and not self.has_attachments():
            self._errors["body"] = ErrorList(
                ["You must provide a body or attachment files"]
            )
            self._errors["attachments"] = ErrorList(
                ["You must provide a body or attachment files"]
            )

        # if purpose=response there must be a related statement
        purpose = LiaisonStatementPurposeName.objects.get(slug="response")
        if self.cleaned_data.get("purpose") == purpose and not self.cleaned_data.get(
            "related_to"
        ):
            self._errors["related_to"] = ErrorList(
                ["You must provide a related statement when purpose is In Response"]
            )
    ...
    def has_attachments(self):
        for key in list(self.files.keys()):
            if key.startswith("attach_file_") and key.replace("file", "title") in list(
                self.data.keys()
            ):
                return True
        return False
    ...
    def save_attachments(self):
        ...
        for key in list(self.files.keys()):
            ...
            if not key.startswith("attach_file_") or not title_key in list(
                self.data.keys()
            ):
                continue
            ...
            LiaisonStatementAttachment.objects.create(
                statement=self.instance, document=attach
            )
            attach_file = io.open(
                os.path.join(settings.LIAISON_ATTACH_PATH, attach.name + extension),
                "wb",
            )
...
class IncomingLiaisonForm(LiaisonModelForm):
    def clean(self):
        if "send" in list(self.data.keys()) and self.get_post_only():
            raise forms.ValidationError(
                "As an IETF Liaison Manager you can not send incoming liaison statements, you only can post them"
            )
    ...
    def get_post_only(self):
        from_groups = self.cleaned_data.get("from_groups")
        if has_role(self.user, "Secretariat") or is_authorized_individual(
            self.user, from_groups
        ):
            return False
        return True
        ...
        if has_role(self.user, "Secretariat"):
            ...
            queryset = (
                Group.objects.filter(
                    ...
                    role__name__in=("liaiman", "auth"),
                )
                .distinct()
                .order_by("name")
            )
            self.fields["from_contact"].initial = (
                self.person.role_set.filter(group=queryset[0]).first().email.address
            )
...
class OutgoingLiaisonForm(LiaisonModelForm):
    ...
    def set_from_fields(self):
        ...
        if has_role(self.user, "Secretariat"):
            self.fields["from_contact"] = SearchableEmailField(
                only_users=True
            )  # secretariat can edit this field!
            return

        if self.person.role_set.filter(name="liaiman", group__state="active"):
            email = (
                self.person.role_set.filter(name="liaiman", group__state="active")
                .first()
                .email.address
            )
        elif self.person.role_set.filter(
            name__in=("ad", "chair"), group__state="active"
        ):
            email = (
                self.person.role_set.filter(
                    name__in=("ad", "chair"), group__state="active"
                )
                .first()
                .email.address
    ...
    def set_to_fields(self):
        ...
        if has_role(self.user, "Liaison Manager") and not self.person.role_set.filter(
            name__in=("ad", "chair"), group__state="active"
        ):
            queryset = (
                Group.objects.filter(
...
class EditLiaisonForm(LiaisonModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["attachments"].initial = (
            self.instance.liaisonstatementattachment_set.exclude(removed=True)
        )
        self.fields["related_to"].initial = [
            x.target for x in self.instance.source_of_set.all()
        ]
    ...
    def save(self, *args, **kwargs):
        ...
        if self.has_changed() and "submitted_date" in self.changed_data:
            event = self.instance.liaisonstatementevent_set.filter(
                type="submitted"
            ).first()
            event.time = datetime_from_date(
                self.cleaned_data.get("submitted_date"), DEADLINE_TZINFO
            )
            event.save()
    ...
    def set_to_fields(self):
        ...
        if self.instance.is_outgoing():
            ...
            if has_role(
                self.user, "Liaison Manager"
            ) and not self.person.role_set.filter(
                name__in=("ad", "chair"), group__state="active"
            ):
                queryset = (
                    Group.objects.filter(
                        type="sdo", role__person=self.person, role__name="liaiman"
                    )
                    .distinct()
                    .order_by("name")
                )
