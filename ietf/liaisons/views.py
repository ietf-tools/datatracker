...
from ietf.liaisons.utils import (
    get_person_for_user,
...
def _can_reply(liaison, user):
    ...
    if user.is_authenticated:
        ...
        if has_role(user, "Secretariat"):
            return True
        if liaison.is_outgoing() and Role.objects.filter(
            group__in=liaison.to_groups.all(), person=person, name="auth"
        ):
            return True
        if not liaison.is_outgoing() and Role.objects.filter(
            group__in=liaison.from_groups.all(), person=person, name="liaiman"
        ):
            return True
    return False
...
def _find_person_in_emails(liaison, person):
    ...
    for email in emails.split(","):
        ...
        if person.email_set.filter(address=addr):
            return True
        elif addr in ("chair@ietf.org", "iesg@ietf.org") and has_role(
            person.user, "IETF Chair"
        ):
            return True
        elif addr in ("iab@iab.org", "iab-chair@iab.org") and has_role(
            person.user, "IAB Chair"
        ):
            return True
        elif addr in ("execd@iab.org",) and has_role(
...
def get_cc(group):
    ...
    if group.acronym in ("ietf", "iesg"):
        ...
        if group.list_email:
            emails.append(
                "{} Discussion List <{}>".format(group.name, group.list_email)
            )
...
def get_contacts_for_group(group):
    ...
    explicit_contacts = contacts_from_roles(
        group.role_set.filter(name="liaison_contact")
    )
    ...
    return [
        ...
        for t in [
            (
                "Statement",
                urlreverse(
                    "ietf.liaisons.views.liaison_detail", kwargs={"object_id": stmt.pk}
                ),
            ),
            (
                "History",
                urlreverse(
                    "ietf.liaisons.views.liaison_history", kwargs={"object_id": stmt.pk}
                ),
            ),
        ]
    ]
...
def needs_approval(group, person):
    ...
    if group.acronym in ("ietf", "iesg") and has_role(user, "IETF Chair"):
        return False
    if group.acronym == "iab" and (
        has_role(user, "IAB Chair") or has_role(user, "IAB Executive Director")
    ):
        return False
    ...
    if (
        group.type_id == "wg"
        and group.parent
        and group.parent.role_set.filter(name="ad", person=person)
    ):
        return False
    return True
...
def add_comment(request, object_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect(
                "ietf.liaisons.views.liaison_history", object_id=statement.id
            )
    else:
        form = AddCommentForm()

    return render(
        request, "liaisons/add_comment.html", dict(liaison=statement, form=form)
    )
...
def liaison_add(request, type=None, **kwargs):
    if type == "incoming" and not can_add_incoming_liaison(request.user):
        permission_denied(
            request,
            "Restricted to users who are authorized to submit incoming liaison statements.",
        )
    ...
    if request.method == "POST":
        form = liaison_form_factory(
            request, data=request.POST.copy(), files=request.FILES, type=type, **kwargs
        )
        ...
        if form.is_valid():
            ...
            if "save" in request.POST:
                ...
                messages.success(
                    request, "The statement has been submitted and is awaiting approval"
                )
    ...
    return render(
        request, "liaisons/edit.html", {"form": form, "liaison": kwargs.get("instance")}
    )
...
def liaison_history(request, object_id):
    ...
    events = (
        liaison.liaisonstatementevent_set.all()
        .order_by("-time", "-id")
        .select_related("by")
    )
...
def liaison_detail(request, object_id):
    ...
    relations_by = [
        i.target for i in liaison.source_of_set.filter(target__state__slug="posted")
    ]
    relations_to = [
        i.source for i in liaison.target_of_set.filter(source__state__slug="posted")
    ]

    return render(
        request,
        "liaisons/detail.html",
        {
            "liaison": liaison,
...
def liaison_edit(request, object_id):
    ...
    if not can_edit_liaison(request.user, liaison):
        permission_denied(
            request, "You do not have permission to edit this liaison statement."
        )
...
def liaison_edit_attachment(request, object_id, doc_id):
    ...
    return render(
        request, "liaisons/edit_attachment.html", {"form": form, "liaison": liaison}
    )
...
def liaison_list(request, state="posted"):
    ...
    liaisons = LiaisonStatement.objects.filter(state=state).prefetch_related(
        Prefetch(
            "from_groups",
            queryset=Group.objects.order_by("acronym").select_related("type"),
            to_attr="prefetched_from_groups",
        ),
        Prefetch(
            "to_groups",
            queryset=Group.objects.order_by("acronym").select_related("type"),
            to_attr="prefetched_to_groups",
        ),
        Prefetch(
            "tags",
            queryset=LiaisonStatementTagName.objects.filter(slug="taken"),
            to_attr="prefetched_tags",
        ),
        Prefetch(
            "liaisonstatementevent_set",
            queryset=LiaisonStatementEvent.objects.filter(type="posted"),
            to_attr="prefetched_posted_events",
        ),
    )
    ...
    entries.append(
        (
            "Posted",
            urlreverse("ietf.liaisons.views.liaison_list", kwargs={"state": "posted"}),
        )
    )
    if can_add_liaison(request.user):
        entries.append(
            (
                "Action Needed",
                urlreverse(
                    "ietf.liaisons.views.liaison_list", kwargs={"state": "posted"}
                )
                + "?tags=required",
            )
        )
        entries.append(
            (
                "Pending",
                urlreverse(
                    "ietf.liaisons.views.liaison_list", kwargs={"state": "pending"}
                ),
            )
        )
        entries.append(
            (
                "Dead",
                urlreverse(
                    "ietf.liaisons.views.liaison_list", kwargs={"state": "dead"}
                ),
            )
        )
...
def liaison_resend(request, object_id):
    ...
    send_liaison_by_email(request, liaison)
    LiaisonStatementEvent.objects.create(
        type_id="resent", by=person, statement=liaison, desc="Statement Resent"
    )
