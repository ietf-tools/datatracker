...
from ietf.nomcom.managers import (
    NomineePositionManager,
    NomineeManager,
    PositionManager,
    FeedbackManager,
)
from ietf.nomcom.utils import (
    initialize_templates_for_group,
...
class NomCom(models.Model):
    ...
    public_key = models.FileField(
        storage=NoLocationMigrationFileSystemStorage(
    ...
    send_questionnaire = models.BooleanField(
    ...
    reminder_interval = models.PositiveIntegerField(
        help_text="If the nomcom user sets the interval field then a cron command will "
    ...
    initial_text = models.TextField(
        verbose_name="Help text for nomination form", blank=True
    )
    show_nominee_pictures = models.BooleanField(
        verbose_name="Show nominee pictures",
        default=True,
        help_text="Display pictures of each nominee (if available) on the feedback pages",
    )
    show_accepted_nominees = models.BooleanField(
    ...
    first_call_for_volunteers = models.DateField(
        verbose_name="Date of the first call for volunteers", blank=True, null=True
    )
    ...
    def encrypt(self, cleartext):
        ...
        command = "%s smime -encrypt -in /dev/stdin %s" % (
        ...
        if not hasattr(self, "_cached_chair_emails"):
            if self.group:
                self._cached_chair_emails = list(
                    self.group.role_set.filter(name_id="chair").values_list(
...
class Nomination(models.Model):
    ...
    candidate_phone = models.CharField(
        verbose_name="Candidate phone", blank=True, max_length=255
    )
    ...
    share_nominator = models.BooleanField(
...
class Nominee(models.Model):
    ...
    nominee_position = models.ManyToManyField(
        "nomcom.Position", through="nomcom.NomineePosition"
    )
...
class NomineePosition(models.Model):
    ...
    def questionnaires(self):
        return Feedback.objects.questionnaires().filter(
            positions__in=[self.position], nominees__in=[self.nominee]
        )
...
class Position(models.Model):
    ...
    requirement = ForeignKey(
        DBTemplate, related_name="requirement", null=True, editable=False
    )
    questionnaire = ForeignKey(
        DBTemplate, related_name="questionnaire", null=True, editable=False
    )
    ...
    accepting_nominations = models.BooleanField(
        verbose_name="Is accepting nominations", default=False
    )
    accepting_feedback = models.BooleanField(
        verbose_name="Is accepting feedback", default=False
    )
    ...
    is_iesg_position = models.BooleanField(
        verbose_name="Is IESG Position", default=False
    )
    ...
    def get_templates(self):
        ...
        self._templates = (
            DBTemplate.objects.filter(group=self.nomcom.group)
            .filter(path__contains="/%s/position/" % self.id)
            .order_by("title")
        )
    ...
    def get_requirement(self):
        ...
        generic_iesg_template = DBTemplate.objects.filter(
            group=self.nomcom.group, path__endswith="iesg_requirements"
        ).first()
        ...
        if self.is_iesg_position and generic_iesg_template:
            ...
            return render_to_string(
                "nomcom/iesg_position_requirements.html",
                dict(
                    position=self,
                    generic_iesg_reqs=generic_iesg_reqs,
...
class Topic(models.Model):
    ...
    description = ForeignKey(
        DBTemplate, related_name="description", null=True, editable=False
    )
    accepting_feedback = models.BooleanField(
        verbose_name="Is accepting feedback", default=False
    )
    audience = ForeignKey(
        TopicAudienceName, verbose_name="Who can provide feedback (intended audience)"
    )
...
class Feedback(models.Model):
    ...
    person = ForeignKey(
        Person, editable=False, blank=True, null=True, on_delete=models.SET_NULL
    )
    time = models.DateTimeField(auto_now_add=True)
