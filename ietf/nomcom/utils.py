...
DISQUALIFYING_ROLE_QUERY_EXPRESSION = (
    Q(
        group__acronym__in=["isocbot", "ietf-trust", "llc-board", "iab"],
        name_id__in=["member", "chair"],
    )
...
def get_hash_nominee_position(date, nominee_position_id):
    return hmac.new(
        settings.NOMCOM_APP_SECRET,
        f"{date}{nominee_position_id}".encode("utf-8"),
        hashlib.sha256,
    ).hexdigest()
...
def initialize_questionnaire_for_position(position):
    ...
    header_questionnaire_path = (
        MAIN_NOMCOM_TEMPLATE_PATH + HEADER_QUESTIONNAIRE_TEMPLATE
    )
...
def store_nomcom_private_key(request, year, private_key):
    ...
    if not private_key:
        ...
        if (
            error
            and error
            != b"*** WARNING : deprecated key derivation used.\nUsing -iter or -pbkdf2 would be better.\n"
        ):
            out = b""
...
def validate_private_key(key):
    ...
    code, out, error = pipe(command % (settings.OPENSSL_COMMAND, key_file.name))
    if code != 0:
        log("openssl error: %s:\n  Error %s: %s" % (command, code, error))
...
def send_accept_reminder_to_nominee(nominee_position):
    ...
    (to_email, cc) = gather_address_lists(
        "nomination_accept_reminder", nominee=nominee.email.address
    )
    ...
    accept_url = reverse(
        ...
        None,
        args=(get_year_by_nomcom(nomcom), nominee_position.id, "accepted", today, hash),
    )
    decline_url = reverse(
        "ietf.nomcom.views.process_nomination_status",
        None,
        args=(get_year_by_nomcom(nomcom), nominee_position.id, "declined", today, hash),
    )
...
def send_questionnaire_reminder_to_nominee(nominee_position):
    ...
    nominee = nominee_position.nominee
    (to_email, cc) = gather_address_lists(
        "nomcom_questionnaire_reminder", nominee=nominee.email.address
    )
    ...
    if type == "accept":
        ...
        for nominee in nominees:
            for (
                ...
                nominee.nomineeposition_set.accepted().without_questionnaire_response()
            ):
...
def make_nomineeposition(nomcom, candidate, position, author):
    ...
    nominee, created = Nominee.objects.get_or_create(
        person=candidate, email=candidate.email(), nomcom=nomcom
    )
    ...
    if nominee_position_created:
        ...
        from_email = settings.NOMCOM_FROM_EMAIL.format(year=nomcom.year())
        (to_email, cc) = gather_address_lists(
            "nomination_new_nominee", nominee=nominee.email.address
        )
        domain = Site.objects.get_current().domain
        today = date_today().strftime("%Y%m%d")
        hash = get_hash_nominee_position(today, nominee_position.id)
        accept_url = reverse(
            "ietf.nomcom.views.process_nomination_status",
            None,
            args=(nomcom.year(), nominee_position.id, "accepted", today, hash),
        )
        decline_url = reverse(
            "ietf.nomcom.views.process_nomination_status",
            None,
            args=(nomcom.year(), nominee_position.id, "declined", today, hash),
        )
        ...
        if nomcom.send_questionnaire:
            ...
            from_email = settings.NOMCOM_FROM_EMAIL.format(year=nomcom.year())
            (to_email, cc) = gather_address_lists(
                "nomcom_questionnaire", nominee=nominee.email.address
            )
            context = {
                "nominee": nominee.person.name,
                "position": position.name,
    ...
    if author:
        context.update(
            {"nominator": author.person.name, "nominator_email": author.address}
        )
    else:
...
def make_nomineeposition_for_newperson(
    ...
    email = Email.objects.create(
        address=candidate_email, origin="nominee: %s" % nomcom.group.acronym
    )
...
def getheader(header_text, default="utf-8"):
    ...
    header_sections = [
        text.decode(charset or default) if isinstance(text, bytes) else text
        for text, charset in tuples
    ]
...
def create_feedback_email(nomcom, msg):
    ...
    feedback = Feedback(
        nomcom=nomcom, author=by, subject=subject or "", comments=nomcom.encrypt(body)
    )
    feedback.save()
    return feedback
...
def is_eligible(person, nomcom=None, date=None):
    return list_eligible(
        nomcom=nomcom, date=date, base_qs=Person.objects.filter(pk=person.pk)
    ).exists()
...
def decorate_volunteers_with_qualifications(
    volunteers, nomcom=None, date=None, base_qs=None
):
    if not base_qs:
        base_qs = Person.objects.all()
    eligibility_date = get_eligibility_date(nomcom, date)
    if eligibility_date.year in (2021, 2022):
        three_of_five_qs, officer_qs, author_qs = get_8989_eligibility_querysets(
            eligibility_date, base_qs
        )
        for v in volunteers:
            qualifications = []
...
def list_eligible_8713(date, base_qs=None):
    ...
    return remove_disqualified(
        three_of_five_eligible_8713(previous_five=previous_five, queryset=base_qs)
    )
...
def list_eligible_8788(date, base_qs=None):
    if not base_qs:
        base_qs = Person.objects.all()
    previous_five = Meeting.objects.filter(
        number__in=["102", "103", "104", "105", "106"]
    )
    return remove_disqualified(
        three_of_five_eligible_8713(previous_five=previous_five, queryset=base_qs)
    )
...
def get_8989_eligibility_querysets(date, base_qs):
    return get_threerule_eligibility_querysets(
        date, base_qs, three_of_five_callable=three_of_five_eligible_8713
    )


def get_9389_eligibility_querysets(date, base_qs):
    return get_threerule_eligibility_querysets(
        date, base_qs, three_of_five_callable=three_of_five_eligible_9389
    )
...
def get_threerule_eligibility_querysets(date, base_qs, three_of_five_callable):
    ...
    three_of_five_qs = three_of_five_callable(
        previous_five=previous_five, queryset=base_qs
    )

    # If date is Feb 29, neither 3 nor 5 years ago has a Feb 29. Use Feb 28 instead.
    if date.month == 2 and date.day == 29:
        three_years_ago = datetime.datetime(
            date.year - 3, 2, 28, tzinfo=DEADLINE_TZINFO
        )
        five_years_ago = datetime.datetime(date.year - 5, 2, 28, tzinfo=DEADLINE_TZINFO)
    else:
        three_years_ago = datetime.datetime(
            date.year - 3, date.month, date.day, tzinfo=DEADLINE_TZINFO
        )
        five_years_ago = datetime.datetime(
            date.year - 5, date.month, date.day, tzinfo=DEADLINE_TZINFO
        )

    officer_qs = base_qs.filter(
        # is currently an officer
        Q(
            role__name_id__in=("chair", "secr"),
        ...
        | Q(
            rolehistory__group__time__gte=three_years_ago,
    ...
    rfc_pks = set(
        DocEvent.objects.filter(
            type="published_rfc", time__gte=five_years_ago, time__lte=date_as_dt
        ).values_list("doc__pk", flat=True)
    )
    iesgappr_pks = set(
        DocEvent.objects.filter(
            type="iesg_approved", time__gte=five_years_ago, time__lte=date_as_dt
        ).values_list("doc__pk", flat=True)
    )
    qualifying_pks = rfc_pks.union(iesgappr_pks.difference(rfc_pks))
    author_qs = (
        base_qs.filter(documentauthor__document__pk__in=qualifying_pks)
        .annotate(document_author_count=Count("documentauthor"))
...
def list_eligible_8989(date, base_qs=None):
    ...
    return remove_disqualified(
        Person.objects.filter(
            pk__in=set(three_of_five_pks).union(set(officer_pks)).union(set(author_pks))
        )
    )
...
def list_eligible_9389(date, base_qs=None):
    if not base_qs:
        base_qs = Person.objects.all()
    three_of_five_qs, officer_qs, author_qs = get_9389_eligibility_querysets(
        date, base_qs
    )
    three_of_five_pks = three_of_five_qs.values_list("pk", flat=True)
    officer_pks = officer_qs.values_list("pk", flat=True)
...
def get_eligibility_date(nomcom=None, date=None):
    if date:
        ...
        last_seated = (
            Role.objects.filter(group__type_id="nomcom", name_id="member")
        ...
        if last_seated:
            ...
            next_nomcom_group = Group.objects.filter(
                acronym=f"nomcom{next_nomcom_year}"
            ).first()
            if (
                next_nomcom_group
                and next_nomcom_group.nomcom_set.first().first_call_for_volunteers
            ):
...
def three_of_five_eligible_8713(previous_five, queryset=None):
    ...
    return (
        queryset.filter(
            meetingregistration__meeting__in=list(previous_five),
            meetingregistration__attended=True,
        )
        .annotate(mtg_count=Count("meetingregistration"))
        .filter(mtg_count__gte=3)
    )
    ...
    if not affiliation:
        recent_draft_revision = (
            NewRevisionDocEvent.objects.filter(
                doc__type_id="draft", doc__documentauthor__person=person
            )
            .order_by("-time")
            .first()
        )
        if recent_draft_revision:
            affiliation = (
                recent_draft_revision.doc.documentauthor_set.filter(person=person)
                .first()
                .affiliation
            )
    return affiliation
...
def extract_volunteers(year):
    ...
    volunteers = sorted(
        volunteers, key=lambda v: (not v.eligible, v.person.last_name())
    )
...
def ingest_feedback_email(message: bytes, year: int):
    ...
    try:
        nomcom = NomCom.objects.get(
            group__acronym__icontains=str(year), group__state__slug="active"
        )
...
def send_reminders():
    ...
    for nomcom in NomCom.objects.filter(group__state__slug="active"):
        ...
        for nominee_position in nps.pending():
            if _is_time_to_send_reminder(
                nomcom, date_today(), nominee_position.time.date()
            ):
                send_accept_reminder_to_nominee(nominee_position)
        ...
        for nominee_position in nps.accepted().without_questionnaire_response():
            if _is_time_to_send_reminder(
                ...
                send_questionnaire_reminder_to_nominee(nominee_position)
                log(
                    f"Sent questionnaire reminder to {nominee_position.nominee.email.address}"
                )
