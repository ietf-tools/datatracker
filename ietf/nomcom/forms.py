...
from ietf.person.fields import (
    SearchableEmailField,
    SearchableEmailsField,
    SearchablePersonField,
    SearchablePersonsField,
)
...
class PositionNomineeField(forms.ChoiceField):
    ...
    def __init__(self, *args, **kwargs):
        ...
        positions = (
            Position.objects.get_by_nomcom(self.nomcom)
            .filter(is_open=True)
            .order_by("name")
        )
        results = []
        for position in positions:
            accepted_nominees = [
                np.nominee
                for np in NomineePosition.objects.filter(
                    position=position, state="accepted"
                ).exclude(nominee__duplicated__isnull=False)
            ]
            nominees = [
                ("%s_%s" % (position.id, i.id), str(i)) for i in accepted_nominees
            ]
            if nominees:
        ...
        for position in positions:
            other_nominees = [
                np.nominee
                for np in NomineePosition.objects.filter(position=position)
...
class MultiplePositionNomineeField(forms.MultipleChoiceField, PositionNomineeField):
    ...
    def clean(self, value):
        nominees = super(PositionNomineeField, self).clean(
            value
        ...
        for nominee in nominees:
            ...
            try:
                ...
                raise forms.ValidationError("Invalid nominee")
            try:
                nominee = position.nominee_set.get_by_nomcom(self.nomcom).get(
                    id=nominee_id
                )
            except Nominee.DoesNotExist:
                raise forms.ValidationError("Invalid nominee")
...
class EditMembersForm(forms.Form):
    ...
    def __init__(self, nomcom, *args, **kwargs):
        ...
        roles = (
            nomcom.group.role_set.filter(name__slug__in=("member", "liaison"))
            .order_by("email__person__name")
            .select_related("email")
        )
        initial["members"] = [r.email for r in roles if r.name.slug == "member"]
        initial["liaisons"] = [r.email for r in roles if r.name.slug == "liaison"]
...
class MergeNomineeForm(forms.Form):
    ...
    def clean_primary_email(self):
        ...
        nominees = (
            Nominee.objects.get_by_nomcom(self.nomcom)
            .not_duplicated()
            .filter(email__address=email)
        )
        if not nominees:
    ...
    def save(self):
        ...
        primary_nominee = Nominee.objects.get_by_nomcom(self.nomcom).get(
            email__address=primary_email
        )
        while primary_nominee.duplicated:
            primary_nominee = primary_nominee.duplicated
        secondary_nominees = Nominee.objects.get_by_nomcom(self.nomcom).filter(
            email__address__in=secondary_emails
        )
        for nominee in secondary_nominees:
            ...
            for nominee_position in nominee.nomineeposition_set.all():
                primary_nominee_positions = NomineePosition.objects.filter(
                    position=nominee_position.position, nominee=primary_nominee
                )
                primary_nominee_position = (
                    primary_nominee_positions and primary_nominee_positions[0] or None
                )
                ...
                if primary_nominee_position:
                    ...
                    if (
                        nominee_position.state.slug == "accepted"
                        or primary_nominee_position.state.slug == "accepted"
                    ):
                        primary_nominee_position.state = (
                            NomineePositionStateName.objects.get(slug="accepted")
                        )
                        primary_nominee_position.save()
                    if (
                        nominee_position.state.slug == "declined"
                        and primary_nominee_position.state.slug == "pending"
                    ):
                        primary_nominee_position.state = (
                            NomineePositionStateName.objects.get(slug="declined")
                        )
                        primary_nominee_position.save()
                else:
...
class MergePersonForm(forms.Form):
    ...
    primary_person = SearchablePersonField(
        help_text="Select the person you want the datatracker to keep"
    )
    duplicate_persons = SearchablePersonsField(
        help_text="Select all the duplicates that should be merged into the primary person record"
    )
...
class NominateForm(forms.ModelForm):
    ...
    qualifications = forms.CharField(
        label="Candidate's qualifications for the position",
        widget=forms.Textarea(),
        strip=False,
    )
    confirmation = forms.BooleanField(
        label="Email comments back to me as confirmation.",
    ...
    def __init__(self, *args, **kwargs):
        ...
        new_person_url_name = "ietf.nomcom.views.%s_nominate_newperson" % (
            "public" if self.public else "private"
        )
        self.fields["searched_email"].label = "Candidate email"
        self.fields["searched_email"].help_text = (
            'Search by name or email address. Click <a href="%s">here</a> if the search does not find the candidate you want to nominate.'
            % reverse(new_person_url_name, kwargs={"year": self.nomcom.year()})
        )
        self.fields["nominator_email"].label = "Nominator email"
        if self.nomcom:
            self.fields["position"].queryset = Position.objects.get_by_nomcom(
                self.nomcom
            ).filter(is_open=True)
            if self.public:
                self.fields["position"].queryset = self.fields[
                    "position"
                ].queryset.filter(accepting_nominations=True)
        ...
        if not self.public:
            ...
            if author:
                ...
                self.fields[
                    "share_nominator"
                ].help_text = """(Nomcom Chair/Member: Check this box if the person providing this nomination
    ...
    def save(self, commit=True):
        ...
        nominee = make_nomineeposition(
            self.nomcom, searched_email.person, position, author
        )
        ...
        if confirmation:
            if author:
                ...
                (to_email, cc) = gather_address_lists(
                    "nomination_receipt_requested", nominator=author.address
                )
                context = {
                    "nominee": nominee.email.person.name,
...
class NominateNewPersonForm(forms.ModelForm):
    ...
    def clean_candidate_email(self):
        ...
        if Email.objects.filter(address=candidate_email).exists():
            normal_url_name = "ietf.nomcom.views.%s_nominate" % (
                "public" if self.public else "private"
            )
            msg = (
                "%s is already in the datatracker. "
                'Use the <a href="%s">normal nomination form</a> to nominate the person '
                "with this address. "
            ) % (
                candidate_email,
                reverse(normal_url_name, kwargs={"year": self.nomcom.year()}),
            )
...
class FeedbackForm(forms.ModelForm):
    ...
    comment_text = forms.CharField(
        label="Comments", widget=forms.Textarea(), strip=False
    )
    confirmation = forms.BooleanField(
        label="Email comments back to me as confirmation (if selected, your comments will be emailed to you in cleartext when you press Save).",
        required=False,
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.public:
            ...
            self.fields["confirmation"].label = (
                "Email these comments in cleartext to the provided commenter email address"
            )
            if author:
                self.fields["nominator_email"].initial = author.address
    ...
    def clean(self):
        if self.nominee and self.position:
            if not NomineePosition.objects.accepted().filter(
                nominee=self.nominee, position=self.position
            ):
                msg = "There isn't a accepted nomination for %s on the %s position" % (
                    self.nominee,
                    self.position,
                )
    ...
    def save(self, commit=True):
        ...
        if confirmation:
            if author:
                ...
                from_email = settings.NOMCOM_FROM_EMAIL.format(year=self.nomcom.year())
                (to_email, cc) = gather_address_lists(
                    "nomcom_comment_receipt_requested", commenter=author.address
                )
                if self.nominee and self.position:
                    about = "%s for the position of\n%s" % (
                        self.nominee.email.person.name,
                        self.position.name,
                    )
                elif self.topic:
...
class FeedbackEmailForm(forms.Form):
    ...
    email_text = forms.CharField(
        label="Email text", widget=forms.Textarea(), strip=False
    )
...
class QuestionnaireForm(forms.ModelForm):
    ...
    comment_text = forms.CharField(
        label="Questionnaire response from this candidate",
        widget=forms.Textarea(),
        strip=False,
    )
...
class PrivateKeyForm(forms.Form):
    ...
    key = forms.CharField(
        label="Private key", widget=forms.Textarea(), required=False, strip=False
    )
...
class MutableFeedbackForm(forms.ModelForm):
    ...
    type = forms.ModelChoiceField(
        queryset=FeedbackTypeName.objects.all(), widget=forms.HiddenInput
    )
    ...
    def set_nomcom(self, nomcom, person, instances=None):
        ...
        for i in instances:
            if i.id == self.instance.id:
                ...
                break
        self.feedback_type = self.feedback_type or self.fields["type"].clean(
            self.fields["type"].widget.value_from_datadict(
                self.data, self.files, self.add_prefix("type")
            )
        )
        ...
        if self.feedback_type.slug != "nomina":
            self.fields["nominee"] = MultiplePositionNomineeField(
                nomcom=self.nomcom,
                widget=forms.SelectMultiple(
                    attrs={"class": "nominee_multi_select", "size": "12"}
                ),
                required=self.feedback_type.slug != "comment",
                help_text='Hold down "Control", or "Command" on a Mac, to select more than one.',
            )
            if self.feedback_type.slug == "comment":
                self.fields["topic"] = ModelMultipleChoiceField(
                    queryset=self.nomcom.topic_set.all(),
                    help_text='Hold down "Control" or "Command" on a Mac, to select more than one.',
                    required=False,
                )
        else:
            self.fields["position"] = forms.ModelChoiceField(
                queryset=Position.objects.get_by_nomcom(self.nomcom).filter(
                    is_open=True
                ),
                label="Position",
            )
            self.fields["searched_email"] = SearchableEmailField(
                only_users=False,
                help_text="Try to find the candidate you are classifying with this field first. Only use the name and email fields below if this search does not find the candidate.",
                label="Candidate",
                required=False,
            )
            self.fields["candidate_name"] = forms.CharField(
                label="Candidate name",
                help_text="Only fill in this name field if the search doesn't find the person you are classifying",
                required=False,
            )
            self.fields["candidate_email"] = forms.EmailField(
                label="Candidate email",
                help_text="Only fill in this email field if the search doesn't find the person you are classifying",
                required=False,
            )
            self.fields["candidate_phone"] = forms.CharField(
                label="Candidate phone", required=False
            )
    ...
    def clean(self):
        ...
        if self.feedback_type.slug == "nomina":
            ...
            if not any(
                [
                    searched_email and not candidate_name and not candidate_email,
                    not searched_email and candidate_name and candidate_email,
                ]
            ):
                raise forms.ValidationError(
                    "You must identify either an existing person (by searching with the candidate field) and leave the name and email fields blank, or leave the search field blank and provide both a name and email address."
                )
            if (
                candidate_email
                and Email.objects.filter(address=candidate_email).exists()
            ):
                raise forms.ValidationError(
                    "%s already exists in the datatracker. Please search within the candidate field to find it and leave both the name and email fields blank."
                    % candidate_email
                )
            ...
            if not (nominees or topics):
                raise forms.ValidationError(
                    "You must choose at least one Nominee or Topic to associate with this comment"
                )
        return cleaned_data
...
class EditNomineeForm(forms.ModelForm):
    ...
    nominee_email = forms.ModelChoiceField(
        queryset=Email.objects.none(), empty_label=None
    )

    def __init__(self, *args, **kwargs):
        super(EditNomineeForm, self).__init__(*args, **kwargs)
        self.fields["nominee_email"].queryset = Email.objects.filter(
            person=self.instance.person, active=True
        )
        self.fields["nominee_email"].initial = self.instance.email
        self.fields["nominee_email"].help_text = (
            "If the address you are looking for does not appear in this list, ask the nominee (or the secretariat) to add the address to their datatracker account and ensure it is marked as active."
        )

    def clean(self):
        nominee_email = self.cleaned_data.get("nominee_email")
        others = Nominee.objects.filter(
            email=nominee_email, nomcom=self.instance.nomcom
        )
        if others.exists():
            msg = (
                "Changing email address for %s (#%s): There already exists a nominee "
                ...
                % (
                    ...
                    reverse(
                        ...
                        kwargs={"year": self.instance.nomcom.year()},
                    ),
                )
            )
            raise forms.ValidationError(mark_safe(msg))
...
class NominationResponseCommentForm(forms.Form):
    comments = forms.CharField(
        widget=forms.Textarea,
        required=False,
        help_text="Any comments provided will be encrypted and will only be visible to the NomCom.",
        strip=False,
    )
...
class VolunteerForm(forms.ModelForm):
    ...
    nomcoms = NomcomVolunteerMultipleChoiceField(
        queryset=NomCom.objects.none(), widget=forms.CheckboxSelectMultiple
    )
    field_order = ("nomcoms", "affiliation")

    def __init__(self, person, *args, **kargs):
        super().__init__(*args, **kargs)
        self.fields["nomcoms"].queryset = NomCom.objects.filter(
            is_accepting_volunteers=True
        ).exclude(volunteer__person=person)
        self.fields["nomcoms"].help_text = (
            "You may volunteer even if the datatracker does not currently calculate that you are eligible. Eligibility will be assessed when the selection process is performed."
        )
        self.fields["affiliation"].help_text = (
            "Affiliation to show in the volunteer list"
        )
