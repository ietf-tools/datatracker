...
from ietf.nomcom.forms import (
    NominateForm,
    NominateNewPersonForm,
    FeedbackForm,
    QuestionnaireForm,
    MergeNomineeForm,
    MergePersonForm,
...
def year_index(request, year):
    ...
    return render(
        request,
        "nomcom/year_index.html",
        {"nomcom": nomcom, "year": year, "template": template},
    )
...
def announcements(request):
    ...
    for n in nomcoms:
        e = GroupEvent.objects.filter(
            group=n, type="changed_state", changestategroupevent__state="active"
        ).order_by("time")[:1]
        n.start_year = e[0].time.year if e else 0
        e = GroupEvent.objects.filter(
            group=n, type="changed_state", changestategroupevent__state="conclude"
        ).order_by("time")[:1]
        ...
        regimes.append(dict(chair=chair, announcements=announcements, group=n))

    regimes.sort(key=lambda x: x["group"].start_year, reverse=True)

    return render(
        request,
        "nomcom/announcements.html",
        {
...
def history(request):
    ...
    for nomcom in nomcom_list:
        ...
        if year > 2012:
            personnel = {}
            for r in (
                Role.objects.filter(group=nomcom)
            ...
            regimes.append(
                dict(year=year, label="%s/%s" % (year, year + 1), nomcom=nomcom)
            )

    regimes.sort(key=lambda x: x["year"], reverse=True)

    return render(
        request, "nomcom/history.html", {"nomcom_list": nomcom_list, "regimes": regimes}
    )
...
def private_key(request, year):
    ...
    back_url = request.GET.get(
        "back_to", reverse("ietf.nomcom.views.private_index", None, args=(year,))
    )
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                store_nomcom_private_key(
                    request, year, force_bytes(form.cleaned_data.get("key", ""))
                )
            except UnicodeError:
                form.add_error(
                    None,
    ...
    if request.session.get("NOMCOM_PRIVATE_KEY_%s" % year, None):
        messages.warning(
            request, "You already have a private decryption key set for this session."
        )
    else:
        messages.warning(
            request, "You don't have a private decryption key set for this session yet"
        )

    return render(
        request,
        "nomcom/private_key.html",
        {"nomcom": nomcom, "year": year, "back_url": back_url, "form": form},
    )
...
def private_index(request, year):
    nomcom = get_nomcom_by_year(year)
    all_nominee_positions = NomineePosition.objects.get_by_nomcom(
        nomcom
    ).not_duplicated()
    ...
    if is_chair and request.method == "POST":
        if nomcom.group.state_id != "active":
            messages.warning(
                request,
                "This nomcom is not active. Request administrative assistance if Nominee state needs to change.",
            )
        else:
            ...
            if nominations_to_modify:
                ...
                if action == "set_as_accepted":
                    ...
                    messages.success(
                        request, "The selected nominations have been set as accepted"
                    )
                elif action == "set_as_declined":
                    nominations.update(state="declined")
                    messages.success(
                        request, "The selected nominations have been set as declined"
                    )
                elif action == "set_as_pending":
                    nominations.update(state="pending")
                    messages.success(
                        request, "The selected nominations have been set as pending"
                    )
                elif action == "email":
                    mailto = ",".join(
                        [np.nominee.email.email_address() for np in nominations]
                    )
            else:
                messages.warning(
                    request, "Please, select some nominations to work with"
                )
    ...
    states = [
        {"slug": questionnaire_state, "name": "Accepted and sent Questionnaire"},
        {"slug": not_declined_state, "name": "Not declined"},
    ] + list(NomineePositionStateName.objects.values("slug", "name"))
    positions = set(
        [n.position for n in all_nominee_positions.order_by("position__name")]
    )
    for s in stats:
        for state in states:
            if state["slug"] == questionnaire_state:
                s[state["slug"]] = Feedback.objects.filter(
                    positions__id=s["position__id"], type="questio"
                ).count()
            else:
                s[state["slug"]] = all_nominee_positions.filter(
                    position__name=s["position__name"], state=state["slug"]
                ).count()
        s["nominations"] = Feedback.objects.filter(
            positions__id=s["position__id"], type="nomina"
        ).count()
        s["nominees"] = all_nominee_positions.filter(
            position__name=s["position__name"]
        ).count()
        s["comments"] = Feedback.objects.filter(
            positions__id=s["position__id"], type="comment"
        ).count()

    totals = dict()
    totals["nominations"] = Feedback.objects.filter(
        nomcom=nomcom, type="nomina"
    ).count()
    totals["nominees"] = all_nominee_positions.count()
    for state in states:
        if state["slug"] == questionnaire_state:
            totals[state["slug"]] = Feedback.objects.filter(
                nomcom=nomcom, type="questio"
            ).count()
        else:
            totals[state["slug"]] = all_nominee_positions.filter(
                state=state["slug"]
            ).count()
    totals["comments"] = Feedback.objects.filter(
        nomcom=nomcom, type="comment", positions__isnull=False
    ).count()
    totals["open"] = nomcom.position_set.filter(is_open=True).count()
    totals["accepting_nominations"] = nomcom.position_set.filter(
        accepting_nominations=True
    ).count()
    totals["accepting_feedback"] = nomcom.position_set.filter(
        accepting_feedback=True
    ).count()

    unique_totals = dict()
    unique_totals["nominees"] = (
        Person.objects.filter(nominee__nomcom=nomcom).distinct().count()
    )
    for state in states:
        if state["slug"] != questionnaire_state:
            unique_totals[state["slug"]] = len(
                set(
                    all_nominee_positions.filter(state=state["slug"]).values_list(
                        "nominee__person", flat=True
                    )
                )
            )
...
def send_reminder_mail(request, year, type):
    ...
    if type == "accept":
        ...
        state_description = NomineePositionStateName.objects.get(
            slug=interesting_state
        ).name
        ...
        state_description = (
            NomineePositionStateName.objects.get(slug=interesting_state).name
            + " but no questionnaire has been received"
        )
    else:
        raise Http404

    nominees = (
        Nominee.objects.get_by_nomcom(nomcom)
        .not_duplicated()
        .filter(nomineeposition__state=interesting_state)
        .distinct()
    )
    annotated_nominees = []
    for nominee in nominees:
        if type == "accept":
            nominee.interesting_positions = [
                x.position.name for x in nominee.nomineeposition_set.pending()
            ]
        else:
            nominee.interesting_positions = [
                x.position.name
    ...
    if request.method == "POST" and nomcom_ready:
        ...
        if selected_nominees:
            ...
            if addrs:
                messages.success(
                    request,
                    'A copy of "%s" has been sent to %s'
                    % (mail_template.title, ", ".join(addrs)),
                )
            else:
    ...
    return render(
        request,
        "nomcom/send_reminder_mail.html",
        {
            "nomcom": nomcom,
            "year": year,
...
def private_merge_person(request, year):
    ...
    if nomcom.group.state_id != "active":
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                form.save()
                messages.success(
                    request, "A merge request has been sent to the secretariat."
                )
    ...
    return render(
        request,
        "nomcom/private_merge_person.html",
        {
            "nomcom": nomcom,
            "year": year,
            "form": form,
...
def private_merge_nominee(request, year):
    ...
    return render(
        request,
        "nomcom/private_merge_nominee.html",
        {
            "nomcom": nomcom,
            "year": year,
            "form": form,
...
def requirements(request, year):
    ...
    return render(
        request,
        "nomcom/requirements.html",
        {"nomcom": nomcom, "positions": positions, "year": year},
    )
...
def nominate(request, year, public, newperson):
    ...
    if not has_publickey:
        ...
        return render(request, template, {"nomcom": nomcom, "year": year})
    ...
    if request.method == "POST":
        if newperson:
            form = NominateNewPersonForm(
                data=request.POST, nomcom=nomcom, person=person, public=public
            )
        else:
            form = NominateForm(
                data=request.POST, nomcom=nomcom, person=person, public=public
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                "Your nomination has been registered. Thank you for the nomination.",
            )
            if newperson:
                return redirect(
                    "ietf.nomcom.views.%s_nominate"
...
def feedback(request, year, public):
    ...
    if nomcom.group.state_id != "conclude":
        ...
        if selected_topic:
            ...
            if topic.audience_id == "nomcom" and not nomcom.group.has_role(
                request.user, ["chair", "advisor", "liaison", "member"]
            ):
                raise Http404()
            if (
                topic.audience_id == "nominees"
                and not nomcom.nominee_set.filter(person=person).exists()
            ):
                raise Http404()

    if public:
        positions = Position.objects.get_by_nomcom(nomcom=nomcom).filter(
            is_open=True, accepting_feedback=True
        )
    ...
    user_comments = Feedback.objects.filter(
        nomcom=nomcom, type="comment", author__in=person.email_set.filter(active="True")
    )
    ...
    if public and position and not (position.is_open and position.accepting_feedback):
        messages.warning(
            request,
            "This Nomcom is not currently accepting feedback for " + position.name,
        )
        return render(
            request,
            "nomcom/feedback.html",
    ...
    if public and topic and not topic.accepting_feedback:
        messages.warning(
            request,
            "This Nomcom is not currently accepting feedback for " + topic.subject,
        )
        return render(
            request,
            "nomcom/feedback.html",
    ...
    if request.method == "POST":
        ...
        if nominee and position:
            ...
            form = FeedbackForm(
                nomcom=nomcom, person=person, public=public, topic=topic
            )
        else:
            form = None
...
def private_feedback_email(request, year):
    ...
    if not has_publickey:
        ...
        messages.warning(
            request, "This Nomcom is not active, and is not accepting feedback email."
        )
        nomcom_ready = False
    else:
        nomcom_ready = True
    ...
    if request.method == "POST":
        form = FeedbackEmailForm(data=request.POST, nomcom=nomcom)
        if form.is_valid():
            try:
                form.save()
                form = FeedbackEmailForm(nomcom=nomcom)
    ...
    return render(request, template, {"form": form, "nomcom": nomcom, "year": year})
...
def private_questionnaire(request, year):
    ...
    if not has_publickey:
        ...
        messages.warning(
            request, "This Nomcom is not active, and is not accepting questionnaires."
        )
        nomcom_ready = False
    else:
        nomcom_ready = True
    ...
    if request.method == "POST":
        form = QuestionnaireForm(data=request.POST, nomcom=nomcom, person=person)
        if form.is_valid():
            form.save()
...
def process_nomination_status(request, year, nominee_position_id, state, date, hash):
    valid = hmac.compare_digest(
        get_hash_nominee_position(date, nominee_position_id), hash
    )
    if not valid:
    ...
    if nominee_position.state.slug != "pending":
        permission_denied(
            request, "The nomination already was %s" % nominee_position.state
        )

    state = get_object_or_404(NomineePositionStateName, slug=state)
    messages.info(
        request,
        "Click on 'Save' to set the state of your nomination to %s to %s (this is not a final commitment - you can notify us later if you need to change this)."
        ...
        if form.is_valid():
            ...
            if form.cleaned_data["comments"]:
                ...
                who = (
                    ...
                    else request.user.person
                )
                f = Feedback.objects.create(
                    nomcom=nomcom,
                    author=nominee_position.nominee.email,
                    subject="%s nomination %s"
                    % (nominee_position.nominee.name(), state),
            ...
            messages.success(
                request,
                "Your nomination on %s has been set as %s"
                % (nominee_position.position.name, state.name),
            )
    else:
    ...
    return render(
        request,
        "nomcom/process_nomination_status.html",
        {
            "nomcom": nomcom,
            "year": year,
...
def view_feedback(request, year):
    ...
    for nominee in sorted_nominees:
        last_seen = FeedbackLastSeen.objects.filter(
            reviewer=reviewer, nominee=nominee
        ).first()
        nominee_feedback = []
    ...
    independent_feedback = [
        ft.feedback_set.get_by_nomcom(nomcom).count()
        for ft in independent_feedback_types
    ]
    for topic in nomcom.topic_set.all():
        last_seen = TopicFeedbackLastSeen.objects.filter(
            reviewer=reviewer, topic=topic
        ).first()
        topic_feedback = []
...
def view_feedback_pending(request, year):
    ...
    if nomcom.group.state_id == "conclude":
        permission_denied(request, "This nomcom is concluded.")
    extra_ids = None
    FeedbackFormSet = modelformset_factory(Feedback, form=PendingFeedbackForm, extra=0)
    feedback_list = Feedback.objects.filter(type__isnull=True, nomcom=nomcom).order_by(
        "-time"
    )
    ...
    if request.method == "POST" and request.POST.get("end"):
        ...
        if formset.is_valid():
            ...
            if extra_ids:
                ...
                formset = FullFeedbackFormSet(
                    queryset=Feedback.objects.filter(id__in=[i.id for i in extra])
                )
                for form in formset.forms:
        ...
        if formset.is_valid():
            ...
            for form in formset.forms:
                if (
                    form.instance.type
                    and form.instance.type.slug in settings.NOMINEE_FEEDBACK_TYPES
                ):
            ...
            if extra or nominations:
                ...
                if nominations:
                    formset = FullFeedbackFormSet(
                        queryset=Feedback.objects.filter(
                            id__in=[i.id for i in nominations]
                        )
                    )
                    for form in formset.forms:
                ...
                if moved:
                    messages.success(
                        request,
                        "%s messages classified. You must enter more information for the following feedback."
                        % moved,
                    )
            else:
                messages.success(request, "Feedback saved")
    ...
    return render(
        request,
        "nomcom/view_feedback_pending.html",
        {
            "year": year,
            "formset": formset,
...
def view_feedback_unrelated(request, year):
    ...
    if request.method == "POST":
        if not nomcom.group.has_role(request.user, ["chair", "advisor"]):
            return HttpResponseForbidden(
                "Restricted to roles: Nomcom Chair, Nomcom Advisor"
            )
        ...
        if type:
            if type == "unclassified":
                ...
                messages.success(
                    request,
                    "The selected feedback has been de-classified. Please reclassify it in the Pending emails tab.",
                )
            else:
                feedback.type = FeedbackTypeName.objects.get(slug=type)
                messages.success(
                    request,
                    f"The selected feedback has been reclassified as {feedback.type.name}.",
                )
            feedback.save()
        else:
            return render(
                request,
                "nomcom/view_feedback_unrelated.html",
                {
                    "year": year,
                    "nomcom": nomcom,
                    "feedback_types": FeedbackTypeName.objects.filter(
                        used=True
                    ).exclude(slug__in=settings.NOMINEE_FEEDBACK_TYPES),
    ...
    feedback_types = []
    for ft in FeedbackTypeName.objects.filter(used=True).exclude(
        slug__in=settings.NOMINEE_FEEDBACK_TYPES
    ):
        feedback_types.append(
            {"ft": ft, "feedback": ft.feedback_set.get_by_nomcom(nomcom)}
        )
    return render(
        request,
        "nomcom/view_feedback_unrelated.html",
...
def view_feedback_topic(request, year, topic_id):
    ...
    last_seen_time = (last_seen and last_seen.time) or datetime.datetime(
        year=1, month=1, day=1, tzinfo=datetime.timezone.utc
    )
    if last_seen:
        last_seen.save()
    else:
    ...
    return render(
        request,
        "nomcom/view_feedback_topic.html",
        {
            "year": year,
            "topic": topic,
            "feedback_types": feedback_types,
...
def view_feedback_nominee(request, year, nominee_id):
    ...
    feedback_types = FeedbackTypeName.objects.filter(
        used=True, slug__in=settings.NOMINEE_FEEDBACK_TYPES
    )
    reviewer = request.user.person
    if request.method == "POST":
        if not nomcom.group.has_role(request.user, ["chair", "advisor"]):
        ...
        if submit == "download":
            ...
            if type:
                ...
                return render(
                    request,
                    "nomcom/view_feedback_nominee.html",
                    {
                        "year": year,
                        "nomcom": nomcom,
                        "feedback_types": feedback_types,
...
def edit_nominee(request, year, nominee_id):
    ...
    if request.method == "POST":
        form = EditNomineeForm(request.POST, instance=nominee)
        if form.is_valid():
            form.save()
            messages.success(
                request,
                "The nomination address for %s has been changed to %s"
                % (nominee.name(), nominee.email.address),
            )
            return redirect("ietf.nomcom.views.private_index", year=year)
    else:
...
def edit_nomcom(request, year):
    ...
    ReminderDateInlineFormSet = inlineformset_factory(
        parent_model=NomCom, model=ReminderDates, form=ReminderDatesForm
    )
    if request.method == "POST":
        ...
        form = EditNomcomForm(request.POST, request.FILES, instance=nomcom)
...
def list_templates(request, year):
    nomcom = get_nomcom_by_year(year)
    template_list = (
        DBTemplate.objects.filter(group=nomcom.group)
        .exclude(path__contains="/position/")
        .exclude(path__contains="/topic/")
    )

    return render(
        request,
        "nomcom/list_templates.html",
        {
...
def edit_template(request, year, template_id):
    ...
    if nomcom.group.state_id == "conclude":
        return group_template_show(
            request,
            nomcom.group.acronym,
            template_id,
...
def list_positions(request, year):
    ...
    if request.method == "POST":
        if nomcom.group.state_id != "active":
            messages.warning(
                request,
                "This nomcom is not active. Request administrative assistance if Position state needs to change.",
            )
        else:
            action = request.POST.get("action")
            ...
            if positions_to_modify:
                ...
                if action == "set_iesg":
                    ...
                    messages.success(
                        request,
                        "The selected positions have been set as IESG Positions",
                    )
                elif action == "unset_iesg":
                    positions.update(is_iesg_position=False)
                    messages.success(
                        request,
                        "The selected positions have been set as NOT IESG Positions",
                    )
                elif action == "set_open":
                    positions.update(is_open=True)
                    messages.success(
                        request, "The selected positions have been set as Open"
                    )
                elif action == "unset_open":
                    positions.update(is_open=False)
                    messages.success(
                        request, "The selected positions have been set as NOT Open"
                    )
                elif action == "set_accept_nom":
                    positions.update(accepting_nominations=True)
                    messages.success(
                        request,
                        "The selected positions have been set as Accepting Nominations",
                    )
                elif action == "unset_accept_nom":
                    positions.update(accepting_nominations=False)
                    messages.success(
                        request,
                        "The selected positions have been set as NOT Accepting Nominations",
                    )
                elif action == "set_accept_fb":
                    positions.update(accepting_feedback=True)
                    messages.success(
                        request,
                        "The selected positions have been set as Accepting Feedback",
                    )
                elif action == "unset_accept_fb":
                    positions.update(accepting_feedback=False)
                    messages.success(
                        request,
                        "The selected positions have been set as NOT Accepting Feedback",
                    )
                positions = nomcom.position_set.order_by("-is_open")
            else:
...
def edit_topic(request, year, topic_id=None):
    ...
    if request.method == "POST":
        ...
        form = TopicForm(
            ...
            nomcom=nomcom,
            initial=(
                {"accepting_feedback": True, "audience": "general"} if not topic else {}
            ),
        )

    return render(
        request,
        "nomcom/edit_topic.html",
        {
            "form": form,
            "topic": topic,
            "year": year,
...
def edit_members(request, year):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            update_role_set(
                nomcom.group, "member", form.cleaned_data["members"], person
            )
            update_role_set(
                nomcom.group, "liaison", form.cleaned_data["liaisons"], person
            )
            return HttpResponseRedirect(
                reverse("ietf.nomcom.views.private_index", kwargs={"year": year})
            )
    else:
    ...
    return render(
        request,
        "nomcom/new_edit_members.html",
        {"nomcom": nomcom, "year": year, "form": form},
    )
...
def extract_email_lists(request, year):
    ...
    noresp = [
        n
        for n in accepted
        if n.nomineeposition_set.without_questionnaire_response().filter(
            state="accepted"
        )
    ]
    ...
    for pos in nomcom.position_set.all():
        bypos[pos] = nomcom.nominee_set.filter(
            nomineeposition__position=pos, nomineeposition__state="accepted"
        ).distinct()

    return render(
        request,
        "nomcom/extract_email_lists.html",
        {
            "nomcom": nomcom,
            "year": year,
            "pending": pending,
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            for nc in form.cleaned_data["nomcoms"]:
                nc.volunteer_set.get_or_create(
                    person=person,
                    defaults={
        ...
        form = VolunteerForm(
            person=person,
            initial=dict(
                nomcoms=can_volunteer, affiliation=suggest_affiliation(person)
            ),
        )
    return render(
        request,
        "nomcom/volunteer.html",
        {
            "form": form,
...
def volunteers(request, year, public=False):
    ...
    return render(
        request,
        "nomcom/volunteers.html",
        dict(year=year, nomcom=nomcom, volunteers=volunteers, public=public),
    )
...
def private_volunteers_csv(request, year, public=False):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="nomcom{year}_volunteers.csv"'
    )
...
def qualified_volunteer_list_for_announcement(request, year, public=False):
    ...
    return render(
        request,
        "nomcom/qualified_volunteer_list_for_announcement.txt",
