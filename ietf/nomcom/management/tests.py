...
class FeedbackEmailTests(TestCase):
    ...
    def test_send_error_to_admins(self, send_smtp_mock):
        ...
        with name_of_file_containing("feedback message") as filename:
            call_command(
                "feedback_email", nomcom_year=self.year + 1, email_file=filename
            )

        self.assertTrue(send_smtp_mock.called)
        (msg,) = send_smtp_mock.call_args.args  # get the message to be sent
        self.assertEqual(
            msg["to"], "admin@example.com", "Email recipient should be the admins"
        )
        self.assertIn("error", msg["subject"], "Email subject should indicate error")
        self.assertFalse(
            msg.is_multipart(),
            "Nomcom feedback error sent to admin should not have attachments",
        )
        ...
        self.assertIn(
            "feedback_email.py",
            content,
            "Admin email should contain file where error occurred",
        )
        self.assertNotIn(
            "traceback", content.lower(), "Admin email should not contain traceback"
        )
        self.assertNotIn(
            f"NomCom {self.year} does not exist",
            content,
    ...
    def test_send_error_to_chair(self, create_feedback_mock, send_smtp_mock):
        ...
        self.assertIn("error", msg["subject"], "Email subject should indicate error")
        self.assertTrue(
            msg.is_multipart(), "Chair feedback error should have attachments"
        )
        ...
        self.assertIn(
            "mock.py", content, "Nomcom email should contain file where error occurred"
        )
        self.assertIn(
            "feedback message",
            attachment,
            "Nomcom email should include original message",
        )
    ...
    def test_feedback_email(self, create_feedback_mock):
        ...
        create_feedback_mock.return_value = namedtuple("mock_feedback", "author")(
            "author@example.com"
        )
        ...
        self.assertEqual(
            create_feedback_mock.call_count,
            1,
            "create_feedback_email() should be called once",
        )
        self.assertEqual(
    ...
    def test_invalid_character_encodings(self, send_smtp_mock):
        ...
        parts = msg.get_payload()
        self.assertEqual(
            len(parts), 2, "Error email should contain message and original message"
        )

    @mock.patch.object(sys.stdin.buffer, "read")
    @mock.patch("ietf.utils.management.base.send_smtp")
    def test_invalid_character_encodings_via_stdin(
        self, send_smtp_mock, stdin_read_mock
    ):
