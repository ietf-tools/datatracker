...
from ietf.nomcom.test_data import (
    nomcom_test_data,
    generate_cert,
    check_comments,
    COMMUNITY_USER,
...
from ietf.nomcom.factories import (
    NomComFactory,
    FeedbackFactory,
    TopicFactory,
    nomcom_kwargs_for_year,
    provide_private_key_to_test_client,
    key,
)
...
class NomcomViewsTest(TestCase):
    ...
    def setUp(self):
        ...
        # private urls
        self.private_index_url = reverse(
            "ietf.nomcom.views.private_index", kwargs={"year": self.year}
        )
        self.private_merge_person_url = reverse(
            "ietf.nomcom.views.private_merge_person", kwargs={"year": self.year}
        )
        self.private_merge_nominee_url = reverse(
            "ietf.nomcom.views.private_merge_nominee", kwargs={"year": self.year}
        )
        self.edit_members_url = reverse(
            "ietf.nomcom.views.edit_members", kwargs={"year": self.year}
        )
        self.edit_nomcom_url = reverse(
            "ietf.nomcom.views.edit_nomcom", kwargs={"year": self.year}
        )
        self.private_nominate_url = reverse(
            "ietf.nomcom.views.private_nominate", kwargs={"year": self.year}
        )
        self.private_nominate_newperson_url = reverse(
            "ietf.nomcom.views.private_nominate_newperson", kwargs={"year": self.year}
        )
        self.add_questionnaire_url = reverse(
            "ietf.nomcom.views.private_questionnaire", kwargs={"year": self.year}
        )
        self.private_feedback_url = reverse(
            "ietf.nomcom.views.private_feedback", kwargs={"year": self.year}
        )
        self.private_feedback_email_url = reverse(
            "ietf.nomcom.views.private_feedback_email", kwargs={"year": self.year}
        )
        self.positions_url = reverse(
            "ietf.nomcom.views.list_positions", kwargs={"year": self.year}
        )
        self.edit_position_url = reverse(
            "ietf.nomcom.views.edit_position", kwargs={"year": self.year}
        )

        # public urls
        self.index_url = reverse(
            "ietf.nomcom.views.year_index", kwargs={"year": self.year}
        )
        self.history_url = reverse("ietf.nomcom.views.history")
        self.requirements_url = reverse(
            "ietf.nomcom.views.requirements", kwargs={"year": self.year}
        )
        self.questionnaires_url = reverse(
            "ietf.nomcom.views.questionnaires", kwargs={"year": self.year}
        )
        self.public_feedback_url = reverse(
            "ietf.nomcom.views.public_feedback", kwargs={"year": self.year}
        )
        self.public_nominate_url = reverse(
            "ietf.nomcom.views.public_nominate", kwargs={"year": self.year}
        )
        self.public_nominate_newperson_url = reverse(
            "ietf.nomcom.views.public_nominate_newperson", kwargs={"year": self.year}
        )
    ...
    def test_private_index_view(self):
        ...
        self.assertTrue(
            row_q('a[href="%s"]' % (person_url)),
            "Nominee table does not link to nominee profile page",
        )
        self.assertTrue(
            row_q('a[href="%s#comment"]' % (feedback_url)),
    ...
    def test_private_index_post_accept(self):
        ...
        test_data = {"action": "set_as_accepted", "selected": [nom_pos.pk]}
    ...
    def test_private_index_post_decline(self):
        nom_pos = self.create_nominees_for_states("pending")
        login_testing_unauthorized(self, CHAIR_USER, self.private_index_url)
        test_data = {"action": "set_as_declined", "selected": [nom_pos.pk]}
        r = self.client.post(self.private_index_url, test_data)
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
        self.assertTrue(q(".alert-success"))
    ...
    def test_private_index_post_pending(self):
        ...
        login_testing_unauthorized(self, CHAIR_USER, self.private_index_url)
        test_data = {"action": "set_as_pending", "selected": [nom_pos.pk]}
        r = self.client.post(self.private_index_url, test_data)
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
        self.assertTrue(q(".alert-success"))
    ...
    def test_private_merge_view(self):
        ...
        self.nominate_view(public=True, nominee_email=nominees[0], position="IAOC")
        self.nominate_view(public=True, nominee_email=nominees[0], position="IAOC")
        self.nominate_view(public=True, nominee_email=nominees[0], position="IAB")
        self.nominate_view(public=True, nominee_email=nominees[0], position="TSV")
        self.nominate_view(public=True, nominee_email=nominees[1], position="IAOC")
        self.nominate_view(public=True, nominee_email=nominees[1], position="IAOC")
        self.nominate_view(public=True, nominee_email=nominees[2], position="IAB")
        self.nominate_view(public=True, nominee_email=nominees[2], position="IAB")
        self.nominate_view(public=True, nominee_email=nominees[3], position="TSV")
        self.nominate_view(public=True, nominee_email=nominees[3], position="TSV")
        ...
        nominee_position = NomineePosition.objects.get(
            position__name="IAOC", nominee__email__address=nominees[0]
        )
        nominee_position.state = NomineePositionStateName.objects.get(slug="accepted")
        nominee_position.save()

        nominee_position = NomineePosition.objects.get(
            position__name="IAOC", nominee__email__address=nominees[1]
        )
        nominee_position.state = NomineePositionStateName.objects.get(slug="declined")
        nominee_position.save()

        nominee_position = NomineePosition.objects.get(
            position__name="IAB", nominee__email__address=nominees[2]
        )
        nominee_position.state = NomineePositionStateName.objects.get(slug="declined")
        nominee_position.save()

        nominee_position = NomineePosition.objects.get(
            position__name="TSV", nominee__email__address=nominees[3]
        )
        nominee_position.state = NomineePositionStateName.objects.get(slug="accepted")
        nominee_position.save()
        ...
        self.add_questionnaire(public=False, nominee_email=nominees[0], position="IAOC")
        self.add_questionnaire(public=False, nominee_email=nominees[1], position="IAOC")
        self.add_questionnaire(public=False, nominee_email=nominees[2], position="IAB")
        self.add_questionnaire(public=False, nominee_email=nominees[3], position="TSV")
        ...
        self.feedback_view(public=False, nominee_email=nominees[0], position="IAOC")
        self.feedback_view(public=False, nominee_email=nominees[1], position="IAOC")
        self.feedback_view(public=False, nominee_email=nominees[2], position="IAB")
        self.feedback_view(public=False, nominee_email=nominees[3], position="TSV")

        self.assertEqual(Feedback.objects.comments().count(), 4)
        self.assertEqual(Feedback.objects.nominations().count(), 18)
        self.assertEqual(
            Feedback.objects.nominations()
            .filter(nominees__email__address=nominees[0])
            .count(),
            6,
        )
        self.assertEqual(
            Feedback.objects.nominations()
            .filter(nominees__email__address=nominees[1])
            .count(),
            4,
        )
        self.assertEqual(
            Feedback.objects.nominations()
            .filter(nominees__email__address=nominees[2])
            .count(),
            4,
        )
        self.assertEqual(
            Feedback.objects.nominations()
            .filter(nominees__email__address=nominees[3])
            .count(),
            4,
        )
        for nominee in nominees:
            self.assertEqual(
                Feedback.objects.comments()
                .filter(nominees__email__address=nominee)
                .count(),
                1,
            )
            self.assertEqual(
                Feedback.objects.questionnaires()
                .filter(nominees__email__address=nominee)
                .count(),
                1,
            )
        ...
        test_data = {
            "secondary_emails": "%s, %s" % (nominees[0], nominees[1]),
        ...
        test_data = {"primary_email": nominees[0], "secondary_emails": ""}
        response = self.client.post(self.private_merge_nominee_url, test_data)
        self.assertEqual(response.status_code, 200)
        q = PyQuery(response.content)
        self.assertTrue(q("form .is-invalid"))

        test_data = {"primary_email": "", "secondary_emails": nominees[0]}
        response = self.client.post(self.private_merge_nominee_url, test_data)
        self.assertEqual(response.status_code, 200)
        q = PyQuery(response.content)
        self.assertTrue(q("form .is-invalid"))
        ...
        test_data = {
            "secondary_emails": [nominees[1], nominees[2], nominees[3]],
            "primary_email": nominees[0],
        }
        ...
        self.assertEqual(
            redirect_path,
            reverse("ietf.nomcom.views.private_index", kwargs={"year": NOMCOM_YEAR}),
        )
        ...
        self.assertEqual(
            Nominee.objects.filter(
                email__address=nominees[1], duplicated__isnull=False
            ).count(),
            1,
        )
        self.assertEqual(
            Nominee.objects.filter(
                email__address=nominees[2], duplicated__isnull=False
            ).count(),
            1,
        )
        self.assertEqual(
            Nominee.objects.filter(
                email__address=nominees[3], duplicated__isnull=False
            ).count(),
            1,
        )
        ...
        self.assertEqual(
            Feedback.objects.nominations().filter(nominees__in=[nominee]).count(), 18
        )
        self.assertEqual(
            Feedback.objects.comments().filter(nominees__in=[nominee]).count(), 4
        )
        self.assertEqual(
            Feedback.objects.questionnaires().filter(nominees__in=[nominee]).count(), 4
        )

        for nominee_email in nominees[1:]:
            self.assertEqual(
                Feedback.objects.nominations()
                .filter(nominees__email__address=nominee_email)
                .count(),
                0,
            )
            self.assertEqual(
        ...
        self.assertEqual(
            NomineePosition.objects.get(
                position__name="TSV", nominee=nominee
            ).state.slug,
            "accepted",
        )
        self.assertEqual(
            NomineePosition.objects.get(
                position__name="IAOC", nominee=nominee
            ).state.slug,
            "accepted",
        )
        self.assertEqual(
            NomineePosition.objects.get(
                position__name="IAB", nominee=nominee
            ).state.slug,
            "declined",
        )
    ...
    def test_edit_members_view(self):
        ...
        self.client.login(
            username=COMMUNITY_USER, password=COMMUNITY_USER + "+password"
        )
    ...
    def test_edit_members_only_removes_member_roles(self):
        ...
        self.assertCountEqual(
            nomcom_group.role_set.filter(name="member").values_list(
                "email__address", flat=True
            ),
            [CHAIR_USER + EMAIL_DOMAIN, COMMUNITY_USER + EMAIL_DOMAIN],
        )
        self.assertCountEqual(
            nomcom_group.role_set.filter(name="liaison").values_list(
                "email__address", flat=True
            ),
            [CHAIR_USER + EMAIL_DOMAIN, COMMUNITY_USER + EMAIL_DOMAIN],
        )
    ...
    def test_edit_nomcom_view(self):
        ...
        response = self.client.post(
            self.edit_nomcom_url,
            {
                "public_key": f,
                "reminderdates_set-TOTAL_FORMS": q(
                    'input[name="reminderdates_set-TOTAL_FORMS"]'
                ).val(),
                "reminderdates_set-INITIAL_FORMS": q(
                    'input[name="reminderdates_set-INITIAL_FORMS"]'
                ).val(),
                "reminderdates_set-MAX_NUM_FORMS": q(
                    'input[name="reminderdates_set-MAX_NUM_FORMS"]'
                ).val(),
        ...
        self.assertEqual(
            check_comments(feedback.comments, comment_text, self.privatekey_file), True
        )

        # Check that the set reminder date is present
        reminder_dates = dict(
            [(d.id, str(d.date)) for d in nomcom.reminderdates_set.all()]
        )
        ...
        reminder_dates = dict(
            [(d.id, str(d.date)) for d in ReminderDates.objects.filter(nomcom=nomcom)]
        )
    ...
    def test_announcements_view(self):
        ...
        self.assertContains(
            r, nomcom.role_set.filter(name="chair")[0].person.email_address()
        )
    ...
    def nominate_view(
        ...
        if not searched_email:
            searched_email = Email.objects.filter(
                address=nominee_email
            ).first() or EmailFactory(
                address=nominee_email, primary=True, origin="test"
            )
        ...
        comment_text = (
            "Test nominate view. Comments with accents äöåÄÖÅ éáíóú âêîôû ü àèìòù."
        )
        candidate_phone = "123456"

        test_data = {
            "searched_email": searched_email.pk,
    ...
    def nominate_newperson_view(
        ...
        if Email.objects.filter(address=nominee_email).exists():
            ...
            self.assertEqual(
                check_comments(feedback.comments, comment_text, self.privatekey_file),
                True,
            )
            Nomination.objects.get(
                position=position,
    ...
    def test_public_feedback(self):
        ...
        self.assertEqual(Message.objects.count(), 2)
        self.assertFalse(
            Message.objects.filter(subject="NomCom comment confirmation").exists()
        )
    ...
    def feedback_view(
        ...
        q = PyQuery(response.content)
        person_url = reverse(
            "ietf.person.views.profile", kwargs={"email_or_name": nominee.email}
        )
        self.assertTrue(
            q('a[href="%s"]' % (person_url)),
            "Nominee feedback page does not link to profile page",
        )

        comments = (
            "Test feedback view. Comments with accents äöåÄÖÅ éáíóú âêîôû ü àèìòù."
        )
        ...
        nominee_position = NomineePosition.objects.get(
            nominee=nominee, position=position
        )
        ...
        if state.slug != "accepted":
            ...
            self.assertTrue(q("form .is-invalid"))
            # accept nomination
            nominee_position.state = NomineePositionStateName.objects.get(
                slug="accepted"
            )
            nominee_position.save()
        ...
        feedback = Feedback.objects.filter(
            positions__in=[position],
            nominees__in=[nominee],
            type=FeedbackTypeName.objects.get(slug="comment"),
        ).latest("id")
        if public:
            self.assertEqual(feedback.author, nominator_email)
...
class NomineePositionStateSaveTest(TestCase):
    ...
    def test_state_autoset(self):
        ...
        nominee_position = NomineePosition.objects.create(
            position=position, nominee=self.nominee
        )
    ...
    def test_nominee_position_unique(self):
        ...
        position = Position.objects.get(name="OAM")
        NomineePosition.objects.create(position=position, nominee=self.nominee)
...
class FeedbackTest(TestCase):
    ...
    def test_ingest_feedback_email(self, mock_create_feedback_email):
        ...
        while NomCom.objects.filter(group__acronym__icontains=no_nomcom_year).exists():
            no_nomcom_year += 1
        inactive_nomcom = NomComFactory(
            group__state_id="conclude", group__acronym=f"nomcom{no_nomcom_year + 1}"
        )
        ...
        for bad_year in (no_nomcom_year, inactive_nomcom.year()):
            ...
            self.assertIsNotNone(
                context.exception.email_body
            )  # error message to be sent
            self.assertIsNone(
                context.exception.email_recipients
            )  # default recipients (i.e., admin)
            self.assertIsNone(
                context.exception.email_original_message
            )  # no original message
...
class ReminderTest(TestCase):
    ...
    def setUp(self):
        ...
        e1 = EmailFactory(
            address="nominee1@example.org",
            person=PersonFactory(name="Nominee 1"),
            origin="test",
        )
        e2 = EmailFactory(
            address="nominee2@example.org",
            person=PersonFactory(name="Nominee 2"),
            origin="test",
        )
    ...
    def test_is_time_to_send_reminder(self):
        ...
        self.assertTrue(
            _is_time_to_send_reminder(
                self.nomcom, today + datetime.timedelta(days=4), today
            )
        )
    ...
    def test_remind_accept_view(self):
        url = reverse(
            ...
            kwargs={"year": NOMCOM_YEAR, "type": "accept"},
        )
        login_testing_unauthorized(self, CHAIR_USER, url)
        messages_before = len(outbox)
        test_data = {
            "selected": [x.id for x in Nominee.objects.filter(nomcom=self.nomcom)]
        }
    ...
    def test_remind_questionnaire_view(self):
        url = reverse(
            "ietf.nomcom.views.send_reminder_mail",
            kwargs={"year": NOMCOM_YEAR, "type": "questionnaire"},
        )
        login_testing_unauthorized(self, CHAIR_USER, url)
        messages_before = len(outbox)
        test_data = {
...
class InactiveNomcomTests(TestCase):
    ...
    def setUp(self):
        super().setUp()
        setup_test_public_keys_dir(self)
        self.nc = NomComFactory.create(
            **nomcom_kwargs_for_year(group__state_id="conclude")
        )
    ...
    def test_feedback_closed(self):
        for view in [
            ...
            url += "?nominee=%d&position=%d" % (
                self.nc.nominee_set.order_by("pk").first().id,
                self.nc.nominee_set.order_by("pk")
                .first()
                .nomineeposition_set.order_by("pk")
                .first()
                .position.id,
            )
            response = self.client.get(url)
            ...
            test_data = {
                "comment_text": "Test feedback view. Comments with accents äöåÄÖÅ éáíóú âêîôû ü àèìòù.",
    ...
    def test_cannot_classify_feedback(self):
        url = reverse(
            "ietf.nomcom.views.view_feedback_pending", kwargs={"year": self.nc.year()}
        )
        login_testing_unauthorized(self, self.chair.user.username, url)
    ...
    def test_cannot_modify_nominees(self):
        ...
        test_data = {"action": "set_as_pending", "selected": [1]}
        response = self.client.post(url, test_data)
        self.assertEqual(response.status_code, 200)
        q = PyQuery(response.content)
    ...
    def test_email_pasting_closed(self):
        url = reverse(
            "ietf.nomcom.views.private_feedback_email", kwargs={"year": self.nc.year()}
        )
        login_testing_unauthorized(self, self.chair.user.username, url)
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_questionnaire_entry_closed(self):
        url = reverse(
            "ietf.nomcom.views.private_questionnaire", kwargs={"year": self.nc.year()}
        )
        login_testing_unauthorized(self, self.chair.user.username, url)
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def _test_send_reminders_closed(self, rtype):
        url = reverse(
            "ietf.nomcom.views.send_reminder_mail",
            kwargs={"year": self.nc.year(), "type": rtype},
        )
        login_testing_unauthorized(self, self.chair.user.username, url)
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_merge_closed(self):
        url = reverse(
            "ietf.nomcom.views.private_merge_person", kwargs={"year": self.nc.year()}
        )
        login_testing_unauthorized(self, self.chair.user.username, url)
        response = self.client.get(url)
        q = PyQuery(response.content)
    ...
    def test_cannot_edit_position(self):
        url = reverse(
            ...
            kwargs={
                "year": self.nc.year(),
                "position_id": self.nc.position_set.first().id,
            },
        )
        login_testing_unauthorized(self, self.chair.user.username, url)
        provide_private_key_to_test_client(self)
    ...
    def test_cannot_add_position(self):
        url = reverse(
            "ietf.nomcom.views.edit_position", kwargs={"year": self.nc.year()}
        )
        login_testing_unauthorized(self, self.chair.user.username, url)
        provide_private_key_to_test_client(self)
        response = self.client.get(url)
    ...
    def test_can_view_but_not_edit_templates(self):
        template = DBTemplateFactory.create(
            group=self.nc.group,
            title="Test template",
        ...
        url = reverse(
            ...
            kwargs={"year": self.nc.year(), "template_id": template.id},
        )
        login_testing_unauthorized(self, self.chair.user.username, url)
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
...
class FeedbackIndexTests(TestCase):
    ...
    def setUp(self):
        ...
        for type_id in ["comment", "nomina", "questio"]:
            f = FeedbackFactory.create(
                author=self.author, nomcom=self.nc, type_id=type_id
            )
    ...
    def test_feedback_index_totals(self):
        url = reverse(
            "ietf.nomcom.views.view_feedback", kwargs={"year": self.nc.year()}
        )
...
class FeedbackLastSeenTests(TestCase):
    ...
    def setUp(self):
        ...
        for type_id in ["comment", "nomina", "questio"]:
            ...
            f.nominees.add(self.nominee)
        f = FeedbackFactory.create(
            author=self.author, nomcom=self.nc, type_id="comment"
        )
    ...
    def test_feedback_nominee_badges(self):
        url = reverse(
            "ietf.nomcom.views.view_feedback_nominee",
            kwargs={"year": self.nc.year(), "nominee_id": self.nominee.id},
        )
        login_testing_unauthorized(self, self.member.user.username, url)
        provide_private_key_to_test_client(self)
        response = self.client.get(url)
    ...
    def test_feedback_topic_badges(self):
        url = reverse(
            ...
            kwargs={"year": self.nc.year(), "topic_id": self.topic.id},
        )
        login_testing_unauthorized(self, self.member.user.username, url)
        provide_private_key_to_test_client(self)
        response = self.client.get(url)
...
class NewActiveNomComTests(TestCase):
    ...
    def setUp(self):
        ...
        self.nc = NomComFactory.create(
            **nomcom_kwargs_for_year(year=random.randint(2008, 2100))
        )
        self.chair = self.nc.group.role_set.filter(name="chair").first().person
        self.saved_days_to_expire_nomination_link = (
            settings.DAYS_TO_EXPIRE_NOMINATION_LINK
        )
    ...
    def test_help(self):
        url = reverse(
            "ietf.nomcom.views.configuration_help", kwargs={"year": self.nc.year()}
        )
        login_testing_unauthorized(self, self.chair.user.username, url)
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_email_pasting(self):
        ...
        response = self.client.post(
            url,
            {
                "email_text": """To: rjsparks@nostrum.com
...
"""
    ...
    def test_simple_feedback_pending(self):
        ...
        fb = FeedbackFactory(nomcom=self.nc, type_id=None)
        np = NomineePosition.objects.filter(
            position__nomcom=self.nc, state="accepted"
        ).first()
        ...
        position = self.nc.position_set.exclude(
            nomineeposition__nominee=nominee
        ).first()
        ...
        nominee = self.nc.nominee_set.get(
            person__email__address="newperson@example.com"
        )
        fb_count_before = nominee.feedback_set.count()
        response = self.client.post(
            url,
    ...
    def test_complicated_feedback_pending(self):
        ...
        nominee = self.nc.nominee_set.order_by("pk").first()
        new_position_for_nominee = self.nc.position_set.exclude(
            nomineeposition__nominee=nominee
        ).first()
        ...
        self.assertEqual(
            q('input[name="extra_ids"]').attr["value"], "%s:comment" % fb2.id
        )
        ...
        )  # Notice that this is also is not a 302
        q = PyQuery(response.content)
    ...
    def test_feedback_unrelated(self):
        ...
        url = reverse(
            "ietf.nomcom.views.view_feedback_unrelated", kwargs={"year": self.nc.year()}
        )
        login_testing_unauthorized(self, self.chair.user.username, url)
        provide_private_key_to_test_client(self)
        response = self.client.get(url)
    ...
    def test_list_templates(self):
        ...
        url = reverse(
            "ietf.nomcom.views.list_templates", kwargs={"year": self.nc.year()}
        )
        login_testing_unauthorized(self, self.chair.user.username, url)
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_list_positions(self):
        url = reverse(
            "ietf.nomcom.views.list_positions", kwargs={"year": self.nc.year()}
        )
        login_testing_unauthorized(self, self.chair.user.username, url)
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_remove_position(self):
        ...
        url = reverse(
            "ietf.nomcom.views.remove_position",
            kwargs={"year": self.nc.year(), "position_id": position.id},
        )
        login_testing_unauthorized(self, self.chair.user.username, url)
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        q = PyQuery(response.content)
        self.assertTrue(
            any(["likely to be harmful" in x.text for x in q(".alert-warning")])
        )
    ...
    def test_remove_invalid_position(self):
        ...
        url = reverse(
            "ietf.nomcom.views.remove_position",
            kwargs={"year": self.nc.year(), "position_id": no_such_position_id},
        )
        login_testing_unauthorized(self, self.chair.user.username, url)
        response = self.client.get(url)
    ...
    def test_edit_nominee(self):
        ...
        url = reverse(
            ...
            kwargs={"year": self.nc.year(), "nominee_id": nominee.id},
        )
        login_testing_unauthorized(self, self.chair.user.username, url)
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_request_merge(self):
        ...
        response = self.client.post(
            url,
            {
                "primary_person": nominee1.person.pk,
        ...
        self.assertTrue(
            all(
                [
                    str(x.person.pk) in outbox[0].get_payload()
                    for x in [nominee1, nominee2]
                ]
            )
        )
    ...
    def test_extract_email(self):
        url = reverse(
            "ietf.nomcom.views.extract_email_lists", kwargs={"year": self.nc.year()}
        )
        login_testing_unauthorized(self, self.chair.user.username, url)
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_eligible(self):
        ...
        for number in range(meeting_start, meeting_start + 10):
            ...
            for person, ascii, email in random.sample(
                [(p, p.ascii, p.email()) for p in Person.objects.all()], samples
            ):
                if not " " in ascii:
                    continue
    ...
    def test_volunteers(self):
        ...
        for number in range(meeting_start, meeting_start + 8):
            ...
            for p in people:
                m.meetingregistration_set.create(
                    person=p, reg_type="onsite", checkedin=True, attended=True
                )
        for p in people:
...
class NomComIndexTests(TestCase):
    def setUp(self):
        ...
        for year in range(2000, 2014):
            NomComFactory.create(
                **nomcom_kwargs_for_year(
                    year=year, populate_positions=False, populate_personnel=False
                )
            )
...
class NoPublicKeyTests(TestCase):
    ...
    def test_not_yet(self):
        ...
        self.do_common_work(
            reverse(
                "ietf.nomcom.views.send_reminder_mail",
                kwargs={"year": self.nc.year(), "type": "accept"},
            ),
            True,
        )
        # No nominations
        self.do_common_work(
            reverse(
                "ietf.nomcom.views.private_nominate", kwargs={"year": self.nc.year()}
            ),
            False,
        )
        # No feedback
        self.do_common_work(
            reverse(
                "ietf.nomcom.views.private_feedback", kwargs={"year": self.nc.year()}
            ),
            False,
        )
        # No feedback email
...
class AcceptingTests(TestCase):
    ...
    def test_public_accepting_nominations(self):
        url = reverse(
            "ietf.nomcom.views.public_nominate", kwargs={"year": self.nc.year()}
        )
    ...
    def test_public_accepting_feedback(self):
        url = reverse(
            "ietf.nomcom.views.public_feedback", kwargs={"year": self.nc.year()}
        )
...
class TopicTests(TestCase):
    ...
    def testAddEditListRemoveTopic(self):
        ...
        url = reverse(
            ...
            kwargs={"year": self.nc.year(), "topic_id": self.nc.topic_set.first().pk},
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        q = PyQuery(response.content)
        ...
        response = self.client.post(
            url,
            {
                "subject": "Test Topic Modified",
                "accepting_feedback": False,
                "audience": "nominees",
            },
        )
        self.assertEqual(response.status_code, 302)
    ...
    def testClassifyTopicFeedback(self):
        ...
        self.assertIn(
            "You must choose at least one Nominee or Topic", unicontent(response)
        )
        response = self.client.post(
            url,
            {
    ...
    def testTopicFeedback(self):
        ...
        login_testing_unauthorized(self, self.plain_person.user.username, url)
        response = self.client.post(
            url, {"comment_text": "junk", "confirmation": False}
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "alert-success")
        self.assertNotContains(response, "feedbackform")
    ...
    def testAudience(self):
        for audience in ["nomcom", "nominees"]:
            ...
            login_testing_unauthorized(
                self, self.plain_person.user.username, feedback_url
            )
            ...
            r = self.client.post(
                topic_url, {"comment_text": "junk", "confirmation": False}
            )
            self.assertEqual(r.status_code, 404)
            ...
            self.client.login(
                username=valid_user.user.username,
                password=valid_user.user.username + "+password",
            )
            r = self.client.get(feedback_url)
...
class EligibilityUnitTests(TestCase):
    ...
    def test_get_eligibility_date(self):
        ...
        self.assertEqual(
            get_eligibility_date(date=datetime.date(2001, 2, 3)),
        ...
        self.assertEqual(
            get_eligibility_date(nomcom=n, date=datetime.date(2001, 2, 3)),
            datetime.date(2001, 2, 3),
        )
        ...
        NomComFactory(
            group__acronym="nomcom2016",
            populate_personnel=False,
            first_call_for_volunteers=datetime.date(2016, 5, 4),
        )
        self.assertEqual(get_eligibility_date(), datetime.date(2016, 5, 4))

        NomComFactory(
            group__acronym=f"nomcom{this_year}",
            first_call_for_volunteers=datetime.date(this_year, 5, 6),
        )
...
class rfc8713EligibilityTests(TestCase):
    ...
    def setUp(self):
        super().setUp()
        self.nomcom = NomComFactory(
            group__acronym="nomcom2019",
            populate_personnel=False,
            first_call_for_volunteers=datetime.date(2019, 5, 1),
        )

        meetings = [
            MeetingFactory(date=date, type_id="ietf")
            for date in (
        ...
        ineligible_person_with_role(
            group__type_id="area", group__state_id="active", name_id="ad"
        )
        ineligible_person_with_role(group=self.nomcom.group, name_id="chair")

        # No-one is eligible for the other_nomcom
        self.other_nomcom = NomComFactory(
            group__acronym="nomcom2018",
            first_call_for_volunteers=datetime.date(2018, 5, 1),
        )
    ...
    def test_list_eligible(self):
        ...
        self.assertEqual(
            set(list_eligible(date=self.other_date)), set(self.other_people)
        )
...
class rfc8788EligibilityTests(TestCase):
    ...
    def setUp(self):
        super().setUp()
        self.nomcom = NomComFactory(
            group__acronym="nomcom2020",
            populate_personnel=False,
            first_call_for_volunteers=datetime.date(2020, 5, 1),
        )

        meetings = [
            MeetingFactory(number=number, date=date, type_id="ietf")
            for number, date in [
...
class rfc8989EligibilityTests(TestCase):
    ...
    def setUp(self):
        ...
        self.nomcoms.append(
            NomComFactory(
                group__acronym="nomcom2021",
                populate_personnel=False,
                first_call_for_volunteers=datetime.date(2021, 5, 15),
            )
        )
        self.nomcoms.append(
            NomComFactory(
                group__acronym="nomcom2022",
                populate_personnel=False,
                first_call_for_volunteers=datetime.date(2022, 5, 15),
            )
        )
    ...
    def test_elig_by_meetings(self):
        ...
        for nomcom in self.nomcoms:
            ...
            prev_five = (
                meetings[2:] if nomcom.group.acronym == "nomcom2021" else meetings[:5]
            )
            for combo_len in range(0, 6):
                for combo in combinations(prev_five, combo_len):
                    ...
                    for m in combo:
                        MeetingRegistrationFactory(
                            person=p, meeting=m, attended=True
                        )  # not checkedin because this forces looking at older meetings
                        AttendedFactory(
                            session__meeting=m, session__type_id="plenary", person=p
                        )
                    if combo_len < 3:
            ...
            Person.objects.filter(
                pk__in=[p.pk for p in eligible_people + ineligible_people]
            ).delete()
    ...
    def test_elig_by_office_active_groups(self):
        ...
        for nomcom in self.nomcoms:
            elig_datetime = datetime_from_date(
                nomcom.first_call_for_volunteers, DEADLINE_TZINFO
            )
    ...
    def test_elig_by_office_edge(self):
        ...
        for nomcom in self.nomcoms:
            elig_date = datetime_from_date(
                get_eligibility_date(nomcom), DEADLINE_TZINFO
            )
    ...
    def test_elig_by_office_closed_groups(self):
        ...
        for nomcom in self.nomcoms:
            ...
            just_after_three_years_before = three_years_before + datetime.timedelta(
                days=1
            )
            just_before_three_years_before = three_years_before - datetime.timedelta(
                days=1
            )
    ...
    def test_elig_by_author(self):
        ...
        for nomcom in self.nomcoms:
            ...
            da = WgDocumentAuthorFactory(person=p)
            DocEventFactory(
                type="published_rfc", doc=da.document, time=day_before_first_date
            )
            ineligible.add(p)
            ...
            da = WgDocumentAuthorFactory(person=p)
            DocEventFactory(
                type="iesg_approved", doc=da.document, time=day_after_last_date
            )
            da = WgDocumentAuthorFactory(person=p)
            DocEventFactory(type="published_rfc", doc=da.document, time=middle_date)
            ineligible.add(p)
            ...
            self.assertEqual(set(list_eligible(nomcom=nomcom)), set(eligible))
            Person.objects.filter(
                pk__in=[p.pk for p in eligible.union(ineligible)]
            ).delete()
...
class rfc9389EligibilityTests(TestCase):
    ...
    def setUp(self):
        super().setUp()
        self.nomcom = NomComFactory(
            group__acronym="nomcom2023",
            populate_personnel=False,
            first_call_for_volunteers=datetime.date(2023, 5, 15),
        )
        self.meetings = [
            MeetingFactory(number=number, date=date, type_id="ietf")
            for number, date in [
    ...
    def test_elig_by_meetings(self):
        ...
        for combo_len in range(
            0, 6
        ):  # Someone might register for 0 to 5 previous meetings
                ...
                for method in attendance_methods:
                    ...
                    for meeting in combo:
                        MeetingRegistrationFactory(
                            ...
                            meeting=meeting,
                            reg_type="onsite",
                            checkedin=(method in ("checkedin", "both")),
                        )
                        if method in ("session", "both"):
                            AttendedFactory(
                                session__meeting=meeting,
                                session__type_id="plenary",
                                person=p,
                            )
                        if combo_len < 3:
                            ineligible_people.append(p)
...
class VolunteerTests(TestCase):
    ...
    def test_volunteer(self):
        ...
        self.assertContains(
            r, "NomCom is not accepting volunteers at this time", status_code=200
        )

        this_year = date_today().year
        nomcom = NomComFactory(
            group__acronym=f"nomcom{this_year}", is_accepting_volunteers=False
        )
        r = self.client.get(url)
        self.assertContains(
            r, "NomCom is not accepting volunteers at this time", status_code=200
        ...
        nomcom.save()
        MeetingRegistrationFactory(
            person=person, affiliation="mtg_affiliation", checkedin=True
        )
        r = self.client.get(url)
        ...
        self.assertEqual(
            person.volunteer_set.get(nomcom=nomcom).affiliation, "something"
        )
        r = self.client.get(url)
        self.assertContains(r, "already volunteered", status_code=200)

        person.volunteer_set.all().delete()
        nomcom2 = NomComFactory(
            group__acronym=f"nomcom{this_year-1}", is_accepting_volunteers=True
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
        ...
        r = self.client.post(
            url, dict(nomcoms=[nomcom.pk, nomcom2.pk], affiliation="something")
        )
        self.assertRedirects(r, reverse("ietf.ietfauth.views.profile"))
...
class VolunteerDecoratorUnitTests(TestCase):
    def test_decorate_volunteers_with_qualifications(self):
        ...
        for m in meetings:
            ...
            AttendedFactory(
                session__meeting=m, session__type_id="plenary", person=meeting_person
            )
...
class ReclassifyFeedbackTests(TestCase):
    ...
    def test_download_feedback_nominee(self):
        ...
        self.client.logout()
        self.client.login(
            username=self.chair.user.username,
            password=self.chair.user.username + "+password",
        )
        provide_private_key_to_test_client(self)
    ...
    def test_reclassify_feedback_nominee(self):
        ...
        provide_private_key_to_test_client(self)
        response = self.client.post(
            url, {"feedback_id": fb.id, "type": "obe", "submit": "reclassify"}
        )
        self.assertEqual(response.status_code, 403)
