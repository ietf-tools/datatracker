...
class AppealTests(TestCase):
    ...
    def test_download_name(self):
        ...
        artifact = AppealArtifactFactory(
            content_type="application/pdf", artifact_type__slug="response"
        )
    ...
    def test_appeal_list_view(self):
        ...
        appeal_artifact = AppealArtifactFactory(
            appeal=appeal, artifact_type__slug="appeal", date=appeal_date
        )
        response_artifact = AppealArtifactFactory(
            appeal=appeal,
            artifact_type__slug="response",
        ...
        self.assertEqual(
            f"{appeal_artifact.display_title()} - {appeal_date}",
            q("#artifact-1-1").text(),
        )
        self.assertEqual(
            f"{response_artifact.display_title()} - {response_date}",
            q("#artifact-1-2").text(),
        )
        self.assertIsNone(q("#artifact-1-1").attr("download"))
        self.assertEqual(
            q("#artifact-1-2").attr("download"), response_artifact.download_name()
        )

    def test_markdown_view(self):
        artifact = AppealArtifactFactory()
        url = urlreverse(
            "ietf.group.views.appeal_artifact",
            kwargs=dict(acronym="iab", artifact_id=artifact.pk),
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
    ...
    def test_markdown_download(self):
        artifact = AppealArtifactFactory()
        url = urlreverse(
            "ietf.group.views.appeal_artifact_markdown",
            kwargs=dict(acronym="iab", artifact_id=artifact.pk),
        )
    ...
    def test_pdf_download(self):
        artifact = AppealArtifactFactory(
            content_type="application/pdf"
        )  # The bits won't _really_ be pdf
        url = urlreverse(
            "ietf.group.views.appeal_artifact",
            kwargs=dict(acronym="iab", artifact_id=artifact.pk),
        )
