...
def group_urlreverse_list(group, viewname):
    return [
        ...
        urlreverse(
            viewname, kwargs=dict(acronym=group.acronym, group_type=group.type_id)
        ),
    ]
...
class GroupPagesTests(TestCase):
    ...
    def test_active_groups(self):
        ...
        for t in (
            ...
            if t in ["dir", "review"]:
                ...
                g.save()
            url = urlreverse(
                "ietf.group.views.active_groups", kwargs=dict(group_type=t)
            )
            r = self.client.get(url)
            self.assertEqual(r.status_code, 200)
            ...
            if t == "area":
                ...
                self.assertEqual(
                    q(f'h2#id-{g.acronym} a[href="{href}"]').text(),
                    f"({g.acronym.upper()})",
                )
        ...
        for slug in GroupTypeName.objects.exclude(
            slug__in=[
                "wg",
                "rg",
                "ag",
            ...
            with self.assertRaises(NoReverseMatch):
                url = urlreverse(
                    "ietf.group.views.active_groups", kwargs=dict(group_type=slug)
                )
    ...
    def test_wg_summary_area(self, mock):
        ...
        self.assertEqual(
            mock.call_args, call(Path(settings.GROUP_SUMMARY_PATH) / "1wg-summary.txt")
        )
    ...
    def test_wg_summary_acronym(self, mock):
        ...
        self.assertEqual(
            mock.call_args,
            call(Path(settings.GROUP_SUMMARY_PATH) / "1wg-summary-by-acronym.txt"),
        )
    ...
    def test_wg_charters(self, mock):
        ...
        self.assertEqual(r.content.decode(), "yay")
        self.assertEqual(
            mock.call_args, call(Path(settings.CHARTER_PATH) / "1wg-charters.txt")
        )
    ...
    def test_wg_charters_by_acronym(self, mock):
        ...
        self.assertEqual(
            mock.call_args,
            call(Path(settings.CHARTER_PATH) / "1wg-charters-by-acronym.txt"),
        )
    ...
    def test_generate_wg_charters_files_task(self):
        ...
        (
            Path(settings.CHARTER_PATH)
            / f"{group.charter.name}-{group.charter.rev}.txt"
        ).write_text("This is a charter.")

        generate_wg_charters_files_task()
        wg_charters_contents = (
            Path(settings.CHARTER_PATH) / "1wg-charters.txt"
        ...
        self.assertIn(
            group.ad_role().person.plain_name(), wg_charters_by_acronym_contents
        )
    ...
    def test_generate_wg_charters_files_task_without_copy(self):
        ...
        self.assertFalse(
            (Path(settings.CHARTER_COPY_PATH) / "1wg-charters.txt").exists()
        )
        self.assertTrue(
    ...
    def test_generate_wg_summary_files_task(self):
        ...
        for summary_by_area_contents in [
            (Path(settings.GROUP_SUMMARY_PATH) / "1wg-summary.txt").read_text(
                encoding="utf8"
            ),
    ...
    def test_chartering_groups(self):
        ...
        self.client.logout()
        replaced_group = CharterFactory(
            group__state_id="replaced",
            group__type_id="wg",
            group__parent=GroupFactory(type_id="area"),
            states=[("charter", "intrev")],
        ).group
        ...
        q = PyQuery(r.content)
        self.assertEqual(
            len(q('#content a:contains("%s")' % replaced_group.acronym)), 0
        )
    ...
    def test_group_documents(self):
        ...
        old_dah.time_added -= datetime.timedelta(
            days=173
        )  # make an "old" action holder
        old_dah.save()

        draft4 = WgDraftFactory(group=group)
        draft4.set_state(
            State.objects.get(type="draft", slug="expired")
        )  # Expired WG draft
        draft5 = IndividualDraftFactory()
        draft5.set_state(
            State.objects.get(type="draft", slug="expired")
        )  # Expired non-WG draft
        ...
        )  # Expired WG draft, marked as dead
        draft7 = WgDraftFactory(group=group)
        draft7.set_state(State.objects.get(type="draft", slug="expired"))
        draft7.set_state(
            State.objects.get(type="draft-stream-%s" % draft7.stream_id, slug="dead")
        )  # Expired WG draft, marked as dead
        ...
        for url in group_urlreverse_list(group, "ietf.group.views.group_documents"):
            ...
            self.assertContains(
                r, "for 173 days", count=1
            )  # the old_dah should be tagged
        ...
        self.assertTrue(
            any(
                [
                    draft2.name in x.attrib["href"]
                    for x in q("table td a.track-untrack-doc")
                ]
            )
        )
    ...
    def test_group_about(self):
        ...
        def verify_cannot_edit_group(url, group, username):
            ...
            self.assertTrue(
                ...
                "%s should not be able to edit %s of type %s"
                % (username, group.acronym, group.type_id),
            )
        ...
        def verify_can_edit_group(url, group, username):
            ...
            self.assertEqual(
                r.status_code,
                200,
                "%s should be able to edit %s of type %s"
                % (username, group.acronym, group.type_id),
            )
        ...
        for group in test_groups:
            ...
            for url in group_urlreverse_list(group, "ietf.group.views.edit"):
                ...
                for username in list(
                    set(interesting_users) - set(can_edit[group.type_id])
                ):
    ...
    def test_group_about_personnel(self):
        ...
        for role_name in group.features.default_used_roles:
            RoleFactory.create_batch(
                2, group=group, name=RoleName.objects.get(slug=role_name)
            )
    ...
    def test_group_about_nosubscribe(self):
        ...
        url = urlreverse(
            "ietf.group.views.group_about", kwargs=dict(acronym=group.acronym)
        )
        r = self.client.get(url)
    ...
    def test_materials(self):
        group = GroupFactory(
            type_id="team", acronym="testteam", name="Test Team", state_id="active"
        )
    ...
    def test_chair_photos(self):
        ...
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
        chairs = Role.objects.filter(
            group__type="wg", group__state="active", name_id="chair"
        )
    ...
    def test_wg_photos(self):
        ...
        url = urlreverse(
            "ietf.group.views.group_photos",
            kwargs={"group_type": "wg", "acronym": "mars"},
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
    ...
    def test_nonactive_group_badges(self):
        ...
        url = urlreverse(
            "ietf.group.views.history", kwargs={"acronym": concluded_group.acronym}
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
        ...
        url = urlreverse(
            "ietf.group.views.history", kwargs={"acronym": replaced_group.acronym}
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
...
class GroupEditTests(TestCase):
    ...
    def test_create(self):
        ...
        url = urlreverse(
            "ietf.group.views.edit", kwargs=dict(group_type="wg", action="charter")
        )
        ...
        r = self.client.post(
            url, dict(acronym="test...", name="Testing WG", state=bof_state.pk)
        )
        self.assertEqual(r.status_code, 200)
        self.assertTrue(len(q("form .is-invalid")) > 0)
        ...
        r = self.client.post(
            url, dict(acronym="test-wg", name="Testing WG", state=bof_state.pk)
        )
        self.assertEqual(r.status_code, 200)
        self.assertTrue(len(q("form .is-invalid")) > 0)
        ...
        # acronym too short
        r = self.client.post(
            url, dict(acronym="t", name="Testing WG", state=bof_state.pk)
        )
        self.assertEqual(r.status_code, 200)
        self.assertTrue(len(q("form .is-invalid")) > 0)
        ...
        r = self.client.post(
            url, dict(acronym="1startwithalpha", name="Testing WG", state=bof_state.pk)
        )
        self.assertEqual(r.status_code, 200)
        self.assertTrue(len(q("form .is-invalid")) > 0)
        ...
        r = self.client.post(
            url, dict(acronym="testwg", name="Testing WG", state=bof_state.pk)
        )
        self.assertEqual(r.status_code, 200)
        self.assertTrue(len(q("form .is-invalid")) > 0)
        ...
        self.assertEqual(
            group.description, "", "Description should be ignored for a WG"
        )

    def test_create_rg(self):

        url = urlreverse(
            "ietf.group.views.edit", kwargs=dict(group_type="rg", action="charter")
        )
    ...
    def test_create_non_chartered_includes_description(self):
        ...
        group_type = GroupTypeName.objects.filter(
            used=True,
            features__has_chartering_process=False,
            features__parent_types="area",
        ).first()
        self.assertIsNotNone(group_type)
        url = urlreverse(
            "ietf.group.views.edit",
            kwargs=dict(group_type=group_type.slug, action="create"),
        )
        login_testing_unauthorized(self, "secretary", url)
        r = self.client.post(
            url,
        ...
        self.assertEqual(
            group.description, "not ignored", "Description should not be ignored"
        )
    ...
    def test_edit_info(self):
        ...
        RoleFactory(
            group=group, name_id="chair", person__user__email="marschairman@example.org"
        )
        RoleFactory(
            group=group,
            name_id="delegate",
            person__user__email="marsdelegate@example.org",
        )

        url = urlreverse(
            "ietf.group.views.edit",
            kwargs=dict(group_type=group.type_id, acronym=group.acronym, action="edit"),
        )
        login_testing_unauthorized(self, "secretary", url)
        ...
        r = self.client.post(
            url,
            dict(
                name="Mars Not Special Interest Group",
                acronym="mars",
                parent=area.pk,
                ad=ad.pk,
                state=state.pk,
        ...
        for k in ("chair", "secr", "techadv", "liaison_cc_contact"):
            self.assertTrue(
                group.role_set.filter(name=k, email__address="aread@example.org")
            )
        self.assertTrue(
            group.role_set.filter(
                name="liaison_contact", email__address="ad1@example.org"
            )
        )
        self.assertFalse(
            group.role_set.filter(
                name="liaison_contact", email__address="aread@example.org"
            )
        )
        self.assertFalse(
            group.role_set.filter(
                name="liaison_contact", email__address="ad2@example.org"
            )
        )
        self.assertFalse(
            group.role_set.filter(
                name="liaison_cc_contact", email__address="ad1@example.org"
            )
        )
        self.assertTrue(
            group.role_set.filter(
                name="liaison_cc_contact", email__address="ad2@example.org"
            )
        )
        self.assertTrue(
            group.role_set.filter(name="delegate", email__address="ad2@example.org")
        )
    ...
    def test_edit_extresources(self):
        ...
        self.assertEqual(
            group.latest_event(GroupEvent, type="info_changed").desc[:20],
            "Resources changed to",
        )
        self.assertIn(
            "github_username githubuser",
            group.latest_event(GroupEvent, type="info_changed").desc,
        )
        self.assertEqual(group.groupextresource_set.count(), 5)
        self.assertEqual(
            group.groupextresource_set.get(name__slug="github_repo").display_name,
            "Some display text",
        )
        self.assertIn(
            group.groupextresource_set.first().name.slug,
            str(group.groupextresource_set.first()),
        )
    ...
    def test_edit_field(self):
        def _test_field(group, field_name, field_content, prohibited_form_names):
            url = urlreverse(
                "ietf.group.views.edit",
                kwargs=dict(acronym=group.acronym, action="edit", field=field_name),
            )
            login_testing_unauthorized(self, "secretary", url)
            r = self.client.get(url)
            self.assertEqual(r.status_code, 200)
            ...
            self.assertEqual(
                len(
                    q(
                        "div#content > form input[name=%s], div#content > form select[name=%s]"
            ...
            for prohibited_name in prohibited_form_names:
                self.assertEqual(
                    len(
                        q(
                            "div#content > form input[name=%s], div#content > form select[name=%s]"
                            % (prohibited_name, prohibited_name)
                        )
                    ),
                    0,
                )
            ...
            if field_name.endswith("_roles"):
                ...
                self.assertSetEqual(
                    ...
                    set(
                        group.role_set.filter(name=role_name).values_list(
                            "email", flat=True
                        )
                    ),
                )
        ...
        _test_field(
            group,
            "liaison_contact_roles",
            ["user@example.com", "other_user@example.com"],
            ["list_email"],
        )
        _test_field(
            group,
            "liaison_cc_contact_roles",
            ["user@example.com", "other_user@example.com"],
            ["liaison_contact"],
        )
    ...
    def test_edit_reviewers(self):
        ...
        other_group = GroupFactory(
            type_id="review", parent=GroupFactory(type_id="area")
        )
        ...
        RoleFactory(
            group=other_group, name_id="reviewer", person=reviewer, email=first_email
        )
        ...
        self.assertEqual(
            group.role_set.get(name="reviewer").email.address, first_email.address
        )
        self.assertTrue("Personnel change" in outbox[0]["Subject"])
        ...
        review_assignment = ReviewAssignmentFactory(
            review_request=review_req, state_id="assigned", reviewer=first_email
        )
        other_review_assignment = ReviewAssignmentFactory(
            review_request=other_review_req, state_id="assigned", reviewer=first_email
        )
        ...
        self.assertEqual(
            other_group.role_set.get(name="reviewer").email.address, first_email.address
        )

        review_assignment, other_review_assignment = reload_db_objects(
            review_assignment, other_review_assignment
        )
        ...
        self.assertEqual(
            group.role_set.get(name="reviewer").email.address, last_email.address
        )
        review_assignment, other_review_assignment = reload_db_objects(
            review_assignment, other_review_assignment
        )
    ...
    def test_edit_info_non_chartered_includes_description(self):
        group_type = GroupTypeName.objects.filter(
            used=True, features__has_chartering_process=False
        ).first()
        self.assertIsNotNone(group_type)
        group = GroupFactory(type_id=group_type.pk, description="Original description")
        url = urlreverse(
            "ietf.group.views.edit", kwargs={"acronym": group.acronym, "action": "edit"}
        )
    ...
    def test_edit_description_field(self):
        ...
        group.type = GroupTypeName.objects.filter(
            used=True, features__has_chartering_process=True
        ).first()
        group.save()
        with patch("ietf.group.views.can_manage_group", return_value=True):
            r = self.client.post(
    ...
    def test_edit_parent(self):
        group = GroupFactory.create(
            type_id="wg", parent=GroupFactory.create(type_id="area")
        )
    ...
    def test_conclude(self):
        ...
        url = urlreverse(
            "ietf.group.views.conclude",
            kwargs=dict(group_type=group.type_id, acronym=group.acronym),
        )
        login_testing_unauthorized(self, "secretary", url)
    ...
    def test_replace(self):
        ...
        BallotDocEventFactory(
            doc=charter, ballot_type__doc_type_id="draft", ballot_type__slug="r-extrev"
        )
        url = urlreverse(
            "ietf.group.views.edit",
            kwargs=dict(
    ...
    def test_add_comment(self):
        group = GroupFactory(acronym="mars", parent=GroupFactory(type_id="area"))
        RoleFactory(
            group=group, person=Person.objects.get(user__username="ad"), name_id="ad"
        )
        ...
        RoleFactory(
            group=group, person__user__username="marsdelegate", name_id="delegate"
        )
        url = urlreverse(
            "ietf.group.views.add_comment", kwargs=dict(acronym=group.acronym)
        )
        empty_outbox()
        for username in [
            "secretary",
            ...
            self.assertTrue(
                GroupEvent.objects.filter(
                    group=group,
                    by=person,
                    type="added_comment",
                    desc="Test comment %s" % username,
                ).exists()
            )
            self.client.logout()
...
class GroupFormTests(TestCase):
    ...
    def _group_post_data(self, group):
        data = dict(
            ...
            resources="\n".join(
                self._format_resource(r) for r in group.groupextresource_set.all()
            ),
        ...
        for rslug in group.get_used_roles():
            data["{}_roles".format(rslug)] = list(
                group.role_set.filter(name_id=rslug)
                .values_list("email__address", flat=True)
                .all()
            )
    ...
    def test_edit_roles(self):
        ...
        for parent_type in GroupTypeName.objects.filter(
            child_features__isnull=False
        ).distinct():
    ...
    def test_used_roles_overrides_default(self):
        ...
        self.assertEqual(
            GroupForm(request_factory.get("/some/url"), group=group).used_roles,
            group.features.default_used_roles,
        )
    ...
    def test_no_description_field_for_chartered_groups(self):
        ...
        self.assertNotIn(
            "description", GroupForm(group=group, group_type=group.type).fields
        )
        self.assertNotIn(
            "description", GroupForm(data={"description": "blah"}, group=group).fields
        )
        self.assertNotIn(
            "description",
            GroupForm(data={"description": "blah"}, group_type=group.type).fields,
        )
        self.assertNotIn(
            "description",
            GroupForm(
                data={"description": "blah"}, group=group, group_type=group.type
            ).fields,
        )
...
class MilestoneTests(TestCase):
    def create_test_milestones(self):
        ...
        CharterFactory(group=group)
        RoleFactory(
            group=group, name_id="ad", person=Person.objects.get(user__username="ad")
        )
        RoleFactory(
            group=group,
            name_id="chair",
    ...
    def test_milestone_sets(self):
        ...
        for url in group_urlreverse_list(
            group, "ietf.group.milestones.edit_milestones;current"
        ):
            login_testing_unauthorized(self, "secretary", url)
        ...
        for url in group_urlreverse_list(
            group, "ietf.group.milestones.edit_milestones;charter"
        ):
            r = self.client.get(url)
            self.assertEqual(r.status_code, 200)
    ...
    def test_add_milestone(self):
        ...
        due = self.last_day_of_month(
            date_today(DEADLINE_TZINFO) + datetime.timedelta(days=365)
        )
    ...
    def test_accept_milestone(self):
        ...
        self.assertEqual(group.groupevent_set.count(), events_before + 1)
        self.assertTrue(
            "to active from review" in m.milestonegroupevent_set.all()[0].desc
        )
    ...
    def test_reset_charter_milestones(self):
        ...
        self.assertEqual(
            GroupMilestone.objects.filter(
                due=m1.due, desc=m1.desc, state="charter"
            ).count(),
            1,
        )

        self.assertEqual(
            group.charter.docevent_set.count(), events_before + 2
        )  # 1 delete, 1 add
    ...
    def test_edit_sort(self):
        ...
        q = PyQuery(r.content)
        self.assertEqual(
            [x.value for x in q("input[id^=id_m][id$=order]")], [None, "0", "1"]
        )
...
class DatelessMilestoneTests(TestCase):
    def test_switch_to_dateless(self):
        ...
        url = urlreverse(
            "ietf.group.milestones.edit_milestones;current",
            kwargs=dict(acronym=ms.group.acronym),
        )
        login_testing_unauthorized(self, chair.user.username, url)
        ...
        self.client.logout()
        self.client.login(
            username=ad.user.username, password="%s+password" % ad.user.username
        )
    ...
    def test_can_switch_date_types_for_initial_charter(self):
        ...
        url = urlreverse(
            "ietf.group.milestones.edit_milestones;charter",
            kwargs=dict(acronym=ms.group.acronym),
        )
        login_testing_unauthorized(self, ad.user.username, url)
...
class CustomizeWorkflowTests(TestCase):
    def test_customize_workflow(self):
        ...
        self.assertEqual(
            len(
                q("form.set-state")
                .find("input[name=state][value='%s']" % state.pk)
                .parents("form")
                .find("input[name=active][value='0']")
            ),
            1,
        )
        ...
        # deactivate state
        r = self.client.post(
            url, dict(action="setstateactive", state=state.pk, active="0")
        )
        self.assertEqual(r.status_code, 302)
        r = self.client.get(url)
        q = PyQuery(r.content)
        self.assertEqual(
            len(
                ...
                .find("input[name=state][value='%s']" % state.pk)
                .parents("form")
                .find("input[name=active][value='1']")
            ),
            1,
        )
        group = Group.objects.get(acronym=group.acronym)
        ...
        next_states = State.objects.filter(
            used=True,
            type=b"draft-stream-ietf",
            slug__in=["parked", "dead", "wait-wgw", "sub-pub"],
        ).values_list("pk", flat=True)
        r = self.client.post(
            url, dict(action="setnextstates", state=state.pk, next_states=next_states)
        )
        self.assertEqual(r.status_code, 302)
        r = self.client.get(url)
        q = PyQuery(r.content)
        self.assertEqual(
            len(
                q("form.set-next-states")
                .find("input[name=state][value='%s']" % state.pk)
                .parents("form")
                .find("input[name=next_states][checked=checked]")
            ),
            len(next_states),
        )
        ...
        self.assertEqual(
            set(transitions[0].next_states.values_list("pk", flat=True)),
            set(next_states),
        )
        ...
        r = self.client.post(url, dict(action="settagactive", tag=tag.pk, active="0"))
        self.assertEqual(r.status_code, 302)
        r = self.client.get(url)
        q = PyQuery(r.content)
        self.assertEqual(
            len(
                q("form")
                .find('input[name=tag][value="%s"]' % tag.pk)
                .parents("form")
...
class EmailAliasesTests(TestCase):
    ...
    def testAliases(self, mock_get_aliases):
        url = urlreverse(
            "ietf.group.urls_info_details.redirect.email", kwargs=dict(acronym="mars")
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 302)
        ...
        mock_get_aliases.return_value = [
            {"acronym": "mars", "alias_type": "-ads", "expansion": "aread@example.org"},
            {
                "acronym": "mars",
                "alias_type": "-chairs",
                "expansion": "mars_chair@ietf.org",
            },
        ]
        for testdict in [dict(acronym="mars"), dict(acronym="mars", group_type="wg")]:
            url = urlreverse(
                "ietf.group.urls_info_details.redirect.email", kwargs=testdict
            )
            r = self.client.get(url, follow=True)
            self.assertEqual(
                mock_get_aliases.call_args,
                mock.call(
                    testdict.get("acronym", None), testdict.get("group_type", None)
                ),
            )
            self.assertTrue(
                all([x in unicontent(r) for x in ["mars-ads@", "mars-chairs@"]])
            )
            self.assertFalse(
                any([x in unicontent(r) for x in ["ames-ads@", "ames-chairs@"]])
            )
        ...
        self.assertTrue(
            all(
                [
                    x in unicontent(r)
                    for x in ["mars-ads@", "mars-chairs@", "ames-ads@", "ames-chairs@"]
                ]
            )
        )
...
class MeetingInfoTests(TestCase):
    ...
    def setUp(self):
        ...
        SessionFactory.create(
            ...
            group=self.group,
            meeting__date=today - datetime.timedelta(days=14),
        )
...
class StatusUpdateTests(TestCase):
    ...
    def test_unsupported_group_types(self):
        ...
        def ensure_updates_dont_show(group, user):
            url = urlreverse(
                "ietf.group.views.group_about", kwargs={"acronym": group.acronym}
            )
            if user:
                self.client.login(
                    username=user.username, password="%s+password" % user.username
                )
            response = self.client.get(url)
            self.assertEqual(response.status_code, 200)
            q = PyQuery(response.content)
        ...
        def ensure_cant_edit(group, user):
            url = urlreverse(
                "ietf.group.views.group_about_status_edit",
        ...
        for type_id in GroupTypeName.objects.exclude(
            slug__in=("wg", "rg", "ag", "rag", "team", "edwg")
        ).values_list("slug", flat=True):
    ...
    def test_see_status_update(self):
        ...
        for url in group_urlreverse_list(chair.group, "ietf.group.views.group_about"):
            ...
            self.client.login(
                ...
                password="%s+password" % chair.person.user.username,
            )
            response = self.client.get(url)
            self.assertEqual(response.status_code, 200)
            q = PyQuery(response.content)
    ...
    def test_view_status_update(self):
        ...
        for url in group_urlreverse_list(
            chair.group, "ietf.group.views.group_about_status"
        ):
            response = self.client.get(url)
            self.assertEqual(response.status_code, 200)
            q = PyQuery(response.content)
            self.assertTrue(
                bleach.linkify(escape(event.desc), parse_email=True) in str(q("pre"))
            )
    ...
    def test_edit_status_update(self):
        ...
        url = urlreverse(
            "ietf.group.views.group_about_status_edit",
            kwargs={"acronym": chair.group.acronym},
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, 404)
        self.client.login(
        ...
        response = self.client.post(
            url, dict(content="Direct content typed into form", submit_response="1")
        )
        self.assertEqual(response.status_code, 302)
        self.assertEqual(
            chair.group.latest_event(type="status_update").desc,
            "Direct content typed into form",
        )
    ...
    def test_view_status_update_for_meeting(self):
        ...
        sess = SessionFactory.create(
            meeting__type_id="ietf",
            group=chair.group,
            meeting__date=date_today() - datetime.timedelta(days=1),
        )
        url = urlreverse(
            "ietf.group.views.group_about_status_meeting",
            kwargs={"acronym": chair.group.acronym, "num": sess.meeting.number},
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        url = urlreverse(
            "ietf.group.views.group_about_status_meeting",
            kwargs={
                "group_type": chair.group.type_id,
                "acronym": chair.group.acronym,
                "num": sess.meeting.number,
            },
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
...
class AcronymValidationTests(TestCase):
    ...
    def test_admin_acronym_validation(self):
        ...
        wg = GroupFactory(
            acronym="bad-idea", type_id="wg"
        )  # There are some existing wg and programs with hyphens in their acronyms.
        form = AdminGroupForm(
            {
                "acronym": wg.acronym,
                "name": wg.name,
    ...
    def test_groupform_acronym_validation(self):
        form = GroupForm(
            {"acronym": "shouldpass", "name": "should pass", "state": "active"},
            group_type="wg",
        )
        self.assertTrue(form.is_valid())
        form = GroupForm(
            {"acronym": "should-fail", "name": "should fail", "state": "active"},
            group_type="wg",
        )
        self.assertIn("acronym", form.errors)
        form = GroupForm(
            {"acronym": "f", "name": "should fail", "state": "active"}, group_type="wg"
        )
        self.assertIn("acronym", form.errors)
        form = GroupForm(
            {"acronym": "should-pass", "name": "should pass", "state": "active"},
            group_type="sdo",
        )
        self.assertTrue(form.is_valid())
        form = GroupForm(
            {"acronym": "shouldfail-", "name": "should fail", "state": "active"},
            group_type="sdo",
        )
        self.assertIn("acronym", form.errors)
        form = GroupForm(
        ...
        form = GroupForm(
            {"acronym": wg.acronym, "name": wg.name, "state": wg.state_id},
            group=wg,
            group_type=wg.type_id,
        )
        self.assertTrue(form.is_valid())
