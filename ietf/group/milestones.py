...
class MilestoneForm(forms.Form):
    ...
    docs = SearchableDocumentsField(
        ...
        required=False,
        help_text="Any Internet-Drafts that the milestone concerns.",
    )
...
def edit_milestones(request, acronym, group_type=None, milestone_set="current"):
    ...
    if can_manage_group(request.user, group):
        ...
        permission_denied(
            request, "You are not authorized to edit the milestones of this group."
        )

    desc_editable = has_role(
    ...
    def milestone_changed(f, m):
        ...
        changed = (
            c["desc"] != m.desc
            or c["resolved"] != m.resolved
            or set(c["docs"]) != set(m.docs.all())
            or c.get("review") in ("accept", "reject")
        )
        if "due" in f.fields:
            changed = changed or due_month_year_to_date(c) != due_month_year_to_date(
                m.due
            )
        else:
    ...
    def save_milestone_form(f):
        ...
        if f.milestone:
            ...
            if "due" in f.fields:
                ...
                if order != m.order:
                    if not history:
                        history = save_milestone_in_history(m)
                    changes.append(
                        "Milestone order changed from %s to %s" % (m.order, order)
                    )
                    m.order = order
            ...
            if new_docs != old_docs:
                ...
                if added:
                    changes.append(
                        "added %s to milestone" % ", ".join(d.name for d in added)
                    )

                removed = old_docs - new_docs
                if removed:
                    changes.append(
                        "removed %s from milestone" % ", ".join(d.name for d in removed)
                    )
            ...
            if len(changes) > 1:
                if c["delete"]:
                    messages.warning(
                        request,
                        "Found conflicting form data: both delete action and milestone changes for '%s'. "
                        "Ignoring the delete; if deletion is wanted, please mark for deletion without making other changes."
                        % (m.desc,),
                    )
    ...
    if request.method == "POST":
        ...
        if action == "switch":
            if can_change_uses_milestone_dates:
                if group.uses_milestone_dates:
                    ...
                    for order, milestone in enumerate(
                        group.groupmilestone_set.filter(state_id="active").order_by(
                            "due", "id"
                        )
                    ):
                ...
                for m in milestones:
                    forms.append(
                        MilestoneForm(
                            ...
                            desc_editable,
                            instance=m,
                            uses_dates=group.uses_milestone_dates,
                        )
                    )
            else:
                permission_denied(
                    request,
                    "You don't have the required permissions to change the 'uses milestone dates' setting",
                )
        else:
            ...
            for prefix in request.POST.getlist("prefix"):
                ...
                if f.is_valid() and f.cleaned_data.get("review") in (
                    "accept",
                    "reject",
                ):
            ...
            if action == "review":
                ...
                if milestone_set == "charter":
                    return redirect(
                        "ietf.doc.views_doc.document_main", name=group.charter.name
                    )
                else:
    ...
    can_reset = (
        milestone_set == "charter"
        and get_chartering_type(group.charter) == "rechartering"
    )

    empty_form = MilestoneForm(
        needs_review, reviewer, True, uses_dates=group.uses_milestone_dates
    )
    ...
    if group.uses_milestone_dates:
        ...
        forms.sort(
            key=lambda f: (
                ...
                (
                    f.milestone.order
                    if f.milestone and f.milestone.order is not None
                    else -1
                ),
            )
        )
...
def reset_charter_milestones(request, acronym, group_type=None):
    ...
    if not can_manage_group(request.user, group):
        permission_denied(
            request, "You are not authorized to change the milestones for this group."
        )
    ...
    if request.method == "POST":
        ...
        for m in current_milestones.filter(id__in=milestone_ids):
            ...
            if group.uses_milestone_dates:
                desc = 'Added milestone "%s", due %s, from current group milestones' % (
                    new.desc,
                    new.due.strftime("%B %Y"),
                )
            else:
                desc = 'Added milestone "%s" from current group milestones' % (
                    new.desc,
                )
            DocEvent.objects.create(
                type="changed_charter_milestone",
    ...
    return render(
        request,
        "group/reset_charter_milestones.html",
        dict(
            group=group,
