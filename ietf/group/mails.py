        ...
        dict(
            ...
            charter_url=(
                ...
                + urlreverse(
                    ...
                    kwargs=dict(name=group.charter.name),
                )
                if group.charter
                else "[no charter]"
            ),
...
def email_personnel_change(request, group, text, changed_personnel):
    (to, cc) = gather_address_lists(
        "group_personnel_change", group=group, changed_personnel=changed_personnel
    )
...
def email_comment(request, event):
    ...
    send_mail(
        ...
        to,
        None,
        "Comment added to %s history" % event.group.acronym,
