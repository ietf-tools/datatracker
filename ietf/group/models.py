...
class GroupInfo(models.Model):
    ...
    def get_used_roles(self):
        return (
            self.used_roles
            if len(self.used_roles) > 0
            else self.features.default_used_roles
        )
...
class Group(GroupInfo):
    ...
    charter = OneToOneField(
        "doc.Document", related_name="chartered_group", blank=True, null=True
    )
    ...
    def latest_event(self, *args, **filter_args):
        ...
        e = (
            model.objects.filter(group=self)
            .filter(**filter_args)
            .order_by("-time", "-id")[:1]
        )
    ...
    def has_role(self, user, role_names):
        ...
        return (
            ...
            and self.role_set.filter(name__in=role_names, person__user=user).exists()
        )
...
class GroupFeatures(models.Model):
    type = OneToOneField(
        GroupTypeName, primary_key=True, null=False, related_name="features"
    )
    # history = HistoricalRecords()

    #
    need_parent = models.BooleanField(
        "Need Parent",
        default=False,
    ...
    default_parent = models.CharField(
        "Default Parent",
        max_length=40,
        blank=True,
        default="",
    ...
    agenda_filter_type = models.ForeignKey(
        AgendaFilterTypeName, default="none", on_delete=PROTECT
    )
    req_subm_approval = models.BooleanField("Subm. Approval", default=False)
    #
    agenda_type = models.ForeignKey(
        AgendaTypeName, null=True, default="ietf", on_delete=CASCADE
    )
    about_page = models.CharField(
        max_length=64, blank=False, default="ietf.group.views.group_about"
    )
    default_tab = models.CharField(
        max_length=64, blank=False, default="ietf.group.views.group_about"
    )
    material_types = IETFJSONField(
        max_length=64, accepted_empty_values=[[], {}], blank=False, default=["slides"]
    )
    default_used_roles = IETFJSONField(
        max_length=256, accepted_empty_values=[[], {}], blank=False, default=[]
    )
    admin_roles = IETFJSONField(
        max_length=64, accepted_empty_values=[[], {}], blank=False, default=["chair"]
    )  # Trac Admin
    docman_roles = IETFJSONField(
        max_length=128,
        accepted_empty_values=[[], {}],
        blank=False,
        default=["ad", "chair", "delegate", "secr"],
    )
    groupman_roles = IETFJSONField(
        max_length=128,
        accepted_empty_values=[[], {}],
        blank=False,
        default=[
    ...
    groupman_authroles = IETFJSONField(
        max_length=128,
        accepted_empty_values=[[], {}],
        blank=False,
        default=[
    ...
    matman_roles = IETFJSONField(
        max_length=128,
        accepted_empty_values=[[], {}],
        blank=False,
        default=["ad", "chair", "delegate", "secr"],
    )
    role_order = IETFJSONField(
        max_length=128,
        accepted_empty_values=[[], {}],
        blank=False,
    ...
    session_purposes = IETFJSONField(
        max_length=256,
        accepted_empty_values=[[], {}],
        blank=False,
        default=[],
...
class GroupStateTransitions(models.Model):
    ...
    group = ForeignKey(Group)
    state = ForeignKey(
        "doc.State", help_text="State for which the next states should be overridden"
    )
    next_states = models.ManyToManyField(
        "doc.State", related_name="previous_groupstatetransitions_states"
    )
    ...
    def __str__(self):
        return '%s "%s" -> %s' % (
            self.group.acronym,
            self.state.name,
            [s.name for s in self.next_states.all()],
        )
...
class Role(models.Model):
    ...
    def __str__(self):
        return "%s is %s in %s" % (
            ...
            self.name.name,
            self.group.acronym or self.group.name,
        )
...
""" % (
            ...
            send_mail_text(
                ...
                to=addr,
                frm=None,
                subject="Group '%s' name change" % instance.acronym,
                txt=msg,
            )
            log.log(
                "Sent notification email: %s: '%s' --> '%s' to %s"
                % (current.acronym, current.name, instance.name, addr)
            )
