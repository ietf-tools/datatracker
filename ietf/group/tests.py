...
class StreamTests(TestCase):
    ...
    def test_stream_documents(self):
        draft = DocumentFactory(
            type_id="draft", group__acronym="iab", states=[("draft", "active")]
        )
        draft.stream_id = "iab"
        ...
        r = self.client.get(
            urlreverse("ietf.group.views.stream_documents", kwargs=dict(acronym="iab"))
        )
        self.assertEqual(r.status_code, 200)
        ...
        r = self.client.get(
            urlreverse(
                "ietf.group.views.stream_documents", kwargs=dict(acronym="editorial")
            )
        )
        self.assertRedirects(
            r,
            expected_url=urlreverse(
                "ietf.group.views.group_documents", kwargs={"acronym": "rswg"}
            ),
        )
    ...
    def test_stream_edit(self):
        ...
        url = urlreverse(
            "ietf.group.views.stream_edit", kwargs=dict(acronym=stream_acronym)
        )
        ...
        self.assertTrue(
            Role.objects.filter(
                name="delegate",
                group__acronym=stream_acronym,
                email__address="ad2@ietf.org",
            )
        )
...
class GroupStatsTests(TestCase):
    def setUp(self):
        ...
        a = WgDraftFactory()
        b = WgDraftFactory()
        RelatedDocument.objects.create(source=a, target=b, relationship_id="refnorm")
...
class GenerateGroupAliasesTests(TestCase):
    def test_generator_class(self):
        ...
        recent.role_set.create(
            name_id="chair", person=recentchair, email=recentchair.email()
        )
        wayold = GroupFactory(
            type_id="wg",
            acronym="wayold",
        ...
        wayold.role_set.create(
            name_id="chair", person=wayoldchair, email=wayoldchair.email()
        )
        ...
        RoleFactory(
            ...
            group__type_id="area",
            group__acronym="done",
            group__state_id="conclude",
        )
        irtf = Group.objects.get(acronym="irtf")
        testrg = GroupFactory(type_id="rg", acronym="testrg", parent=irtf)
        testrgchair = PersonFactory(user__username="testrgchair")
        testrg.role_set.create(
            name_id="chair", person=testrgchair, email=testrgchair.email()
        )
        testrag = GroupFactory(type_id="rg", acronym="testrag", parent=irtf)
        testragchair = PersonFactory(user__username="testragchair")
        testrag.role_set.create(
            name_id="chair", person=testragchair, email=testragchair.email()
        )

        output = [
            (alias, (domains, alist)) for alias, domains, alist in GroupAliasGenerator()
        ]
        ...
        expected_dict = {
            ...
            + "-chairs": (
                ["ietf"],
                [marschair.email_address(), marssecr.email_address()],
            ),
            ...
            testrg.acronym
            + "-chairs": (["ietf", "irtf"], [testrgchair.email_address()]),
            testrag.acronym
            + "-chairs": (["ietf", "irtf"], [testragchair.email_address()]),
        }
        # Sort lists for comparison
        self.assertEqual(
            {
                k: (sorted(doms), sorted(addrs))
                for k, (doms, addrs) in alias_dict.items()
            },
            {
                k: (sorted(doms), sorted(addrs))
                for k, (doms, addrs) in expected_dict.items()
            },
        )
...
class GroupRoleEmailTests(TestCase):
        ...
        for area in Group.objects.filter(type_id="area"):
            for iter_count in range(2):
                ...
                RoleFactory(
                    group=group,
                    name_id="chair",
                    person__user__email="{%s}chairman@ietf.org" % group.acronym,
                )
                RoleFactory(
                    group=group,
                    name_id="secr",
                    person__user__email="{%s}secretary@ietf.org" % group.acronym,
                )
    ...
    def test_role_holder_emails(self):
        ...
        RoleFactory(
            name_id="member", group__type_id="adhoc"
        )  # arbitrary type not in the of-interest list
