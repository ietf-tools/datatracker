    ...
    def get_absolute_url(self):
        ...
        if self.person:
            return urlreverse(
                ...
                kwargs={"email_or_name": self.person.email()},
            )
        elif self.group:
            return urlreverse(
...
class SearchRule(models.Model):
    ...
    text = models.CharField(
        verbose_name="Text/RegExp", max_length=255, blank=True, default=""
    )
...
class EmailSubscription(models.Model):
    ...
    notify_on = models.CharField(
        max_length=30, choices=NOTIFICATION_CHOICES, default="all"
    )

    def __str__(self):
        return "%s to %s (%s changes)" % (
            self.email,
            self.community_list,
            self.notify_on,
        )
