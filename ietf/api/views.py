...
@method_decorator(
    (csrf_exempt, require_api_key, role_required("Robot")), name="dispatch"
)
    ...
    if request.method == "POST":
        ...
        if request.POST.get("cancelled", "false") == "true":
            MeetingRegistration.objects.filter(
                meeting_id=meeting.pk, email=email, reg_type=reg_type
            ).delete()
            ...
            try:
                # Update attributes
                for key in set(data.keys()) - set(
                    ["attended", "apikey", "meeting", "email"]
                ):
                    if key == "checkedin":
            ...
            response = (
                ...
                if created
                else "Accepted, Updated registration"
            )
            if (
                User.objects.filter(username__iexact=email).exists()
                or Email.objects.filter(address=email).exists()
            ):
                pass
            else:
            ...
            if (
                request.POST.get("is_nomcom_volunteer", "false").lower() == "true"
                and object.person
            ):
                try:
...
def version(request):
    ...
    if dumpinfo:
        ...
        if dumpinfo.tz != "UTC":
            dumpdate = pytz.timezone(dumpinfo.tz).localize(
                dumpinfo.date.replace(tzinfo=None)
            )
...
def find_doc_for_rfcdiff(name, rev):
    ...
    if found.matched_rev is None or doc.rev == found.matched_rev:
        ...
        candidate = (
            doc.history_set.filter(rev=found.matched_rev).order_by("-time").first()
        )
        if candidate:
...
def rfcdiff_latest_json(request, name, rev=None):
    ...
    if condition == "no such document":
        ...
        if doc.type_id == "rfc":
            ...
            doc.type_id == "draft"
            and not found_rev
            and doc.relateddocument_set.filter(relationship_id="became_rfc").exists()
        ):
            ...
            if doc.rev == "00":
                replaces_docs = (
                    history.doc if condition == "historic version" else doc
                ).related_that_doc("replaces")
                if replaces_docs:
                    replaces = replaces_docs[0]
                    response["previous"] = f"{replaces.name}-{replaces.rev}"
                    response["previous_url"] = get_previous_url(
                        replaces.name, replaces.rev
                    )
                else:
...
def directauth(request):
    if request.method == "POST":
        ...
        if raw_data is None or data is None:
            ...
            return HttpResponse(
                json.dumps(dict(result="failure", reason="invalid post")),
                content_type="application/json",
            )
        ...
        if any([item is None for item in (authtoken, username, password)]):
            log.log(
                "One or more mandatory fields are missing: authtoken, username, password"
            )
            return HttpResponse(
                json.dumps(dict(result="failure", reason="invalid post")),
                content_type="application/json",
        ...
        if not is_valid_token("ietf.api.views.directauth", authtoken):
            ...
            return HttpResponse(
                json.dumps(dict(result="failure", reason="invalid authtoken")),
                content_type="application/json",
            )
        ...
        if user_count == 1 and authenticate(
            username=user_query.first().username, password=password
        ):
            user = user_query.get()
            if (
                user_query.filter(person__isnull=True).count() == 1
            ):  # Can't inspect user.person direclty here
                log.log(
                    f"Direct auth success (personless user): {user.pk}:{user.username}"
                )
            else:
                log.log(f"Direct auth success: {user.pk}:{user.person.plain_name()}")
            return HttpResponse(
                json.dumps(dict(result="success")), content_type="application/json"
            )

        log.log(f"Direct auth failure: {username} ({user_count} user(s) found)")
        return HttpResponse(
            json.dumps(dict(result="failure", reason="authentication failed")),
            content_type="application/json",
        )
...
class EmailIngestionError(Exception):
    ...
    def as_emailmessage(self) -> Optional[EmailMessage]:
        ...
        msg.set_content(self.email_body.format(**format_values))
...
def ingest_email_handler(request, test_mode=False):
    ...
    try:
        ...
        return _http_err(
            400, f"JSON parse error at line {err.lineno} col {err.colno}: {err.msg}"
        )
    ...
    try:
        ...
        if error_email is not None:
            with suppress(
                Exception
            ):  # send_smtp logs its own exceptions, ignore them here
