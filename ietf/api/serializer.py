...
def filter_from_queryargs(request):
    ...
    filter = fix_ranges(
        dict([(k, params[k]) for k in list(params.keys()) if not k.startswith("not__")])
    )
    exclude = fix_ranges(
        dict([(k[5:], params[k]) for k in list(params.keys()) if k.startswith("not__")])
    )
...
def clear_top_level_cache_m2m(
    sender, instance, action, reverse, model, *args, **kwargs
):
...
class AdminJsonSerializer(Serializer):
    ...
    def serialize(self, queryset, **options):
        ...
        if len(list(queryset)) == 1:
            ...
            if is_cached:
                value = cached_get(
                    key,
                    lambda: super(AdminJsonSerializer, self).serialize(
                        queryset, **options
                    ),
                )
            else:
        ...
        for name in expansions:
            try:
                ...
                if not isinstance(field, Field):
                    ...
                    options["expand"] = [
                        v[len(name) + 2 :]
                        for v in options["expand"]
                        if v.startswith(name + "__")
                    ]
                    if hasattr(field, "all"):
                        if options["expand"]:
                            ...
                            self._current[name] = dict(
                                [
                                    (rel.pk, self.expand_related(rel, name))
                                    for rel in field.all().select_related()
                                ]
                            )
                        ...
                        if callable(field):
                            ...
                            field_value = field
                        if isinstance(field_value, QuerySetAny) or isinstance(
                            field_value, list
                        ):
                            self._current[name] = dict(
                                [
                            ...
                            if hasattr(field_value, "_meta"):
                                self._current[name] = self.expand_related(
                                    field_value, name
                                )
                            else:
                ...
                if name in names and hasattr(obj, "%s_set" % name):
                    ...
                    if self.options["expand"]:
                        self._current[name] = dict(
                            [
                                (rel.pk, self.expand_related(rel, name))
                                for rel in related_objects.select_related()
                            ]
                        )
                    else:
                        self._current[name] = dict(
    ...
    def expand_related(self, related, name):
        options = self.options.copy()
        options["expand"] = [
            v[len(name) + 2 :] for v in options["expand"] if v.startswith(name + "__")
        ]
    ...
    def handle_fk_field(self, obj, field):
        ...
        if related is not None:
            if field.name in self.options.get("expand", []):
                ...
                related = smart_str(
                    getattr(related, field.remote_field.field_name), strings_only=True
                )
    ...
    def handle_m2m_field(self, obj, field):
        if field.remote_field.through._meta.auto_created:
            ...
            elif self.use_natural_keys and hasattr(
                field.remote_field.to, "natural_key"
            ):
...
class JsonExportMixin(object):
    ...
    def json_view(self, request, filter=None, expand=None):
        ...
        try:
            ...
            return HttpResponse(
                json.dumps({"error": str(e)}, sort_keys=True, indent=3),
        ...
        try:
            ...
            items = [
                (
                    getattr(o, key),
                    serializer.serialize([o], expand=expand, query_info=query_info),
                )
                for o in qs
            ]
