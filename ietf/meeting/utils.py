...
def session_time_for_sorting(session, use_meeting_date):
    if hasattr(session, "_otsa"):
        ...
        official_timeslot = TimeSlot.objects.filter(
            sessionassignments__session=session,
            sessionassignments__schedule__in=[
    ...
    if official_timeslot:
        ...
        first_event = (
            SchedulingEvent.objects.filter(session=session)
            .order_by("time", "id")
            .first()
        )
        if first_event:
...
def session_requested_by(session):
    first_event = (
        SchedulingEvent.objects.filter(session=session).order_by("time", "id").first()
    )
    if first_event:
...
def group_sessions(sessions):
    ...
    for s in sessions:
        ...
        if s.meeting.date > today:
            ...
            s, "cached_is_cutoff", lambda: s.is_material_submission_cutoff
        ):
            recent.append(s)
        else:
            past.append(s)
...
def get_upcoming_manageable_sessions(user):
    ...
    candidate_sessions = (
        add_event_info_to_session_qs(
            Session.objects.filter(
                meeting__date__gte=today - datetime.timedelta(days=15)
            )
        )
        .exclude(current_status__in=["canceled", "disappr", "notmeet", "deleted"])
    ...
    return [
        ...
        if sess.meeting.end_date() >= today and can_manage_materials(user, sess.group)
    ]
...
def sort_sessions(sessions):
    return sorted(
        ...
        key=lambda s: (
            s.meeting.number,
            s.group.acronym,
            session_time_for_sorting(s, use_meeting_date=False),
        ),
    )
            ...
            affiliation=Coalesce(
                Subquery(
                    MeetingRegistration.objects.filter(
                        ...
                        Q(person=OuterRef("person"))
                        | Q(email=OuterRef("person__email")),
...
def generate_bluesheet(request, session):
    ...
    return save_bluesheet(
        ...
        session,
        ContentFile(
            text.encode("utf-8"), name="unusednamepartsothereisanextension.txt"
        ),
    )
...
def finalize(request, meeting):
    ...
    for session in meeting.session_set.all():
        for sp in session.presentations.filter(document__type="draft", rev=None):
            rev_before_end = [
                e
                for e in sp.document.docevent_set.filter(
                    newrevisiondocevent__isnull=False
                ).order_by("-time")
                if e.time <= end_time
            ]
            if rev_before_end:
...
def add_event_info_to_session_qs(
    qs, current_status=True, requested_by=False, requested_time=False
):
...
def data_for_meetings_overview(meetings, interim_status=None):
    ...
    sessions = (
        Session.objects.filter(meeting__in=meetings)
        .order_by("meeting", "pk")
        .with_current_status()
    ...
    if interim_status == "apprw":
        meetings = [
            m
            for m in meetings
            if not m.type_id == "interim"
            or any(s.current_status == "apprw" for s in m.sessions)
        ]
        ...
        meetings = [
            ...
            for m in meetings
            if not m.type_id == "interim"
            or any(s.current_status == "scheda" for s in m.sessions)
        ]
        ...
        meetings = [
            ...
            if not m.type_id == "interim"
            or not all(
                s.current_status in ["apprw", "scheda", "canceledpa"]
                for s in m.sessions
            )
        ]
...
def preprocess_constraints_for_meeting_schedule_editor(meeting, sessions):
    ...
    constraint_names[joint_with_groups_constraint_name.slug] = (
        joint_with_groups_constraint_name
    )
    ...
    constraints = list(
        meeting.enabled_constraints().prefetch_related("target", "person", "timeranges")
    )
    ...
    session_groups = set(
        s.group
        for s in sessions
        if s.group and s.group.parent and s.group.parent.type_id == "area"
    )
    ...
    for group_id, history_time, pk in (
        Person.objects.filter(
            rolehistory__name="ad",
            rolehistory__group__group__in=session_groups,
            rolehistory__group__time__lte=meeting_time,
        )
        .values_list("rolehistory__group__group", "rolehistory__group__time", "pk")
        .order_by("rolehistory__group__time")
    ):
        responsible_ad_for_group[group_id] = pk
    for group_id, pk in Person.objects.filter(
        role__name="ad",
        role__group__in=session_groups,
        role__group__time__lte=meeting_time,
    ).values_list("role__group", "pk"):
        responsible_ad_for_group[group_id] = pk

    ad_person_lookup = {
        p.pk: p
        for p in Person.objects.filter(pk__in=set(responsible_ad_for_group.values()))
    }
    for group in session_groups:
        ...
        if ad is not None:
            constraints.append(
                Constraint(
                    meeting=meeting, source=group, person=ad, name=ad_constraint_name
                )
            )
    ...
    person_needed_for_groups = {
        cn.slug: defaultdict(set) for cn in constraint_names.values()
    }
        ...
        if g1_pk != g2_pk:
            for s1_pk in sessions_for_group.get(g1_pk, []):
                for s2_pk in sessions_for_group.get(g2_pk, []):
                    if s1_pk != s2_pk:
                        constraints_for_sessions[s1_pk].append(
                            (name_id, s2_pk, person_id)
                        )
    ...
    for c in constraints:
        if c.target_id and c.name_id != "wg_adjacent":
            ...
            seen_forward_constraints_for_groups.add(
                (c.source_id, c.target_id, c.name_id)
            )
    ...
    for c in reverse_constraints:
        ...
        if (
            ...
            c.source_id,
            c.name_id,
        ) not in seen_forward_constraints_for_groups:
            add_group_constraints(
                c.target_id, c.source_id, c.name_id + "-reversed", c.person_id
            )
    ...
    for (group_pk, cn_pk), cs in itertools.groupby(
        sorted(
            constraints,
            key=lambda c: (
                c.source_id,
                constraint_names[c.name_id].order,
                c.name_id,
                c.pk,
            ),
        ),
        key=lambda c: (c.source_id, c.name_id),
    ):
        cs = list(cs)
        for s_pk in sessions_for_group.get(group_pk, []):
            formatted_constraints_for_sessions[s_pk][constraint_names[cn_pk]] = [
                format_constraint(c) for c in cs
            ]
    ...
    for s in sessions:
        ...
        if joint_groups:
            formatted_constraints_for_sessions[s.pk][
                joint_with_groups_constraint_name
            ] = [g.acronym for g in joint_groups]
...
def diff_meeting_schedules(from_schedule, to_schedule):
    ...
    from_session_timeslots = {
        a.session_id: a.timeslot_id for a in from_schedule.assignments.all()
    }
    ...
    for a in to_schedule.assignments.all():
        ...
        if from_timeslot_id is None:
            diffs.append(
                {"change": "schedule", "session": a.session_id, "to": a.timeslot_id}
            )
...
def prefetch_schedule_diff_objects(diffs):
    ...
    timeslot_lookup = {
        t.pk: t
        for t in TimeSlot.objects.filter(pk__in=timeslot_ids).prefetch_related(
            "location"
        )
    }
    ...
    res = []
    for d in diffs:
        d_objs = {"change": d["change"], "session": session_lookup.get(d["session"])}
...
def swap_meeting_schedule_timeslot_assignments(
    schedule, source_timeslots, target_timeslots, source_target_offset
):
    ...
    for a in SchedTimeSessAssignment.objects.filter(
        schedule=schedule, timeslot__in=source_timeslots + target_timeslots
    ):
        assignments_by_timeslot[a.timeslot_id].append(a)

    timeslots_to_match_up = [
        (source_timeslots, target_timeslots, source_target_offset),
        (target_timeslots, source_timeslots, -source_target_offset),
    ]
                ...
                [
                    (
                        ...
                        max(
                            ...
                            min(lts.end_time() + lhs_offset, rts.end_time())
                            - max(lts.time + lhs_offset, rts.time),
                        ),
                    )
...
def preprocess_meeting_important_dates(meetings):
    for m in meetings:
        ...
        m.cached_updated = m.updated() or pytz.utc.localize(
...
def save_session_minutes_revision(
    session, file, ext, request, encoding=None, apply_to_all=False, narrative=False
):
    ...
    """
    document_type = DocTypeName.objects.get(
        slug="narrativeminutes" if narrative else "minutes"
    )
...
def create_recording(session, url, title=None, user=None):
    ...
    name = "recording-{}-{}-{}".format(
        session.meeting.number, session.group.acronym, sequence
    )
    ...
    pres = SessionPresentation.objects.create(
        session=session, document=doc, rev=doc.rev
    )
...
def delete_recording(session_presentation, user=None):
    ...
    if document.type_id != "recording":
        raise ValueError(
            f"Document {document.pk} is not a recording (type_id={document.type_id})"
        )
...
def get_next_sequence(group, meeting, type):
    ...
    docs = Document.objects.filter(
        name__startswith="{}-{}-{}-".format(type, meeting.number, group.acronym)
    )
    if not docs:
        return 1
...
def get_activity_stats(sdate, edate):
    ...
    events = DocEvent.objects.filter(
        doc__type="draft", time__gte=sdatetime, time__lt=edatetime
    )
    ...
    data["new_drafts_updated_count"] = events.filter(
        doc__id__in=new_drafts, newrevisiondocevent__rev="01"
    ).count()
    data["new_drafts_updated_more_count"] = events.filter(
        doc__id__in=new_drafts, newrevisiondocevent__rev="02"
    ).count()
    ...
    ffw_new_count = events.filter(
        time__gte=ffwdate, newrevisiondocevent__rev="00"
    ).count()
    try:
    ...
    ffw_update_events = events.filter(time__gte=ffwdate, type="new_revision").exclude(
        doc__id__in=new_drafts
    )
    ffw_update_count = len(set([e.doc_id for e in ffw_update_events]))
    try:
        ffw_update_percent = format(
            ffw_update_count / float(data["updated_drafts_count"]), ".0%"
        )
    except ZeroDivisionError:
    ...
    data["rfcs"] = (
        rfcs.select_related("doc")
        .select_related("doc__group")
        .select_related("doc__intended_std_level")
    )

    data["counts"] = {
        "std": rfcs.filter(doc__intended_std_level__in=("ps", "ds", "std")).count(),
...
def post_process(doc):
    ...
    if is_powerpoint(doc) and hasattr(settings, "PPT2PDF_COMMAND"):
        try:
            cmd = list(
                settings.PPT2PDF_COMMAND
            )  # Don't operate on the list actually in settings
...
def participants_for_meeting(meeting):
    ...
    """
    checked_in = (
        meeting.meetingregistration_set.filter(reg_type="onsite", checkedin=True)
        .values_list("person", flat=True)
        .distinct()
    )
    sessions = meeting.session_set.filter(
...
def generate_proceedings_content(meeting, force_refresh=False):
    ...
    cache_version = Document.objects.filter(
        session__meeting__number=meeting.number
    ...
    plenaries, _ = organize_proceedings_sessions(
        sessions.filter(name__icontains="plenary").exclude(current_status="notmeet")
    )
    ...
    irtf_not_meeting = [
        item for item in irtf_not_meeting if item["group"].type_id != "rag"
    ]
    ...
    iab, _ = organize_proceedings_sessions(
        sessions.filter(group__parent__acronym="iab").exclude(current_status="notmeet")
    )
    editorial, _ = organize_proceedings_sessions(
        sessions.filter(group__acronym__in=["rsab", "rswg"]).exclude(
            current_status="notmeet"
        )
    )

    ietf = sessions.filter(group__parent__type__slug="area").exclude(
        group__acronym__in=["edu", "iepg", "tools"]
    )
    ietf = list(ietf)
    ietf.sort(key=lambda s: area_and_group_acronyms_from_session(s))
    ietf_areas = []
    for area, area_sessions in itertools.groupby(
        ietf, key=lambda s: s.group_parent_at_the_time()
    ):
...
def organize_proceedings_sessions(sessions):
    ...
    meeting_groups = []
    not_meeting_groups = []
    for group_acronym, group_sessions in itertools.groupby(
        sessions, key=lambda s: s.group.acronym
    ):
        by_name = {}
        is_meeting = False
        all_canceled = True
        ...
        for s in sorted(
            ...
            key=lambda gs: (
                ...
                if gs.official_timeslotassignment()
                else datetime.datetime(datetime.MAXYEAR, 1, 1)
            ),
        ):
            group = s.group
            ...
            if s.current_status != "notmeet" or s.presentations.exists():
                by_name[s.name].append(
                    s
                )  # for notmeet, only include sessions with materials
        for sess_name, ss in by_name.items():
            ...
            def _format_materials(items):
                ...
                material_times = (
                    {}
                )  # key is material, value is first timestamp it appeared
                for s, mats in items:
                ...
                if n_mats == 1:
                    result.append(
                        {"material": list(material_times)[0]}
                    )  # no 'time' when only a single material
                elif n_mats > 1:
            ...
            entry = {
                ...
                "meetecho_recordings": _format_materials(
                    (s, [s.session_recording_url()]) for s in ss
                ),
                ...
                "last_update": (
                    session.last_update if hasattr(session, "last_update") else None
                ),
            }
            if (
                session
                and session.meeting.type_id == "ietf"
                and not session.meeting.proceedings_final
            ):
                entry["attendances"] = _format_materials(
                    (s, s) for s in ss if Attended.objects.filter(session=s).exists()
                )
            if is_meeting:
