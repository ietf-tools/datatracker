...
class ScheduleGeneratorTest(TestCase):
    def setUp(self):
        ...
        self.meeting = MeetingFactory(
            type_id="ietf", days=2, date=datetime.date(2020, 5, 31)
        )
        self.rooms = [
        ...
        self.wg2 = GroupFactory(
            acronym="wg2",
        )
        self.wg3 = GroupFactory(
            acronym="wg3",
        )
        ...
        self.prg1 = GroupFactory(
            acronym="prg1", parent=self.area2, type_id="rg", state_id="proposed"
        )
        self.all_groups = [
            self.area1,
            self.area2,
    ...
    def test_normal_schedule(self):
        ...
        generator = generate_schedule.ScheduleHandler(
            self.stdout, self.meeting.number, verbosity=3
        )
        ...
        wg2_no_attendees_session_pk = [
            s.session_pk
            for s in generator.schedule.sessions
            if s.group == "wg2" and not s.attendees
        ][0]
        self.assertIn(
            f"WARNING: session wg2 (pk {wg2_no_attendees_session_pk}) has no attendees set",
            output,
        )
    ...
    def test_unresolvable_schedule(self):
        ...
        for group in self.all_groups:
            ...
            c = Constraint.objects.create(
                meeting=self.meeting, source=group, name_id="timerange"
            )
            c.timeranges.set(
                TimerangeName.objects.filter(slug__startswith=self.first_meeting_day)
            )
        ...
        generator = generate_schedule.ScheduleHandler(
            self.stdout, self.meeting.number, verbosity=2
        )
        violations, cost = generator.run()
    ...
    def test_too_many_sessions(self):
        ...
        self._create_basic_sessions()
        filterwarnings(
            "ignore", '"time relation" constraint only makes sense for 2 sessions'
        )
        generator = generate_schedule.ScheduleHandler(
            self.stdout, self.meeting.number, verbosity=1
        )
        generator.run()
        self.stdout.seek(0)
    ...
    def test_invalid_meeting_number(self):
        with self.assertRaises(CommandError):
            generator = generate_schedule.ScheduleHandler(
                self.stdout, "not-valid-meeting-number-aaaa", verbosity=0
            )
            generator.run()
    ...
    def test_base_schedule(self):
        ...
        self.assertIn(
            "scheduling 13 sessions in 19 timeslots", output
        )  # 19 because base is using one
        self.assertIn("Optimiser starting run 1", output)
        self.assertIn("Optimiser found an optimal schedule", output)
    ...
    def test_base_schedule_dynamic_cost(self):
        ...
        SchedTimeSessAssignment.objects.create(
            ...
            .exclude(sessionassignments__schedule=base_schedule)
            .first(),
        )
        ...
        conflict_session = SessionFactory(
            meeting=self.meeting,
            group=self.wg1,
            add_to_schedule=False,
            attendees=10,
        ...
        conflict_timeslot = (
            self.meeting.timeslot_set.filter(
        ...
        self.assertCountEqual(
            violations,
            [
                "{}: group conflict with {}".format(
                    base_session.group.acronym, self.wg1.acronym
                )
            ],
        )
        self.assertEqual(
        ...
        self.assertCountEqual(
            ...
            [
                ...
                "{}: missing adjacency with {}, adjacents are: ".format(
                    base_session.group.acronym, self.wg2.acronym
                ),
                "{}: scheduled in too small room".format(base_session.group.acronym),
            ],
        )
    ...
    def _create_basic_sessions(self):
        ...
        joint_session = SessionFactory(
            meeting=self.meeting, group=self.wg2, add_to_schedule=False
        )
        ...
        Constraint.objects.create(
            meeting=self.meeting, source=self.wg2, name_id="conflict", target=self.bof1
        )
        Constraint.objects.create(
            meeting=self.meeting,
            source=self.bof1,
        ...
        Constraint.objects.create(
            ...
            source=self.bof2,
            name_id="time_relation",
            time_relation="one-day-separation",
        )

        timerange_c1 = Constraint.objects.create(
            meeting=self.meeting, source=self.wg2, name_id="timerange"
        )
        timerange_c1.timeranges.set(
            TimerangeName.objects.filter(slug__startswith=self.first_meeting_day)
        )

        self.fixed_violations = [
            "No timeslot with sufficient duration available for wg2, "
            "requested 2:00:00, trimmed to 1:00:00",
            "No timeslot with sufficient capacity available for wg2, "
            "requested 500, trimmed to 100",
        ]
        self.fixed_cost = (
            BusinessConstraint.objects.get(slug="session_requires_trim").penalty * 2
        )
