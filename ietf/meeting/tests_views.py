...
from ietf.meeting.test_data import (
    make_meeting_test_data,
    make_interim_meeting,
...
from ietf.meeting.views import (
    session_draft_list,
    parse_agenda_filter_params,
    sessions_post_save,
    agenda_extract_schedule,
)
from ietf.meeting.views import (
    get_summary_by_area,
    get_summary_by_type,
    get_summary_by_purpose,
...
if os.path.exists(settings.GHOSTSCRIPT_COMMAND):
    ...
    skip_pdf_tests = True
    skip_message = (
        "Skipping pdf test: The binary for ghostscript wasn't found in the\n       "
...
class BaseMeetingTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.patcher = patch(
            "ietf.meeting.models.BlobShadowFileSystemStorage.base_location",
    ...
    def write_materials_file(
        self, meeting, doc, content, charset="utf-8", with_ext=None
    ):
        if with_ext is None:
        ...
        path = os.path.join(
            self.materials_dir, "%s/%s/%s" % (meeting.number, doc.type_id, filename)
        )
    ...
    def write_materials_files(self, meeting, session):
        ...
        self.write_materials_file(
            meeting,
            session.materials.filter(type="slides")
            .exclude(states__type__slug="slides", states__slug="deleted")
            .first(),
            "This is a slideshow",
        )
    ...
    def test_agenda_extract_schedule_location(self):
        ...
        processed = preprocess_assignments_for_agenda(
            SchedTimeSessAssignment.objects.filter(
                session__in=[hidden_sess, shown_sess]
            ),
            meeting,
        )
        AgendaKeywordTagger(assignments=processed).apply()
        extracted = {
            item.session.pk: agenda_extract_schedule(item) for item in processed
        }
        ...
        self.assertEqual(
            shown["location"],
            {"name": room.floorplan.name, "short": room.floorplan.short},
        )

    def test_agenda_extract_schedule_names(self):
        meeting = MeetingFactory(type_id="ietf")
        named_timeslots = TimeSlotFactory.create_batch(
            2, meeting=meeting, name="Timeslot Name"
        )
        unnamed_timeslots = TimeSlotFactory.create_batch(2, meeting=meeting, name="")
        named_sessions = SessionFactory.create_batch(
            2, meeting=meeting, name="Session Name"
        )
        unnamed_sessions = SessionFactory.create_batch(2, meeting=meeting, name="")
        pk_with = {
            "both named": named_sessions[0]
            .timeslotassignments.create(
                schedule=meeting.schedule,
                timeslot=named_timeslots[0],
            )
            .pk,
            "session named": named_sessions[1]
            .timeslotassignments.create(
                schedule=meeting.schedule,
                timeslot=unnamed_timeslots[0],
            )
            .pk,
            "timeslot named": unnamed_sessions[0]
            .timeslotassignments.create(
                schedule=meeting.schedule,
                timeslot=named_timeslots[1],
            )
            .pk,
            "neither named": unnamed_sessions[1]
            .timeslotassignments.create(
                schedule=meeting.schedule,
        ...
        processed = preprocess_assignments_for_agenda(
            meeting.schedule.assignments.all(), meeting
        )
        AgendaKeywordTagger(assignments=processed).apply()
        ...
        self.assertEqual(
            extracted[pk_with["timeslot named"]]["slotName"], "Timeslot Name"
        )
...
class MeetingTests(BaseMeetingTestCase):
    ...
    def test_meeting_agenda(self):
        ...
        meeting.importantdate_set.create(
            name_id="prelimagenda", date=date_today() + datetime.timedelta(days=20)
        )
        #
        ...
        r = self.client.get(
            urlreverse("agenda", kwargs=dict(num=meeting.number, utc="-utc"))
        )
        ...
        self.assertEqual(
            generated_data,
            {
                "meeting": {
                    ...
                    "updated": generated_data.get("meeting").get(
                        "updated"
                    ),  # Just expect the value to exist
                    "timezone": meeting.time_zone,
                    "infoNote": meeting.agenda_info_note,
                    "warningNote": meeting.agenda_warning_note,
                    "prelimAgendaDate": (
                        date_today() + datetime.timedelta(days=20)
                    ).isoformat(),
                },
                "categories": generated_data.get(
                    "categories"
                ),  # Just expect the value to exist
                "isCurrentMeeting": True,
                "usesNotes": False,  # make_meeting_test_data sets number=72
                "schedule": generated_data.get(
                    "schedule"
                ),  # Just expect the value to exist
                "floors": [],
            },
        ...
        with patch(
            "ietf.meeting.views.generate_agenda_data", return_value=generated_data
        ):
            r = self.client.get(
                urlreverse(
        ...
        self.assertJSONEqual(
            r.content.decode("utf8"), json.dumps(generated_data, cls=DjangoJSONEncoder)
        )
        # -> Session MaterialM
        r = self.client.get(
            urlreverse(
        ...
        updated = (
            meeting.updated().astimezone(meeting.tz()).strftime("%Y-%m-%d %H:%M:%S %Z")
        )
        ...
        ical_url = urlreverse(
            "ietf.meeting.views.agenda_ical", kwargs=dict(num=meeting.number)
        )
        ...
        self.assertContains(
            r,
            meeting.time_zone,
            msg_prefix="time_zone should appear in its original case",
        )
        self.assertNotEqual(
            meeting.time_zone,
        ...
        self.assertNotContains(
            r,
            meeting.time_zone.lower(),
            msg_prefix="time_zone should not be lower-cased",
        )
        self.assertNotEqual(
            meeting.time_zone,
        ...
        self.assertNotContains(
            r,
            meeting.time_zone.upper(),
            msg_prefix="time_zone should not be upper-cased",
        )
    ...
    def test_session_recordings_via_factories(self):
        session = SessionFactory(
            ...
            meeting__date=date_today() - datetime.timedelta(days=180),
            meeting__number=str(random.randint(108, 150)),
        )
        self.assertEqual(session.meetecho_recording_name, "")
        self.assertEqual(len(session.recordings()), 0)
        url = urlreverse(
            "ietf.meeting.views.session_details",
            kwargs=dict(num=session.meeting.number, acronym=session.group.acronym),
        )
    ...
    def test_delete_recordings(self):
        ...
        self.assertEqual(
            doc.get_state("recording").slug, "deleted", "recording state updated"
        )
        ...
        user = (
            PersonFactory()
        )  # naming matches the methods - user is a Person, not a User
    ...
    def test_agenda_redirects_for_old_meetings(self):
        ...
        meeting_with_schedule = MeetingFactory(
            type_id="ietf", number="36", populate_schedule=True
        )
        r = self.client.get(
            urlreverse(
                "agenda",
    ...
    @override_settings(
        ...
        MEETING_DOC_HREFS=settings.MEETING_DOC_CDN_HREFS,
    )
    def test_materials_through_cdn(self):
        ...
        doc = DocumentFactory.create(
            name="agenda-172-mars",
            type_id="agenda",
            title="Agenda",
            uploaded_filename="agenda-172-mars.txt",
            group=session107.group,
            rev="00",
            states=[("agenda", "active")],
        )
        pres = SessionPresentation.objects.create(
            session=session107, document=doc, rev=doc.rev
        )
        session107.presentations.add(pres)  #
        doc = DocumentFactory.create(
            name="minutes-172-mars",
            type_id="minutes",
            title="Minutes",
            uploaded_filename="minutes-172-mars.md",
            group=session107.group,
            rev="00",
            states=[("minutes", "active")],
        )
        pres = SessionPresentation.objects.create(
            session=session107, document=doc, rev=doc.rev
        )
        session107.presentations.add(pres)
        doc = DocumentFactory.create(
            name="slides-172-mars-1-active",
            type_id="slides",
            title="Slideshow",
            uploaded_filename="slides-172-mars.txt",
            group=session107.group,
            rev="00",
        ...
        for session in (
            ...
            for document in (session.agenda(), session.minutes(), session.slides()[0]):
                ...
                r = self.client.get(url)
                if (
                    session.meeting.number.isdigit()
                    and int(session.meeting.number) <= 96
                ):
                    self.assertEqual(r.status_code, 200)
                else:
    ...
    def test_named_session(self):
        ...
        meeting.importantdate_set.create(
            name_id="revsub", date=date_today() + datetime.timedelta(days=20)
        )
        group = GroupFactory()
        plain_session = SessionFactory(meeting=meeting, group=group)
        named_session = SessionFactory(
            meeting=meeting, group=group, name="I Got a Name"
        )
    ...
    def do_test_materials(self, meeting, session):
        ...
        if meeting.number.isdigit():
            url = urlreverse(
                "ietf.meeting.views.materials", kwargs=dict(num=meeting.number)
            )
            r = self.client.get(url)
            self.assertEqual(r.status_code, 200)
            q = PyQuery(r.content)
            ...
            for section in sections:
                self.assertEqual(
                    len(q(f"#{section}")),
                    1,
                    f"{section} section should exists in proceedings",
                )
            ...
            for doc in session.materials.exclude(states__slug="deleted"):
                url = urlreverse(
                    "ietf.meeting.views.materials_document",
                    kwargs=dict(num=meeting.number, document=doc.name),
                )
                r = self.client.get(url)
    ...
    def test_materials_has_edit_links(self):
        ...
        self.assertEqual(
            len(q(f'a[href^="{edit_url}"]')),
            1,
            "Link to mars session_details for mars chairman",
        )
        for acro in ["ietf", "ames"]:  # other groups with materials
            ...
            self.assertEqual(
                len(q(f'a[href^="{edit_url}"]')),
                0,
                f"No link to {acro} session_details for mars chairman",
            )
        ...
        for acro in ["mars", "ames"]:  # wgs
            ...
            self.assertEqual(
                len(q(f'a[href^="{edit_url}"]')),
                1,
                f"Link to session_details page for {acro}",
            )
        ...
        self.assertEqual(
            len(q(f'a[href^="{edit_url}#session"]')),
            1,
            f"Link to session_details page for {acro}",
        )
    ...
    def test_materials_document_extension_choice(self):
        ...
        self.write_materials_file(
            meeting, presentation.document, "Hi I'm a txt", with_ext=".txt"
        )
        self.write_materials_file(
            meeting, presentation.document, "Hi I'm a pptx", with_ext=".pptx"
        )

        # with no rev, prefers the uploaded_filename
        r = self.client.get(
            _url(document="slides-whatever", num=meeting.number)
        )  # no rev
        self.assertEqual(r.status_code, 200)
        ...
        # now create a pdf
        self.write_materials_file(
            meeting, presentation.document, "Hi I'm a pdf", with_ext=".pdf"
        )
        ...
        for ext in ["pdf", "pptx", "txt"]:
            r = self.client.get(
                _url(document="slides-whatever-00", num=meeting.number, ext=f".{ext}")
            )
            self.assertEqual(r.status_code, 200)
        ...
        r = self.client.get(
            _url(document="slides-whatever-00", num=meeting.number, ext=".docx")
        )
        self.assertEqual(r.status_code, 404)
    ...
    def test_materials_name_endswith_hyphen_number_number(self):
        ...
        url = urlreverse(
            "ietf.meeting.views.materials_document",
            kwargs=dict(
                document=sp.document.name + "-00", num=sp.session.meeting.number
            ),
        )
        r = self.client.get(url)
        self.assertContains(
        ...
        url = urlreverse(
            "ietf.meeting.views.materials_document",
            kwargs=dict(
                document=sp.document.name + "-02", num=sp.session.meeting.number
            ),
        )
        r = self.client.get(url)
    ...
    def test_important_dates(self):
        ...
        url = urlreverse(
            "ietf.meeting.views.important_dates", kwargs={"num": meeting.number}
        )
        r = self.client.get(url)
    ...
    def test_important_dates_ical(self):
        ...
        url = urlreverse(
            ...
            kwargs={"num": meeting.number, "output_format": "ics"},
        )
        r = self.client.get(url)
        ...
        expected_updated = updated.astimezone(datetime.timezone.utc).strftime(
            "%Y%m%dT%H%M%SZ"
        )
        ...
        with patch("ietf.meeting.models.Meeting.updated", return_value=None):
            ...
            self.assertContains(r, f"DTSTAMP:{expected_updated}")
            dtstamps_count = r.content.decode("utf-8").count(
                f"DTSTAMP:{expected_updated}"
            )
            self.assertEqual(dtstamps_count, meeting.importantdate_set.count())
    ...
    def test_group_ical(self):
        ...
        s2 = SessionFactory.create(
            meeting=meeting, group=s1.group, add_to_schedule=False
        )
        SchedTimeSessAssignment.objects.create(
            timeslot=t2, session=s2, schedule=meeting.schedule
        )
        #
        url = urlreverse(
            ...
            kwargs={
                "num": meeting.number,
                "acronym": s1.group.acronym,
            },
        )
        r = self.client.get(url)
        assert_ical_response_is_valid(
            self,
            r,
    ...
    def test_parse_agenda_filter_params(self):
        def _r(show=(), hide=(), showtypes=(), hidetypes=()):
            ...
            return dict(
                show=set(show),
                hide=set(hide),
                showtypes=set(showtypes),
                hidetypes=set(hidetypes),
            )
        ...
        for qstr, expected in (
            ...
            ("show=x", _r(show=["x"])),
            ("hide=x", _r(hide=["x"])),
            ("showtypes=x", _r(showtypes=["x"])),
            ("hidetypes=x", _r(hidetypes=["x"])),
            ...
            (
                "show=a&hide=b&showtypes=c&hidetypes=d",
                _r(show=["a"], hide=["b"], showtypes=["c"], hidetypes=["d"]),
            ),
        ):
            self.assertEqual(
    ...
    def do_ical_filter_test(self, meeting, querystring, expected_session_summaries):
        url = urlreverse(
            "ietf.meeting.views.agenda_ical", kwargs={"num": meeting.number}
        )
    ...
    def build_session_setup(self):
        ...
        agenda = DocumentFactory(
            type_id="agenda",
            group=session.group,
            uploaded_filename="agenda-%s-%s"
        ...
        for d in (draft1, draft2):
            file, _ = submission_file(
                name_in_doc=f"{d.name}-00",
                name_in_post=f"{d.name}-00.txt",
                templatename="test_submission.txt",
                group=session.group,
            )
        ...
        self.assertEqual(
            len(session_draft_list(session.meeting.number, session.group.acronym)), 2
        )
    ...
    def test_edit_schedule_properties(self):
        ...
        new_base = Schedule.objects.create(
            name="newbase", owner=schedule.owner, meeting=schedule.meeting
        )
    ...
    def test_agenda_by_type_ics(self):
        ...
        url = urlreverse(
            ...
            kwargs={"num": session.meeting.number, "type": "lead"},
        )
    ...
    def test_cancelled_ics(self):
        ...
        url = urlreverse(
            "ietf.meeting.views.agenda_ical", kwargs=dict(num=session.meeting.number)
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
    ...
    def test_session_materials(self):
        ...
        url = urlreverse(
            "ietf.meeting.views.session_materials", kwargs=dict(session_id=session.pk)
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
        ...
        not_deleted_slides = session.materials.filter(type="slides").exclude(
            states__type__slug="slides", states__slug="deleted"
        )
        self.assertGreater(
            not_deleted_slides.count(), 0
        )  # make sure this isn't a pointless test

        deleted_slides = session.materials.filter(
            type="slides", states__type__slug="slides", states__slug="deleted"
        )
        self.assertGreater(
            deleted_slides.count(), 0
        )  # make sure this isn't a pointless test
    ...
    def test_meetinghost_logo(self):
        ...
        url = urlreverse(
            ...
            kwargs=dict(host_id=host.pk, num=host.meeting.number),
        )
        r = self.client.get(url)
...
class EditMeetingScheduleTests(TestCase):
    ...
    def test_room_grouping(self):
        ...
        times = [
            ...
            [
                ...
                datetime.time(12, 0),
                datetime.time(13, 0),
            ],  # same times, but durations will differ
            [
                datetime.time(11, 30),
                datetime.time(12, 0),
                datetime.time(13, 0),
        ...
        for time_row, duration_row in zip(times, durations):
            room_groups.append(
                RoomFactory.create_batch(rooms_in_group, meeting=meeting)
            )
    ...
    def test_bof_session_tag(self):
        ...
        url = urlreverse(
            "ietf.meeting.views.edit_meeting_schedule", kwargs=dict(num=meeting.number)
        )
        ...
        q = PyQuery(r.content)
        self.assertEqual(
            len(q("#session{} .bof-tag".format(non_bof_session.pk))),
            0,
        ...
        self.assertEqual(
            len(bof_tags), 1, "BOF session should have one BOF session tag"
        )
        self.assertIn("BOF", bof_tags.eq(0).text(), 'BOF tag should contain text "BOF"')
    ...
    def _setup_for_swap_timeslots(self):
        ...
        t0 = (
            meeting.tz()
            .localize(datetime.datetime.combine(meeting.date, datetime.time(11, 0)))
            .astimezone(pytz.utc)
        )
        ...
        for room in room_groups[0]:
            ...
            TimeSlotFactory(
                ...
                location=room,
                duration=dur,
                time=t0 + datetime.timedelta(days=1, hours=2),
            )
            TimeSlotFactory(
                meeting=meeting,
                location=room,
                duration=dur,
                time=t0 + datetime.timedelta(days=2, hours=4),
            )
        ...
        for room in room_groups[1]:
            TimeSlotFactory(
                ...
                location=room,
                duration=dur,
                time=t0 + datetime.timedelta(hours=1),
            )
            TimeSlotFactory(
                meeting=meeting,
                location=room,
                duration=dur,
                time=t0 + datetime.timedelta(days=1, hours=3),
            )
            TimeSlotFactory(
                meeting=meeting,
                location=room,
                duration=dur,
                time=t0 + datetime.timedelta(days=2, hours=5),
            )
    ...
    def test_swap_timeslots(self):
        ...
        for index, room in enumerate(room_groups[0]):
            ...
            self.assertEqual(
                timeslots[0].session.name,
                str(timeslots[-1].pk),
                "Session from last timeslot in room (0, {}) should now be in first".format(
                    index
                ),
            )
            self.assertEqual(
                timeslots[-1].session.name,
                str(timeslots[0].pk),
                "Session from first timeslot in room (0, {}) should now be in last".format(
                    index
                ),
            )
            self.assertEqual(
    ...
    def test_swap_timeslots_denies_past(self):
        ...
        official_url = urlreverse(
            "ietf.meeting.views.edit_meeting_schedule", kwargs=dict(num=meeting.number)
        )
        unofficial_url = urlreverse(
            "ietf.meeting.views.edit_meeting_schedule",
            kwargs=dict(
                num=meeting.number,
        ...
        for room in room_groups[0]:
            ts = room.timeslot_set.last()
            ts.time = right_now + datetime.timedelta(
                minutes=9
            )  # must be < MEETING_SESSION_LOCK_TIME
            ts.save()
        self.assertTrue(
            room_groups[0][0].timeslot_set.last().time
            < right_now + settings.MEETING_SESSION_LOCK_TIME
        )
        self.assertTrue(
            room_groups[0][0].timeslot_set.first().time
            > right_now + settings.MEETING_SESSION_LOCK_TIME
        )
        post_data = dict(
            action="swaptimeslots",
    ...
    def test_swap_timeslots_handles_unmatched(self):
        ...
        for index, room in enumerate(room_groups[0]):
            ...
            if index == 1:
                ...
                self.assertIsNone(
                    timeslots[0].session,
                    "Unmatched timeslot should be empty after swap",
                )
                session_that_should_be_unassigned = Session.objects.get(
                    name=str(timeslots[0].pk)
                )
                self.assertEqual(
                    session_that_should_be_unassigned.timeslotassignments.count(),
                    0,
        ...
        for room in room_groups[0]:
            ...
            ts.time = meeting.tz().localize(
                datetime.datetime.combine(
                    yesterday, ts.time.astimezone(meeting.tz()).time()
                ),
            )
            ts.save()
        # timeslot_set is ordered by -time, so check that we know which is past/future
    ...
    def test_assign_session(self):
        ...
        timeslots = dict(
            past=TimeSlotFactory(
                meeting=meeting, time=right_now - datetime.timedelta(hours=1)
            ),
            future=TimeSlotFactory(
                meeting=meeting, time=right_now + datetime.timedelta(hours=1)
            ),
        )
        ...
        r = self.client.post(
            url_for(schedules["official"]), post_data(timeslots["past"])
        )
        ...
        r = self.client.post(
            url_for(schedules["unofficial"]), post_data(timeslots["past"])
        )
        self.assertEqual(r.status_code, 200)
        ...
        r = self.client.post(
            url_for(schedules["official"]), post_data(timeslots["future"])
        )
        self.assertEqual(r.status_code, 200)
        self.assertTrue(self._decode_json_response(r)["success"])
        ...
        r = self.client.post(
            url_for(schedules["unofficial"]), post_data(timeslots["future"])
        )
        self.assertEqual(r.status_code, 200)
        self.assertTrue(self._decode_json_response(r)["success"])
    ...
    def test_reassign_session(self):
        ...
        timeslots = dict(
            ...
            other_past=TimeSlotFactory(
                meeting=meeting, time=right_now - datetime.timedelta(hours=2)
            ),
            barely_future=TimeSlotFactory(
                meeting=meeting, time=right_now + datetime.timedelta(minutes=9)
            ),
            future=TimeSlotFactory(
                meeting=meeting, time=right_now + datetime.timedelta(hours=1)
            ),
            other_future=TimeSlotFactory(
                meeting=meeting, time=right_now + datetime.timedelta(hours=2)
            ),
        )
        ...
        r = self.client.post(
            url_for(schedules["official"]), post_data(session, timeslots["other_past"])
        )
        self.assertEqual(r.status_code, 400)
        self.assertEqual(
            self._decode_json_response(r),
        ...
        session = _new_session_in(timeslots["past"], schedules["official"])
        r = self.client.post(
            url_for(schedules["official"]), post_data(session, timeslots["future"])
        )
        self.assertEqual(r.status_code, 400)
        self.assertEqual(
            self._decode_json_response(r),
        ...
        r = self.client.post(
            url_for(schedules["official"]), post_data(session, timeslots["past"])
        )
        self.assertEqual(r.status_code, 400)
        self.assertEqual(
            self._decode_json_response(r),
        ...
        r = self.client.post(
            url_for(schedules["unofficial"]),
            post_data(session, timeslots["other_future"]),
        )
        self.assertEqual(r.status_code, 200)
        self.assertTrue(self._decode_json_response(r)["success"])
        session.delete()  # takes the SchedTimeSessAssignment with it
        ...
        session = _new_session_in(timeslots["future"], schedules["official"])
        r = self.client.post(
            url_for(schedules["official"]),
            post_data(session, timeslots["barely_future"]),
        )
        self.assertEqual(r.status_code, 400)
        self.assertEqual(
            self._decode_json_response(r),
        ...
        r = self.client.post(
            url_for(schedules["unofficial"]),
            post_data(session, timeslots["other_past"]),
        )
        self.assertEqual(r.status_code, 200)
        self.assertTrue(self._decode_json_response(r)["success"])
        session.delete()  # takes the SchedTimeSessAssignment with it
        ...
        session = _new_session_in(timeslots["past"], schedules["unofficial"])
        r = self.client.post(
            url_for(schedules["unofficial"]), post_data(session, timeslots["future"])
        )
        self.assertEqual(r.status_code, 200)
        self.assertTrue(self._decode_json_response(r)["success"])
        session.delete()  # takes the SchedTimeSessAssignment with it
        ...
        session = _new_session_in(timeslots["future"], schedules["unofficial"])
        r = self.client.post(
            url_for(schedules["unofficial"]), post_data(session, timeslots["past"])
        )
        self.assertEqual(r.status_code, 200)
        self.assertTrue(self._decode_json_response(r)["success"])
        session.delete()  # takes the SchedTimeSessAssignment with it
    ...
    def test_unassign_session(self):
        ...
        r = self.client.post(
            url_for(schedules["official"]),
            post_data(timeslots["past"], schedules["official"]),
        )
        self.assertEqual(r.status_code, 400)
        self.assertEqual(
            self._decode_json_response(r),
        ...
        r = self.client.post(
            url_for(schedules["unofficial"]),
            post_data(timeslots["past"], schedules["unofficial"]),
        )
        self.assertEqual(r.status_code, 200)
        self.assertTrue(self._decode_json_response(r)["success"])
        ...
        r = self.client.post(
            url_for(schedules["official"]),
            post_data(timeslots["barely_future"], schedules["official"]),
        )
        self.assertEqual(r.status_code, 400)
        self.assertEqual(
            self._decode_json_response(r),
        ...
        r = self.client.post(
            url_for(schedules["unofficial"]),
            post_data(timeslots["barely_future"], schedules["unofficial"]),
        )
        self.assertEqual(r.status_code, 200)
        self.assertTrue(self._decode_json_response(r)["success"])
        ...
        r = self.client.post(
            url_for(schedules["official"]),
            post_data(timeslots["future"], schedules["official"]),
        )
        self.assertEqual(r.status_code, 200)
        self.assertTrue(self._decode_json_response(r)["success"])
        ...
        r = self.client.post(
            url_for(schedules["unofficial"]),
            post_data(timeslots["future"], schedules["unofficial"]),
        )
        self.assertEqual(r.status_code, 200)
        self.assertTrue(self._decode_json_response(r)["success"])
    ...
    def test_editor_with_no_timeslots(self):
        ...
        self.assertEqual(
            meeting.timeslot_set.count(),
            0,
            "Test problem - meeting should not have any timeslots",
        )
        url = urlreverse(
            "ietf.meeting.views.edit_meeting_schedule", kwargs={"num": meeting.number}
        )
        self.assertTrue(
            self.client.login(username="secretary", password="secretary+password")
        )
        ...
        self.assertContains(
            r,
            urlreverse(
                "ietf.meeting.views.edit_timeslots", kwargs={"num": meeting.number}
            ),
        )
    ...
    def test_editor_time_zone(self):
        ...
        self.assertEqual(
            ...
            ts_start.astimezone(datetime.timezone.utc).isoformat(),
        )
        self.assertEqual(
            time_label.attr("data-end"),
            ts_end.astimezone(datetime.timezone.utc).isoformat(),
        )
        ...
        self.assertIn(
            f"{ts_start.hour:d}:{ts_start.minute:02d}-{ts_end.hour:d}:{ts_end.minute:02d}",
            origin_label,
        )
...
class EditTimeslotsTests(TestCase):
    ...
    def create_timeslots_url(meeting):
        return urlreverse(
            "ietf.meeting.views.create_timeslot", kwargs={"num": meeting.number}
        )
    ...
    def test_view_permissions(self):
        ...
        def _assert_permissions(comment):
            ...
            self.assertEqual(
                r.status_code,
                200,
                "secretary should be able to access the edit timeslots page {}".format(
                    comment
                ),
            )
    ...
    def test_linked_from_agenda_list(self):
        ...
        url = urlreverse(
            "ietf.meeting.views.list_schedules", kwargs={"num": meeting.number}
        )
    ...
    def test_with_no_rooms(self):
        ...
        self.assert_helpful_url(
            r,
            urlreverse(
                "ietf.secr.meetings.views.view", kwargs={"meeting_id": meeting.number}
            ),
    ...
    def test_shows_timeslots(self):
        ...
        day_columns = (
            dict()
        )  # map datetime to iterable with table col indices for that day
        ...
        self.assertEqual(
            len(timeslot_elts),
            len(timeslots),
            "Unexpected or missing timeslot elements",
        )
        for ts in timeslots:
            pk_elts = timeslot_elts.filter("#timeslot{}".format(ts.pk))
            self.assertEqual(
                len(pk_elts), 1, "Expect exactly one element for each timeslot"
            )
            elt = pk_elts.eq(0)
            self.assertIn(
                ts.name,
                elt.text(),
                "Timeslot name should appear in the element for {}".format(ts),
            )
            self.assertIn(
                str(ts.type),
                elt.text(),
                "Timeslot type should appear in the element for {}".format(ts),
            )
            self.assertEqual(
                _col_index(elt),
                expected_columns[ts.time.date()][ts.time.time()],
            ...
            self.assertEqual(
                len(edit_btn), 1, "Timeslot {} should have one edit button".format(ts)
            )
            # find the room heading for the row
            tr = elt.closest("tr")
            self.assertIn(
                ts.location.name,
                tr.children("th").eq(0).text(),
    ...
    def test_bulk_delete_buttons_exist(self):
        ...
        for day_label in days.items():
            self.assertEqual(
                len(day_label.find('.delete-button[data-delete-scope="day"]')),
                1,
        ...
        for slot_label in slots.items():
            self.assertEqual(
                len(slot_label.find('.delete-button[data-delete-scope="column"]')),
                1,
    ...
    def test_timeslot_collision_flag(self):
        ...
        TimeSlotFactory(
            meeting=meeting, time=t1.time, duration=t1.duration, location=t1.location
        )
        TimeSlotFactory(
            meeting=meeting, time=t1.time, duration=t1.duration
        )  # other location
        TimeSlotFactory(
            meeting=meeting,
            time=t1.time.replace(hour=t1.time.hour + 1),
        ...
        self.assertEqual(
            len(collision), 1, "Wrong number of timeslot collisions flagged"
        )
        self.assertEqual(
            len(no_collision), 3, "Wrong number of non-colliding timeslots"
        )
        # check that the cell containing t1 is the one flagged as a conflict
        self.assertEqual(
            len(collision.find("#timeslot{}".format(t1.pk))),
            1,
    ...
    def test_timeslot_in_use_flag(self):
        ...
        empty, has_official, has_other = TimeSlotFactory.create_batch(
            3, meeting=meeting, location=meeting.room_set.first()
        )
        SchedTimeSessAssignment.objects.create(
        ...
        self.assertEqual(
            empty_elt.attr("data-unofficial-use"),
            "false",
            "Unused timeslot should not be in use",
        )
        self.assertEqual(
            empty_elt.attr("data-official-use"),
            "false",
            "Unused timeslot should not be in use",
        )

        self.assertEqual(
            has_other_elt.attr("data-unofficial-use"),
            "true",
            "Unofficially used timeslot should be flagged",
        )
        self.assertEqual(
            has_other_elt.attr("data-official-use"),
            "false",
            "Unofficially used timeslot is not in official use",
        )

        self.assertEqual(
            has_official_elt.attr("data-unofficial-use"),
            "false",
            "Officially used timeslot not in unofficial use",
        )
        self.assertEqual(
            has_official_elt.attr("data-official-use"),
            "true",
    ...
    def test_edit_timeslot(self):
        ...
        self.assertContains(
            r, "?sched=1234", status_code=200
        )  # could check in more detail

        name_after = "New Name (tm)"
        type_after = "plenary"
        time_after = (time_utc + datetime.timedelta(days=1, hours=2)).astimezone(
            meeting.tz()
        )
        ...
        self.assertEqual(
            r["Location"],
            self.edit_timeslots_url(meeting) + "?sched=1234",
    ...
    def test_create_single_timeslot(self):
        ...
        self.assertEqual(
            str(ts.local_start_time().date().toordinal()), post_data["days"]
        )
        self.assertEqual(ts.local_start_time().strftime("%H:%M"), post_data["time"])
        self.assertEqual(
            str(ts.duration), "{}:00".format(post_data["duration"])
        )  # add seconds
    ...
    def test_invalid_create_timeslot(self):
        ...
        self.assertContains(
            r,
            "Please select a day or specify a date",
            status_code=400,
        ...
        self.assertEqual(
            TimeSlot.objects.count(), timeslot_count, "TimeSlot unexpectedly created"
        )
    ...
    def test_create_bulk_timeslots(self):
        ...
        self.assertEqual(
            meeting.timeslot_set.count(), len(timeslots_before) + new_slot_count
        )

        day_locs = set(
            (day, loc) for day in days for loc in locations
        )  # cartesian product
    ...
    def test_ajax_delete_timeslot(self):
        ...
        self.assertEqual(
            meeting.timeslot_set.filter(pk=ts_to_del.pk).count(),
            0,
            "Timeslot not deleted",
        )
        self.assertEqual(
            meeting.timeslot_set.filter(pk=ts_to_keep.pk).count(),
            1,
    ...
    def test_ajax_delete_timeslots_invalid(self):
        ...
        r = self.client.post(self.edit_timeslots_url(meeting), data=dict())
        ...
        self.assertEqual(
            meeting.timeslot_set.filter(pk=ts.pk).count(),
            1,
...
class ReorderSlidesTests(TestCase):
    ...
    def test_add_slides_to_session(self, mock_slides_manager_cls):
        for type_id in ("ietf", "interim"):
            ...
            self.client.login(
                ...
                password=chair_role.person.user.username + "+password",
            )
            ...
            SessionPresentationFactory.create_batch(
                3, document__type_id="slides", session=session2
            )
            for num, sp in enumerate(
                session2.presentations.filter(document__type_id="slides"), start=1
            ):
                sp.order = num
                sp.save()
            ...
            url = urlreverse(
                "ietf.meeting.views.ajax_add_slides_to_session",
                kwargs={"session_id": session2.pk, "num": session2.meeting.number},
            )
            ...
            self.assertEqual(r.json()["success"], True)
            self.assertEqual(
                session2.presentations.get(document=more_slides[0]).order, 1
            )
            self.assertEqual(
                list(
                    session2.presentations.order_by("order").values_list(
                        "order", flat=True
                    )
                ),
                list(range(1, 5)),
            ...
            self.assertEqual(
                mock_slides_manager_cls.return_value.add.call_args,
                call(session=session2, slides=more_slides[0], order=1),
            )
            mock_slides_manager_cls.reset_mock()
            ...
            self.assertEqual(r.json()["success"], True)
            self.assertEqual(
                session2.presentations.get(document=more_slides[1]).order, 5
            )
            self.assertEqual(
                list(
                    session2.presentations.order_by("order").values_list(
            ...
            self.assertEqual(
                mock_slides_manager_cls.return_value.add.call_args,
                call(session=session2, slides=more_slides[1], order=5),
            )
            mock_slides_manager_cls.reset_mock()
            ...
            self.assertEqual(r.json()["success"], True)
            self.assertEqual(
                session2.presentations.get(document=more_slides[2]).order, 3
            )
            self.assertEqual(
                list(
                    session2.presentations.order_by("order").values_list(
            ...
            self.assertEqual(
                mock_slides_manager_cls.return_value.add.call_args,
                call(session=session2, slides=more_slides[2], order=3),
            )
            mock_slides_manager_cls.reset_mock()
    ...
    def test_remove_slides_from_session(self, mock_slides_manager_cls):
        for type_id in ["ietf", "interim"]:
            ...
            self.assertEqual(
                ...
                call(session=session, slides=slides),
            )
            mock_slides_manager_cls.reset_mock()

            session2 = SessionFactory(group=session.group, meeting=session.meeting)
            sp_list = SessionPresentationFactory.create_batch(
                5, document__type_id="slides", session=session2
            )
            for num, sp in enumerate(
                session2.presentations.filter(document__type_id="slides"), start=1
            ):
            ...
            self.assertEqual(
                mock_slides_manager_cls.return_value.delete.call_args,
                call(session=session2, slides=sp_list[0].document),
            )
            mock_slides_manager_cls.reset_mock()
            ...
            self.assertEqual(
                mock_slides_manager_cls.return_value.delete.call_args,
                call(session=session2, slides=sp_list[4].document),
            )
            mock_slides_manager_cls.reset_mock()
            ...
            self.assertEqual(
                mock_slides_manager_cls.return_value.delete.call_args,
                call(session=session2, slides=sp_list[2].document),
            )
            mock_slides_manager_cls.reset_mock()
    ...
    def test_reorder_slides_in_session(self, mock_slides_manager_cls):
        ...
        sp_list = SessionPresentationFactory.create_batch(
            5, document__type_id="slides", session=session
        )
        ...
        for type_id in ["ietf", "interim"]:
            ...
            self.assertEqual(
                list(
                    session.presentations.order_by("order").values_list("pk", flat=True)
                ),
                _sppk_at(sppk, [2, 3, 1, 4, 5]),
            )
            self.assertTrue(mock_slides_manager_cls.called)
            self.assertEqual(
                mock_slides_manager_cls.call_args, call(api_config="fake settings")
    ...
    def test_slide_order_reconditioning(self):
        ...
        self.assertEqual(
            list(
                session.presentations.order_by("order").values_list("order", flat=True)
            ),
            list(range(1, 6)),
        )
...
class EditTests(TestCase):
    ...
    def test_official_record_schedule_is_read_only(self):
        ...
        #######################################################
        r, q = _set_date_offset_and_retrieve_page(
            meeting, 0 - 2 - meeting.days, self.client  # Meeting ended 2 days ago
        )
        self.assertTrue(q(""".alert:contains("You can't edit this schedule")"""))
        self.assertTrue(
            q(
                """.alert:contains("This is the official schedule for a meeting in the past")"""
            )
        )
        ...
        #######################################################
        r, q = _set_date_offset_and_retrieve_page(
            meeting, 0, self.client  # Meeting starts today
        )
        ...
        #######################################################
        r, q = _set_date_offset_and_retrieve_page(
            meeting, 7, self.client  # Meeting starts next week
        )
        self.assertFalse(q(""".alert:contains("You can't edit this schedule")"""))
        self.assertFalse(
            q(
    ...
    def test_edit_meeting_schedule(self):
        ...
        s2 = (
            Session.objects.filter(meeting=meeting, type="regular")
            .exclude(group=s1.group)
            .first()
        )
        ...
        base_timeslot = TimeSlot.objects.create(
            meeting=meeting,
            type_id="regular",
            location=room,
            ...
            time=meeting.tz().localize(
                datetime.datetime.combine(
                    meeting.date + datetime.timedelta(days=2), datetime.time(9, 30)
                )
            ),
        )

        timeslots = list(
            TimeSlot.objects.filter(meeting=meeting, type="regular").order_by("time")
        )

        base_session = SessionFactory(
            meeting=meeting,
        ...
        SchedTimeSessAssignment.objects.create(
            timeslot=base_timeslot, session=base_session, schedule=meeting.schedule.base
        )
        ...
        for s in [s1, s2]:
            ...
            self.assertTrue(e.find(f'a[href="{edit_session_url}"]'))
            self.assertTrue(
                e.find(
                    'a[href="{}?sched={}"]'.format(
                        urlreverse(
                            "ietf.meeting.views.cancel_session",
                            kwargs={"session_id": s.pk},
                        ),
                        meeting.schedule.pk,
                    )
                )
            ...
            self.assertEqual(
                constraints.eq(0).find(".bi-people-fill").parent().text(), "1"
            )  # 1 person in the constraint
            self.assertEqual(constraints.eq(1).attr("data-sessions"), str(s_other.pk))
            self.assertEqual(
                constraints.eq(1).find(".encircled").text(),
                "1" if s_other == s2 else "-1",
            )
            ...
            self.assertIn(
                str(round(s.requested_duration.total_seconds() / 60.0 / 60, 1)),
                e.find(".session-info .title").text(),
            )
        ...
        formatted_constraints1 = q(
            "#session{} .session-info .formatted-constraints > *".format(s1.pk)
        )
        self.assertIn(s2.group.acronym, formatted_constraints1.eq(0).html())
        self.assertIn(p.name, formatted_constraints1.eq(1).html())

        formatted_constraints2 = q(
            "#session{} .session-info .formatted-constraints > *".format(s2.pk)
        )
        ...
        url = urlreverse(
            "ietf.meeting.views.edit_meeting_schedule",
            kwargs=dict(
                num=meeting.number, owner=schedule.owner_email(), name=schedule.name
            ),
        )
        r = self.client.get(url)
        q = PyQuery(r.content)
        ...
        self.assertEqual(
            SchedTimeSessAssignment.objects.get(
                schedule=schedule, session=s_tombstone
            ).timeslot,
            timeslots[1],
        )
        self.assertTrue(
            PyQuery(json_content["tombstone"])(
                "#session{}.readonly".format(s_tombstone.pk)
            ).html()
        )
        ...
        self.assertEqual(json.loads(r.content)["success"], True)
        self.assertEqual(
            list(SchedTimeSessAssignment.objects.filter(schedule=schedule, session=s1)),
            [],
        )

        # try swapping days
        SchedTimeSessAssignment.objects.create(
            schedule=schedule, session=s1, timeslot=timeslots[0]
        )
        self.assertEqual(
            len(
                SchedTimeSessAssignment.objects.filter(
        ...
        self.assertEqual(
            len(
                SchedTimeSessAssignment.objects.filter(
                    schedule=schedule, session=s2, timeslot=timeslots[1]
                )
            ),
            1,
        )
        self.assertEqual(
            list(
                SchedTimeSessAssignment.objects.filter(
                    schedule=schedule, timeslot=timeslots[2]
                )
            ),
            [],
        )
        ...
        self.assertEqual(
            list(
                SchedTimeSessAssignment.objects.filter(
                    schedule=schedule, timeslot=timeslots[0]
                )
            ),
            [],
        )
        self.assertEqual(
            list(
                SchedTimeSessAssignment.objects.filter(
                    schedule=schedule, timeslot=timeslots[1]
                )
            ),
            [],
        )
        self.assertEqual(
            len(
                SchedTimeSessAssignment.objects.filter(
                    schedule=schedule, session=s1, timeslot=timeslots[2]
                )
            ),
            1,
        )
        self.assertEqual(
            list(SchedTimeSessAssignment.objects.filter(schedule=schedule, session=s2)),
            [],
        )
    ...
    def test_edit_meeting_timeslots_and_misc_sessions(self):
        ...
        url = urlreverse(
            ...
            kwargs=dict(
                num=meeting.number,
                owner=meeting.schedule.base.owner_email(),
                name=meeting.schedule.base.name,
            ),
        )
        r = self.client.get(url)
        q = PyQuery(r.content)
        ...
        for i in range(meeting.days):
            self.assertTrue(
                q(
                    '[data-day="{}"]'.format(
                        (meeting.date + datetime.timedelta(days=i)).isoformat()
                    )
                )
            )
        ...
        room_row = q(
            '.room-row[data-day="{}"][data-room="{}"]'.format(
                break_slot.time.date().isoformat(), break_slot.location_id
            )
        )
        ...
        self.assertEqual(
            test_timeslot.duration, datetime.timedelta(hours=1, minutes=30)
        )
        ...
        test_session = Session.objects.get(
            meeting=meeting, timeslotassignments__timeslot=test_timeslot
        )
        self.assertEqual(test_session.short, "ietf-testing")
        self.assertEqual(test_session.group, ietf_group)

        self.assertTrue(
            SchedulingEvent.objects.filter(session=test_session, status="sched")
        )
        ...
        # delete timeslot
        test_presentation = Document.objects.create(
            name="slides-test", type_id="slides"
        )
        SessionPresentation.objects.create(
            document=test_presentation, rev="1", session=test_session
        )
        ...
        # set agenda note
        assignment = SchedTimeSessAssignment.objects.filter(
            session__group__acronym="mars", schedule=meeting.schedule
        ).first()
    ...
    def test_edit_meeting_schedule_conflict_types(self):
        ...
        self.assertEqual(
            tostring(s1_constraints[0][0]), chair_conf_label
        )  # [0][0] is the innermost <span>
        ...
        conf_label_reversed = (
            b'<span class="encircled">-1</span>'  # the '-' is inside the span!
        )
        ...
        self.assertEqual(
            tostring(s1_constraints[1][0]), conf_label_reversed
        )  # [0][0] is the innermost <span>
    ...
    def test_new_meeting_schedule(self):
        ...
        # new from scratch
        url = urlreverse(
            "ietf.meeting.views.new_meeting_schedule", kwargs=dict(num=meeting.number)
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        ...
        new_schedule = Schedule.objects.get(
            meeting=meeting, owner__user__username="secretary", name="scratch"
        )
    ...
    def test_copy_meeting_schedule(self):
        ...
        new_schedule = Schedule.objects.get(
            meeting=meeting, owner__user__username="secretary", name="copy"
        )
        self.assertEqual(new_schedule.public, True)
        ...
        old_assignments = {
            ...
            for a in SchedTimeSessAssignment.objects.filter(schedule=meeting.schedule)
        }
    ...
    def test_schedule_read_permissions(self):
        ...
        url = urlreverse(
            "ietf.meeting.views.edit_meeting_schedule",
            kwargs=dict(num=meeting.number, owner=schedule.owner_email(), name="foo"),
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 404)
    ...
    def test_edit_session(self):
        session = SessionFactory(
            meeting__type_id="ietf", group__type_id="team"
        )  # type determines allowed session purposes
        ...
        self.client.login(username="secretary", password="secretary+password")
        url = urlreverse(
            "ietf.meeting.views.edit_session", kwargs={"session_id": session.pk}
        )
        r = self.client.get(url)
    ...
    def test_cancel_session(self):
        ...
        url = urlreverse(
            "ietf.meeting.views.cancel_session", kwargs={"session_id": session.pk}
        )
        return_url = urlreverse(
            "ietf.meeting.views.edit_meeting_schedule",
            kwargs={"num": session.meeting.number},
        )
        ...
        self.assertRedirects(
            r, return_url
        )  # should redirect immediately when session is already canceled
    ...
    def test_edit_timeslots(self):
        ...
        r = self.client.get(
            urlreverse(
                "ietf.meeting.views.edit_timeslots", kwargs=dict(num=meeting.number)
            )
        )
    ...
    def test_edit_timeslot_type(self):
        ...
        url = urlreverse(
            ...
            kwargs=dict(num=timeslot.meeting.number, slot_id=timeslot.id),
        )
        login_testing_unauthorized(self, "secretary", url)
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
    ...
    def test_slot_to_the_right(self):
        ...
        mars_slot = TimeSlot.objects.get(
            sessionassignments__session=session,
            sessionassignments__schedule__name="test-schedule",
        )
    ...
    def test_persistent_enabled_timeslot_types(self):
        ...
        q = PyQuery(r.content)
        self.assertEqual(
            len(q('#timeslot-type-toggles-modal input[value="regular"][checked]')), 1
        )
        self.assertEqual(
            len(q('#timeslot-type-toggles-modal input[value="other"]:not([checked])')),
            1,
        )
        self.assertEqual(
            len(q('#timeslot-type-toggles-modal input[value="reg"]:not([checked])')), 1
        )

        # test with 'regular' and 'other' enabled via session store
        client_session = (
            self.client.session
        )  # must store as var, new session is created on access
        ...
        self.assertEqual(
            len(q('#timeslot-type-toggles-modal input[value="other"][checked]')), 1
        )
        self.assertEqual(
            len(q('#timeslot-type-toggles-modal input[value="reg"]:not([checked])')), 1
        )
...
class SessionDetailsTests(TestCase):
    ...
    def test_session_details(self):
        ...
        r = self.client.get(url)
        self.assertTrue(
            all([x in unicontent(r) for x in ("slides", "agenda", "minutes", "draft")])
        )
    ...
    def test_session_details_has_import_minutes_buttons(self):
        ...
        with patch(
            "ietf.meeting.views.can_manage_session_materials", return_value=False
        ):
            r = self.client.get(session_details_url)
            self.assertEqual(r.status_code, 200)
            q = PyQuery(r.content)
            self.assertEqual(
                len(q(f'a[href="{import_minutes_url}"]')),
                0,
                "Do not show import new minutes buttons to non-materials manager",
            )
        with patch(
            "ietf.meeting.views.can_manage_session_materials", return_value=True
        ):
            r = self.client.get(session_details_url)
            self.assertEqual(r.status_code, 200)
            q = PyQuery(r.content)
    ...
    def test_session_details_past_interim(self):
        ...
        self.assertEqual(r.status_code, 200)
        self.assertNotIn(
            "The materials upload cutoff date for this session has passed",
            unicontent(r),
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        self.client.login(
            username=chair.person.user.username,
            password=chair.person.user.username + "+password",
        )
        self.assertTrue(
            all([x in unicontent(r) for x in ("slides", "agenda", "minutes", "draft")])
        )
    ...
    def test_add_session_drafts(self):
        ...
        old_draft = (
            session.presentations.filter(document__type="draft").first().document
        )
        ...
        q = PyQuery(r.content)
        self.assertEqual(
            1, len(q(".alert-warning:contains('may affect published proceedings')"))
        )
...
class EditScheduleListTests(TestCase):
    ...
    def test_list_schedules(self):
        url = urlreverse(
            "ietf.meeting.views.list_schedules", kwargs={"num": self.mtg.number}
        )
        login_testing_unauthorized(self, "secretary", url)
        r = self.client.get(url)
        self.assertTrue(r.status_code, 200)
    ...
    def test_diff_schedules(self):
        ...
        url = urlreverse(
            "ietf.meeting.views.diff_schedules", kwargs={"num": meeting.number}
        )
        login_testing_unauthorized(self, "secretary", url)
        r = self.client.get(url)
        self.assertTrue(r.status_code, 200)

        from_schedule = Schedule.objects.get(
            meeting=meeting, name="test-unofficial-schedule"
        )

        session1 = Session.objects.filter(
            meeting=meeting, group__acronym="mars"
        ).first()
        session2 = Session.objects.filter(
            meeting=meeting, group__acronym="ames"
        ).first()
        session3 = SessionFactory(
            meeting=meeting,
            group=Group.objects.get(acronym="mars"),
            attendees=10,
            requested_duration=datetime.timedelta(minutes=70),
            add_to_schedule=False,
        )
        SchedulingEvent.objects.create(
            session=session3, status_id="schedw", by=Person.objects.first()
        )

        slot2 = (
            TimeSlot.objects.filter(meeting=meeting, type="regular")
            .order_by("-time")
            .first()
        )
        ...
        new_url = urlreverse(
            ...
            kwargs=dict(
                num=meeting.number,
                owner=from_schedule.owner_email(),
                name=from_schedule.name,
            ),
        )
        r = self.client.post(
            new_url,
        ...
        edit_url = urlreverse(
            ...
            kwargs=dict(
                num=meeting.number,
                owner=to_schedule.owner_email(),
                name=to_schedule.name,
            ),
        )
...
class InterimTests(TestCase):
    ...
    def do_interim_skip_announcement_test(
        self, base_session=False, extra_session=False, canceled_session=False
    ):
        make_meeting_test_data()
        group = Group.objects.get(acronym="irg")
        ...
        if base_session:
            base_session = SessionFactory(
                meeting=meeting, status_id="apprw", add_to_schedule=False
            )
        ...
        if base_session:
            self.assertEqual(
                meeting_sessions.get(pk=base_session.pk).current_status, "sched"
            )
        if extra_session:
            self.assertEqual(
                meeting_sessions.get(pk=extra_session.pk).current_status, "sched"
            )
        if canceled_session:
            self.assertEqual(
                meeting_sessions.get(pk=canceled_session.pk).current_status,
                "canceledpa",
            )
    ...
    def test_interim_skip_announcement_with_extra_and_canceled_sessions(self):
        ...
        self.do_interim_skip_announcement_test(
            extra_session=True, canceled_session=True
        )

    def test_interim_skip_announcement_with_extra_and_canceled_sessions_and_base_sched(
        self,
    ):
        """skip_announcement should schedule multiple sessions and leave canceled session alone"""
        self.do_interim_skip_announcement_test(
            extra_session=True, canceled_session=True, base_session=True
        )

    def do_interim_send_announcement_test(
        self, base_session=False, extra_session=False, canceled_session=False
    ):
        make_interim_test_data(meeting_tz="America/Los_Angeles")
        session = (
            Session.objects.with_current_status()
            .filter(
                meeting__type="interim", group__acronym="mars", current_status="apprw"
            )
            .first()
        )
        meeting = session.meeting
        ...
        for sess in [session, base_session, extra_session]:
            if sess:
                ...
                self.assertRegex(
                    ...
                    r"(%s\s+to\s+%s\s+UTC)"
                    % (
                        timeslot.utc_start_time().strftime("%H:%M"),
                        timeslot.utc_end_time().strftime("%H:%M"),
                    ),
                )
    ...
    def test_interim_send_announcement_with_extra_and_canceled_sessions_and_base_sched(
        self,
    ):
        self.do_interim_send_announcement_test(
            extra_session=True, canceled_session=True, base_session=True
    ...
    def do_interim_approve_by_ad_test(
        self, base_session=False, extra_session=False, canceled_session=False
    ):
        make_interim_test_data()
        session = (
    ...
    def test_interim_approve_by_ad_with_extra_and_canceled_sessions_and_base_sched(
        self,
    ):
        self.do_interim_approve_by_ad_test(
            extra_session=True, canceled_session=True, base_session=True
    ...
    def do_interim_approve_by_secretariat_test(
        self, base_session=False, extra_session=False, canceled_session=False
    ):
        make_interim_test_data()
        session = (
    ...
    def test_interim_approve_by_secretariat_with_extra_session_and_base_sched(self):
        self.do_interim_approve_by_secretariat_test(
            extra_session=True, base_session=True
        )
    ...
    def test_interim_approve_by_secretariat_with_canceled_session_and_base_sched(self):
        self.do_interim_approve_by_secretariat_test(
            canceled_session=True, base_session=True
        )
    ...
    def test_interim_approve_by_secretariat_with_extra_and_canceled_sessions_and_base_sched(
        self,
    ):
        self.do_interim_approve_by_secretariat_test(
            extra_session=True, canceled_session=True, base_session=True
    ...
    def do_upcoming_test(self, querystring=None, create_meeting=True):
        ...
        today = date_today()
        interims = dict(
            mars=add_event_info_to_session_qs(
                Session.objects.filter(
                    meeting__type="interim",
                    meeting__date__gt=today,
                    group__acronym="mars",
                )
            )
            ...
            .first()
            .meeting,
            ames=add_event_info_to_session_qs(
                Session.objects.filter(
                    meeting__type="interim",
                    meeting__date__gt=today,
                    group__acronym="ames",
    ...
    def test_upcoming_ical(self):
        ...
        Session.objects.filter(meeting__type_id="interim").update(
            remote_instructions=""
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        ...
        with patch("ietf.meeting.models.Meeting.updated", return_value=None):
            ...
            expected_updated = "19700101T000000Z"
            self.assertEqual(
                1, r.content.decode("utf-8").count(f"DTSTAMP:{expected_updated}")
            )
    ...
    def test_interim_request_options(self):
        ...
        self.assertEqual(
            ...
            .filter(state__in=("active", "proposed", "bof"))
            .count(),
            len(q("#id_group option")) - 1,
        )  # -1 for options placeholder
        self.client.logout()
    ...
    def do_interim_request_single_virtual(self, emails_expected):
        ...
        meeting_count = Meeting.objects.filter(
            number__contains="-%s-" % group.acronym, date__year=date.year
        ).count()
        ...
        with patch(
            "ietf.meeting.views.sessions_post_save", wraps=sessions_post_save
        ) as mock:
        ...
        self.assertEqual(
            meeting.number, "interim-%s-%s-%s" % (date.year, group.acronym, next_num)
        )
        ...
        self.assertEqual(retrieve_str("agenda", doc.uploaded_filename), agenda)
    ...
    def test_interim_request_single_virtual_settings_approval_required(self):
        ...
        self.assertEqual(
            meeting.session_set.last().schedulingevent_set.last().status_id, "apprw"
        )
    ...
    def test_interim_request_single_virtual_settings_approval_not_required(self):
        ...
        self.assertEqual(
            meeting.session_set.last().schedulingevent_set.last().status_id, "scheda"
        )
    ...
    def test_interim_request_single_in_person(self):
        ...
        self.assertEqual(timeslot.duration, duration)
        self.assertEqual(
            retrieve_str("agenda", session.agenda().uploaded_filename), agenda
        )
    ...
    def test_interim_request_series(self):
        ...
        self.assertEqual(
            meeting.number, "interim-%s-%s-%s" % (date2.year, group.acronym, next_num2)
        )
        self.assertEqual(meeting.city, city)
        self.assertEqual(meeting.country, country)
        self.assertEqual(meeting.time_zone, time_zone)
    ...
    def test_can_approve_interim_request(self):
        ...
        meeting = (
            add_event_info_to_session_qs(
                Session.objects.filter(meeting__type="interim", group=group)
            )
            .filter(current_status="apprw")
            .first()
            .meeting
        )
    ...
    def test_interim_request_details(self):
        ...
        self.assertEqual(r.status_code, 200)
        start_time = (
            meeting.session_set.first()
            .official_timeslotassignment()
            .timeslot.local_start_time()
            .strftime("%H:%M")
        )
        utc_start_time = (
            meeting.session_set.first()
    ...
    def test_interim_request_details_cancel(self):
        ...
        make_interim_test_data()
        mars_sessions = Session.objects.with_current_status().filter(
            meeting__type="interim",
            group__acronym="mars",
        )
        ...
        for meeting in (meeting_apprw, meeting_sched):
            ...
            for username, password in usernames_and_passwords:
                ...
                self.assertEqual(
                    ...
                    1,
                    "Should be exactly one cancel meeting button for user %s"
                    % username,
                )
                self.assertEqual(
                    cancel_meeting_btns.eq(0).attr("href"),
                    urlreverse(
                        "ietf.meeting.views.interim_request_cancel",
                        kwargs={"number": meeting.number},
                    ),
                ...
                self.assertEqual(
                    len(q("a.btn:contains('Cancel Session')")),
                    0,
    ...
    def test_interim_request_details_status(self):
        ...
        sessions = list(
            Session.objects.with_current_status().filter(
                meeting__type="interim", group__acronym="mars"
            )
        )
        ...
        for session in [
            Session.objects.with_current_status().get(pk=s.pk) for s in sessions
        ]:
            url = urlreverse(
                "ietf.meeting.views.interim_request_details",
        ...
        for session in [
            ...
            SchedulingEvent.objects.create(
                session=session, status=canceledpa_status, by=some_person
            )
            url = urlreverse(
                "ietf.meeting.views.interim_request_details",
                kwargs={"number": session.meeting.number},
        ...
        for session in [
            ...
            SchedulingEvent.objects.create(
                session=second_session, status=canceled_status, by=some_person
            )
            url = urlreverse(
                "ietf.meeting.views.interim_request_details",
                kwargs={"number": session.meeting.number},
    ...
    def do_interim_request_disapprove_test(
        self, extra_session=False, canceled_session=False
    ):
        make_interim_test_data()
        session = (
            Session.objects.with_current_status()
    ...
    def test_interim_request_cancel(self, mock):
        ...
        meeting = (
            add_event_info_to_session_qs(
                Session.objects.filter(meeting__type="interim", group__acronym="mars")
            )
            .filter(current_status="sched")
            .first()
            .meeting
    ...
    def test_interim_request_session_cancel(self, mock):
        ...
        self.client.login(username="marschairman", password="marschairman+password")
        length_before = len(outbox)
        canceled_count_before = (
            meeting.session_set.with_current_status()
            .filter(current_status__in=["canceled", "canceledpa"])
            .count()
        )
        r = self.client.post(url, {"comments": comments})
        self.assertRedirects(
        ...
        session = (
            Session.objects.with_current_status()
            .filter(
                meeting__type="interim", group__acronym="mars", current_status="sched"
            )
            .first()
        )
        meeting = session.meeting
        ...
        # Add another session
        SessionFactory(
            meeting=meeting, status_id="sched"
        )  # two sessions so canceling a session makes sense
    ...
    def test_interim_request_edit_no_notice(self):
        ...
        url = urlreverse(
            "ietf.meeting.views.interim_request_edit", kwargs={"number": meeting.number}
        )
        ...
        self.assertEqual(
            ...
            meeting.tz().localize(
                datetime.datetime.combine(
                    formset_initial["date"], datetime.time(12, 34)
                )
            ),
        )
    ...
    def test_interim_request_edit(self):
        ...
        data = {
            ...
            "session_set-0-date": formset_initial["date"].strftime("%Y-%m-%d"),
            "session_set-0-time": "12:34",
            "session_set-0-requested_duration": self.strfdelta(
                new_duration, "{hours}:{minutes}"
            ),
            "session_set-0-remote_instructions": formset_initial["remote_instructions"],
            #'session_set-0-agenda':formset_initial['agenda'],
            "session_set-0-agenda_note": formset_initial["agenda_note"],
    ...
    def test_interim_request_edit_agenda_updates_doc(self):
        ...
        self.assertEqual(
            agenda_doc.rev,
            f"{int(rev_before) + 1:02}",
            "Revision of agenda should increase",
        )
        self.assertNotEqual(
            agenda_doc.uploaded_filename,
            uploaded_filename_before,
            "Uploaded filename should be updated",
        )
        with (
            Path(agenda_doc.get_file_path()) / agenda_doc.uploaded_filename
        ).open() as f:
            self.assertEqual(
                f.read(),
                "modified agenda contents",
                "New agenda contents should be saved",
            )
        self.assertEqual(
    ...
    def test_group_ical(self):
        ...
        meeting = Meeting.objects.filter(
            type="interim", session__group__acronym="mars"
        ).first()
        s1 = Session.objects.filter(meeting=meeting, group__acronym="mars").first()
        self.assertGreater(
            len(s1.remote_instructions), 0, "Expected remote_instructions to be set"
        )
        a1 = s1.official_timeslotassignment()
        t1 = a1.timeslot
        # Create an extra session
...
class IphoneAppJsonTests(TestCase):
    def test_iphone_app_json_interim(self):
        ...
        url = urlreverse(
            "ietf.meeting.views.agenda_json", kwargs={"num": meeting.number}
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        data = r.json()
        ...
        msessions = meeting.session_set.exclude(
            type__in=["lead", "offagenda", "break", "reg"]
        )
        self.assertEqual(len(jsessions), msessions.count())
        for s in jsessions:
            self.assertTrue(
                msessions.filter(group__acronym=s["group"]["acronym"]).exists()
            )
...
class FinalizeProceedingsTests(TestCase):
    def test_finalize_proceedings(self):
        make_meeting_test_data()
        meeting = Meeting.objects.filter(type_id="ietf").order_by("id").last()
        meeting.session_set.filter(group__acronym="mars").first().presentations.create(
            document=Document.objects.filter(type="draft").first(), rev=None
        )

        url = urlreverse(
            "ietf.meeting.views.finalize_proceedings", kwargs={"num": meeting.number}
        )
        login_testing_unauthorized(self, "secretary", url)
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        ...
        self.assertEqual(
            meeting.session_set.filter(group__acronym="mars")
            .first()
            .presentations.filter(document__type="draft")
            .first()
            .rev,
            None,
        )
    ...
    def test_bluesheet_generation(self, mock):
        meeting = MeetingFactory(
            type_id="ietf", number="107"
        )  # number where generate_bluesheets should not be called
        ...
        meeting = MeetingFactory(
            type_id="ietf", number="108"
        )  # number where generate_bluesheets should be called
        SessionFactory.create_batch(5, meeting=meeting)
        url = urlreverse(
            "ietf.meeting.views.finalize_proceedings", kwargs={"num": meeting.number}
        )
...
class MaterialsTests(TestCase):
    ...
    def test_upload_bluesheets(self):
        ...
        self.assertEqual(r.status_code, 302)
        bs_doc = (
            session.presentations.filter(document__type_id="bluesheets")
            .first()
            .document
        )
        self.assertEqual(bs_doc.rev, "00")
        self.assertEqual(
            retrieve_str("bluesheets", f"{bs_doc.name}-{bs_doc.rev}.pdf"), test_content
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
    ...
    def test_upload_bluesheets_interim_chair_access(self):
        ...
        mars = Group.objects.get(acronym="mars")
        session = SessionFactory(
            meeting__type_id="interim", group=mars, meeting__date=date_today()
        )
        url = urlreverse(
            "ietf.meeting.views.upload_session_bluesheets",
            kwargs={"num": session.meeting.number, "session_id": session.id},
    ...
    def test_upload_minutes_agenda(self):
        for doctype in ("minutes", "agenda"):
            for future in (True, False):
                mtg_date = date_today() + datetime.timedelta(
                    days=180 if future else -180
                )
                session = SessionFactory(
                    meeting__type_id="ietf", meeting__date=mtg_date
                )
                if doctype == "minutes":
                    url = urlreverse(
                ...
                r = self.client.post(
                    url, dict(submission_method="upload", file=test_file)
                )
                self.assertEqual(r.status_code, 200)
                q = PyQuery(r.content)
                self.assertTrue(q("form .is-invalid"))
                ...
                test_file = BytesIO(
                    b'<html><frameset><frame src="foo.html"></frame><frame src="bar.html"></frame></frameset></html>'
                )
                ...
                test_file = BytesIO(
                    b"<html><head><title>Title</title></head><body><h1>Title</h1><section>Some text</section></body></html>"
                )
                ...
                self.assertEqual(r.status_code, 302)
                doc = (
                    session.presentations.filter(document__type_id=doctype)
                    .first()
                    .document
                )
                ...
                r = self.client.post(
                    url,
                    dict(
                        submission_method="upload", file=test_file, apply_to_all=False
                    ),
                )
                self.assertEqual(r.status_code, 302)
                doc = (
                ...
                self.assertFalse(
                    session2.presentations.filter(document__type_id=doctype)
                )
                ...
                r = self.client.post(
                    url,
                    dict(submission_method="upload", file=test_file, apply_to_all=True),
                )
                self.assertEqual(r.status_code, 302)
                ...
                # Test bad encoding
                test_file = BytesIO(
                    "<html><h1>Title</h1><section>Some\x93text</section></html>".encode(
                        "latin1"
                    )
                )
                test_file.name = "some.html"
                ...
                self.requests_mock.get(
                    f"{session.notes_url()}/download", text="markdown notes"
                )
                self.requests_mock.get(
                    f"{session.notes_url()}/info",
                    text=json.dumps(
                        {"title": "title", "updatetime": "2021-12-01T17:11:00z"}
                    ),
                )
    ...
    def test_upload_minutes_agenda_unscheduled(self):
        for doctype in ("minutes", "agenda"):
            ...
            r = self.client.post(
                url,
                dict(submission_method="upload", file=test_file, apply_to_all=False),
            )
        ...
        for doctype in ("minutes", "agenda"):
            for future in (True, False):
                session = SessionFactory(
                    ...
                    + datetime.timedelta(days=180 if future else -180),
                )
                if doctype == "minutes":
                    url = urlreverse(
                        "ietf.meeting.views.upload_session_minutes",
    ...
    def test_upload_narrativeminutes(self):
        for type_id in ["interim", "ietf"]:
            ...
            url = urlreverse(
                "ietf.meeting.views.upload_session_narrativeminutes",
                kwargs={"num": session.meeting.number, "session_id": session.id},
            )
            self.client.logout()
            login_testing_unauthorized(self, "secretary", url)
            ...
            self.assertEqual(r.status_code, 302)
            doc = (
                session.presentations.filter(document__type_id=doctype).first().document
            )
            self.assertEqual(doc.rev, "00")
            retrieved_bytes = retrieve_bytes(doctype, f"{doc.name}-{doc.rev}.txt")
            self.assertEqual(retrieved_bytes, test_bytes)
    ...
    def test_enter_agenda(self):
        ...
        redirect_url = urlreverse(
            "ietf.meeting.views.session_details",
            kwargs={"num": session.meeting.number, "acronym": session.group.acronym},
        )
        login_testing_unauthorized(self, "secretary", url)
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
    ...
    def test_upload_slides(self, mock_slides_manager_cls):
        ...
        url = urlreverse(
            ...
            kwargs={"num": session1.meeting.number, "session_id": session1.id},
        )
        login_testing_unauthorized(self, "secretary", url)
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        ...
        self.assertEqual(
            retrieve_bytes("slides", f"{sp.document.name}-{sp.document.rev}.txt"),
            test_bytes,
        )
        ...
        url = urlreverse(
            "ietf.meeting.views.upload_session_slides",
            kwargs={"num": session2.meeting.number, "session_id": session2.id},
        )
        ...
        url = urlreverse(
            ...
            kwargs={
                ...
                "name": session2.presentations.get(order=2).document.name,
            },
        )
        r = self.client.get(url)
        self.assertTrue(r.status_code, 200)
        ...
        self.assertEqual(
            retrieve_bytes(
                "slides",
                f"{replacement_sp.document.name}-{replacement_sp.document.rev}.txt",
            ),
            test_bytes,
        )
        self.assertEqual(
    ...
    def test_upload_slide_title_bad_unicode(self):
        ...
        test_file.name = "not_really.txt"
        r = self.client.post(
            url, dict(file=test_file, title="title with bad character \U0001fabc ")
        )
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
        self.assertTrue(q("form .is-invalid"))
    ...
    def test_remove_sessionpresentation(self, mock_slides_manager_cls):
        ...
        url = urlreverse(
            "ietf.meeting.views.remove_sessionpresentation",
            kwargs={"num": session.meeting.number, "session_id": 0, "name": doc.name},
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, 404)
        self.assertFalse(mock_slides_manager_cls.called)
    ...
    def test_propose_session_slides(self):
        for type_id in ["ietf", "interim"]:
            ...
            session.meeting.importantdate_set.create(
                name_id="revsub", date=date_today() + datetime.timedelta(days=20)
            )
            newperson = PersonFactory()

            session_overview_url = urlreverse(
                "ietf.meeting.views.session_details",
                kwargs={
                    "num": session.meeting.number,
                    "acronym": session.group.acronym,
            ...
            r = self.client.post(
                ...
                dict(
                    file=test_file,
                    title="a selfapproved test slide file",
                    apply_to_all=True,
                    approved=True,
                ),
            )
            ...
            self.assertFalse(
                exists_in_storage("staging", sp.document.uploaded_filename)
            )
            self.assertEqual(
                retrieve_bytes("slides", sp.document.uploaded_filename), test_bytes
            )
            self.client.logout()
    ...
    def test_disapprove_proposed_slides(self):
        ...
        submission.session.meeting.importantdate_set.create(
            name_id="revsub", date=date_today() + datetime.timedelta(days=20)
        )
        self.assertEqual(
            SlideSubmission.objects.filter(status__slug="pending").count(), 1
        )
        chair = RoleFactory(group=submission.session.group, name_id="chair").person
        url = urlreverse(
            "ietf.meeting.views.approve_proposed_slides",
            kwargs={
                "slidesubmission_id": submission.pk,
                "num": submission.session.meeting.number,
            },
        )
        ...
        self.assertEqual(r.status_code, 302)
        self.assertEqual(
            SlideSubmission.objects.filter(status__slug="rejected").count(), 1
        )
        self.assertEqual(
            SlideSubmission.objects.filter(status__slug="pending").count(), 0
        )
        ...
        self.assertEqual(r.status_code, 200)
        self.assertRegex(
            r.content.decode(), r"These\s+slides\s+have\s+already\s+been\s+rejected"
        )
    ...
    def test_approve_proposed_slides(self, mock_slides_manager_cls):
        ...
        self.assertEqual(
            SlideSubmission.objects.filter(status__slug="approved").count(), 1
        )
        ...
        self.assertEqual(session.presentations.count(), 1)
        self.assertEqual(
            session.presentations.first().document.title, "different title"
        )
        ...
        self.assertEqual(r.status_code, 200)
        self.assertRegex(
            r.content.decode(), r"These\s+slides\s+have\s+already\s+been\s+approved"
        )
        self.assertFalse(mock_slides_manager_cls.called)
        self.assertEqual(len(outbox), 1)
    ...
    @patch("ietf.meeting.views.SlidesManager")
    def test_approve_proposed_slides_multisession_apply_one(
        ...
        session2 = SessionFactory(
            group=submission.session.group, meeting=submission.session.meeting
        )
        submission.session.meeting.importantdate_set.create(
            name_id="revsub", date=date_today() + datetime.timedelta(days=20)
        )
    ...
    @patch("ietf.meeting.views.SlidesManager")
    def test_approve_proposed_slides_multisession_apply_all(
        self, mock_slides_manager_cls
    ):
        TestBlobstoreManager().emptyTestBlobstores()
        submission = SlideSubmissionFactory(session__meeting__type_id="ietf")
        ...
        self.assertEqual(r.status_code, 200)
        r = self.client.post(
            url, dict(title="yet another title", apply_to_all=1, approve="approve")
        )
        submission.refresh_from_db()
        self.assertEqual(r.status_code, 302)
        self.assertEqual(session1.presentations.count(), 1)
    ...
    def test_submit_and_approve_multiple_versions(self, mock_slides_manager_cls):
        ...
        r = self.client.post(
            approve_url, dict(title=submission.title, approve="approve")
        )
        submission.refresh_from_db()
        self.assertEqual(r.status_code, 302)
        self.client.logout()
        ...
        login_testing_unauthorized(self, newperson.user.username, upload_url)
        test_file = BytesIO(
            b"this is not really a slide, but it is another version of it"
        )
        test_file.name = "not_really.txt"
        r = self.client.post(
            upload_url,
            dict(file=test_file, title="a test slide file", apply_to_all=True),
        )
        self.assertEqual(r.status_code, 302)

        test_file = BytesIO(
            b"this is not really a slide, but it is third version of it"
        )
        test_file.name = "not_really.txt"
        r = self.client.post(
            upload_url,
        ...
        (first_submission, second_submission) = SlideSubmission.objects.filter(
            session=session, status__slug="pending"
        ).order_by("id")
        ...
        approve_url = urlreverse(
            "ietf.meeting.views.approve_proposed_slides",
            kwargs={
                "slidesubmission_id": second_submission.pk,
                "num": second_submission.session.meeting.number,
            },
        )
        login_testing_unauthorized(self, chair.user.username, approve_url)
        r = self.client.post(
        ...
        self.assertTrue(
            exists_in_storage("slides", second_submission.doc.uploaded_filename)
        )
        self.assertEqual(r.status_code, 302)
        self.assertEqual(mock_slides_manager_cls.call_count, 1)
        self.assertEqual(
        ...
        disapprove_url = urlreverse(
            "ietf.meeting.views.approve_proposed_slides",
            kwargs={
                "slidesubmission_id": first_submission.pk,
                "num": first_submission.session.meeting.number,
            },
        )
        r = self.client.post(
            disapprove_url, dict(title="some title", disapprove="disapprove")
        )
        self.assertEqual(r.status_code, 302)
        self.client.logout()
        self.assertFalse(mock_slides_manager_cls.called)
        ...
        filename = os.path.join(
            path, session.presentations.first().document.name + "-01.txt"
        )
...
class ImportNotesTests(TestCase):
    ...
    def test_retrieves_note(self):
        ...
        with requests_mock.Mocker() as mock:
            mock.get(
                f"https://notes.ietf.org/{self.session.notes_id()}/download",
                text="markdown text",
            )
            mock.get(
                f"https://notes.ietf.org/{self.session.notes_id()}/info",
                text=json.dumps(
                    {"title": "title", "updatetime": "2021-12-02T11:22:33z"}
                ),
            )
            r = self.client.get(url)
            self.assertEqual(r.status_code, 200)
    ...
    def test_refuses_identical_import(self):
        ...
        with requests_mock.Mocker() as mock:
            ...
            q = PyQuery(r.content)
            r = self.client.post(
                url, {"markdown_text": q('input[name="markdown_text"]').attr["value"]}
            )
            self.assertEqual(r.status_code, 302)
    ...
    def test_allows_import_on_existing_bad_unicode(self):
        ...
        with open(
            self.session.presentations.filter(document__type="minutes")
            .first()
    ...
    def test_handles_missing_previous_revision_file(self):
        ...
        self.client.login(username="secretary", password="secretary+password")
        r = self.client.post(
            url, {"markdown_text": "original markdown text"}
        )  # create a rev
...
class SessionTests(TestCase):
    ...
    def test_meeting_requests(self):
        ...
        self.assertTrue(
            GroupFeatures.objects.get(pk=group_type_with_meetings).has_meetings
        )
        group_type_without_meetings = "sdo"
        self.assertFalse(
            GroupFeatures.objects.get(pk=group_type_without_meetings).has_meetings
        )

        area = GroupFactory(type_id="area", acronym="area")
        requested_session = SessionFactory(
            meeting=meeting,
            group__parent=area,
            status_id="schedw",
            add_to_schedule=False,
        )
        conflicting_session = SessionFactory(
            meeting=meeting,
            group__parent=area,
            status_id="schedw",
            add_to_schedule=False,
        ...
        ConstraintFactory(
            name_id="key_participant",
            meeting=meeting,
            source=requested_session.group,
            target=conflicting_session.group,
        )
        not_meeting = SessionFactory(
            meeting=meeting,
            group__parent=area,
            status_id="notmeet",
            add_to_schedule=False,
        ...
        url = urlreverse(
            "ietf.meeting.views.meeting_requests", kwargs={"num": meeting.number}
        )
        r = self.client.get(url)
        # requested_session group should be listed with a link to the request
        self.assertContains(r, requested_session.group.acronym)
        self.assertContains(
            r, _sreq_edit_link(requested_session)
        )  # link to the session request
    ...
    def test_request_minutes(self):
        ...
        SessionPresentation.objects.create(
            session=has_minutes, document=DocumentFactory(type_id="minutes")
        )

        empty_outbox()
        url = urlreverse(
            "ietf.meeting.views.request_minutes", kwargs={"num": meeting.number}
        )
        login_testing_unauthorized(self, "secretary", url)
        r = self.client.get(url)
    ...
    def test_add_session_recordings(self):
        ...
        self.assertEqual(
            ...
            "Video recording of {acro} for {timestamp}".format(
                ...
                timestamp=session.official_timeslotassignment()
                .timeslot.utc_start_time()
                .strftime("%Y-%m-%d %H:%M"),
            ),
        )
        ...
        with patch(
            "ietf.meeting.views.delete_recording", side_effect=ValueError("oh joy!")
        ) as mock_delete:
            r = self.client.post(
                url,
                data={
    ...
    def test_show_chatlog_links(self):
        ...
        SessionPresentation.objects.create(
            session=session, document=DocumentFactory(type_id="chatlog", name=doc_name)
        )

        session_url = urlreverse(
            "ietf.meeting.views.session_details",
        ...
        self.assertTrue(
            link_chatlog_box,
            'Expected <a> element with title "Chat logs for {group.acronym}" not found.',
        )
        ...
        self.assertTrue(
            link_chatlog_list, 'Expected <li> element containing "Chat logs" not found.'
        )
    ...
    def test_can_request_interim(self):
        ...
        for gf in GroupFeatures.objects.filter(has_meetings=True):
            ...
            for role in gf.groupman_roles:
                ...
                self.do_request_interim(
                    url, role.group, role.person.user, meeting_count
                )
...
class AgendaFilterTests(TestCase):
    ...
    def test_agenda_filter_template(self):
        ...
        self.assertEqual(
            len(header_buttons), 3
        )  # last column has disabled header, so only 3
        ...
        # area0
        _assert_button_ok(
            header_cells.eq(0)("button.keyword0"),
        ...
        # area1
        _assert_button_ok(
            header_cells.eq(1)("button.keyword1"),
        ...
        # area2
        _assert_button_ok(
            header_cells.eq(2)("button.keyword2"),
        ...
        # area3
        _assert_button_ok(
            header_cells.eq(3)("button.keyword2"),
            expected_label=None,
            expected_filter_item=None,
        )
...
class MeetingHostTests(BaseMeetingTestCase):
    ...
    def test_add(self):
        ...
        self.assertRedirects(
            r,
            urlreverse("ietf.meeting.views.materials", kwargs=dict(num=meeting.number)),
        )
    ...
    def test_logo_types_checked(self):
        ...
        for fmt, ext in allowed_formats:
            r = self._create_first_host(meeting, logo_file(format=fmt, ext=ext), url)
            self.assertRedirects(
                r,
                urlreverse(
                    "ietf.meeting.views.materials", kwargs=dict(num=meeting.number)
...
class ProceedingsTests(BaseMeetingTestCase):
    ...
    def _create_proceedings_materials(self, meeting):
        ...
        MeetingHostFactory.create_batch(
            2, meeting=meeting
        )  # create a couple of meeting hosts/logos
        ProceedingsMaterialFactory(
    ...
    def _assertMeetingHostsDisplayed(self, pq: PyQuery, meeting):
        ...
        self.assertEqual(
            len(host_divs),
            meeting.meetinghosts.count(),
            "Should have a logo for every meeting host",
        )
        self.assertEqual(
    ...
    def _assertProceedingsMaterialsDisplayed(self, pq: PyQuery, meeting):
        ...
        expected_materials = [
            m
            for m in meeting.proceedings_materials.order_by("type__order")
            if m.active()
        ]
        ...
        self.assertEqual(
            ...
            "Should have an entry for each active ProceedingsMaterial",
        )
        self.assertEqual(
    ...
    def test_generate_proceedings_content(self, mock_caches):
        ...
        self.assertTrue(mock_default_cache.get.called)
        self.assertEqual(
            mock_default_cache.get.call_args.args[0],
            cache_key,
            "same cache key each time",
        )
        self.assertTrue(mock_default_cache.set.called)
        self.assertEqual(
            mock_default_cache.set.call_args,
            call(cache_key, proceedings_content, timeout=86400),
        )
        mock_default_cache.get.reset_mock()
        ...
        self.assertEqual(
            result, proceedings_content
        )  # should have recomputed the same thing
        self.assertFalse(
            mock_default_cache.get.called,
            "don't bother reading cache when force_refresh is True",
        )
        self.assertTrue(mock_default_cache.set.called)
        self.assertEqual(
            mock_default_cache.set.call_args,
    ...
    def test_named_session(self):
        ...
        url = urlreverse(
            "ietf.meeting.views.proceedings", kwargs={"num": meeting.number}
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
    ...
    def test_proceedings_no_agenda(self):
        ...
        meeting = MeetingFactory(
            type_id="ietf", populate_schedule=False, date=date_today(), number="100"
        )
    ...
    def test_proceedings_acknowledgements(self):
        make_meeting_test_data()
        meeting = MeetingFactory(
            type_id="ietf", date=datetime.date(2016, 7, 14), number="97"
        )
    ...
    def test_proceedings_acknowledgements_link(self):
        ...
        """
        meeting_with_acks = MeetingFactory(
            type_id="ietf", date=datetime.date(2020, 7, 25), number="108"
        )
        SessionFactory(
            meeting=meeting_with_acks
        )  # make sure meeting has a scheduled session
        meeting_with_acks.acknowledgements = "these acknowledgements should appear"
        meeting_with_acks.save()
        url = urlreverse(
            "ietf.meeting.views.proceedings", kwargs={"num": meeting_with_acks.number}
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        q = PyQuery(response.content)
        ...
        meeting_without_acks = MeetingFactory(
            type_id="ietf", date=datetime.date(2022, 7, 25), number="113"
        )
        SessionFactory(
            meeting=meeting_without_acks
        )  # make sure meeting has a scheduled session
        meeting_without_acks.acknowledgements = (
            "these acknowledgements should not appear"
        )
    ...
    def test_proceedings_attendees(self):
        ...
        meeting = MeetingFactory(
            type_id="ietf", date=datetime.date(2023, 11, 4), number="118"
        )
        ...
        MeetingRegistrationFactory(
            meeting=meeting, person=person_a, reg_type="onsite", checkedin=True
        )
        MeetingRegistrationFactory(
            meeting=meeting, person=person_b, reg_type="onsite", checkedin=False
        )
        MeetingRegistrationFactory(meeting=meeting, person=person_a, reg_type="remote")
        AttendedFactory(
            session__meeting=meeting, session__type_id="plenary", person=person_a
        )
        MeetingRegistrationFactory(meeting=meeting, person=person_c, reg_type="remote")
        AttendedFactory(
            session__meeting=meeting, session__type_id="plenary", person=person_c
        )
        MeetingRegistrationFactory(meeting=meeting, person=person_d, reg_type="remote")
        url = urlreverse(
            "ietf.meeting.views.proceedings_attendees", kwargs={"num": 118}
        )
        response = self.client.get(url)
    ...
    def test_proceedings_activity_report(self):
        ...
        url = urlreverse(
            "ietf.meeting.views.proceedings_activity_report", kwargs={"num": 97}
        )
        response = self.client.get(url)
    ...
    def test_proceedings_material_details(self):
        ...
        for row, mat_type in zip(body_rows.items(), procmat_types.all()):
            ...
            self.assertEqual(
                cells.eq(0).text(),
                str(mat_type),
                "First column should be material type name",
            )
            ...
            self.assertEqual(
                len(cells.eq(2).find(f'a[href="{add_url}"]')),
                1,
        ...
        for row, mat in zip(
            body_rows.items(), meeting.proceedings_materials.order_by("type__order")
        ):
            add_url = urlreverse(
                "ietf.meeting.views_proceedings.upload_material",
            ...
            remove_url = urlreverse(
                "ietf.meeting.views_proceedings.upload_material",
                kwargs=dict(num=meeting.number, material_type=mat.type.slug),
            )
            restore_url = urlreverse(
                "ietf.meeting.views_proceedings.upload_material",
                kwargs=dict(num=meeting.number, material_type=mat.type.slug),
            )
            cells = row.find("td")
            ...
            if mat.active():
                ...
                self.assertEqual(
                    cells.eq(1).text(),
                    str(mat),
                    "Second column should be active material title",
                )
                self.assertEqual(
                    cells.eq(2).text(),
                    "{} ({})".format(
                ...
                self.assertIn(
                    mod_time.strftime("%Y-%m-%d"), c3text, "Updated date incorrect"
                )
                self.assertIn(
                    mod_time.strftime("%H:%M:%S"), c3text, "Updated time incorrect"
                )
                self.assertEqual(
                    len(cells.eq(4).find(f'a[href="{add_url}"]')),
                    1,
                    "Fourth column should have a Replace link",
                )
                self.assertEqual(
                    len(cells.eq(4).find(f'a[href="{edit_url}"]')),
                    1,
                    "Fourth column should have an Edit link",
                )
                self.assertEqual(
                    len(cells.eq(4).find(f'a[href="{remove_url}"]')),
                    1,
                ...
                self.assertEqual(
                    cells.eq(1).text(), "", "Second column should be empty"
                )
                add_url = urlreverse(
                    "ietf.meeting.views_proceedings.upload_material",
                    kwargs=dict(num=meeting.number, material_type=mat.type.slug),
                ...
                self.assertEqual(
                    len(cells.eq(2).find(f'a[href="{restore_url}"]')),
                    1,
        ...
        with NamedTemporaryFile("w+", suffix=".png") as invalid_file:
            ...
            for mat_type in ProceedingsMaterialTypeName.objects.filter(used=True):
                ...
                self.assertFormError(
                    r.context["form"],
                    "file",
                    "Found an unexpected extension: .png.  Expected one of .pdf",
                )
    ...
    def test_add_proceedings_material_url_invalid(self):
        ...
        for mat_type in ProceedingsMaterialTypeName.objects.filter(used=True):
            ...
            r = self.client.post(
                url, {"use_url": "on", "external_url": "Ceci n'est pas une URL"}
            )
            self.assertEqual(r.status_code, 200)
            self.assertFormError(
                r.context["form"], "external_url", "Enter a valid URL."
            )
    ...
    def test_add_proceedings_material_url_empty(self):
        ...
        for mat_type in ProceedingsMaterialTypeName.objects.filter(used=True):
            ...
            self.assertEqual(r.status_code, 200)
            self.assertFormError(
                r.context["form"], "external_url", "This field is required"
            )
    ...
    def test_replace_proceedings_material(self):
        ...
        pm_url = ProceedingsMaterialFactory(
            meeting=meeting, document__external_url="https://example.com/first"
        )
        success_url = urlreverse(
            "ietf.meeting.views_proceedings.material_details",
            kwargs=dict(num=meeting.number),
        )
        ...
        r = self.client.post(
            pm_doc_url, {"use_url": "on", "external_url": "https://example.com/second"}
        )
        self.assertRedirects(r, success_url)
        self.assertEqual(meeting.proceedings_materials.count(), 2)
        pm_doc = meeting.proceedings_materials.get(pk=pm_doc.pk)  # refresh from DB
        ...
        r = self.client.post(
            pm_url_url, {"use_url": "on", "external_url": "https://example.com/third"}
        )
        self.assertRedirects(r, success_url)
        self.assertEqual(meeting.proceedings_materials.count(), 2)
    ...
    def test_restore_proceedings_material(self):
        """Proceedings material can be removed"""
        meeting = self._procmat_test_meeting()
        pm = ProceedingsMaterialFactory(
            meeting=meeting, document__states=[("procmaterials", "removed")]
        )
    ...
    def test_create_recording(self):
        session = SessionFactory(
            meeting__type_id="ietf", meeting__number=72, group__acronym="mars"
        )
        filename = "ietf42-testroomt-20000101-0800.mp3"
        url = settings.IETF_AUDIO_URL + "ietf{}/{}".format(
            session.meeting.number, filename
        )
    ...
    def test_participants_for_meeting(self):
        ...
        MeetingRegistrationFactory(
            meeting=m, person=person_a, reg_type="onsite", checkedin=True
        )
        MeetingRegistrationFactory(
            meeting=m, person=person_b, reg_type="onsite", checkedin=False
        )
    ...
    def test_session_attendance(self):
        ...
        attendance_url = urlreverse(
            "ietf.meeting.views.session_attendance",
            kwargs={"num": meeting.number, "session_id": session.id},
        )
        r = self.client.get(session_url)
        ...
        recmanrole = RoleFactory(
            group__type_id="ietf",
            name_id="recman",
            person__user__last_login=timezone.now(),
        )
        ...
        r = self.client.post(
            ...
            {
                "apikey": apikey.hash(),
                "attended": f'{{"session_id":{session.pk},"attendees":{attendees}}}',
            },
        )
        self.assertEqual(r.status_code, 200)
        ...
        meeting.importantdate_set.update(
            name_id="revsub", date=date_today() + datetime.timedelta(days=20)
        )
        ...
        meeting = make_interim_meeting(
            group=GroupFactory(acronym="mars"), date=date_today()
        )
        session = Session.objects.filter(meeting=meeting, group__acronym="mars").first()
        attendance_url = urlreverse(
            "ietf.meeting.views.session_attendance",
    ...
    def test_bluesheet_data(self):
        session = SessionFactory(meeting__type_id="ietf")
        attended_with_affil = MeetingRegistrationFactory(
            meeting=session.meeting, affiliation="Somewhere"
        )
        AttendedFactory(
            session=session,
            person=attended_with_affil.person,
            time="2023-03-13T01:24:00Z",
        )  # joined 2nd
        attended_no_affil = MeetingRegistrationFactory(meeting=session.meeting)
        AttendedFactory(
            session=session,
            person=attended_no_affil.person,
            time="2023-03-13T01:23:00Z",
        )  # joined 1st
        ...
        self.assertEqual(
            ...
            [
                ...
                {
                    "name": attended_with_affil.person.plain_name(),
