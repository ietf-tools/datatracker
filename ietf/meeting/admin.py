    ...
    def session_desc(self, instance):
        if instance.session:
            if instance.session.name:
                ...
                return "%s (%s)" % (
                    instance.session.group.name,
                    instance.session.group.acronym,
                )
...
class SessionAdmin(admin.ModelAdmin):
    ...
    def current_status(self, instance):
        events = sorted(
            instance.schedulingevent_set.all(), key=lambda e: (e.time, e.id)
        )
        if events:
