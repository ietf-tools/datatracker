...
@override_settings(
    MEETING_APPLICATION_OCTET_STREAM_OVERRIDES={
        ".md": "text/markdown"
    },  # test relies on .txt not mapping
    MEETING_VALID_UPLOAD_EXTENSIONS={
        "minutes": [".txt", ".md"]
    },  # test relies on .exe being absent
...
class FileUploadFormTests(TestCase):
    ...
    def test_accepts_valid_data(self):
        ...
        self.assertEqual(
            cleaned_file.name, "file.txt", "Uploaded filename should not be changed"
        )
        with cleaned_file.open("rb") as f:
            self.assertEqual(
                f.read(), b"plain text", "Uploaded file contents should not be changed"
            )
        self.assertEqual(
            cleaned_file.content_type, "text/plain", "Content-Type should be overridden"
        )
    ...
    def test_overrides_content_type_application_octet_stream(self):
        ...
        self.assertEqual(
            cleaned_file,
            test_file,
            "Cleaning should return the file object that was passed in",
        )
        self.assertEqual(
            cleaned_file.name, "file.md", "Uploaded filename should not be changed"
        )
        with cleaned_file.open("rb") as f:
            self.assertEqual(
                f.read(), b"plain text", "Uploaded file contents should not be changed"
    ...
    def test_overrides_only_application_octet_stream(self):
        test_file = SimpleUploadedFile(
            name="file.md", content=b"plain text", content_type="application/json"
        )

        form = FileUploadFormTests.TestClass(files={"file": test_file})
        self.assertFalse(form.is_valid(), "Test data are invalid input")
        self.assertEqual(
            test_file.name, "file.md", "Uploaded filename should not be changed"
        )
        self.assertEqual(
            test_file.content_type,
            "application/json",
            "Uploaded Content-Type should not be changed",
        )
    ...
    def test_overrides_only_requested_extensions_when_valid_ext(self):
        ...
        self.assertFalse(form.is_valid(), "Test data are invalid input")
        self.assertEqual(
            test_file.name, "file.txt", "Uploaded filename should not be changed"
        )
        self.assertEqual(
            test_file.content_type,
    ...
    def test_overrides_only_requested_extensions_when_invalid_ext(self):
        ...
        self.assertFalse(form.is_valid(), "Test data are invalid input")
        self.assertEqual(
            test_file.name, "file.exe", "Uploaded filename should not be changed"
        )
        self.assertEqual(
            test_file.content_type,
            "application/octet-stream",
...
class InterimSessionModelFormTests(TestCase):
    @override_settings(
        MEETECHO_API_CONFIG={}
    )  # setting needs to exist, don't care about its value in this test
    def test_remote_participation_options(self):
        ...
        choice_vals = [
            choice[0] for choice in form.fields["remote_participation"].choices
        ]
    ...
    def test_edits_in_meeting_time_zone(self):
        ...
        session = SessionFactory(
            meeting__type_id="interim", meeting__time_zone="America/Halifax"
        )
