...
class MeetingFactory(factory.django.DjangoModelFactory):
    ...
    def number(self, n):
        if self.type_id == "ietf":
            if Meeting.objects.filter(type="ietf").exists():
                so_far = max(
                    [int(x.number) for x in Meeting.objects.filter(type="ietf")]
                )
                return "%02d" % (so_far + 1)
            else:
                return "%02d" % (n + 80)
    ...
    def date(self):
        if self.type_id == "ietf":
            ...
            return datetime.date(2010, 1, 1) + datetime.timedelta(
                days=random.randint(0, 3652)
            )
    ...
    def group_conflicts(
        obj, create, extracted, **kwargs
    ):  # pulint: disable=no-self-argument
        """Add conflict types
        ...
        if extracted is None:
            extracted = [
                ConstraintName.objects.get(slug=s)
                for s in ["chair_conflict", "tech_overlap", "key_participant"]
            ]
        if create:
            for cn in extracted:
                obj.group_conflict_types.add(
                    cn
                    if isinstance(cn, ConstraintName)
                    else ConstraintName.objects.get(slug=cn)
                )
...
class SessionFactory(factory.django.DjangoModelFactory):
    ...
    on_agenda = factory.lazy_attribute(
        lambda obj: SessionPurposeName.objects.get(pk=obj.purpose_id).on_agenda
    )
    ...
    def add_to_schedule(
        ...
        if create and extracted:
            ...
            obj.timeslotassignments.create(
                timeslot=ts[random.randrange(len(ts))], schedule=obj.meeting.schedule
            )
...
class SlideSubmissionFactory(factory.django.DjangoModelFactory):
    ...
    make_file = factory.PostGeneration(
        lambda obj, create, extracted, **kwargs: open(
            obj.staged_filepath(), "a"
        ).close()
    )
