            ...
            for date in dates[1:]:
                if date - last_date != datetime.timedelta(days=1):
                    raise forms.ValidationError(
                        "For Multi-Day meetings, days must be consecutive"
                    )
                last_date = date
            self.days = len(dates)
...
class InterimMeetingModelForm(forms.ModelForm):
    group = GroupModelChoiceField(
        queryset=Group.objects.with_meetings()
    ...
    def set_group_options(self):
        ...
        queryset = (
            ...
            .filter(state_id__in=["active", "proposed", "bof"])
            .order_by("acronym")
        )
        ...
        if kwargs.get("commit", True):
            ...
            if not meeting.schedule:
                meeting.schedule = Schedule.objects.create(
                    meeting=meeting, owner=Person.objects.get(name="(System)")
                )
...
class InterimSessionModelForm(forms.ModelForm):
    ...
    time = forms.TimeField(
        ...
        required=True,
        help_text="Start time in meeting time zone",
    )
    time.widget.attrs["placeholder"] = "HH:MM"
    requested_duration = CustomDurationField(required=True)
    end_time = forms.TimeField(
        required=False, help_text="End time in meeting time zone"
    )
    ...
    agenda_note = forms.CharField(
        max_length=255, required=False, label=" Additional information"
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.is_edit:
            self.initial["date"] = (
                self.instance.official_timeslotassignment()
        ...
        field_order = [
            field for field in self.fields if field != "remote_participation"
        ]
        field_order.insert(
            field_order.index("remote_instructions"), "remote_participation"
        )
    ...
    def clean_requested_duration(self):
        ...
        if (
            not duration
            or duration < datetime.timedelta(minutes=min_minutes)
            or duration > datetime.timedelta(minutes=max_minutes)
        ):
            raise forms.ValidationError(
                "Provide a duration, %s-%smin." % (min_minutes, max_minutes)
            )
        return duration

    def clean(self):
        if self.cleaned_data.get("remote_participation", None) == "meetecho":
            self.cleaned_data["remote_instructions"] = (
                ""  # blank this out if we're creating a Meetecho conference
            )
    ...
    def save_agenda(self):
        ...
        # write file
        path = os.path.join(
            self.instance.meeting.get_materials_path(),
            "agenda",
            doc.filename_with_rev(),
        )
...
class InterimCancelForm(forms.Form):
    ...
    comments = forms.CharField(
        max_length=512,
        required=False,
        widget=forms.Textarea(attrs={"placeholder": "enter optional comments here"}),
        strip=False,
    )
...
class FileUploadForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.mime_types:
            label += "Note that you can only upload files with these formats: %s." % (
                ", ".join(
                    self.mime_types,
                )
            )
    ...
    def clean_file(self):
        ...
        if self.mime_types:
            if (
                ...
                in settings.MEETING_VALID_UPLOAD_MIME_FOR_OBSERVED_MIME[mime_type]
            ):
                raise ValidationError(
                    "Upload Content-Type (%s) is different from the observed mime-type (%s)"
            ...
            if file.content_type in settings.MEETING_VALID_MIME_TYPE_EXTENSIONS:
                if (
                    not ext
                    in settings.MEETING_VALID_MIME_TYPE_EXTENSIONS[file.content_type]
                ):
                    raise ValidationError(
                        "Upload Content-Type (%s) does not match the extension (%s)"
...
class UploadSlidesForm(ApplyToAllFileUploadForm):
    ...
    def __init__(
        self, session, show_apply_to_all_checkbox, can_manage, *args, **kwargs
    ):
    ...
    def clean_title(self):
        ...
        if self.session.meeting.type_id == "interim":
            if re.search(r"-\d{2}$", title):
                raise forms.ValidationError(
                    "Interim slides currently may not have a title that ends with something that looks like a revision number (-nn)"
                )
        return title
...
class TimeSlotEditForm(forms.ModelForm):
    class Meta:
        ...
        field_classes = dict(
            time=forms.SplitDateTimeField, duration=TimeSlotDurationField
        )
        widgets = dict(
...
class TimeSlotCreateForm(forms.Form):
    ...
    type = forms.ModelChoiceField(
        queryset=TimeSlotTypeName.objects.all(), initial="regular"
    )
    ...
    def __init__(self, meeting, *args, **kwargs):
        ...
        meeting_days = [
            meeting.date + datetime.timedelta(days=n) for n in range(meeting.days)
        ]

        # Fill in dynamic field properties
        self.fields["days"].choices = self._day_choices(meeting_days)
        self.fields["other_date"].widget.attrs[
            "data-date-default-view-date"
        ] = meeting.date
    ...
    def clean(self):
        ...
        if len(self.cleaned_data["days"]) == 0:
            self.add_error(
                "days", ValidationError("Please select a day or specify a date")
            )
...
class DurationChoiceField(forms.ChoiceField):
    ...
    def to_python(self, value):
        if value in self.empty_values or (
            isinstance(value, str) and not value.isnumeric()
        ):
...
class SessionDetailsForm(forms.ModelForm):
    ...
    def __init__(self, group, *args, **kwargs):
        ...
        self.fields["purpose"].queryset = SessionPurposeName.objects.filter(
            pk__in=session_purposes
        )
        if not group.features.acts_like_wg:
            self.fields["requested_duration"].durations = [
                datetime.timedelta(minutes=m) for m in range(30, 241, 30)
            ]
    ...
    def clean(self):
        ...
        if "purpose" in self.cleaned_data and (
            ...
            or (self.instance.purpose != self.cleaned_data["purpose"])
        ):
