...
class UploadProceedingsMaterialForm(FileUploadForm):
    ...
    external_url = forms.URLField(
        required=False, help_text="External URL to link from the proceedings"
    )
    field_order = ["use_url", "external_url"]  # will precede superclass fields

    class Media:
        js = ("ietf/js/upload-material.js",)

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["file"].label = (
            "Select a file to upload. Allowed format{}: {}".format(
...
def save_proceedings_material_doc(
    meeting, material_type, title, request, file=None, external_url=None, state=None
):
    events = []
    ...
    doc_name = "-".join(
        [
            "proceedings",
            meeting.number,
            xslugify(getattr(material_type, "slug", material_type)).replace("_", "-")[
                :128
            ],
        ]
    )
    ...
    if doc is None:
        if file is None and external_url is None:
            raise ValueError(
                "Cannot create a new document without a file or external URL"
            )
    ...
    new_state = (
        ...
        else State.objects.get(type_id="procmaterials", slug="active")
    )
...
def upload_material(request, num, material_type):
    ...
    return render(
        request,
        "meeting/proceedings/upload_material.html",
        {
            "form": form,
            "material": material,
...
def edit_material(request, num, material_type):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect(
                "ietf.meeting.views_proceedings.material_details", num=meeting.number
            )
    else:
    ...
    return render(
        request,
        "meeting/proceedings/edit_material.html",
        {
            "form": form,
            "material": material,
    ...
    if request.method == "POST":
        ...
        if new_state != prev_state:
            ...
            add_state_change_event(
                material.document, request.user.person, prev_state, new_state
            )
        return redirect("ietf.meeting.views_proceedings.material_details", num=num)
...
def edit_meetinghosts(request, num):
    ...
    return render(
        request,
        "meeting/proceedings/edit_meetinghosts.html",
        {
            "formset": formset,
            "meeting": meeting,
        },
