...
def materials(request, num=None):
    ...
    if settings.SERVER_MODE != "production" and "_testoverride" in request.GET:
        ...
        if meeting.number.isdigit() and int(meeting.number) > 96:
            ...
            with timezone.override(meeting.tz()):
                return render(
                    request,
                    "meeting/materials_upload_closed.html",
                    {
    ...
    sessions = add_event_info_to_session_qs(
        Session.objects.filter(
            ...
            timeslotassignments__schedule__in=[
                schedule,
                schedule.base if schedule else None,
            ],
        )
        .distinct()
        .select_related("meeting__schedule", "group__state", "group__parent")
    ).order_by("group__acronym")

    plenaries = sessions.filter(name__icontains="plenary")
    ietf = (
        sessions.filter(group__parent__type__slug="area")
        .exclude(group__acronym="edu")
        .order_by("group__parent__acronym", "group__acronym")
    )
    irtf = sessions.filter(group__parent__acronym="irtf")
    training = sessions.filter(
        group__acronym__in=["edu", "iaoc"],
        type_id__in=[
            "regular",
            "other",
        ],
    ...
    session_pks = [
        s.pk for ss in [plenaries, ietf, irtf, training, iab, editorial] for s in ss
    ]
    other = sessions.filter(
        type__in=["regular"], group__type__features__has_meetings=True
    ).exclude(pk__in=session_pks)

    for topic in [plenaries, ietf, training, irtf, iab, editorial]:
        for event in topic:
            date_list = []
            for slide_event in event.all_meeting_slides():
                date_list.append(slide_event.time)
            for agenda_event in event.all_meeting_agendas():
                date_list.append(agenda_event.time)
            if date_list:
                setattr(event, "last_update", sorted(date_list, reverse=True)[0])
    ...
    ietf_areas = []
    for area, area_sessions in itertools.groupby(ietf, key=lambda s: s.group.parent):
        meeting_groups, not_meeting_groups = organize_proceedings_sessions(
            area_sessions
        )
...
def current_materials(request):
    today = date_today()
    meetings = (
        Meeting.objects.exclude(number__startswith="interim-")
...
def _get_materials_doc(meeting, name):
    ...
    if "-" in name:
        ...
        if len(rev) == 2 and rev.isdigit():
            ...
            if (
                doc.get_related_meeting() == meeting
...
def materials_editable_groups(request, num=None):
    ...
    return render(
        request,
        "meeting/materials_editable_groups.html",
        {"meeting_num": meeting.number},
    )
            ...
            if action == "delete":
                ...
                if len(missing_ids) != 0:
                    return HttpResponseNotFound(
                        "TimeSlot ids not found in meeting {}: {}".format(
                            meeting.number, ", ".join(sorted(missing_ids))
                        )
                    )
                timeslots.delete()
                return HttpResponse(
                    content="; ".join(
                        "Deleted TimeSlot {}".format(id) for id in slot_ids
                    )
                )
            else:
        ...
        for room in rooms:
            for day in time_slices:
                for slice in date_slices[day]:
                    ts_list.append(
                        room.timeslot_set.filter(
                            time=slice[0], duration=datetime.timedelta(seconds=slice[2])
                        )
                    )

        # Grab these in one query each to identify sessions that are in use and should be handled with care
        ts_with_official_assignments = meeting.timeslot_set.filter(
            sessionassignments__schedule=meeting.schedule
        )
        ts_with_any_assignments = meeting.timeslot_set.filter(
            sessionassignments__isnull=False
        )

        return render(
            request,
            "meeting/timeslot_edit.html",
            {
                "rooms": rooms,
...
class NewScheduleForm(forms.ModelForm):
    ...
    def __init__(self, meeting, schedule, new_owner, *args, **kwargs):
        ...
        existing_names = set(
            Schedule.objects.filter(meeting=meeting, owner=new_owner).values_list(
                "name", flat=True
            )
        )
        ...
        if schedule:
            ...
            self.fields["base"].queryset = self.fields["base"].queryset.exclude(
                pk=schedule.pk
            )
    ...
    def clean_name(self):
        ...
        if name and Schedule.objects.filter(
            meeting=self.meeting, owner=self.new_owner, name=name
        ):
...
def new_meeting_schedule(request, num, owner=None, name=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect(
                edit_meeting_schedule,
                meeting.number,
                new_schedule.owner_email(),
                new_schedule.name,
            )
    ...
    return render(
        request,
        "meeting/new_meeting_schedule.html",
        {
            "meeting": meeting,
            "schedule": schedule,
            "form": form,
...
def edit_meeting_schedule(request, num=None, owner=None, name=None):
    ...
    can_see, can_edit, secretariat = schedule_permissions(
    ...
    if "type" in request.GET:
        ...
        include_timeslot_types = (
            None  # disables filtering by type (other than IGNORE_TIMESLOT_TYPES)
        )
    ...
    timeslots_qs = (
        timeslots_qs.that_can_be_scheduled()
        .prefetch_related("type")
        .order_by("location", "time", "name")
    )
    ...
    def timedelta_to_css_ems(timedelta):
        ...
        # interpolate
        scale = (
            (capped_timedelta - capped_min_d) / (capped_max_d - capped_min_d)
            if capped_min_d != capped_max_d
            else 1
        )
    ...
    def prepare_sessions_for_display(sessions):
        ...
        requested_by_lookup = {
            ...
            for p in Person.objects.filter(
                pk__in=set(s.requested_by for s in sessions if s.requested_by)
            )
        }

        # constraints
        (
            constraints_for_sessions,
            formatted_constraints_for_sessions,
            constraint_names,
        ) = preprocess_constraints_for_meeting_schedule_editor(meeting, sessions)
        ...
        for s in sessions:
            ...
            if s.group:
                if s.purpose.slug in ("none", "regular"):
                    ...
                    s.scheduling_label = (
                        s.name if s.name else f"??? [{s.group.acronym}]"
                    )
            ...
            s.requested_duration_in_hours = round(
                s.requested_duration.seconds / 60.0 / 60.0, 1
            )

            session_layout_margin = 0.2
            s.layout_width = (
                timedelta_to_css_ems(s.requested_duration) - 2 * session_layout_margin
            )
            s.parent_acronym = (
                s.group.parent.acronym if s.group and s.group.parent else ""
            )
            ...
            for name_id, ts in itertools.groupby(
                sorted(constraints_for_sessions.get(s.pk, [])), key=lambda t: t[0]
            ):  # name_id same for each set of ts
                ts = list(ts)
                ...
                for session_pk, grouped_session_pks in itertools.groupby(session_pks):
                    ...
                    count = len(
                        list(grouped_session_pks)
                    )  # list() needed because iterator has no len()
                    constraint_hints[
                        ConstraintHint(constraint_names[name_id], count)
                    ].add(session_pk)
            ...
            s.other_sessions = [
                s_other
                for s_other in sessions_for_group.get(s.group_id)
                if s != s_other
            ]

            s.readonly = s.current_status in tombstone_states or any(
                a.schedule_id != schedule.pk
    ...
    def prepare_timeslots_for_display(timeslots, rooms):
        ...
        sorted_rooms = sorted(
            ...
            key=lambda room: (
                ...
                (
                    room.capacity if room.capacity is not None else math.inf
                ),  # sort rooms with capacity = None at end
        ...
        room_groups = []
        last_start_and_duration = (
            None  # Used to watch for changes in start_and_duration
        )
        ...
        days = OrderedDict(
            (
                ...
                [
                    ...
                    OrderedDict((room.pk, dict(room=room, timeslots=[])) for room in rg)
    ...
    if request.method == "POST":
        ...
        if action == "updateview":
            ...
            enabled_types = [
                ts_type.slug
                for ts_type in TimeSlotTypeName.objects.filter(
                    used=True,
        ...
        if (
            action == "assign"
            and request.POST.get("session", "").isdigit()
            and request.POST.get("timeslot", "").isdigit()
        ):
            ...
            existing_assignments = SchedTimeSessAssignment.objects.filter(
            ...
            if existing_assignments:
                assertion(
                    ...
                    note="Multiple assignments for {} in schedule {}".format(
                        session, schedule
                    ),
                )
                ...
                if (
                    schedule.pk == meeting.schedule_id
                    and session.current_status == "sched"
                ):
                    ...
                    tombstone_session.current_status = (
                        "resched"  # rematerialize status for the rendering
                    )
            ...
            if tombstone_session:
                ...
                return _json_response(
                    True,
                    tombstone=render_to_string(
                        "meeting/edit_meeting_schedule_session.html",
                        {"session": tombstone_session},
                    ),
                )
            else:
            ...
            if not swap_days_form.is_valid():
                return HttpResponseBadRequest(
                    "Invalid swap: {}".format(swap_days_form.errors)
                )
            ...
            source_timeslots = [
                ts for ts in timeslots_qs if ts.local_start_time().date() == source_day
            ]
            target_timeslots = [
                ts for ts in timeslots_qs if ts.local_start_time().date() == target_day
            ]
            if any(timeslot_locked(ts) for ts in source_timeslots + target_timeslots):
                return HttpResponseBadRequest("Can't swap these days.")

            swap_meeting_schedule_timeslot_assignments(
                schedule, source_timeslots, target_timeslots, target_day - source_day
            )
            ...
            if not swap_timeslots_form.is_valid():
                return HttpResponseBadRequest(
                    "Invalid swap: {}".format(swap_timeslots_form.errors)
                )
            ...
            if any(timeslot_locked(ts) for ts in origin_timeslots) or any(
                timeslot_locked(ts) for ts in target_timeslots
            ):
    ...
    for ts in timeslots_qs:
        ts.start_end_group = "ts-group-{}-{}".format(
            ts.local_start_time().strftime("%Y%m%d-%H%M"),
            int(ts.duration.total_seconds() / 60),
        )
        timeslot_groups[ts.local_start_time().date()].add(
            (ts.local_start_time(), ts.local_end_time(), ts.start_end_group)
        )
    ...
    def cubehelix(i, total, hue=1.2, start_angle=0.5):
        ...
        return (
            max(
                0, min(x + a * (-0.14861 * math.cos(phi) + 1.78277 * math.sin(phi)), 1)
            ),
            max(
                0, min(x + a * (-0.29227 * math.cos(phi) + -0.90649 * math.sin(phi)), 1)
            ),
    ...
    session_parents = sorted(
        set(
            ...
            and s.group.parent
            and (
                s.group.parent.type_id == "area"
                or s.group.parent.acronym in ("irtf", "iab")
            )
        ),
    ...
    for i, p in enumerate(session_parents):
        if p.acronym in liz_preferred_colors:
            ...
            p.scheduling_color = "rgb({}, {}, {})".format(
                *tuple(int(round(x * 255)) for x in rgb_color)
            )
            p.light_scheduling_color = "rgb({}, {}, {})".format(
                *tuple(int(round((0.9 + 0.1 * x) * 255)) for x in rgb_color)
            )

    session_purposes = sorted(
        set(s.purpose for s in sessions if s.purpose), key=lambda p: p.name
    )
    timeslot_types = sorted(
        set(s.type for s in sessions if s.type).union(
    ...
    with timezone.override(meeting.tz()):
        return render(
            request,
            "meeting/edit_meeting_schedule.html",
            {
                "meeting": meeting,
                "schedule": schedule,
                "can_edit": can_edit,
                ...
                "days": days,
                "timeslot_groups": sorted(
                    (d, list(sorted(t_groups)))
                    for d, t_groups in timeslot_groups.items()
                ),
...
class TimeSlotForm(forms.Form):
    day = forms.TypedChoiceField(
        coerce=lambda t: datetime.datetime.strptime(t, "%Y-%m-%d").date()
    )  # all dates, no tz
    time = forms.TimeField()
    duration = (
        CustomDurationField()
    )  # this is just to make 1:30 turn into 1.5 hours instead of 1.5 minutes
    location = RoomNameModelChoiceField(
        queryset=Room.objects.all(), required=False, empty_label="(No location)"
    )
    show_location = forms.BooleanField(initial=True, required=False)
    type = forms.ModelChoiceField(
        queryset=TimeSlotTypeName.objects.filter(used=True),
        empty_label=None,
        required=False,
    )
    purpose = forms.ModelChoiceField(
        queryset=SessionPurposeName.objects.filter(used=True),
        required=False,
    ...
    short = forms.CharField(
        max_length=32,
        label="Short name",
        help_text="Abbreviated session name used for material file names",
        required=False,
    )
    ...
    def __init__(self, meeting, schedule, *args, timeslot=None, **kwargs):
        ...
        self.fields["day"].choices = [
            (
                ...
                (meeting.date + datetime.timedelta(days=i)).strftime("%a %b %d"),
            )
        ...
        self.fields["group"].widget.attrs["data-ietf"] = Group.objects.get(
            acronym="ietf"
        ).pk
        ...
        self.fields["type"].queryset = self.fields["type"].queryset.filter(
            pk__in=timeslot_types_with_purpose
        )
        ...
        if timeslot:
            ...
            assignments = sorted(
                ...
                key=lambda a: 0 if a.schedule_id == schedule.pk else 1,
            )
        ...
        if not self.active_assignment or timeslot.type_id != "regular":
            del self.fields[
                "agenda_note"
            ]  # at the moment, the UI only shows this field for regular sessions
    ...
    def clean(self):
        ...
        if not ts_type:
            ...
            self.cleaned_data["purpose"] = SessionPurposeName.objects.get(
                slug="open_meeting"
            )
        else:
            if ts_type.slug in ["break", "reg", "reserved", "unavail", "regular"]:
                if ts_type.slug != "regular":
                    self.cleaned_data["group"] = self.fields["group"].queryset.get(
                        acronym="secretariat"
                    )
            else:
                if not group:
                    self.add_error(
                        "group",
                        "When scheduling this type of timeslot, a group must be associated",
                    )
                if not short:
                    self.add_error(
                        "short",
                        "When scheduling this type of timeslot, a short name is required",
                    )

            if (
                self.timeslot
                and self.timeslot.type.slug == "regular"
                and self.active_assignment
                and ts_type.slug != self.timeslot.type.slug
            ):
                self.add_error(
                    "type",
                    "Can't change type on timeslots for regular sessions when a session has been assigned",
                )
        ...
        if (
            ...
            self.add_error(
                "group", "Can't change group after materials have been uploaded"
            )
...
def edit_meeting_timeslots_and_misc_sessions(request, num=None, owner=None, name=None):
    ...
    rooms = list(
        Room.objects.filter(meeting=meeting)
        .prefetch_related("session_types")
        .order_by("-capacity", "name")
    )
    rooms.append(Room(name="(No location)"))

    timeslot_qs = (
        TimeSlot.objects.filter(meeting=meeting)
        .prefetch_related("type")
        .order_by("time")
    )
    ...
    with timezone.override(meeting.tz()):
        if request.method == "GET" and request.GET.get("action") == "edit-timeslot":
            ...
            assigned_session = add_event_info_to_session_qs(
                Session.objects.filter(
            ...
            timeslot.can_cancel = (
                not assigned_session
                or assigned_session.current_status
                not in ["canceled", "canceled", "resched"]
            )

            return JsonResponse(
                {
                    "form": render_to_string(
                        "meeting/edit_timeslot_form.html",
                        {
                            "timeslot_form_action": "edit",
                            "timeslot_form": TimeSlotForm(
                                meeting, schedule, timeslot=timeslot
                            ),
                            "timeslot": timeslot,
                            "schedule": schedule,
                            "meeting": meeting,
        ...
        if (
            request.method == "POST"
            and request.POST.get("action") == "add-timeslot"
            and can_edit
        ):
            ...
            if add_timeslot_form.is_valid():
                ...
                timeslot, created = TimeSlot.objects.get_or_create(
                    ...
                    time=meeting.tz().localize(
                        datetime.datetime.combine(c["day"], c["time"])
                    ),
                    duration=c["duration"],
                    location=c["location"],
        ...
        if (
            request.method == "POST"
            and request.POST.get("action") == "edit-timeslot"
            and can_edit
        ):
            ...
            edit_timeslot_form = TimeSlotForm(
                meeting, schedule, request.POST, timeslot=timeslot
            )
            if (
                edit_timeslot_form.is_valid()
                and edit_timeslot_form.active_assignment.schedule_id == schedule.pk
            ):
                ...
                timeslot.time = meeting.tz().localize(
                    datetime.datetime.combine(c["day"], c["time"])
                )
        ...
        if (
            request.method == "POST"
            and request.POST.get("action") == "cancel-timeslot"
            and can_edit
        ):
            timeslot_pk = request.POST.get("timeslot")
            if not timeslot_pk or not timeslot_pk.isdecimal():
        ...
        if (
            request.method == "POST"
            and request.POST.get("action") == "delete-timeslot"
            and can_edit
        ):
            timeslot_pk = request.POST.get("timeslot")
            if not timeslot_pk or not timeslot_pk.isdecimal():
            ...
            if timeslot.type_id != "regular":
                for session in Session.objects.filter(
                    ...
                    for doc in session.materials.all():
                        doc.set_state(
                            State.objects.get(type=doc.type_id, slug="deleted")
                        )
                        e = DocEvent(
                            doc=doc, rev=doc.rev, by=request.user.person, type="deleted"
                        )
        ...
        for a in SchedTimeSessAssignment.objects.filter(
            schedule__in=[schedule, schedule.base]
        ):
        ...
        min_time = min(
            [t.local_start_time().time() for t in timeslot_qs] + [datetime.time(8)]
        )
        max_time = max(
            [t.local_end_time().time() for t in timeslot_qs] + [datetime.time(22)]
        )
        min_max_delta = datetime.datetime.combine(
            meeting.date, max_time
        ) - datetime.datetime.combine(meeting.date, min_time)
        ...
        for d in days:
            ...
            for r in rooms:
                ...
                for t in timeslots_by_day_and_room.get((d, r.pk), []):
                    ...
                    # away
                    if t.type_id == "regular" and not any(
                        t.slug == "regular" for t in r.session_types.all()
                    ):
                        continue
                    ...
                    t.left_offset = (
                        100.0 * (local_start_dt - local_min_dt) / min_max_delta
                    )
                    t.layout_width = min(
                        100.0 * t.duration / min_max_delta, 100 - t.left_offset
                    )
                    ts.append(t)

                room_timeslots.append((r, ts))

            day_grid.append({"day": d, "room_timeslots": room_timeslots})

        return render(
            request,
            "meeting/edit_meeting_timeslots_and_misc_sessions.html",
            {
                "meeting": meeting,
                "schedule": schedule,
                "can_edit": can_edit,
...
def edit_schedule_properties(request, num, owner, name):
    ...
    if schedule is None:
        raise Http404(
            "No agenda information for meeting %s owner %s schedule %s available"
            % (num, owner, name)
        )
    ...
    if request.method == "POST":
        ...
        form = SchedulePropertiesForm(
            meeting, instance=Schedule.objects.get(pk=schedule.pk), data=request.POST
        )
        if form.is_valid():
            form.save()
...
def natural_sort_key(
    ...
    return [
        int(text) if text.isdecimal() else text.lower() for text in nat_sort_re.split(s)
    ]
...
def list_schedules(request, num):
    ...
    schedules = (
        Schedule.objects.filter(meeting=meeting)
        .prefetch_related(
            "owner", "assignments", "origin", "origin__assignments", "base"
        )
        .order_by("owner", "-name", "-public")
        .distinct()
    )
    ...
    schedule_groups = [
        (sorted(l, reverse=True, key=lambda s: natural_sort_key(s.name)), own, *t)
        for l, own, *t in schedule_groups
        if l or own
    ]

    return render(
        request,
        "meeting/schedule_list.html",
        {
            "meeting": meeting,
...
class DiffSchedulesForm(forms.Form):
    ...
    def __init__(self, meeting, user, *args, **kwargs):
        ...
        sorted_schedules = sorted(
            qs, reverse=True, key=lambda s: natural_sort_key(s.name)
        )

        schedule_choices = [
            (schedule.name, "{} ({})".format(schedule.name, schedule.owner))
...
def diff_schedules(request, num):
    ...
    if "from_schedule" in request.GET:
        ...
        if form.is_valid():
            from_schedule = get_object_or_404(
                Schedule, name=form.cleaned_data["from_schedule"], meeting=meeting
            )
            to_schedule = get_object_or_404(
                Schedule, name=form.cleaned_data["to_schedule"], meeting=meeting
            )
            ...
            for d in diffs:
                ...
                if s.requested_duration:
                    s.session_label = "{} ({}h)".format(
                        s.session_label,
                        round(s.requested_duration.seconds / 60.0 / 60.0, 1),
                    )
    else:
...
@ensure_csrf_cookie
def agenda_plain(
    request, num=None, name=None, base=None, ext=None, owner=None, utc=None
):
    ...
    if meeting is None:
        ...
        return HttpResponseRedirect(
            f"{settings.PROCEEDINGS_V1_BASE_URL.format(meeting=meeting)}"
        )
    else:
        pass
    ...
    filtered_assignments = preprocess_assignments_for_agenda(
        get_assignments_for_agenda(schedule), meeting
    )
...
def agenda(request, num=None, name=None, base=None, ext=None, owner=None, utc=""):
    ...
    return render(
        request, "meeting/agenda.html", {"meetingData": {"meetingNumber": num}}
    )
...
def generate_agenda_data(num=None, force_refresh=False):
    ...
    prelimAgendaDate = ImportantDate.objects.filter(
        name_id="prelimagenda", meeting=meeting
    ).first()
    ...
    result = {
        "meeting": {
            ...
            "prelimAgendaDate": (
                prelimAgendaDate.date.isoformat() if prelimAgendaDate else ""
            ),
        },
...
def api_get_session_materials(request, session_id=None):
    ...
    slides_actions = []
    if (
        can_manage_session_materials(request.user, session.group, session)
        or not session.is_material_submission_cutoff()
    ):
...
def agenda_extract_schedule(item):
    return {
        ...
        "location": (
            ...
            if (
                item.timeslot.show_location
                and item.timeslot.location
                and item.timeslot.location.floorplan
            )
            else {}
        ),
        ...
        "groupParent": (
            ...
            if item.session.group_parent_at_the_time()
            else {}
        ),
        ...
        "flags": {
            ...
            "showAgenda": (
                True
                if (
                    item.session.agenda() is not None
                    or item.session.remote_instructions
                )
                else False
            ),
        },
        "agenda": (
            ...
            if item.session.agenda() is not None
            else {"url": None}
        ),
...
def agenda_csv(schedule, filtered_assignments, utc=False):
    ...
    def agenda_field(item):
        ...
        if agenda_doc:
            return "http://www.ietf.org/proceedings/{schedule.meeting.number}/agenda/{agenda.uploaded_filename}".format(
    ...
    def slides_field(item):
        return "|".join(
            "http://www.ietf.org/proceedings/{schedule.meeting.number}/slides/{slide.uploaded_filename}".format(
                schedule=schedule, slide=slide
            )
    ...
    for item in filtered_assignments:
        ...
        if item.slot_type().slug == "break":
            ...
            row.append(
                item.session.group_parent_at_the_time().acronym.upper()
                if item.session.group_parent_at_the_time()
                else ""
            )
            ...
            row.append(
                "BOF"
                if item.session.group_at_the_time().state_id in ("bof", "bof-conc")
                else item.session.group_at_the_time().type.name
            )
...
def agenda_by_type_ics(request, num=None, type=None):
    ...
    return render(
        request,
        "meeting/agenda.ics",
        {"schedule": schedule, "updated": updated, "assignments": assignments},
...
def session_draft_list(num, acronym):
    ...
    for sp in SessionPresentation.objects.filter(
        session__meeting__number=num,
        session__group__acronym=acronym,
...
def ical_session_status(assignment):
    if assignment.session.current_status == "canceled":
        ...
        if assignment.session.tombstone_for_id is not None:
            other_assignment = SchedTimeSessAssignment.objects.filter(
                schedule=assignment.schedule_id,
                session=assignment.session.tombstone_for_id,
            ).first()
            if other_assignment:
...
def should_include_assignment(filter_params, assignment):
    ...
    hidden = (
        len(set(filter_params["hide"]).intersection(assignment.filter_keywords)) > 0
    )
...
def agenda_ical(request, num=None, acronym=None, session_id=None):
    ...
    if num is None:
        ...
        meeting = get_meeting(
            num, type_in=None
        )  # get requested meeting, whatever its type
    schedule = get_schedule(meeting)
    updated = meeting.updated()
    ...
    if filt_params is not None:
        # Apply the filter
        assignments = [
            a for a in assignments if should_include_assignment(filt_params, a)
        ]

    if acronym:
        assignments = [
            a for a in assignments if a.session.group_at_the_time().acronym == acronym
        ]
    elif session_id:
...
def agenda_json(request, num=None):
    ...
    assignments = SchedTimeSessAssignment.objects.filter(
        schedule__in=[
            meeting.schedule,
            meeting.schedule.base if meeting.schedule else None,
        ],
        session__on_agenda=True,
    ).exclude(session__type__in=["break", "reg"])
    # Update the assignments with historic information, i.e., valid at the
    # time of the meeting
    assignments = preprocess_assignments_for_agenda(
        assignments,
        meeting,
        extra_prefetches=[
    ...
    for asgn in assignments:
        ...
        if asgn.session.group_at_the_time():
            ...
            if asgn.session.group_at_the_time().type_id in [
                "wg",
                "rg",
                "ag",
                "rag",
            ] or asgn.session.group_at_the_time().acronym in [
                "iesg",
            ]:  # TODO: should that first list be groupfeatures driven?
                if asgn.session.group_parent_at_the_time():
                    sessdict["group"][
                        "parent"
                    ] = asgn.session.group_parent_at_the_time().acronym
                ...
                sessdict["presentations"].append(
                    {
                        ...
                        "order": pres.order,
                        "rev": pres.rev,
                        "resource_uri": "/api/v1/meeting/sessionpresentation/%s/"
                        % pres.id,
                    }
                )
    ...
    for obj in meetinfo:
        obj["modified"] = (
            obj["modified"].astimezone(pytz.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
        )
...
def get_summary_by_area(sessions):
    ...
    for duration in sorted(durations):
        rows.append(
            [duration]
            + [data[(duration, a)] for a in sorted_areas]
            + [duration_totals[duration]]
            + [duration_totals[duration] * duration]
        )
    # add total row
    rows.append(
        ["Total Slots"]
        + [
            sum([rows[r][c] for r in range(1, len(rows))])
            for c in range(1, len(rows[0]))
        ]
    )
    rows.append(
        ["Total Hours"]
        + [sum([d * data[(d, area)] for d in durations]) for area in sorted_areas]
    )
    return rows
...
def meeting_requests(request, num=None):
    ...
    sessions = list(
        ...
        .with_current_status()
        .with_requested_by()
        .exclude(requested_by=0)
        .prefetch_related("group", "group__ad_role__person", "group__type")
    )
    ...
    session_requesters = {
        p.pk: p
        for p in Person.objects.filter(
            pk__in=[s.requested_by for s in sessions if s.requested_by is not None]
        )
    }
...
def session_details(request, num, acronym):
    ...
    for session in sessions:
        ...
        if ss:
            ...
            session.times = []
            session.cancelled = session.current_status in Session.CANCELED_STATUSES
            session.status = status_names.get(
                session.current_status, session.current_status
            )
        ...
        if session.meeting.type_id == "ietf" and not session.meeting.proceedings_final:
            ...
            if Attended.objects.filter(session=session).exists():
                ...
                session.bluesheet_title = "Attendance IETF%s: %s : %s" % (
        ...
        session.filtered_artifacts = list(
            session.presentations.filter(document__type__slug__in=artifact_types)
        )
        session.filtered_artifacts.sort(
            key=lambda d: artifact_types.index(d.document.type.slug)
        )
        session.filtered_slides = session.presentations.filter(
            document__type__slug="slides"
        ).order_by("order")
        session.filtered_drafts = session.presentations.filter(
            document__type__slug="draft"
        )

        filtered_polls = session.presentations.filter(document__type__slug=("polls"))
        filtered_chatlogs = session.presentations.filter(
            document__type__slug=("chatlog")
        )
        ...
        for qs in [
            session.filtered_artifacts,
            session.filtered_slides,
    ...
    pending_suggestions = SlideSubmission.objects.filter(
        session__in=sessions, status__slug="pending"
    )
    if can_manage:
    ...
    return render(
        ...
        "meeting/session_details.html",
        {
            ...
            "meeting": meeting,
            "group": group,
            "is_materials_manager": session.group.has_role(
                request.user, session.group.features.matman_roles
            ),
...
def add_session_drafts(request, session_id, num):
    ...
    already_linked = [
        sp.document for sp in session.presentations.filter(document__type_id="draft")
    ]
...
def session_attendance(request, session_id, num):
    ...
    if session.meeting.type_id != "ietf" or session.meeting.proceedings_final:
        bluesheets = session.presentations.filter(document__type_id="bluesheets")
        if bluesheets:
...
def upload_session_bluesheets(request, session_id, num):
    ...
    if not session.can_manage_materials(request.user):
        permission_denied(
            request, "You don't have permission to upload bluesheets for this session."
        )
    if session.is_material_submission_cutoff() and not has_role(
        request.user, "Secretariat"
    ):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            save_error = save_bluesheet(
                request, session, file, encoding=form.file_encoding[file.name]
            )
            if save_error:
...
def upload_session_minutes(request, session_id, num):
    ...
    if not session.can_manage_materials(request.user):
        permission_denied(
            request, "You don't have permission to upload minutes for this session."
        )
    if session.is_material_submission_cutoff() and not has_role(
        request.user, "Secretariat"
    ):
    ...
    if request.method == "POST":
        form = UploadMinutesForm(
            show_apply_to_all_checkbox, request.POST, request.FILES
        )
        if form.is_valid():
            file = request.FILES["file"]
            ...
            try:
                ...
                messages.success(
                    request,
                    f"Successfully uploaded minutes as revision {session.minutes().rev}.",
                )
                return redirect(
                    "ietf.meeting.views.session_details",
                    num=num,
    ...
    return render(
        request,
        "meeting/upload_session_minutes.html",
        {
            "session": session,
            "session_number": session_number,
...
def upload_session_narrativeminutes(request, session_id, num):
    ...
    narrativeminutes_sp = session.presentations.filter(
        document__type="narrativeminutes"
    ).first()

    if request.method == "POST":
        form = UploadNarrativeMinutesForm(
            show_apply_to_all_checkbox, request.POST, request.FILES
        )
        if form.is_valid():
            file = request.FILES["file"]
            ...
            try:
                ...
                messages.success(
                    request,
                    f"Successfully uploaded narrative minutes as revision {session.narrative_minutes().rev}.",
                )
                return redirect(
                    "ietf.meeting.views.session_details",
                    num=num,
    ...
    return render(
        request,
        "meeting/upload_session_narrativeminutes.html",
        {
            "session": session,
            "session_number": session_number,
...
class UploadOrEnterAgendaForm(UploadAgendaForm):
    ...
    content = forms.CharField(
        widget=forms.Textarea, required=False, strip=False, label="Agenda text"
    )
...
def upload_session_agenda(request, session_id, num):
    ...
    if not session.can_manage_materials(request.user):
        permission_denied(
            request, "You don't have permission to upload an agenda for this session."
        )
    if session.is_material_submission_cutoff() and not has_role(
        request.user, "Secretariat"
    ):
    ...
    sessions = get_sessions(session.meeting.number, session.group.acronym)
    show_apply_to_all_checkbox = (
        len(sessions) > 1 if session.type.slug == "regular" else False
    )
    if len(sessions) > 1:
        session_number = 1 + sessions.index(session)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if save_error:
                ...
                messages.success(
                    request, f"Successfully uploaded agenda as revision {doc.rev}."
                )
                return redirect(
                    "ietf.meeting.views.session_details",
                    num=num,
    ...
    return render(
        request,
        "meeting/upload_session_agenda.html",
        {
            "session": session,
            "session_number": session_number,
            "agenda_sp": agenda_sp,
...
def upload_session_slides(request, session_id, num, name=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not approved:
                title = form.cleaned_data["title"]
                submission = SlideSubmission.objects.create(
                    session=session,
                    title=title,
                    filename="",
                    apply_to_all=apply_to_all,
                ...
                destination = io.open(
                    os.path.join(settings.SLIDE_STAGING_PATH, filename), "wb+"
                )
                ...
                (to, cc) = gather_address_lists(
                    "slides_proposed", group=session.group, proposer=request.user.person
                ).as_strings()
                msg_txt = render_to_string(
                    "meeting/slides_proposed.txt",
                    {
                        "to": to,
                        "cc": cc,
        ...
        form = UploadSlidesForm(
            session, show_apply_to_all_checkbox, can_manage, initial=initial
        )
    ...
    return render(
        ...
        "meeting/upload_session_slides.html",
        {
            ...
            "session_number": session_number,
            "slides_sp": (
                session.presentations.filter(document=doc).first() if doc else None
            ),
...
def make_schedule_official(request, num, owner, name):
    ...
    if request.method == "POST":
        ...
        meeting.save()
        return HttpResponseRedirect(
            reverse("ietf.meeting.views.list_schedules", kwargs={"num": num})
        )

    if not schedule.public:
        messages.warning(
            request, "This schedule will be made public as it is made official."
        )
    if not schedule.visible:
        messages.warning(
            request, "This schedule will be made visible as it is made official."
        )
    if schedule.base:
        if not schedule.base.public:
            messages.warning(
                request, "The base schedule will be made public as it is made official."
            )
        if not schedule.base.visible:
            messages.warning(
                request,
                "The base schedule will be made visible as it is made official.",
            )

    return render(
        request,
        "meeting/make_schedule_official.html",
        {
            "schedule": schedule,
            "meeting": meeting,
        },
...
def delete_schedule(request, num, owner, name):
    ...
    if schedule == meeting.schedule:
        permission_denied(
            request, "You may not delete the official schedule for %s" % meeting
        )

    if Schedule.objects.filter(base=schedule).exists():
        return HttpResponseForbidden(
            "You may not delete a schedule serving as the base for other schedules"
        )
...
def interim_announce(request):
    ...
    meetings = data_for_meetings_overview(
        Meeting.objects.filter(type="interim").order_by("date"), interim_status="scheda"
    )
...
def interim_send_announcement(request, number):
    ...
    return render(
        request,
        "meeting/interim_send_announcement.html",
...
def interim_skip_announcement(request, number):
    ...
    return render(request, "meeting/interim_skip_announce.html", {"meeting": meeting})


def interim_pending(request):
    """View which shows interim meeting requests pending approval"""
    meetings = data_for_meetings_overview(
        Meeting.objects.filter(type="interim").order_by("date"), interim_status="apprw"
    )
...
def interim_request(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and formset.is_valid():
            ...
            requires_approval = not (
                is_approved
                or (is_virtual and not settings.VIRTUAL_INTERIMS_REQUIRE_APPROVAL)
            )
    ...
    return render(
        request, "meeting/interim_request.html", {"form": form, "formset": formset}
    )
...
def interim_request_cancel(request, number):
    ...
    if not can_manage_group(request.user, group):
        permission_denied(
            request, "You do not have permissions to cancel this meeting request"
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            result_status = SessionStatusName.objects.get(
                slug="canceled" if was_scheduled else "canceledpa"
            )
    ...
    return render(
        request,
        "meeting/interim_request_cancel.html",
        {
            "form": form,
            "meeting": meeting,
...
def interim_request_session_cancel(request, sessionid):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            remaining_sessions = (
                session.meeting.session_set.with_current_status().exclude(
                    current_status__in=["canceled", "canceledpa"]
                )
            )
            if remaining_sessions.count() <= 1:
                return HttpResponse(
                    "Cannot cancel only remaining session. Cancel the request instead.",
                    status=409,
                )
        ...
        form = InterimCancelForm(
            initial={"group": group.acronym, "date": session_time.date()}
        )
...
def interim_request_details(request, number):
    ...
    if not can_manage_group(request.user, group):
        permission_denied(
            request, "You do not have permissions to manage this meeting request"
        )
    ...
    ).select_related("session", "timeslot")
    ...
    return render(
        request,
        "meeting/interim_request_details.html",
        {
            "meeting": meeting,
            ...
            or SessionStatusName.objects.get(slug="canceled"),
            "can_edit": can_edit,
...
def interim_request_edit(request, number):
    ...
    if not can_edit_interim_request(meeting, request.user):
        permission_denied(
            request, "You do not have permissions to edit this meeting request"
        )

    SessionFormset = inlineformset_factory(
        Meeting, Session, form=InterimSessionModelForm, can_delete=False, extra=1
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and formset.is_valid():
            ...
            meeting_is_scheduled = (
                add_event_info_to_session_qs(meeting.session_set)
    ...
    return render(
        request,
        "meeting/interim_request_edit.html",
        {"meeting": meeting, "form": form, "formset": formset},
    )
...
def past(request):
    ...
    meetings = data_for_meetings_overview(
        Meeting.objects.filter(date__lte=today).order_by("-date")
    )
...
def upcoming(request):
    ...
    ietf_meetings = Meeting.objects.filter(
        type_id="ietf", date__gte=today - datetime.timedelta(days=7)
    )
    ...
    filter_organizer = AgendaFilterOrganizer(
        sessions=interim_sessions, single_category=True
    )
    ...
    entries.sort(
        key=lambda o: (
            (
                pytz.utc.localize(
                    datetime.datetime.combine(o.date, datetime.datetime.min.time())
                )
                if isinstance(o, Meeting)
                else o.official_timeslotassignment().timeslot.utc_start_time()
            ),
    ...
    for o in entries:
        if isinstance(o, Meeting):
            o.start_timestamp = int(
                pytz.utc.localize(
                    datetime.datetime.combine(o.date, datetime.time.min)
                ).timestamp()
            )
            o.end_timestamp = int(
                pytz.utc.localize(
                    datetime.datetime.combine(o.end_date(), datetime.time.max)
                ).timestamp()
            )
        else:
            o.start_timestamp = int(
                o.official_timeslotassignment().timeslot.utc_start_time().timestamp()
            )
            o.end_timestamp = int(
                o.official_timeslotassignment().timeslot.utc_end_time().timestamp()
            )
...
def upcoming_ical(request):
    ...
    meetings = data_for_meetings_overview(
        Meeting.objects.filter(date__gte=today - datetime.timedelta(days=7))
    ...
    assignments = list(
        SchedTimeSessAssignment.objects.filter(
            schedule__in=[m.schedule_id for m in meetings]
            + [m.schedule.base_id for m in meetings if m.schedule],
            session__in=[
                s.pk for m in meetings for s in m.sessions if m.type_id != "ietf"
            ],
            timeslot__time__gte=today,
        )
        .order_by(
            "schedule__meeting__date",
            "session__type",
            "timeslot__time",
            "schedule__meeting__number",
        )
        .select_related(
            "session__group",
            "session__group__parent",
    ...
    if filter_params is not None:
        assignments = [
            a for a in assignments if should_include_assignment(filter_params, a)
        ]
    ...
    if filter_params is None or (
        "ietf-meetings" in filter_params["show"]
        and "ietf-meetings" not in filter_params["hide"]
    ):
...
def upcoming_json(request):
    ...
    # get meetings starting 7 days ago -- we'll filter out sessions in the past further down
    meetings = data_for_meetings_overview(
        Meeting.objects.filter(date__gte=today - datetime.timedelta(days=7)).order_by(
            "date"
        )
    )
...
def proceedings(request, num=None):
    ...
    if meeting.proceedings_format_version == 1:
        return HttpResponseRedirect(
            settings.PROCEEDINGS_V1_BASE_URL.format(meeting=meeting)
        )
    ...
    with timezone.override(meeting.tz()):
        return render(
            request,
            "meeting/proceedings_wrapper.html",
            {
                "meeting": meeting,
...
def finalize_proceedings(request, num=None):
    ...
    meeting = get_meeting(num)
    if (
        (meeting.number.isdigit() and int(meeting.number) <= 64)
        or not meeting.schedule
        or not meeting.schedule.assignments.exists()
    ...
    if request.method == "POST":
        ...
        return HttpResponseRedirect(
            reverse("ietf.meeting.views.proceedings", kwargs={"num": meeting.number})
        )

    return render(
        request,
        "meeting/finalize.html",
        {
            "meeting": meeting,
        },
    )
...
def proceedings_acknowledgements(request, num=None):
    ...
    if meeting.proceedings_format_version == 1:
        return HttpResponseRedirect(
            f"{settings.PROCEEDINGS_V1_BASE_URL.format(meeting=meeting)}/acknowledgement.html"
        )
    return render(
        request,
        "meeting/proceedings_acknowledgements.html",
        {
            "meeting": meeting,
        },
    )
...
def proceedings_attendees(request, num=None):
    ...
    if meeting.proceedings_format_version == 1:
        return HttpResponseRedirect(
            f"{settings.PROCEEDINGS_V1_BASE_URL.format(meeting=meeting)}/attendee.html"
        )
    ...
    if int(meeting.number) >= 118:
        ...
        regs = list(
            MeetingRegistration.objects.filter(
                meeting__number=num, reg_type="onsite", checkedin=True
            )
        )

        for mr in MeetingRegistration.objects.filter(
            meeting__number=num, reg_type="remote"
    ...
    return render(
        request,
        "meeting/proceedings_attendees.html",
        {
            "meeting": meeting,
...
def proceedings_overview(request, num=None):
    ...
    if meeting.proceedings_format_version == 1:
        return HttpResponseRedirect(
            f"{settings.PROCEEDINGS_V1_BASE_URL.format(meeting=meeting)}/overview.html"
        )
    ...
    return render(
        request,
        "meeting/proceedings_overview.html",
        {
            "meeting": meeting,
            "template": template,
        },
...
def proceedings_activity_report(request, num=None):
    ...
    if meeting.proceedings_format_version == 1:
        return HttpResponseRedirect(
            f"{settings.PROCEEDINGS_V1_BASE_URL.format(meeting=meeting)}/progress-report.html"
        )
...
def api_set_session_video_url(request):
    ...
    recordings = [
        (r.name, r.title, r) for r in session.recordings() if "video" in r.title.lower()
    ]
    if recordings:
        r = recordings[-1][-1]
        if r.external_url != incoming_url:
            e = DocEvent.objects.create(
                ...
                type="added_comment",
                by=request.user.person,
                desc="External url changed from %s to %s"
...
def deprecated_api_set_session_video_url(request):
    ...
    if request.method == "POST":
        ...
        if not sessions.exists():
            return err(
                400,
                "No sessions found in meeting '%s' for group '%s'" % (number, acronym),
            )
        session_times = [
            (s.official_timeslotassignment().timeslot.time, s.id, s)
            for s in sessions
        ...
        try:
            ...
            return err(
                400, "No item '%s' found in list of sessions for group" % (item,)
            )
...
@role_required(
    "Recording Manager"
)  # TODO : Rework how Meetecho interacts via APIs. There may be better paths to pursue than Personal API keys as they are currently defined.
@csrf_exempt
        ...
        if type(attendees[0]) == int:
            ...
            join_time_by_pk = {
                att["user_id"]: datetime.datetime.fromisoformat(
                    att["join_time"].replace(
                        "Z", "+00:00"
                    )  # Z not understood until py311
                )
            ...
            to_create = [
                Attended(
                    session=session, person=person, time=join_time_by_pk[person.user_id]
                )
...
def api_upload_chatlog(request):
    ...
    if not (
        "chatlog" in apidata
        and type(apidata["chatlog"]) is list
        and all([type(el) is dict for el in apidata["chatlog"]])
    ):
        return err(400, "Malformed post")
    session = Session.objects.filter(pk=session_id).first()
    if not session:
...
def api_upload_polls(request):
    ...
    if not (
        "polls" in apidata
        and type(apidata["polls"]) is list
        and all([type(el) is dict for el in apidata["polls"]])
    ):
        return err(400, "Malformed post")
    session = Session.objects.filter(pk=session_id).first()
    if not session:
...
def important_dates(request, num=None, output_format=None):
    ...
    for i in range(1, 3):
        ...
        if future_meeting and (
            future_meeting.show_important_dates
    ...
    if output_format == "ics":
        ...
        ics = render_to_string(
            "meeting/important_dates.ics",
            {
                "meetings": meetings,
            },
            request=request,
    ...
    return render(request, "meeting/important-dates.html", {"meetings": meetings})
...
def edit_timeslot_type(request, num, slot_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            return HttpResponseRedirect(
                reverse("ietf.meeting.views.edit_timeslots", kwargs={"num": num})
            )
    ...
    return render(
        request,
        "meeting/edit_timeslot_type.html",
...
def edit_timeslot(request, num, slot_id):
    ...
    with timezone.override(
        meeting.tz()
    ):  # specifies current_timezone used for rendering and form handling
        if request.method == "POST":
            form = TimeSlotEditForm(instance=timeslot, data=request.POST)
            if form.is_valid():
                form.save()
                redirect_to = reverse(
                    "ietf.meeting.views.edit_timeslots", kwargs={"num": num}
                )
                if "sched" in request.GET:
...
def _schedule_edit_url(meeting, schedule):
    ...
    if schedule and not schedule.is_official:
        url_args.update(
            {
                "name": (
                    schedule.name if schedule and not schedule.is_official else None
                ),
                "owner": (
...
def request_minutes(request, num=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(
                reverse("ietf.meeting.views.materials", kwargs={"num": num})
            )
    else:
        needs_minutes = set()
        session_qs = (
            add_event_info_to_session_qs(
                Session.objects.filter(
                    ...
                    timeslotassignments__schedule__meeting__schedule=F(
                        "timeslotassignments__schedule"
                    ),
                    group__type__in=["wg", "rg", "ag", "rag", "program"],
                )
            )
            .filter(~Q(current_status="canceled"))
        ...
        needs_minutes.sort(
            key=lambda g: ("zzz" if g.parent.acronym == "irtf" else g.parent.acronym)
            + ":"
            + g.acronym
        )
        body_context = {
...
def approve_proposed_slides(request, slidesubmission_id, num):
    ...
    if not submission.session.can_manage_materials(request.user):
        permission_denied(
            request, "You don't have permission to manage slides for this session."
        )
    if submission.session.is_material_submission_cutoff() and not has_role(
        request.user, "Secretariat"
    ):
        permission_denied(
            request,
    ...
    sessions = get_sessions(
        submission.session.meeting.number, submission.session.group.acronym
    )
    show_apply_to_all_checkbox = (
        len(sessions) > 1 if submission.session.type_id == "regular" else False
    )
    if len(sessions) > 1:
    ...
    if request.method == "POST" and submission.status.slug == "pending":
        ...
        if form.is_valid():
            ...
            if request.POST.get("approve"):
                ...
                if (
                    ...
                    or not os.path.isfile(submission.staged_filepath())
                ):
                    return HttpResponseNotFound(
                        "The slides you attempted to approve could not be found.  Please disapprove and delete them instead."
                    )
                title = form.cleaned_data["title"]
                if existing_doc:
                    doc = Document.objects.get(name=name)
                ...
                if submission.session.presentations.filter(document=doc).exists():
                    ...
                    max_order = (
                        submission.session.presentations.filter(
                            document__type="slides"
                        ).aggregate(Max("order"))["order__max"]
                        or 0
                    )
                    added_presentations.append(
                        submission.session.presentations.create(
                            document=doc, rev=doc.rev, order=max_order + 1
                        )
                    )
                if apply_to_all:
                    for other_session in sessions:
                        if (
                            other_session != submission.session
                ...
                doc.save_with_history([e])
                path = os.path.join(
                    submission.session.meeting.get_materials_path(), "slides"
                )
                if not os.path.exists(path):
                    os.makedirs(path)
                shutil.move(
                    submission.staged_filepath(), os.path.join(path, target_filename)
                )
                doc.store_bytes(
                    target_filename, retrieve_bytes("staging", submission.filename)
                )
                remove_from_storage("staging", submission.filename)
                post_process(doc)
                DocEvent.objects.create(
                    type="approved_slides",
                    doc=doc,
                    rev=doc.rev,
                    by=request.user.person,
                    desc="Slides approved",
                )
                ...
                submission.status = SlideSubmissionStatusName.objects.get(
                    slug="approved"
                )
                submission.doc = doc
                submission.save()
                ...
                return redirect(
                    "ietf.meeting.views.session_details", num=num, acronym=acronym
                )
        ...
        return render(
            request,
            "meeting/previously_approved_slides.html",
            {"submission": submission},
        )
    else:
        initial = {
    ...
    return render(
        request,
        "meeting/approve_proposed_slides.html",
        {
            "submission": submission,
            "session_number": session_number,
...
def import_session_minutes(request, session_id, num):
    ...
    if not session.can_manage_materials(request.user):
        permission_denied(
            request, "You don't have permission to import minutes for this session."
        )
    if session.is_material_submission_cutoff() and not has_role(
        request.user, "Secretariat"
    ):
    ...
    if request.method == "POST":
        ...
        if not form.is_valid():
            ...
            try:
                ...
                messages.success(
                    request,
                    f"Successfully imported minutes as revision {session.minutes().rev}.",
                )
                return redirect(
                    "ietf.meeting.views.session_details",
                    num=num,
        ...
        try:
            with open(current_minutes.get_file_name()) as f:
                if import_contents == f.read():
                    ...
                    messages.warning(
                        request,
                        "This document is identical to the current revision, no need to import.",
                    )
        except Exception:
