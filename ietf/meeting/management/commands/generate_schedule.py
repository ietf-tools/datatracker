...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "-n", "--name", default=None, help="a name for the generated schedule"
        )
        parser.add_argument(
            "-r",
            "--max-runs",
        ...
        parser.add_argument(
            ...
            choices=[
                spn.slug
                for spn in SessionPurposeName.objects.all()
                if "regular"
                in spn.timeslot_types  # scheduler only works with "regular" timeslots
            ],
            default=None,
            help=(
    ...
    def handle(
        self, meeting, name, max_cycles, verbosity, base_id, purposes, *args, **kwargs
    ):
        ScheduleHandler(
            self.stdout, meeting, name, max_cycles, verbosity, base_id, purposes
        ).run()
...
class ScheduleHandler(object):
    def __init__(
        ...
        meeting_number,
        name=None,
        max_cycles=OPTIMISER_MAX_CYCLES,
        verbosity=1,
        base_id=None,
        session_purposes=None,
    ):
        self.stdout = stdout
        ...
        if meeting_number:
            try:
                self.meeting = models.Meeting.objects.get(
                    type="ietf", number=meeting_number
                )
        ...
        if base_id is None:
            ...
            base_candidates = models.Schedule.objects.filter(
                meeting=self.meeting, name=base_id.name
            )
            if base_id.owner is not None:
                base_candidates = base_candidates.filter(
                    owner=get_person_by_email(base_id.owner)
                )
            if base_candidates.count() == 0:
                raise CommandError('Base schedule "{}" not found'.format(base_id))
            elif base_candidates.count() >= 2:
                raise CommandError(
                    'Base schedule "{}" not unique (candidates are {})'.format(
                        base_id,
                        ", ".join(
                            str(ScheduleId.from_schedule(sched))
                            for sched in base_candidates
                        ),
                    )
                )
            else:
        ...
        if self.verbosity >= 1:
            msgs = [
                "Running automatic schedule layout for meeting IETF {}".format(
                    self.meeting.number
                )
            ]
            if self.base_schedule is not None:
                msgs.append(
                    "Applying schedule {} as base schedule".format(
                        ScheduleId.from_schedule(self.base_schedule)
                    )
                )
            self.stdout.write("\n{}\n\n".format("\n".join(msgs)))
        self._load_meeting()
    ...
    def run(self):
        ...
        if self.verbosity >= 1:
            self.stdout.write(
                "Initial schedule completed with %s violations, total cost %s, in %dm %.2fs"
        ...
        if self.verbosity >= 1:
            vc = len(violations)
            self.stdout.write(
                "Optimisation completed with %s violation%s, cost %s, %s runs in %dm %.2fs"
                % (
                    vc,
                    "" if vc == 1 else "s",
                    intcomma(cost),
    ...
    def _save_schedule(self, cost):
        if not self.name:
            count = models.Schedule.objects.filter(
                name__startswith="auto-%s-" % self.meeting.number
            ).count()
            self.name = "auto-%s-%02d" % (self.meeting.number, count)
        if models.Schedule.objects.filter(name=self.name).exists():
            self.stdout.write(
                "WARNING: A schedule with the name '%s' already exists.  Picking another random one."
                % self.name
            )
            self.name = "auto-%s-%s" % (
                self.meeting.number,
    ...
    def _available_timeslots(self):
        ...
        """
        timeslots_db = (
            self.meeting.timeslot_set.that_can_be_scheduled()
            .filter(
                type_id="regular",
            )
            .exclude(
        ...
        if self.base_schedule is None:
            ...
            fixed_timeslots = timeslots_db.filter(
                pk__in=self.base_schedule.qs_timeslots_in_use()
            )
        free_timeslots = timeslots_db.exclude(pk__in=fixed_timeslots)
        timeslots = {
            DatatrackerTimeSlot(t, verbosity=self.verbosity)
            for t in free_timeslots.select_related("location")
        }
        timeslots.update(
            DatatrackerTimeSlot(t, verbosity=self.verbosity, is_fixed=True)
            for t in fixed_timeslots.select_related("location")
        )
    ...
    def _sessions_to_schedule(self, *args, **kwargs):
        ...
        """
        sessions_db = self.meeting.session_set.that_can_be_scheduled().filter(
            type_id="regular"
        )
        ...
        if self.base_schedule is None:
            ...
            fixed_sessions = sessions_db.filter(
                pk__in=self.base_schedule.qs_sessions_scheduled()
            )
    ...
    def _load_meeting(self):
        ...
        business_constraint_costs = {
            bc.slug: bc.penalty for bc in models.BusinessConstraint.objects.all()
        }
...
class Schedule(object):
    ...
    def __init__(
        ...
        self.max_cycles = max_cycles
        self.base_schedule = (
            self._load_base_schedule(base_schedule) if base_schedule else None
        )
...
-----------------""".format(
            ...
            print(
                "{}: {}{}".format(
                    ...
                    (
                        " [BASE]"
                        if self.base_schedule and slot in self.base_schedule
                        else ""
                    ),
                )
            )
...
-----------------"""
    ...
    def _load_base_schedule(self, db_base_schedule):
        ...
        base_schedule = dict()
        for assignment in db_base_schedule.assignments.filter(
            session__in=session_lut, timeslot__in=timeslot_lut
        ):
            base_schedule[timeslot_lut[assignment.timeslot.pk]] = session_lut[
        ...
        if num_to_schedule > num_free_timeslots:
            raise CommandError(
                "More sessions ({}) than timeslots ({})".format(
                    num_to_schedule, num_free_timeslots
                )
            )

        def make_capacity_adjustments(t_attr, s_attr):
            availables = [
                getattr(timeslot, t_attr)
                for timeslot in self.free_timeslots
                if timeslot.is_scheduled
            ]
            availables.sort()
            sessions = sorted(
                self.free_sessions, key=lambda s: getattr(s, s_attr), reverse=True
            )
            ...
            availables.extend(
                [availables[-1]] * n_unscheduled
            )  # availables is sorted, so [-1] is max
            violations, cost = [], 0
            for session in sessions:
                found_fit = False
    ...
    def total_schedule_cost(self):
        ...
        if self.base_schedule is not None:
            ...
            self.add_fixed_cost(
                "base_schedule",
                *self.calculate_dynamic_cost(self.base_schedule, include_fixed=True),
            )
    ...
    def calculate_dynamic_cost(self, schedule=None, include_fixed=False):
        ...
        for timeslot, session in schedule.items():
            group_sessions[session.group].add(
                (timeslot, session)
            )  # (timeslot, session), not just session!
            overlapping_sessions[timeslot].update(
                {schedule[t] for t in timeslot.overlaps if t in schedule}
            )
        ...
        for timeslot, session in schedule.items():
            session_violations, session_cost = session.calculate_cost(
                schedule,
                timeslot,
                overlapping_sessions[timeslot],
                group_sessions[session.group],
                include_fixed,
            )
    ...
    def fill_initial_schedule(self):
        ...
        for session in sessions:
            possible_slots = [
                t for t in self.free_timeslots if t not in self.schedule.keys()
            ]
            ...
            def timeslot_preference(t):
                ...
                return (
                    ...
                    (
                        t.duration if t.is_scheduled else datetime.timedelta(hours=1000)
                    ),  # unscheduled slots sort to the end
                    (
                        t.capacity if t.is_scheduled else math.inf
                    ),  # unscheduled slots sort to the end
                )
            ...
            if self.verbosity >= 3:
                if possible_slots[0].is_scheduled:
                    self.stdout.write(
                        "Scheduled {} at {} in location {}".format(
                            session.group,
                            possible_slots[0].start,
    ...
    def optimise_schedule(self):
        ...
        for _ in range(self.max_cycles):
            ...
            if self.verbosity >= 2:
                self.stdout.write(
                    "== Optimiser starting run {}, dynamic cost after last run {} ==".format(
                        run_count, last_run_cost
                    )
                )
                self.stdout.write(
                    "Dynamic violations in last optimiser run: {}".format(
                        last_run_violations
                    )
                )
            if shuffle_next_run:
                shuffle_next_run = False
                last_run_cost = (
                    None  # After a shuffle, attempt at least two regular runs
                )
            ...
            for original_timeslot, session in items:
                ...
                if best_timeslot:
                    switched_with = self._switch_sessions(
                        original_timeslot, best_timeslot
                    )
                    switched_with = (
                        switched_with.group if switched_with else "<empty slot>"
                    )
                    if self.verbosity >= 3:
                        self.stdout.write(
                            "Run {:2}: found cost reduction to {:,} by switching {} with {}".format(
                                run_count, best_cost, session.group, switched_with
                            )
                        )
        ...
        if self.verbosity >= 2:
            self.stdout.write(
                "Optimiser did not find perfect schedule, using best schedule at dynamic cost {:,}".format(
                    self.best_cost
                )
            )
    ...
    def _shuffle_conflicted_sessions(self, items):
        ...
        if self.verbosity >= 2:
            self.stdout.write(
                "Optimiser has no more improvements, shuffling sessions {}".format(
                    ", ".join([s.group for t, s in to_reschedule])
                )
            )

        for original_timeslot, rescheduling_session in to_reschedule:
            possible_new_slots = list(
                t for t in self.free_timeslots if t != original_timeslot
            )
            random.shuffle(possible_new_slots)

            for possible_new_slot in possible_new_slots:
                switched_with = self._switch_sessions(
                    original_timeslot, possible_new_slot
                )
                if switched_with is not False:
                    ...
                    if self.verbosity >= 3:
                        self.stdout.write(
                            "Moved {} to random new slot, previously in slot was {}".format(
                                rescheduling_session.group, switched_group
                            )
                        )
                    break
    ...
    def optimise_timeslot_capacity(self):
        ...
        for timeslot in list(self.schedule.keys()):
            if (
                timeslot in optimised_timeslots
                or timeslot.is_fixed
                or not timeslot.is_scheduled
            ):
                continue
            timeslot_overlaps = sorted(
                timeslot.full_overlaps, key=lambda t: t.capacity, reverse=True
            )
...
class DatatrackerTimeSlot(GeneratorTimeSlot):
    ...
    def store_relations(self, other_timeslots):
        ...
        for other in other_timeslots:
            ...
            if (
                abs(self.start - other.end) <= datetime.timedelta(minutes=30)
                or abs(other.start - self.end) <= datetime.timedelta(minutes=30)
...
class Session(object):
    ...
    def __init__(
        ...
        self.parent = (
            session_db.group.parent.acronym if session_db.group.parent else None
        )
        self.ad = (
            session_db.group.ad_role().person.pk if session_db.group.ad_role() else None
        )
        ...
        self.is_prg = (
            session_db.group.type_id == "rg" and session_db.group.state_id == "proposed"
        )
        ...
        if not self.attendees:
            if self.verbosity >= 1:
                self.stdout.write(
                    "WARNING: session {} (pk {}) has no attendees set, assuming any room fits".format(
                        self.group, self.session_pk
                    )
                )
            self.attendees = 0
        ...
        constraints_db = models.Constraint.objects.filter(
            Q(source=session_db.group)
            | Q(source__in=session_db.joint_with_groups.all()),
            meeting=meeting,
        )
        ...
        for constraint_db in constraints_db:
            if constraint_db.name.is_group_conflict:
                self.conflict_groups[
                    constraint_db.target.acronym
                ] += constraint_db.name.penalty
                ...
                self.timeranges_unavailable.update(
                    {t.slug for t in constraint_db.timeranges.all()}
                )
            ...
            self.complexity += sum(
                [
                    sum(
                        [
                            cost
                            for group, cost in other_session.conflict_groups.items()
                            if group == self.group
                        ]
                    ),
                    self.conflict_people_penalty
                    * len(
    ...
    def fits_in_timeslot(self, timeslot):
        return timeslot.has_space_for(self.attendees) and timeslot.has_time_for(
    ...
    def calculate_cost(
        ...
        if include_fixed or (not self.is_fixed):
            ...
            if not my_timeslot.has_time_for(self.requested_duration):
                violations.append(
                    "{}: scheduled in too short timeslot".format(self.group)
                )
                cost += self.business_constraint_costs["session_requires_trim"]

            if my_timeslot.time_group in self.timeranges_unavailable:
                violations.append(
                    "{}: scheduled in unavailable timerange {}".format(
                        self.group, my_timeslot.time_group
                    )
                )
        ...
        if self.wg_adjacent and (include_fixed or not self.is_fixed):
            adjacent_groups = tuple(
                [schedule[t].group for t in my_timeslot.adjacent if t in schedule]
            )
            if self.wg_adjacent not in adjacent_groups:
                violations.append(
                    "{}: missing adjacency with {}, adjacents are: {}".format(
                        self.group, self.wg_adjacent, ", ".join(adjacent_groups)
                    )
                )
    ...
    def _calculate_cost_overlapping_groups(self, overlapping_sessions):
        ...
        for other in overlapping_sessions:
            ...
            if other.group == self.group:
                violations.append(
                    "{}: scheduled twice in overlapping slots".format(self.group)
                )
                cost += math.inf
            if other.group in self.conflict_groups:
                violations.append(
                    "{}: group conflict with {}".format(self.group, other.group)
                )
            ...
            for person in conflict_people:
                violations.append(
                    "{}: conflict w/ key person {}, also in {}".format(
                        self.group, person, other.group
                    )
                )
    ...
    def _calculate_cost_business_logic(self, overlapping_sessions):
        ...
        for other in overlapping_sessions:
            ...
            if self.is_bof and other.is_prg:
                violations.append(
                    "{}: BOF overlaps with PRG: {}".format(self.group, other.group)
                )
            ...
            if self.is_bof and other.is_bof:
                violations.append(
                    "{}: BOF overlaps with other BOF: {}".format(
            ...
            if self.is_bof and self.parent == other.parent:
                violations.append(
                    "{}: BOF overlaps with other session from same area: {}".format(
                        self.group, other.group
                    )
                )
            ...
            if self.is_bof and other.is_area_meeting:
                violations.append(
                    "{}: BOF overlaps with area meeting {}".format(
                        self.group, other.group
                    )
                )
            ...
            if self.is_area_meeting and other.parent == self.group:
                violations.append(
                    "{}: area meeting overlaps with session from same area: {}".format(
                        self.group, other.group
                    )
                )
            ...
            if self.is_area_meeting and other.is_area_meeting:
                violations.append(
                    "{}: area meeting overlaps with other area meeting: {}".format(
                        self.group, other.group
                    )
                )
            ...
            if self.ad and self.ad == other.ad:
                violations.append(
                    "{}: has same AD as {}".format(self.group, other.group)
                )
    ...
    def _calculate_cost_my_other_sessions(self, my_sessions):
        ...
        if len(my_sessions) >= 2:
            ...
            fixed_sessions_in_order = my_fixed_sessions == sort_sessions(
                my_fixed_sessions
            )
            # Only possible to keep sessions in order if fixed sessions are in order - ignore cost if not.
            if fixed_sessions_in_order and (
                list(my_sessions) != sort_sessions(my_sessions)
            ):
                session_order = [s.session_pk for t, s in list(my_sessions)]
                violations.append(
                    "{}: sessions out of order: {}".format(self.group, session_order)
                )
            ...
            if self.time_relation:
                if len(my_sessions) != 2:
                    ...
                    warn(
                        '"time relation" constraint only makes sense for 2 sessions but {} has {}'.format(
                            self.group, len(my_sessions)
                        )
                    )
                ...
                if all(session.is_fixed for _, session in my_sessions):
                    ...
                    if self.time_relation == "subsequent-days" and difference_days != 1:
                        violations.append(
                            "{}: has time relation subsequent-days but difference is {}".format(
                                self.group, difference_days
                            )
                        )
                        cost += self.time_relation_penalty
                    elif (
                        self.time_relation == "one-day-seperation"
                        and difference_days == 1
                    ):
                        violations.append(
                            "{}: has time relation one-day-seperation but difference is {}".format(
                                self.group, difference_days
                            )
                        )
                        cost += self.time_relation_penalty
