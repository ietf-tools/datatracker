...
def make_interim_meeting(group, date, status="sched", tz="UTC"):
    ...
    time = meeting.tz().localize(datetime.datetime.combine(date, datetime.time(9)))
    ...
    SchedTimeSessAssignment.objects.create(
        timeslot=slot, session=session, schedule=session.meeting.schedule
    )
    # agenda
...
def make_meeting_test_data(meeting=None, create_interims=False):
    ...
    base_schedule = Schedule.objects.create(
        meeting=meeting, owner=plainman, name="base", visible=True, public=True
    )
    ...
    unofficial_schedule = Schedule.objects.create(
        meeting=meeting,
        owner=plainman,
        name="test-unofficial-schedule",
        visible=True,
        public=True,
        base=base_schedule,
    )
    ...
    projector = ResourceAssociation.objects.create(
        name=pname, icon="notfound.png", desc="Basic projector"
    )
    room = Room.objects.create(
        meeting=meeting,
        name="Test Room",
        capacity=123,
        functional_name="Testing Ground",
    )
    room.session_types.add("regular")
    room.resources.add(projector)
    asname = RoomResourceName.objects.get(slug="audiostream")
    UrlResource.objects.create(
        name=asname,
        room=room,
        url="http://ietf{number}streaming.dnsalias.net/ietf/ietf{number}1.m3u".format(
            number=meeting.number
        ),
    )

    # other rooms
    breakfast_room = Room.objects.create(
        meeting=meeting, name="Breakfast Room", capacity=40
    )
    ...
    reg_room = Room.objects.create(
        meeting=meeting, name="Registration Area", capacity=500
    )
    ...
    breakfast_slot = TimeSlot.objects.create(
        meeting=meeting,
        type_id="lead",
    ...
    reg_slot = TimeSlot.objects.create(
        meeting=meeting,
        type_id="reg",
        location=reg_room,
    ...
    plenary_slot = TimeSlot.objects.create(
        meeting=meeting,
        type_id="plenary",
        location=room,
        duration=datetime.timedelta(minutes=60),
    ...
    mars_session = SessionFactory(
        meeting=meeting,
        group=mars,
        attendees=10,
        requested_duration=datetime.timedelta(minutes=50),
        add_to_schedule=False,
    )
    SchedulingEvent.objects.create(
        session=mars_session, status_id="schedw", by=system_person
    )
    SchedTimeSessAssignment.objects.create(
        timeslot=slot1, session=mars_session, schedule=schedule
    )
    SchedTimeSessAssignment.objects.create(
        timeslot=slot2, session=mars_session, schedule=unofficial_schedule
    )

    # ames WG
    ames_session = SessionFactory(
        meeting=meeting,
    ...
    SchedulingEvent.objects.create(
        session=ames_session, status_id="schedw", by=system_person
    )
    SchedTimeSessAssignment.objects.create(
        timeslot=slot2, session=ames_session, schedule=schedule
    )
    SchedTimeSessAssignment.objects.create(
        timeslot=slot1, session=ames_session, schedule=unofficial_schedule
    )

    # IESG breakfast
    iesg_session = SessionFactory(
        meeting=meeting,
        group=Group.objects.get(acronym="iesg"),
        name="IESG Breakfast",
        attendees=25,
        requested_duration=datetime.timedelta(minutes=60),
        type_id="lead",
        purpose_id="closed_meeting",
        add_to_schedule=False,
    )
    SchedulingEvent.objects.create(
        session=iesg_session, status_id="schedw", by=system_person
    )
    SchedTimeSessAssignment.objects.create(
        timeslot=breakfast_slot, session=iesg_session, schedule=schedule
    )
    # No breakfast on unofficial schedule

    # Registration
    reg_session = SessionFactory(
        meeting=meeting,
    ...
    SchedulingEvent.objects.create(
        session=reg_session, status_id="schedw", by=system_person
    )
    SchedTimeSessAssignment.objects.create(
        timeslot=reg_slot, session=reg_session, schedule=base_schedule
    )

    # Break
    break_session = SessionFactory(
        meeting=meeting,
        group=Group.objects.get(acronym="secretariat"),
        name="Morning Break",
        attendees=250,
    ...
    SchedulingEvent.objects.create(
        session=break_session, status_id="schedw", by=system_person
    )
    SchedTimeSessAssignment.objects.create(
        timeslot=break_slot, session=break_session, schedule=base_schedule
    )

    # IETF Plenary
    plenary_session = SessionFactory(
        meeting=meeting,
    ...
    SchedulingEvent.objects.create(
        session=plenary_session, status_id="schedw", by=system_person
    )
    SchedTimeSessAssignment.objects.create(
        timeslot=plenary_slot, session=plenary_session, schedule=schedule
    )
    ...
    doc = DocumentFactory.create(
        ...
        type_id="agenda",
        title="Agenda",
        uploaded_filename="agenda-72-mars.txt",
        group=mars,
        rev="00",
        states=[("agenda", "active")],
    )
    pres = SessionPresentation.objects.create(
        session=mars_session, document=doc, rev=doc.rev
    )
    mars_session.presentations.add(pres)  #

    doc = DocumentFactory.create(
        name="minutes-72-mars",
        type_id="minutes",
        title="Minutes",
        uploaded_filename="minutes-72-mars.md",
        group=mars,
        rev="00",
    ...
    doc = DocumentFactory.create(
        name="slides-72-mars-1-active",
        type_id="slides",
        title="Slideshow",
        uploaded_filename="slides-72-mars.txt",
        group=mars,
        rev="00",
        states=[("slides", "active"), ("reuse_policy", "single")],
    )
    ...
    doc = DocumentFactory.create(
        name="slides-72-mars-2-deleted",
        type_id="slides",
        title="Bad Slideshow",
        uploaded_filename="slides-72-mars-2-deleted.txt",
        group=mars,
        rev="00",
...
def make_interim_test_data(meeting_tz="UTC"):
    ...
    mars = GroupFactory(
        acronym="mars", parent=area, name="Martian Special Interest Group"
    )
    ames = GroupFactory(
        acronym="ames",
        parent=area,
