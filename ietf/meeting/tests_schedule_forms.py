...
class CsvModelPkInputTests(TestCase):
    ...
    def test_render_value(self):
        ...
        self.assertHTMLEqual(
            result, '<input type="text" name="csv_model" value="1,2,3">'
        )

    def test_value_from_datadict(self):
        result = self.widget.value_from_datadict(
            {"csv_model": "11,23,47"}, {}, "csv_model"
        )
...
class CustomDurationFieldTests(TestCase):
    def test_duration_string(self):
        ...
        self.assertEqual(
            duration_string(timedelta(hours=3, minutes=17, seconds=43)), "03:17"
        )
        self.assertEqual(
            duration_string(timedelta(days=1, hours=3, minutes=17, seconds=43)),
            "1 03:17",
        )
        self.assertEqual(
            duration_string(
                timedelta(hours=3, minutes=17, seconds=43, microseconds=37438)
            ),
            "03:17",
        )
...
class TimeSlotCreateFormTests(TestCase):
    def setUp(self):
        ...
        self.meeting = MeetingFactory(
            type_id="ietf", date=date(2021, 11, 16), days=3, populate_schedule=False
        )
    ...
    def test_other_date(self):
        ...
        self.assertCountEqual(
            form.cleaned_data["days"], [date(2021, 11, 17), date(2021, 11, 15)]
        )
...
class SessionDetailsFormTests(TestCase):
    ...
    def test_initial_purpose(self):
        ...
        self.assertEqual(
            SessionDetailsForm(group=self.group).initial["purpose"], "coding"
        )
        self.group.features.session_purposes = ["admin", "coding", "closed_meeting"]
        self.group.features.save()
        self.assertEqual(
            SessionDetailsForm(group=self.group).initial["purpose"], "admin"
        )
    ...
    def test_session_purposes(self):
        ...
        self.group.features.save()
        self.assertCountEqual(
            SessionDetailsForm(group=self.group)
            .fields["purpose"]
            .queryset.values_list("slug", flat=True),
    ...
    def test_allowed_types(self):
        ...
        SessionPurposeName.objects.filter(slug="regular").update(
            timeslot_types=["other"]
        )
        SessionPurposeName.objects.filter(slug="admin").update(
            timeslot_types=["break", "regular"]
        )
        SessionPurposeName.objects.exclude(slug__in=["regular", "admin"]).update(
            timeslot_types=[]
        )
        ...
        allowed = json.loads(
            SessionDetailsForm(group=self.group)
            .fields["type"]
            .widget.attrs["data-allowed-options"]
        )
        self.assertEqual(allowed["regular"], ["other"])
        self.assertEqual(allowed["admin"], ["break", "regular"])
        for purpose in SessionPurposeName.objects.exclude(
            slug__in=["regular", "admin"]
        ):
    ...
    def test_on_agenda(self):
        ...
        session = SessionFactory(
            meeting=self.meeting, add_to_schedule=False, on_agenda=True
        )
        form = SessionDetailsForm(
            group=self.group,
            instance=session,
...
class SessionEditFormTests(TestCase):
    def test_rejects_group_mismatch(self):
        session = SessionFactory(
            ...
            meeting__populate_schedule=False,
            add_to_schedule=False,
        )
        other_group = GroupFactory()
        with self.assertRaisesMessage(
            ValueError, "Session group does not match group keyword"
        ):
...
class SessionDetailsInlineFormset(TestCase):
    ...
    def test_initial_sessions(self):
        ...
        sessions = SessionFactory.create_batch(
            2, meeting=self.meeting, group=self.group, add_to_schedule=False
        )
    ...
    def test_add_instance(self):
        session = SessionFactory(
            meeting=self.meeting, group=self.group, add_to_schedule=False
        )
        formset_class = sessiondetailsformset_factory()
