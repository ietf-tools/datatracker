...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        secretariat = orm['group.Group'].objects.get(acronym='secretariat')
        system = orm["person.Person"].objects.get(id=1, name='(System)')
        scheduled = orm["name.SessionStatusName"].objects.get(slug='sched')

        for meeting in orm.Meeting.objects.filter(type="ietf").order_by("date"):
            print "Running reg and break forwards data migration for %s" % meeting

            try:
                brk = orm.Session.objects.get(meeting=meeting, group=secretariat, status=scheduled, requested_by=system, name='Break', )
            except orm.Session.DoesNotExist:
                brk = orm.Session(meeting=meeting, group=secretariat, status=scheduled, requested_by=system, name='Break', )
                brk.save()
                
            try:
                reg = orm.Session.objects.get(meeting=meeting, group=secretariat, status=scheduled, requested_by=system, name='Registration', )
            except orm.Session.DoesNotExist:
                reg = orm.Session(meeting=meeting, group=secretariat, status=scheduled, requested_by=system, name='Registration', )
                reg.save()
                
            for schedule in meeting.schedule_set.all():
                print "  Adding Break and Reg sessions to %s" % str(schedule)
                for timeslot in meeting.timeslot_set.all():
                    if timeslot.type_id == 'break':
                        try:
                            assignment = orm.ScheduledSession.objects.get(timeslot=timeslot, session=brk, schedule=schedule)
                        except orm.ScheduledSession.DoesNotExist:
                            assignment = orm.ScheduledSession(timeslot=timeslot, session=brk, schedule=schedule)
                            assignment.save()
                            print "    Added %s break assignment" % str(timeslot)
                    if timeslot.type_id == 'reg':
                        try:
                            assignment = orm.ScheduledSession.objects.get(timeslot=timeslot, session=reg, schedule=schedule)
                        except orm.ScheduledSession.DoesNotExist:
                            assignment = orm.ScheduledSession(timeslot=timeslot, session=reg, schedule=schedule)
                            assignment.save()
                            print "    Added %s registration assignment" % str(timeslot)
    ...
    def backwards(self, orm):
        ...
        for meeting in orm.Meeting.objects.filter(type="ietf").order_by("date"):
            print "Running reg and break backwards data migration for %s" % meeting

            brk = orm.Session.objects.get(meeting=meeting, group=secretariat, status=scheduled, requested_by=system, name='Break', )
            reg = orm.Session.objects.get(meeting=meeting, group=secretariat, status=scheduled, requested_by=system, name='Registration', )

            for schedule in meeting.schedule_set.all():
                print "  Removing Break and Reg sessions from %s" % str(schedule)
                for timeslot in meeting.timeslot_set.all():
                    if timeslot.type_id == 'break':
                        orm.ScheduledSession.objects.filter(timeslot=timeslot, session=brk, schedule=schedule).delete()
                        print "    Removed %s break assignment" % str(timeslot)
                    if timeslot.type_id == 'reg':
                        orm.ScheduledSession.objects.filter(timeslot=timeslot, session=reg, schedule=schedule).delete()
                        print "    Removed %s registration assignment" % str(timeslot)
    ...
    models = {
        ...
        u'meeting.scheduledsession': {
            ...
            'timeslot': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'assignments'", 'to': u"orm['meeting.TimeSlot']"})
        },
        u'meeting.session': {
        ...
        u'meeting.timeslot': {
            'Meta': {'ordering': "['time']", 'object_name': 'TimeSlot'},
