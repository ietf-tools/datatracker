...
def describe_with_tz(session):
       ...
       # and displays the timeslot in the meeting's timezone.
        ...
        if status_id in ('canceled','disappr','notmeet','deleted'):
            ...
            ss0name = "(unscheduled)"
            ss = session.timeslotassignments.filter(schedule__in=[session.meeting.schedule, session.meeting.schedule.base if session.meeting.schedule else None]).order_by('timeslot__time')
            if ss:
                ss0name = ','.join(x.timeslot.time.astimezone(session.meeting.tz()).strftime("%a-%H%M") for x in ss)
