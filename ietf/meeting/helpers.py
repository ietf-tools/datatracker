...
def get_current_ietf_meeting():
    meetings = Meeting.objects.filter(
        type="ietf", date__gte=timezone.now() - datetime.timedelta(days=31)
    ).order_by("date")
    return meetings.first()
...
def preprocess_assignments_for_agenda(
    assignments_queryset, meeting, extra_prefetches=()
):
    ...
    assignments_queryset = assignments_queryset.prefetch_related(
        ...
        "timeslot__meeting",
        "timeslot__location",
        "timeslot__location__floorplan",
        "timeslot__location__urlresource_set",
        Prefetch(
            "session",
            queryset=add_event_info_to_session_qs(
                Session.objects.all().prefetch_related(
                    "group",
                    "group__charter",
                    "group__charter__group",
                    Prefetch(
                        "materials",
    ...
    for a in assignments:
        if a.session:
            ...
            a.session.order_number = (
                a.session.order_in_meeting() if a.session.group else None
            )
    ...
    for a in assignments:
        ...
        if a.session.current_status == "resched":
            a.session.rescheduled_to = timeslot_by_session_pk.get(
                a.session.tombstone_for_id
            )
...
class AgendaKeywordTool:
    ...
    def __init__(self, *, assignments=None, sessions=None):
        ...
        if assignments is not None and sessions is None:
            ...
            raise RuntimeError(
                "Exactly one of assignments or sessions must be specified"
            )
...
class AgendaFilterOrganizer(AgendaKeywordTool):
    ...
    def __init__(self, *, single_category=False, **kwargs):
        ...
        if self._use_legacy_keywords():
            self.extra_labels += (
                "Plenary",
            )  # need this when not using session purpose
    ...
    def _group_filter_headings(self):
        ...
        groups = set(
            self._get_group(s) for s in self.sessions if s and self._get_group(s)
        )
        ...
        group_parents = set(
            self._get_group_parent(g) for g in groups if self._get_group_parent(g)
        )
        ...
        all_groups.difference_update(
            [g for g in all_groups if g.acronym in self.exclude_acronyms]
        )
        headings = {
            g: set()
            for g in all_groups
        ...
        special = set(
            g for g in all_groups if g.features.agenda_filter_type_id == "special"
        )
                ...
                for name, session in sessions_by_name.items():
                    children.append(
                        self._filter_entry(
                            ...
                            toggled_by=(
                                [self._group_keyword(session.group)]
                                if session.group
                                else None
                            ),
                            is_bof=False,
            ...
            column = self._filter_column(
                ...
                children=[
                    ...
                    for label, group in sorted(
                        office_hours_items, key=lambda item: item[0].upper()
                    )
                ],
            )
    ...
    def _extra_filters(self):
        ...
        return self._filter_column(
            ...
            children=[
                self._filter_entry(
                    label=label, keyword=xslugify(label), toggled_by=[], is_bof=False
                )
                for label in candidates
                if label in self.manual_extra_labels
                or any(
...
class AgendaKeywordTagger(AgendaKeywordTool):
    ...
    def _legacy_extra_session_keywords(session):
        ...
        office_hours_match = re.match(
            r"^ *\w+(?: +\w+)* +office hours *$", session.name, re.IGNORECASE
        )
        if office_hours_match is not None:
            ...
            extra.extend(
                [
                    "officehours",
                    session.name.lower().replace(" ", "")[: -len(suffix)]
                    + "-officehours",
                ]
            )
        return extra
...
def read_session_file(type, num, doc):
    ...
    path = os.path.join(
        settings.AGENDA_PATH, "%s/%s/%s" % (num, type, doc.uploaded_filename)
    )
...
def convert_draft_to_pdf(doc_name):
    ...
    for line in infile:
        ...
        if re.search("^[ \t]*\f", line):
            ...
            continue
        if (
            re.search("^ *INTERNET.DRAFT.+[0-9]+ *$", line)
            or re.search("^ *Internet.Draft.+[0-9]+ *$", line)
            or re.search("^draft-[-a-z0-9_.]+.*[0-9][0-9][0-9][0-9]$", line)
            or re.search("^RFC.+[0-9]+$", line)
        ):
            newpage = 1
...
def can_approve_interim_request(meeting, user):
    ...
    if group.type.slug in ["wg", "ag"]:
        if group.parent.role_set.filter(
            name="ad", person=person
        ) or group.role_set.filter(name="ad", person=person):
            return True
    if group.type.slug in ["rg", "rag"] and group.parent.role_set.filter(
...
def can_edit_interim_request(meeting, user):
    ...
    if has_role(
        user, "Secretariat"
    ):  # Consider removing - can_manage_group should handle this
        return True
    session = meeting.session_set.first()
    if not session:
        return False
...
def create_interim_meeting(
    group, date, city="", country="", timezone="UTC", person=None
):
...
def get_announcement_initial(meeting, is_change=False):
    ...
    initial["cc"] = addrs.cc
    initial["frm"] = (
        settings.INTERIM_ANNOUNCE_FROM_EMAIL_PROGRAM
        if group.type_id == "program"
        else settings.INTERIM_ANNOUNCE_FROM_EMAIL_DEFAULT
    )
    if in_person:
        desc = "Interim"
    else:
    ...
    initial["subject"] = (
        "{name} ({acronym}) {type} {desc} Meeting: {date}{change}".format(
            name=group.name,
    ...
    body = render_to_string(
        "meeting/interim_announcement.txt", locals() | {"settings": settings}
    )
    initial["body"] = body
    return initial
...
def get_earliest_session_date(formset):
    ...
    return sorted(
        [f.cleaned_data["date"] for f in formset.forms if f.cleaned_data.get("date")]
    )[0]


def is_interim_meeting_approved(meeting):
    return (
        add_event_info_to_session_qs(meeting.session_set.all()).first().current_status
        == "apprw"
    )
...
def send_interim_approval_request(meetings):
    ...
    for (
        ...
        group.features.groupman_authroles
    ):  # NOTE: This makes an assumption that the authroles are exactly the set of approvers
...
def send_interim_approval(user, meeting):
    ...
    (to_email, cc_list) = gather_address_lists(
        "interim_approved", group=first_session.group, person=user.person
    )
    from_email = settings.SESSION_REQUEST_FROM_EMAIL
...
def send_interim_announcement_request(meeting):
    ...
    announce_url = settings.IDTRACKER_BASE_URL + reverse(
        "ietf.meeting.views.interim_request_details", kwargs={"number": meeting.number}
    )
    context = locals()
...
def send_interim_meeting_cancellation_notice(meeting):
    ...
    subject = (
        "{group} ({acronym}) {type} Interim Meeting Cancelled (was {date})".format(
            group=group.name,
            acronym=group.acronym,
            type=group.type.slug.upper(),
    ...
    is_multi_day = (
        session.meeting.session_set.with_current_status()
...
def sessions_post_save(request, forms):
    ...
    for form in forms:
        ...
        if (
            ...
            and not SchedulingEvent.objects.filter(session=form.instance).exists()
        ):
...
def delete_interim_session_conferences(sessions):
    ...
    ):  # do nothing if Meetecho API not configured
        meetecho_manager = meetecho.ConferenceManager(settings.MEETECHO_API_CONFIG)
        for session in sessions:
...
def sessions_post_cancel(request, sessions):
    ...
    try:
        ...
        log.log(
            f"Exception deleting Meetecho conferences for sessions [{sess_pks}]: {err}"
        )
        messages.warning(
            request,
...
def update_interim_session_assignment(form):
    ...
    if session.official_timeslotassignment():
        ...
        SchedTimeSessAssignment.objects.create(
            timeslot=slot, session=session, schedule=meeting.schedule
        )
