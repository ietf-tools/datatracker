...
class AgendaRedirectView(RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        kwargs = {
            ...
            if v is not None and k not in self.ignore_kwargs
        }
...
safe_for_all_meeting_types = [
    ...
    url(
        r"^session/(?P<session_id>\d+)/narrativeminutes$",
        views.upload_session_narrativeminutes,
    ),
    ...
    url(
        r"^session/(?P<session_id>\d+)/slides(?:/%(name)s)?$" % settings.URL_REGEXPS,
        views.upload_session_slides,
    ),
    url(
        r"^session/(?P<session_id>\d+)/add_to_session$",
        views.ajax_add_slides_to_session,
    ),
    url(
        r"^session/(?P<session_id>\d+)/remove_from_session$",
        views.ajax_remove_slides_from_session,
    ),
    url(
        r"^session/(?P<session_id>\d+)/reorder_in_session$",
        views.ajax_reorder_slides_in_session,
    ),
    url(
        r"^session/(?P<session_id>\d+)/doc/%(name)s/remove$" % settings.URL_REGEXPS,
        views.remove_sessionpresentation,
    ),
    url(r"^session/(?P<session_id>\d+)\.ics$", views.agenda_ical),
    url(r"^sessions/(?P<acronym>[-a-z0-9]+)\.ics$", views.agenda_ical),
    url(
        r"^slidesubmission/(?P<slidesubmission_id>\d+)$", views.approve_proposed_slides
    ),
]
...
type_ietf_only_patterns = [
    url(
        r"^agenda/%(owner)s/%(schedule_name)s/edit/?$" % settings.URL_REGEXPS,
        views.edit_meeting_schedule,
    ),
    url(
        r"^agenda/%(owner)s/%(schedule_name)s/timeslots/$" % settings.URL_REGEXPS,
        views.edit_meeting_timeslots_and_misc_sessions,
    ),
    url(
        r"^agenda/%(owner)s/%(schedule_name)s/details$" % settings.URL_REGEXPS,
        views.edit_schedule_properties,
    ),
    url(
        r"^agenda/%(owner)s/%(schedule_name)s/delete$" % settings.URL_REGEXPS,
        views.delete_schedule,
    ),
    url(
        r"^agenda/%(owner)s/%(schedule_name)s/make_official$" % settings.URL_REGEXPS,
        views.make_schedule_official,
    ),
    url(
        r"^agenda/%(owner)s/%(schedule_name)s(\.(?P<ext>.html))?/?$"
        % settings.URL_REGEXPS,
        views.agenda,
    ),
    url(
        r"^agenda/%(owner)s/%(schedule_name)s/week-view(?:.html)?/?$"
        % settings.URL_REGEXPS,
        AgendaRedirectView.as_view(pattern_name="agenda", permanent=True),
    ),
    url(
        r"^agenda/%(owner)s/%(schedule_name)s/by-room/?$" % settings.URL_REGEXPS,
        AgendaRedirectView.as_view(pattern_name="agenda", permanent=True),
    ),
    url(
        r"^agenda/%(owner)s/%(schedule_name)s/by-type/?$" % settings.URL_REGEXPS,
        AgendaRedirectView.as_view(pattern_name="agenda", permanent=True),
    ),
    url(
        r"^agenda/%(owner)s/%(schedule_name)s/by-type/(?P<type>[a-z]+)$"
        % settings.URL_REGEXPS,
        AgendaRedirectView.as_view(pattern_name="agenda", permanent=True),
    ),
    url(
        r"^agenda/%(owner)s/%(schedule_name)s/new/$" % settings.URL_REGEXPS,
        views.new_meeting_schedule,
    ),
    ...
    url(
        ...
        RedirectView.as_view(
            pattern_name="ietf.meeting.views.list_schedules", permanent=True
        ),
    ),
...
type_interim_patterns = [
    ...
    url(
        r"^materials/%(document)s(?P<ext>\.[a-z0-9]+)$" % settings.URL_REGEXPS,
        views.materials_document,
    ),
...
type_ietf_only_patterns_id_optional = [
    ...
    url(
        r"^agenda/edit$",
        RedirectView.as_view(
            pattern_name="ietf.meeting.views.edit_meeting_schedule", permanent=True
        ),
    ...
    url(
        r"^proceedings/materials/(?P<material_type>[a-z_]+)/$",
        views_proceedings.edit_material,
    ),
    url(
        r"^proceedings/materials/(?P<material_type>[a-z_]+)/new/$",
        views_proceedings.upload_material,
    ),
    url(
        r"^proceedings/materials/(?P<material_type>[a-z_]+)/remove/$",
        views_proceedings.remove_restore_material,
        {"action": "remove"},
        "ietf.meeting.views_proceedings.remove_material",
    ),
    url(
        r"^proceedings/materials/(?P<material_type>[a-z_]+)/restore/$",
        views_proceedings.remove_restore_material,
        {"action": "restore"},
    ...
    url(
        r"^proceedings/meetinghosts/(?P<host_id>\d+)/logo/$",
        views_proceedings.meetinghost_logo,
    ),
]
...
urlpatterns = [
    ...
    url(
        r"^interim/announce/(?P<number>[A-Za-z0-9._+-]+)/?$",
        views.interim_send_announcement,
    ),
    url(
        r"^interim/skip_announce/(?P<number>[A-Za-z0-9._+-]+)/?$",
        views.interim_skip_announcement,
    ),
    url(r"^interim/request/?$", views.interim_request),
    url(
        r"^interim/request/(?P<number>[A-Za-z0-9._+-]+)/?$",
        views.interim_request_details,
    ),
    url(
        r"^interim/request/(?P<number>[A-Za-z0-9._+-]+)/edit/?$",
        views.interim_request_edit,
    ),
    url(
        r"^interim/request/(?P<number>[A-Za-z0-9._+-]+)/cancel/?$",
        views.interim_request_cancel,
    ),
    url(
        r"^interim/session/(?P<sessionid>[A-Za-z0-9._+-]+)/cancel/?$",
        views.interim_request_session_cancel,
    ),
