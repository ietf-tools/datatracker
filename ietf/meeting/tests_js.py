...
class EditMeetingScheduleTests(IetfSeleniumTestCase):
    def test_edit_meeting_schedule(self):
        ...
        schedule = Schedule.objects.filter(
            meeting=meeting, owner__user__username="plain"
        ).first()

        room1 = Room.objects.get(name="Test Room")
        slot1 = (
            TimeSlot.objects.filter(meeting=meeting, location=room1, type="regular")
            .order_by("time")
            .first()
        )
        slot1b = (
        ...
        slot3 = TimeSlot.objects.create(
            ...
            location=room2,
            duration=datetime.timedelta(hours=2),
            time=max(slot1.end_time(), slot2.end_time())
        ...
        url = self.absreverse(
            ...
            kwargs=dict(
                num=meeting.number, name=schedule.name, owner=schedule.owner_email()
            ),
        )
        self.driver.get(url)
        ...
        WebDriverWait(self.driver, 2).until(
            expected_conditions.presence_of_element_located(
                (By.CSS_SELECTOR, ".edit-meeting-schedule")
            )
        )

        self.assertEqual(
            len(self.driver.find_elements(By.CSS_SELECTOR, ".session.purpose-regular")),
            3,
        )

        # select - show session info
        s2_element = self.driver.find_element(
            By.CSS_SELECTOR, "#session{}".format(s2.pk)
        )
        s2b_element = self.driver.find_element(
            By.CSS_SELECTOR, "#session{}".format(s2b.pk)
        )
        ...
        session_info_container = self.driver.find_element(
            By.CSS_SELECTOR, ".session-info-container"
        )
        self.assertIn(
            s2.group.acronym,
            session_info_container.find_element(By.CSS_SELECTOR, ".title").text,
        )
        self.assertEqual(
            session_info_container.find_element(
                By.CSS_SELECTOR, ".other-session .time"
            ).text,
            "not yet scheduled",
        )
        ...
        self.driver.find_element(
            By.CSS_SELECTOR, '.timeslot[data-type="regular"] .drop-target'
        ).click()

        self.assertEqual(
            session_info_container.find_elements(By.CSS_SELECTOR, ".title"), []
        )
        self.assertNotIn("other-session-selected", s2b_element.get_attribute("class"))
        ...
        WebDriverWait(self.driver, 2).until(
            expected_conditions.presence_of_element_located(
                (By.CSS_SELECTOR, ".unassigned-sessions #session{}".format(s2.pk))
            )
        )

        self.assertEqual(
            list(SchedTimeSessAssignment.objects.filter(session=s2, schedule=schedule)),
            [],
        )
        ...
        sorted_pks = [
            ...
            for s in sorted(
                [s1, s2, s2b],
                key=lambda s: (s.group.acronym, s.requested_duration, s.pk),
            )
        ]
        self.driver.find_element(
            By.CSS_SELECTOR, "[name=sort_unassigned] option[value=name]"
        ).click()
        self.assertTrue(
            self.driver.find_element(
                By.CSS_SELECTOR,
                ".unassigned-sessions .drop-target #session{} + #session{} + #session{}".format(
                    *sorted_pks
                ),
            )
        )
        ...
        self.driver.find_element(
            By.CSS_SELECTOR, "[name=sort_unassigned] option[value=parent]"
        ).click()
        self.assertTrue(
            self.driver.find_element(
                By.CSS_SELECTOR,
                ".unassigned-sessions .drop-target #session{} + #session{}".format(
                    *sorted_pks
                ),
            )
        )
        ...
        self.driver.find_element(
            By.CSS_SELECTOR, "[name=sort_unassigned] option[value=duration]"
        ).click()
        self.assertTrue(
            self.driver.find_element(
                By.CSS_SELECTOR,
        ...
        self.driver.find_element(
            By.CSS_SELECTOR, "[name=sort_unassigned] option[value=comments]"
        ).click()
        self.assertTrue(
            self.driver.find_element(
                By.CSS_SELECTOR,
        ...
        # schedule
        self.driver.execute_script(
            "jQuery('#session{}').simulateDragDrop({{dropTarget: '#timeslot{} .drop-target'}});".format(
                s2.pk, slot1.pk
            )
        )
        ...
        WebDriverWait(self.driver, 2).until(
            expected_conditions.presence_of_element_located(
                (By.CSS_SELECTOR, "#timeslot{} #session{}".format(slot1.pk, s2.pk))
            )
        )
        ...
        s1_element = self.driver.find_element(
            By.CSS_SELECTOR, "#session{}".format(s1.pk)
        )
        s1_element.click()
        ...
        WebDriverWait(self.driver, 2).until(
            expected_conditions.presence_of_element_located(
                (By.CSS_SELECTOR, "#timeslot{} #session{}".format(slot2.pk, s2.pk))
            )
        )
        ...
        self.assertTrue(
            self.driver.find_elements(
                By.CSS_SELECTOR, "#session{}.too-many-attendees".format(s2.pk)
            )
        )
        ...
        self.assertTrue(
            self.driver.find_elements(
                By.CSS_SELECTOR, "#timeslot{}.overfull".format(slot2.pk)
            )
        )
        ...
        constraint_element = s2_element.find_element(
            By.CSS_SELECTOR,
            '.constraints span[data-sessions="{}"].would-violate-hint'.format(s1.pk),
        )
        ...
        WebDriverWait(self.driver, 2).until(
            expected_conditions.presence_of_element_located(
                (By.CSS_SELECTOR, "#timeslot{} #session{}".format(slot1.pk, s1.pk))
            )
        )

        constraint_element = s2_element.find_element(
            By.CSS_SELECTOR,
            '.constraints span[data-sessions="{}"].violated-hint'.format(s1.pk),
        )
        self.assertTrue(constraint_element.is_displayed())
        ...
        self.driver.find_element(
            By.CSS_SELECTOR,
            '.session-parent-toggles [value="{}"]'.format(s1.group.parent.acronym),
        ).click()
        self.assertTrue(s1_element.is_displayed())  # should still be displayed
        self.assertIn(
            "hidden-parent",
            s1_element.get_attribute("class"),
        ...
        # hide timeslots
        modal_open = self.driver.find_element(
            By.CSS_SELECTOR, "#timeslot-toggle-modal-open"
        )
        self.driver.execute_script(
            "arguments[0].click();", modal_open
        )  # FIXME: not working:
        # modal_open.click()

        self.assertTrue(
            self.driver.find_element(
                By.CSS_SELECTOR, "#timeslot-group-toggles-modal"
            ).is_displayed()
        )
        self.driver.find_element(
            By.CSS_SELECTOR,
        ...
        self.driver.find_element(
            By.CSS_SELECTOR, '#timeslot-group-toggles-modal [data-bs-dismiss="modal"]'
        ).click()
        self.assertTrue(
        ...
        self.assertTrue(
            self.driver.find_element(By.CSS_SELECTOR, "#swap-days-modal").is_displayed()
        )
        self.driver.find_element(
            By.CSS_SELECTOR,
        ...
        self.driver.find_element(
            By.CSS_SELECTOR, '#swap-days-modal button[type="submit"]'
        ).click()

        self.assertTrue(
            self.driver.find_elements(
                By.CSS_SELECTOR, "#timeslot{} #session{}".format(slot4.pk, s1.pk)
            ),
        ...
        self.driver.find_element(
            By.CSS_SELECTOR,
            '#swap-timeslot-col-modal input[name="target_timeslot"][value="{}"]'.format(
                slot4.pk
            ),
        ).click()  # select room1 timeslot that has a session in it
        self.driver.find_element(
            By.CSS_SELECTOR, '#swap-timeslot-col-modal button[type="submit"]'
        ).click()

        self.assertTrue(
            self.driver.find_elements(
                By.CSS_SELECTOR, "#timeslot{} #session{}".format(slot1b.pk, s1.pk)
            ),
    ...
    def test_past_flags(self):
        ...
        right_now = timezone.now().astimezone(pytz.timezone(meeting.time_zone))
        ...
        past_timeslots = [
            TimeSlotFactory(
                meeting=meeting,
                time=right_now - datetime.timedelta(hours=n),
        ...
        now_timeslots = [
            ...
            TimeSlotFactory(
                meeting=meeting,
                time=right_now - datetime.timedelta(seconds=1),
                duration=datetime.timedelta(hours=1),
                location=room,
            ),
            ...
            TimeSlotFactory(
                meeting=meeting,
                time=right_now + datetime.timedelta(minutes=9),
                duration=datetime.timedelta(hours=1),
                location=room,
            ),
        ]
        ...
        for flag in past_flags:
            self.assertTrue(
                flag.is_displayed(), "Past timeslot or session not flagged as past"
            )

        for flag in now_flags:
            self.assertTrue(
                flag.is_displayed(), '"Now" timeslot or session not flagged as past'
            )

        for flag in future_flags:
            self.assertFalse(
                flag.is_displayed(), "Future timeslot or session is flagged as past"
            )
    ...
    def test_past_swap_days_buttons(self):
        ...
        past_timeslots = [
            TimeSlotFactory(
                meeting=meeting,
                time=right_now - datetime.timedelta(days=n),
                duration=datetime.timedelta(hours=1),
                location=room,
            )
        ...
        past_swap_days_buttons = self.driver.find_elements(
            By.CSS_SELECTOR,
            ",".join(
                '.swap-days[data-start="{}"]'.format(
                    ts.time.astimezone(ts.tz()).date().isoformat()
                )
                for ts in past_timeslots
            ),
        )
        self.assertEqual(
            len(past_swap_days_buttons),
            len(past_timeslots),
            "Missing past swap days buttons",
        )

        future_swap_days_buttons = self.driver.find_elements(
            By.CSS_SELECTOR,
            ",".join(
                '.swap-days[data-start="{}"]'.format(
                    ts.time.astimezone(ts.tz()).date().isoformat()
        ...
        self.assertEqual(
            len(future_swap_days_buttons),
            len(future_timeslots),
            "Missing future swap days buttons",
        )

        now_swap_days_buttons = self.driver.find_elements(
            By.CSS_SELECTOR,
            ",".join(
                '.swap-days[data-start="{}"]'.format(
                    ts.time.astimezone(ts.tz()).date().isoformat()
        ...
        wait.until(
            expected_conditions.presence_of_element_located(
                (
                    By.CSS_SELECTOR,
                    ".timeslot.past",
                )  # wait until timeslots are updated by JS
            )
        )
        ...
        self.assertFalse(
            any(
                radio.is_enabled()
                for radio in modal.find_elements(
                    By.CSS_SELECTOR,
                    ",".join(
                        'input[name="target_day"][value="{}"]'.format(
                            ts.time.astimezone(ts.tz()).date().isoformat()
                        )
                        for ts in past_timeslots
                    ),
        ...
        enabled_timeslots = (
            ts for ts in future_timeslots if ts != future_timeslots[clicked_index]
        )
        self.assertTrue(
            all(
                radio.is_enabled()
    ...
    def test_past_swap_timeslot_col_buttons(self):
        ...
        wait = WebDriverWait(self.driver, 2)
        meeting = MeetingFactory(
            type_id="ietf", date=timezone.now() - datetime.timedelta(days=3), days=7
        )
        room = RoomFactory(meeting=meeting)
        ...
        past_swap_ts_buttons = self.driver.find_elements(
            By.CSS_SELECTOR,
            ",".join(
                '*[data-start="{}"] .swap-timeslot-col'.format(
                    ts.utc_start_time().isoformat()
                )
                for ts in past_timeslots
            ),
        )
        self.assertEqual(
            ...
            len(past_timeslots),
            "Missing past swap timeslot col buttons",
        )

        future_swap_ts_buttons = self.driver.find_elements(
            By.CSS_SELECTOR,
            ",".join(
                '*[data-start="{}"] .swap-timeslot-col'.format(
                    ts.utc_start_time().isoformat()
        ...
        self.assertEqual(
            len(future_swap_ts_buttons),
            len(future_timeslots),
            "Missing future swap timeslot col buttons",
        )

        now_swap_ts_buttons = self.driver.find_elements(
            By.CSS_SELECTOR,
            ",".join(
                '[data-start="{}"] .swap-timeslot-col'.format(
                    ts.utc_start_time().isoformat()
        ...
        self.assertEqual(
            ...
            len(now_timeslots),
            'Missing "now" swap timeslot col buttons',
        )
        ...
        clicked_index = 1
        self.driver.execute_script(
            "arguments[0].click();", future_swap_ts_buttons[clicked_index]
        )  # FIXME: not working:
        ...
        self.assertFalse(
            any(
                ...
                for radio in modal.find_elements(
                    By.CSS_SELECTOR,
                    ",".join(
                        'input[name="target_timeslot"][value="{}"]'.format(ts.pk)
                        for ts in past_timeslots
                    ),
                )
            ),
    ...
    def test_unassigned_sessions_sort(self):
        ...
        def wait_for_order(sessions, expected_order, fail_message):
            ...
            self.assertEqual(
                sort_by_position(self.driver, sessions), expected_order, fail_message
            )
        ...
        for _ in range(2):
            ...
            for area in areas:
                wgs.append(
                    GroupFactory(acronym=group_acro(num), type_id="wg", parent=area)
                )
                num += 1
    ...
    def test_unassigned_sessions_drop_target_visible_when_empty(self):
        ...
        schedule = ScheduleFactory(
            meeting__type_id="ietf", owner__user__username="plain"
        )
        ...
        drop_target = self.driver.find_element(
            By.CSS_SELECTOR, ".unassigned-sessions .drop-target"
        )
        self.assertEqual(
            len(drop_target.find_elements(By.CLASS_NAME, "session")),
            0,
    ...
    def test_session_constraint_hints(self):
        ...
        """
        meeting = MeetingFactory(
            type_id="ietf", date=date_today(), populate_schedule=False
        )
        ...
        constraint_names = meeting.enabled_constraint_names().filter(
            is_group_conflict=True
        )
        self.assertGreaterEqual(
            len(constraint_names),
            2,
            "Not enough constraint names enabled to perform test",
        )
        ...
        url = self.absreverse(
            "ietf.meeting.views.edit_meeting_schedule",
            kwargs=dict(
                num=meeting.number, owner=schedule.owner.email(), name=schedule.name
            ),
        )
        self.login(schedule.owner.user.username)
        self.driver.get(url)
        session_elements = [
            self.driver.find_element(By.CSS_SELECTOR, f"#session{sess.pk}")
        ...
        self.assertIn(
            "would-violate-hint",
            session_elements[1].get_attribute("class"),
            "Constraint violation should be indicated on conflicting session",
        )
        self.assertIn(
            "would-violate-hint",
            session_elements[2].get_attribute("class"),
            "Constraint violation should be indicated on conflicting session",
        )
        self.assertIn(
            "would-violate-hint",
            session_elements[3].get_attribute("class"),
            "Constraint violation should be indicated on conflicting session",
        )
        ...
        self.assertNotIn(
            "would-violate-hint",
            session_elements[4].get_attribute("class"),
...
class SlideReorderTests(IetfSeleniumTestCase):
    def setUp(self):
        ...
        self.session.presentations.create(
            document=DocumentFactory(type_id="slides", name="one"), order=1
        )
        self.session.presentations.create(
            document=DocumentFactory(type_id="slides", name="two"), order=2
        )
        self.session.presentations.create(
            document=DocumentFactory(type_id="slides", name="three"), order=3
        )
...
class InterimTests(IetfSeleniumTestCase):
    def setUp(self):
        ...
        sg_interim = make_interim_meeting(
            somegroup, date_today() + datetime.timedelta(days=20)
        )
    ...
    def all_ietf_meetings(self):
        meetings = Meeting.objects.filter(
            type_id="ietf", date__gte=timezone.now() - datetime.timedelta(days=7)
        )
        for m in meetings:
    ...
    def assert_upcoming_meeting_visibility(self, visible_meetings=None):
        ...
        for entry in entries:
            entry_text = entry.get_attribute(
                "innerHTML"
            ).strip()  # gets text, even if element is hidden
        ...
        self.assertEqual(
            expected, set(), "Missing entries for expected iterim meetings."
        )
        self.assertEqual(
            not_visible, set(), "Hidden rows for expected interim meetings."
        )
    ...
    def assert_upcoming_meeting_calendar(self, visible_meetings=None):
        ...
        def advance_month():
            ...
            )  # FIXME-LARS: no idea why this fails:
        ...
        for _ in range(13):
            entries = self.driver.find_elements(
                By.CSS_SELECTOR, "div#calendar div.fc-event-main"
            )
            for entry in entries:
                meetings = [
                    m for m in visible_meetings if m.calendar_label in entry.text
                ]
                if len(meetings) > 0:
                    seen.add(meetings[0])
                    if not entry.is_displayed():
        ...
        self.assertCountEqual(
            seen, visible_meetings, "Expected calendar entries not shown."
        )
        self.assertCountEqual(
            not_visible, set(), "Hidden calendar entries for expected interim meetings."
        )
    ...
    def do_upcoming_view_filter_test(self, querystring, visible_meetings=()):
        ...
        if simplified_querystring in ["?show=", "?hide=", "?show=&hide="]:
            simplified_querystring = (
                ""  # these empty querystrings will be dropped (not an exhaustive list)
            )
    ...
    def assert_upcoming_view_filter_matches_ics_filter(self, filter_string):
        ...
        for meeting_elt in visible_meetings:
            ...
            if match.group("ietf"):
                ...
                expected_interim_sessions.extend(
                    [s.pk for s in meeting.session_set.all()]
                )
                if meeting.schedule:
    ...
    def test_upcoming_view_default(self):
        ...
        self.do_upcoming_view_filter_test(
            "", ietf_meetings.union(self.displayed_interims())
        )

    def test_upcoming_view_show_ietf_meetings(self):
        self.do_upcoming_view_filter_test(
            "?show=ietf-meetings", self.all_ietf_meetings()
        )
    ...
    def test_upcoming_view_filter_show_group(self):
        ...
        # Show one
        self.do_upcoming_view_filter_test(
            "?show=mars", self.displayed_interims(groups=["mars"])
        )

        # Show two
        self.do_upcoming_view_filter_test(
            "?show=mars,ames", self.displayed_interims(groups=["mars", "ames"])
        )

        # Show two plus ietf-meetings
        self.do_upcoming_view_filter_test(
            "?show=ietf-meetings,mars,ames",
            set(self.all_ietf_meetings()).union(
    ...
    def test_upcoming_view_filter_show_area(self):
        ...
        area = mars.parent
        self.do_upcoming_view_filter_test(
            "?show=%s" % area.acronym, self.displayed_interims(groups=["mars", "ames"])
        )

    def test_upcoming_view_filter_show_type(self):
        self.do_upcoming_view_filter_test(
            "?show=plenary", self.displayed_interims(groups=["sg"])
        )
    ...
    def test_upcoming_view_filter_hide_group(self):
        ...
        # With group shown
        self.do_upcoming_view_filter_test(
            "?show=ames,mars&hide=mars", self.displayed_interims(groups=["ames"])
        )
        # With area shown
        self.do_upcoming_view_filter_test(
            "?show=%s&hide=mars" % area.acronym,
    ...
    def test_upcoming_view_filter_hide_area(self):
        ...
        # With area shown
        self.do_upcoming_view_filter_test(
            "?show=%s&hide=%s" % (area.acronym, area.acronym)
        )
        ...
        self.do_upcoming_view_filter_test(
            "?show=ietf-meetings,hide=%s" % area.acronym, self.all_ietf_meetings()
        )
    ...
    def test_upcoming_view_filter_hide_type(self):
        ...
        # With type shown
        self.do_upcoming_view_filter_test(
            "?show=plenary,regular&hide=regular", self.displayed_interims(groups=["sg"])
        )
    ...
    def test_upcoming_view_filter_whitespace(self):
        ...
        self.do_upcoming_view_filter_test(
            "?show=mars , ames &hide=   ames", self.displayed_interims(groups=["mars"])
        )
    ...
    def test_upcoming_view_time_zone_selection(self):
        def _assert_interim_tz_correct(sessions, tz):
            ...
            for session in sessions:
                ...
                meeting_link = self.driver.find_element(
                    By.LINK_TEXT, session.meeting.number
                )
                time_td = meeting_link.find_element(
                    By.XPATH, '../../td[contains(@class, "session-time")]'
                )
        ...
        def _assert_ietf_tz_correct(meetings, tz):
            ...
            for meeting in meetings:
                ...
                start_dt = meeting_zone.localize(
                    datetime.datetime.combine(meeting.date, datetime.time.min)
                )
                end_dt = meeting_zone.localize(
                ...
                meeting_link = self.driver.find_element(
                    By.LINK_TEXT, "IETF " + meeting.number
                )
                time_td = meeting_link.find_element(
                    By.XPATH, '../../td[contains(@class, "meeting-time")]'
                )
        ...
        tz_select_bottom_input = self.driver.find_element(
            By.ID, "timezone-select-bottom"
        )

        # For things we click, need to click the labels / actually visible items. The actual inputs are hidden
        # and managed by the JS.
        local_tz_link = self.driver.find_element(
            By.CSS_SELECTOR, 'label[for="local-timezone"]'
        )
        utc_tz_link = self.driver.find_element(
            By.CSS_SELECTOR, 'label[for="utc-timezone"]'
        )
        local_tz_bottom_link = self.driver.find_element(
            By.CSS_SELECTOR, 'label[for="local-timezone-bottom"]'
        )
        utc_tz_bottom_link = self.driver.find_element(
            By.CSS_SELECTOR, 'label[for="utc-timezone-bottom"]'
        )
        ...
        arbitrary_tz_opt = self.wait.until(
            expected_conditions.presence_of_element_located(
                (
                    By.CSS_SELECTOR,
                    '#timezone-select > option[value="%s"]' % arbitrary_tz,
                )
            )
        )
        tz_selector_clickables = self.driver.find_elements(
            By.CSS_SELECTOR, ".tz-display .select2"
        )
        self.assertEqual(len(tz_selector_clickables), 2)
        (tz_selector_top, tz_selector_bottom) = tz_selector_clickables

        arbitrary_tz_bottom_opt = tz_select_bottom_input.find_element(
            By.CSS_SELECTOR,
            '#timezone-select-bottom > option[value="%s"]' % arbitrary_tz,
        )

        utc_tz_opt = tz_select_input.find_element(
            By.CSS_SELECTOR, 'option[value="UTC"]'
        )
        utc_tz_bottom_opt = tz_select_bottom_input.find_element(
            By.CSS_SELECTOR, 'option[value="UTC"]'
        )
        ...
        local_tz_opt = tz_select_input.find_element(
            By.CSS_SELECTOR, "option[value=%s]" % local_tz
        )
        local_tz_bottom_opt = tz_select_bottom_input.find_element(
            By.CSS_SELECTOR, 'option[value="%s"]' % local_tz
        )
        ...
        self.wait.until(
            expected_conditions.presence_of_element_located(
                (
                    By.CSS_SELECTOR,
                    'span.select2-container .select2-results li[id$="America/Halifax"]',
                )
            )
        ).click()
    ...
    def test_upcoming_materials_modal(self):
        ...
        open_modal_button_locator = (
            By.CSS_SELECTOR,
            '[data-bs-target="#modal-%s"]' % slug,
        )
...
class ProceedingsMaterialTests(IetfSeleniumTestCase):
    ...
    def test_add_proceedings_material(self):
        ...
        self.assertTrue(
            choose_file_button.is_displayed(), "File chooser should be shown by default"
        )
        self.assertFalse(
            external_url_field.is_displayed(), "URL field should be hidden by default"
        )

        # enable URL
        use_url_checkbox.click()
        self.wait.until(
            expected_conditions.invisibility_of_element(choose_file_button),
            "File chooser should be hidden when URL option is checked",
        )
        self.wait.until(
            expected_conditions.visibility_of(external_url_field),
        ...
        use_url_checkbox.click()
        self.wait.until(
            expected_conditions.visibility_of(choose_file_button),
            "File chooser should appear when URL option is unchecked",
        )
        self.wait.until(
            expected_conditions.invisibility_of_element(external_url_field),
    ...
    def test_replace_proceedings_material_shows_correct_default(self):
        ...
        url_mat = ProceedingsMaterialFactory(
            meeting=self.meeting, document__external_url="https://example.com"
        )
        ...
        self.assertFalse(
            use_url_checkbox.is_selected(),
            "URL option should be unchecked for a document material",
        )
        self.assertTrue(
            choose_file_button.is_displayed(), "File chooser should be shown by default"
        )
        ...
        self.assertTrue(
            use_url_checkbox.is_selected(),
            "URL option should be checked for URL material",
        )
        self.assertFalse(
            choose_file_button.is_displayed(),
            "File chooser should be hidden by default",
        )
        self.assertTrue(
            external_url_field.is_displayed(), "URL field should be shown by default"
        )
...
class EditTimeslotsTests(IetfSeleniumTestCase):
    ...
    def do_delete_time_interval_test(self, cancel=False):
        delete_time_local = datetime_from_date(
            self.meeting.date, self.meeting.tz()
        ).replace(hour=10)
        ...
        keep: [TimeSlot] = [  # type: ignore[annotation-unchecked]
            TimeSlotFactory(meeting=self.meeting, time=keep_time, duration=duration)
            for keep_time in (
                # same day, but 2 hours later
                delete_time + datetime.timedelta(hours=2),
                # next day, but same wall clock time
                datetime_from_date(
                    self.meeting.get_meeting_date(1), self.meeting.tz()
                ).replace(hour=10),
            )
        ]
    ...
    def do_delete_day_test(self, cancel=False):
        ...
        delete: [TimeSlot] = [  # type: ignore[annotation-unchecked]
            TimeSlotFactory(
                meeting=self.meeting,
                time=datetime_from_date(delete_day, self.meeting.tz()).replace(
                    hour=hour
                ),
            )
            for hour in hours
