...
class Person(models.Model):
    ...
    )  # When this Person record entered the system
    ...
    name = models.CharField(
        ...
        validators=[name_character_validator],
    )
    plain = models.CharField(
        "Plain Name correction (Unicode)",
        max_length=64,
        default="",
        blank=True,
    ...
    ascii_short = models.CharField(
        "Abbreviated Name (ASCII)",
        max_length=32,
        null=True,
        blank=True,
    ...
    pronouns_selectable = jsonfield.JSONCharField(
        "Pronouns", max_length=120, blank=True, null=True, default=list
    )
    pronouns_freetext = models.CharField(
        " ",
        max_length=30,
        null=True,
        blank=True,
    ...
    name_from_draft = models.CharField(
        "Full Name (from submission)",
        null=True,
        max_length=255,
        editable=False,
    ...
    def get_absolute_url(self):
        return urlreverse(
            "ietf.person.views.profile", kwargs={"email_or_name": self.name}
        )
    ...
    def role_email(self, role_name, group=None):
        ...
        if group:
            ...
            e = Email.objects.filter(
                person=self, role__group=group, role__name=role_name
            )
        else:
            e = Email.objects.filter(
                person=self, role__group__state="active", role__name=role_name
            )
        if e:
            return e[0]
    ...
    def email_allowing_inactive(self):
        if not hasattr(self, "_cached_email_allowing_inactive"):
            ...
            log.assertion(
                statement="e is not None", note=f"Person {self.pk} has no Email objects"
            )
    ...
    def has_drafts(self):
        ...
        return Document.objects.filter(
            documentauthor__person=self, type="draft"
        ).exists()
        ...
        return list(
            set(
                [
                    ...
                    if r == None or has_role(self.user, r)
                ]
            )
        )
    ...
    def cdn_photo_url(self, size=80):
        if self.photo:
            if settings.SERVE_CDN_PHOTOS:
                ...
                datatracker_photo_path = urlreverse(
                    "ietf.person.views.photo", kwargs={"email_or_name": self.email()}
                )
                datatracker_photo_url = (
                    settings.IDTRACKER_BASE_URL + datatracker_photo_path
                )
...
class Alias(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if created:
            if (
                Alias.objects.filter(name=self.name).exclude(person=self.person).count()
                > 0
            ):
                msg = render_to_string(
                    "person/mail/possible_duplicates.txt",
...
class Email(models.Model):
    ...
    )  # User.username or Document.name
    active = models.BooleanField(
        default=True
    )  # Old email addresses are *not* purged, as history
...
PERSON_API_KEY_ENDPOINTS = sorted(
    list(set([(v, n) for (v, n, r) in PERSON_API_KEY_VALUES]))
)
...
class PersonalApiKey(models.Model):
    ...
    endpoint = models.CharField(
        max_length=128, null=False, blank=False, choices=PERSON_API_KEY_ENDPOINTS
    )
