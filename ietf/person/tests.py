...
class PersonTests(TestCase):
    def test_ajax_search_emails(self):
        ...
        r = self.client.get(
            urlreverse(
                "ietf.person.views.ajax_select2_search", kwargs={"model_name": "email"}
            ),
            dict(q=person.name),
        )
    ...
    def test_ajax_person_email_json(self):
        ...
        bad_url = urlreverse(
            "ietf.person.ajax.person_email_json", kwargs=dict(personid=12345)
        )
        url = urlreverse(
            "ietf.person.ajax.person_email_json", kwargs=dict(personid=person.pk)
        )
        ...
        self.assertCountEqual(
            ...
            [
                dict(address=email.address, primary=email.primary)
    ...
    def test_case_insensitive(self):
        ...
        url = urlreverse(
            "ietf.person.views.profile", kwargs={"email_or_name": "test person"}
        )
        r = self.client.get(url)
        self.assertContains(r, person.name, status_code=200)
    ...
    def test_person_profile_duplicates(self):
        ...
        person = PersonFactory(
            name="bazquux@example.com", user__email="bazquux@example.com"
        )
        url = urlreverse(
            "ietf.person.views.profile", kwargs={"email_or_name": person.plain_name()}
        )
    ...
    def test_person_profile_404(self):
        urls = [
            urlreverse(
                ...
                kwargs={"email_or_name": "nonexistent@example.com"},
            ),
            urlreverse(
                "ietf.person.views.profile",
                kwargs={"email_or_name": "Nonexistent Person"},
            ),
        ]
    ...
    def test_person_photo(self):
        ...
        url = urlreverse(
            "ietf.person.views.photo", kwargs={"email_or_name": person.email()}
        )
        r = self.client.get(url)
    ...
    def test_person_photo_duplicates(self):
        person = PersonFactory(
            name="bazquux@example.com",
            user__username="bazquux@example.com",
            with_bio=True,
        )
        PersonFactory(
            name="bazquux@example.com",
            user__username="foobar@example.com",
            with_bio=True,
        )

        url = urlreverse(
            "ietf.person.views.photo", kwargs={"email_or_name": person.plain_name()}
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 404)
    ...
    def test_merge_with_params(self):
        ...
        p2 = PersonFactory()
        url = urlreverse("ietf.person.views.merge") + "?source={}&target={}".format(
            p1.pk, p2.pk
        )
        login_testing_unauthorized(self, "secretary", url)
        r = self.client.get(url)
    ...
    def test_invalid_name_characters_rejected(self):
        for disallowed in "/:@":
            ...
            person_with_bad_name = PersonFactory.build(
                name=f"I have a {disallowed}", user=None
            )
            with self.assertRaises(
                ValidationError, msg=f"Name with a {disallowed} char should be rejected"
            ):
...
class PersonUtilsTests(TestCase):
    ...
    def test_handle_reviewer_settings(self):
        ...
        source = PersonFactory()
        target = PersonFactory()
        rs1 = ReviewerSettings.objects.create(
            team=groups[0], person=source, min_interval=14
        )
        ...
        self.assertEqual(
            result,
            [
                "REVIEWER SETTINGS ACTION: dropping duplicate ReviewSettings for team: {}".format(
                    rs1.team
                )
            ],
        )
    ...
    def test_handle_users(self):
        ...
        self.assertTrue(
            "DATATRACKER LOGIN ACTION: retaining login {}".format(target2.user)
            in result
        )
        ...
        self.assertTrue(
            "DATATRACKER LOGIN ACTION: retaining login {}".format(user) in result
        )
        ...
        self.assertTrue(
            "DATATRACKER LOGIN ACTION: retaining login: {}, removing login: {}".format(
    ...
    def test_merge_persons(self):
        ...
        feedback = FeedbackFactory(
            person=source, author=source.email().address, nomcom=nomcom
        )
        feedback.nominees.add(nominee)
        nomination = NominationFactory(
            nominee=nominee, person=source, position=position, comments=feedback
        )
    ...
    def test_dots(self):
        ...
        ncmember = RoleFactory(
            group__acronym="nomcom2020", group__type_id="nomcom", name_id="member"
        ).person
        self.assertEqual(get_dots(ncmember), ["nomcom"])
        ncchair = RoleFactory(
            group__acronym="nomcom2020", group__type_id="nomcom", name_id="chair"
        ).person
...
class TaskTests(TestCase):
    ...
    def test_purge_personal_api_key_events_task(self, mock_log):
        now = timezone.now()
        old_event = PersonApiKeyEventFactory(
            time=now - datetime.timedelta(days=1, minutes=1)
        )
        young_event = PersonApiKeyEventFactory(
            time=now - datetime.timedelta(days=1, minutes=-1)
        )
