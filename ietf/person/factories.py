...
class PersonFactory(factory.django.DjangoModelFactory):
    ...
    name = factory.LazyAttribute(
        lambda p: normalize_name("%s %s" % (p.user.first_name, p.user.last_name))
    )
    ...
    ascii = factory.LazyAttribute(
        lambda p: force_str(unidecode_name(p.name)).rstrip(".;")
    )
...
class AliasFactory(factory.django.DjangoModelFactory):
    ...
    def _create(cls, model_class, *args, **kwargs):
        ...
        existing_aliases = set(
            model_class.objects.filter(person=person).values_list("name", flat=True)
        )
...
def fake_email_address(n):
    ...
    while True:
        ...
        if count >= 10:
            raise RuntimeError(
                "Failed generating a fake email address to fit in Email.address(max_length=%s)"
                % address_field.max_lenth
            )
    return address
...
class EmailFactory(factory.django.DjangoModelFactory):
    ...
    origin = factory.LazyAttribute(
        lambda obj: obj.person.user.username if obj.person.user else ""
    )
