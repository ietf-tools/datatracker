...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "-f", "--file", help="Process an mbox file containing bounce messages"
        )
        parser.add_argument(
            "-r",
            "--reason",
            default="bounce",
            help='The reason for deactivation. Default: "%(default)s".',
        )
    ...
    def handle(self, *args, **options):
        ...
        if options["file"]:
            self.stderr.write(
                'Extracting bounced addresses from mbox file "%s":\n'
                % (options["file"])
            )
            ...
            for msg in tqdm(mbox):
                ...
                recipients = [
                    r.replace("@exch.cisco.com", "@cisco.com") for r in recipients
                ]
                for r in recipients:
                    messages[r] = msg
            ...
            for a in tqdm(addresses):
                ...
                if email and email.person_id:
                    ...
                    email.origin = (
                        ...
                        else ("script: %s deactivation" % options["reason"])
                    )
                    email.save()
                    PersonEvent.objects.create(
                        person=email.person,
                        type="email_address_deactivated",
                        desc="Deactivated the email address <%s>. Reason: %s"
                        % (email.address, options["reason"]),
                    )
                else:
                    if email is None:
                        not_found.append(a)
                        ...
                        self.stderr.write(
                            "Could not deactivate <%s>: Null person record\n" % (a,)
                        )
                    else:
                        self.stderr.write(
                            "Unexpected error when processing <%s>: Quitting." % (a,)
                        )
                        sys.exit(1)
            for a in not_found:
                self.stderr.write("Address not found: <%s>\n" % (a,))
