...
def merge_persons(request, source, target, file=sys.stdout, verbose=False):
    ...
    for email in get_extra_primary(source, target):
        ...
        email.save()
        changes.append(
            "EMAIL ACTION: {} no longer marked as primary".format(email.address)
        )
...
def merge_nominees(source, target):
    ...
    for nominee in source.nominee_set.all():
        try:
            ...
            target_nominee = target.nominee_set.create(
                nomcom=nominee.nomcom, email=target.email()
            )
        ...
        for np in nominee.nomineeposition_set.all():
            existing_target_np = target_nominee.nomineeposition_set.filter(
...
def determine_merge_order(source, target):
    ...
    if source.user and target.user:
        source, target = sorted(
            [source, target],
            key=lambda a: (
                a.user.last_login if a.user.last_login else datetime.datetime.min
            ),
        )
...
def get_active_irsg():
    ...
    if not active_irsg_balloters:
        active_irsg_balloters = list(
            Person.objects.filter(
                role__group__acronym="irsg",
                role__name__in=["chair", "member", "atlarge"],
            ).distinct()
        )
...
def get_active_rsab():
    ...
    if not active_rsab_balloters:
        active_rsab_balloters = list(
            Person.objects.filter(
                role__group__acronym="rsab", role__name="member"
            ).distinct()
        )
...
def get_dots(person):
    ...
    if roles.filter(group__type_id="wg", name_id="chair").exists():
        dots.append("chair")
    if roles.filter(
        Q(group__acronym="iesg", name_id="ad")
        | Q(group__acronym="iab", name_id="chair")
    ).exists():
        dots.append("iesg")
    ...
    if roles.filter(group__acronym="ietf-trust").exists():
        dots.append("trust")
    if roles.filter(
        group__acronym__startswith="nomcom", name_id__in=("chair", "member")
    ).exists():
        dots.append("nomcom")
    return dots
