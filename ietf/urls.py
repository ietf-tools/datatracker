...
urlpatterns = [
    ...
    url(
        r"^(?P<group_type>(wg|rg|ag|rag|team|dir|review|area|program|iabasg|iabworkshop|adhoc|ise|adm|rfcedtyp|edwg|edappr))/",
        include(grouptype_urls),
    ),
    # Redirects
    url(r"^(?P<path>public)/", include("ietf.redirects.urls")),
    # Google webmaster tools verification url
    url(
        r"^googlea30ad1dacffb5e5b.html",
        TemplateView.as_view(template_name="googlea30ad1dacffb5e5b.html"),
    ),
    # Android webmanifest
    url(
        r"^site.webmanifest",
...
if settings.SERVER_MODE in ("development", "test"):
    ...
    urlpatterns += [
        ...
        url(
            r"^(?P<path>(?:images|css|js)/.*)$",
            static_view.serve,
            {"document_root": settings.STATIC_ROOT + "ietf/"},
        ),
    ]
