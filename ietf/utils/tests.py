...
"""
        ...
        send_mail_preformatted(
            request=None, preformatted=msg, extra={}, override=override
        )
        recv = outbox[-1]
...
class TestSMTPServer(TestCase):
    ...
    def test_address_rejected(self):
        ...
        def send_simple_mail(to):
            send_mail_text(
                None, to=to, frm=None, subject="Test for rejection", txt="dummy body"
            )
...
def get_callbacks(urllist, namespace=None):
    ...
    for entry in urllist:
        if hasattr(entry, "url_patterns"):
            ...
            if (
                hasattr(entry, "callback")
                and entry.callback
                and type(entry.callback) in [types.FunctionType, types.MethodType]
            ):
                callbacks.add(
                    qualified(
                        "%s.%s" % (entry.callback.__module__, entry.callback.__name__)
                    )
                )
...
class TemplateChecksTestCase(TestCase):
    ...
    def test_parse_templates(self):
        ...
        if errors:
            messages = [
                "Parsing template '%s' failed with error: %s" % (path, ex)
                for (path, ex) in errors
            ]
            raise self.failureException(
                "Template parsing failed for %s templates:\n  %s"
                % (len(errors), "\n  ".join(messages))
            )
    ...
    def apply_template_test(self, func, node_type, msg, *args, **kwargs):
        ...
        if errors:
            ...
            raise self.failureException(
                "Found %s errors when trying to %s:\n  %s"
                % (len(errors), func.__name__.replace("_", " "), "\n  ".join(messages))
            )
    ...
    def test_template_url_lookup(self):
        ...
        self.apply_template_test(
            check_that_url_tag_callbacks_exists,
            URLNode,
            'In %s: Could not find urlpattern for "%s"',
            callbacks,
        )
    ...
    def test_template_statics_exists(self):
        ...
        settings.DEBUG = True
        self.apply_template_test(
            check_that_static_tags_resolve,
            StaticNode,
            'In %s: Could not find static file for "%s"',
            checked,
        )
    ...
    def test_all_model_admins_exist(self):
        ...
        for name in self.apps:
            ...
            for model in model_list:
                if model.__name__.startswith("Historical") and hasattr(
                    model, "get_history_type_display"
                ):
                    continue
                else:
                    self.assertContains(
                        ...
                        msg_prefix="There doesn't seem to be any admin API for model %s.models.%s"
                        % (
                            app.__name__,
                            model.__name__,
                        ),
...
class PlaintextDraftTests(TestCase):
    ...
    def setUp(self):
        ...
        file, _ = submission_file(
            name_in_doc="draft-test-draft-class-00",
            name_in_post="draft-test-draft-class-00.txt",
            templatename="test_submission.txt",
            group=None,
        )
        self.draft = PlaintextDraft(
            text=file.getvalue(),
            source="draft-test-draft-class-00.txt",
    ...
    def test_get_authors_with_firm(self):
        self.assertTrue(
            all(["@" in author for author in self.draft.get_authors_with_firm()])
        )
...
class XMLDraftTests(TestCase):
    ...
    def test_parse_creation_date(self):
        ...
        with capture_xml2rfc_output(), patch(
            "ietf.utils.xmldraft.date_today", return_value=today
        ):
            ...
            self.assertEqual(
                XMLDraft.parse_creation_date({"year": "", "month": "", "day": ""}),
                today,
            )
            self.assertEqual(
                XMLDraft.parse_creation_date(
    ...
    def test_parse_docname(self):
        ...
        # implementation.
        with self.assertRaises(ValueError) as cm:
            XMLDraft.parse_docname(
                lxml.etree.Element("xml", docName="")
            )  # not a valid docName
        self.assertIn("Unable to parse docName", str(cm.exception))

        self.assertEqual(
            XMLDraft.parse_docname(
                lxml.etree.Element("xml", docName="draft-foo-bar-baz-01")
            ),
        ...
        self.assertEqual(
            XMLDraft.parse_docname(
                lxml.etree.Element("xml", docName="draft-foo-bar-baz")
            ),
    ...
    def test_render_author_name(self):
        self.assertEqual(
            XMLDraft.render_author_name(
                lxml.etree.Element("author", fullname="Joanna Q. Public")
            ),
            "Joanna Q. Public",
        )
        self.assertEqual(
    ...
    def test_capture_xml2rfc_output(self):
        ...
        with capture_xml2rfc_output() as outer_log_streams:  # ensure no output
            ...
            self.assertEqual(
                xml2rfc_log.write_out,
                outer_log_streams["stdout"],
                "out stream should be restored",
            )
            self.assertEqual(
                xml2rfc_log.write_err,
                outer_log_streams["stderr"],
                "err stream should be restored",
            )
        self.assertEqual(
            xml2rfc_log.write_out,
            orig_write_out,
            "original out stream should be restored",
        )
        self.assertEqual(
            xml2rfc_log.write_err,
            orig_write_err,
            "original err stream should be restored",
        )

        # don't happen to get any output on stdout and not paranoid enough to force some, just test stderr
        self.assertGreater(
            len(inner_log_streams["stderr"].getvalue()),
            0,
            "want output on inner streams",
        )
        self.assertEqual(
            len(outer_log_streams["stdout"].getvalue()), 0, "no output on outer streams"
        )
        self.assertEqual(
            len(outer_log_streams["stderr"].getvalue()), 0, "no output on outer streams"
        )
    ...
    def test_capture_xml2rfc_output_exception_handling(self):
        ...
        with capture_xml2rfc_output() as outer_log_streams:  # ensure no output
            with self.assertRaises(
                RuntimeError
            ), capture_xml2rfc_output() as inner_log_streams:
...
class TimezoneTests(TestCase):
    ...
    @patch(
        ...
        return_value=pytz.timezone("America/Chicago").localize(
            datetime.datetime(2022, 7, 1, 23, 15, 0)
        ),  # 23:15:00
    )
