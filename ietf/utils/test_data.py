...
def create_person(
    ...
    person = Person.objects.create(
        name=name, ascii=unidecode_name(smart_str(name)), user=user
    )
    email = Email.objects.create(
        address=email_address, person=person, origin=user.username
    )
...
def make_immutable_base_data():
    ...
    old = TelechatDate.objects.create(
        date=t - datetime.timedelta(days=14)
    ).date  # pyflakes:ignore
    date1 = TelechatDate.objects.create(date=t).date  # pyflakes:ignore
    date2 = TelechatDate.objects.create(
        date=t + datetime.timedelta(days=14)
    ).date  # pyflakes:ignore
    date3 = TelechatDate.objects.create(
        date=t + datetime.timedelta(days=14 * 2)
    ).date  # pyflakes:ignore
    date4 = TelechatDate.objects.create(
        date=t + datetime.timedelta(days=14 * 3)
    ).date  # pyflakes:ignore
    ...
    secretariat = create_group(
        name="IETF Secretariat", acronym="secretariat", type_id="ietf"
    )
    create_person(
        secretariat,
        "secr",
    ...
    iab = create_group(
        name="Internet Architecture Board", acronym="iab", type_id="ietf", parent=ietf
    )
    create_person(iab, "chair")
    create_person(iab, "member")

    ise = create_group(
        name="Independent Submission Editor", acronym="ise", type_id="ise"
    )
    create_person(ise, "chair")

    rsoc = create_group(
        name="RFC Series Oversight Committee", acronym="rsoc", type_id="rfcedtyp"
    )
    ...
    create_person(
        iana, "auth", name="Iña Iana", username="iana", email_address="iana@ia.na"
    )

    rpc = create_group(name="RFC Production Center", acronym="rpc", type_id="rfcedtyp")
    create_person(
        rpc, "auth", name="Rfc Editor", username="rfc", email_address="rfc@edit.or"
    )
    ...
    irsg = create_group(
        name="Internet Research Steering Group",
        acronym="irsg",
        type_id="irtf",
        parent=irtf,
    )  # pyflakes:ignore

    individ = create_group(
        name="Individual submissions", acronym="none", type_id="individ"
    )  # pyflakes:ignore

    # one area
    area = create_group(
        name="Far Future", acronym="farfut", type_id="area", parent=ietf
    )
    create_person(
        area,
        "ad",
        name="Areað Irector",
        username="ad",
        email_address="aread@example.org",
    )

    # second area
    opsarea = create_group(
        name="Operations", acronym="ops", type_id="area", parent=ietf
    )
    ops_ad = create_person(opsarea, "ad")
    sops = create_group(
        name="Server Operations", acronym="sops", type_id="wg", parent=opsarea
    )
    ...
    for i in range(1, 10):
        ...
        email = Email.objects.create(
            address="ad%s@example.org" % i, person=p, origin=u.username
        )
        if i < 6:
            # active
            ...
            RoleHistory.objects.create(
                name_id="ad", group=areahist, person=p, email=email
            )
    ...
    create_person(
        ...
        name="R. Searcher",
        username="rsearcher",
        email_address="rsearcher@example.org",
    )
    ...
    for i in range(1, 5):
        ...
        p = Person.objects.create(
            name="IRSG Member No%s" % i, ascii="IRSG Member No%s" % i, user=u
        )
        email = Email.objects.create(
            address="irsgmember%s@example.org" % i, person=p, origin=u.username
        )
...
def make_test_data():
    ...
    frfarea.role_set.get_or_create(
        name_id="chair", person=ad, email=ad.role_email("ad")
    )
    frfarea.save()
    ...
    doc = IndividualDraftFactory(
        name="draft-imaginary-irtf-submission",
        stream_id="irtf",
        title="The Importance of Research Imagination",
    )
    ConflictReviewFactory(
        name="conflict-review-imaginary-irtf-submission",
        review_of=doc,
        notify="fsm@ietf.org",
        title="Conflict Review of IRTF Imagination Document",
    )
    ...
    def rfc_for_status_change_test_factory(name, rfc_num, std_level_id):
        ...
        source_draft = WgDraftFactory(
            name=name, states=[("draft", "rfc")], notify=f"{name}@ietf.org"
        )
        source_draft.relateddocument_set.create(
            relationship_id="became_rfc", target=target_rfc
        )
        return target_rfc
    ...
    ResourceAssociation.objects.create(
        name=name, icon="projector.png", desc="Projector in room"
    )
...
def make_review_data(doc):
    ...
    team2 = create_group(
        ...
        name="Review Team 2",
        type_id="review",
        list_email="reviewteam2@ietf.org",
        parent=Group.objects.get(acronym="farfut"),
    )
    team3 = create_group(
        acronym="reviewteam3",
    ...
    for team in (team1, team2, team3):
        ...
        for r in ReviewResultName.objects.filter(
            slug__in=["issues", "ready-issues", "ready", "not-ready"]
        ):
    ...
    u.save()
    reviewer = Person.objects.create(
        name="Some Réviewer", ascii="Some Reviewer", user=u
    )
    email = Email.objects.create(
        address="reviewer@example.com", person=reviewer, origin=u.username
    )

    for team in (team1, team2, team3):
        Role.objects.create(
            name_id="reviewer", person=reviewer, email=email, group=team
        )
        ReviewerSettings.objects.create(
            team=team, person=reviewer, min_interval=14, skip_next=0
        )
    ...
    Role.objects.create(
        name_id="reviewer", person=p, email=p.email_set.first(), group=team1
    )

    u = User.objects.create(username="reviewsecretary")
    u.set_password("reviewsecretary+password")
    u.save()
    reviewsecretary = Person.objects.create(
        name="Réview Secretary", ascii="Review Secretary", user=u
    )
    reviewsecretary_email = Email.objects.create(
        address="reviewsecretary@example.com", person=reviewsecretary, origin=u.username
    )
    Role.objects.create(
        name_id="secr", person=reviewsecretary, email=reviewsecretary_email, group=team1
    )

    u = User.objects.create(username="reviewsecretary3")
    u.set_password("reviewsecretary3+password")
    u.save()
    reviewsecretary3 = Person.objects.create(
        name="Réview Secretary3", ascii="Review Secretary3", user=u
    )
    reviewsecretary3_email = Email.objects.create(
        address="reviewsecretary3@example.com",
        person=reviewsecretary,
        origin=u.username,
    )
    Role.objects.create(
        name_id="secr",
        person=reviewsecretary3,
        email=reviewsecretary3_email,
        group=team3,
    )
