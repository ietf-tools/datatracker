...
class APITests(TestCase):
    ...
    def test_retrieve_wg_tokens(self):
        ...
        self.requests_mock.post(
            self.retrieve_token_url, status_code=200, json=data_to_fetch
        )
    ...
    def test_schedule_meeting(self):
        ...
        api_response = api.schedule_meeting(
            ...
            start_time=datetime.datetime(
                2021, 9, 14, 10, 0, 0, tzinfo=datetime.timezone.utc
            ),
        ...
        for start_time in [
            ...
            datetime.datetime(
                2021, 9, 14, 11, 0, 0, tzinfo=ZoneInfo("Africa/Porto-Novo")
            ),
        ]:
            self.assertEqual(
                api_response,
                ...
                "rooms": {
                    ...
                    "e68e96d4-d38f-475b-9073-ecab46ca96a5": {
                        "room": {
                            "id": 23,
                            "start_time": datetime.datetime(
                                2021, 9, 15, 14, 30, 0, tzinfo=datetime.timezone.utc
                            ),
    ...
    def test_delete_meeting(self):
        data_to_fetch = {}
        self.requests_mock.post(
            self.delete_meetings_url, status_code=200, json=data_to_fetch
        )
        ...
        self.assertEqual(
            request.headers["Authorization"],
            "bearer delete-this-meeting-please",
            "Incorrect request authorization header",
        )
    ...
    def test_request_helper_failed_requests(self):
        self.requests_mock.register_uri(
            requests_mock.ANY,
            urljoin(API_BASE, "unauthorized/url/endpoint"),
            status_code=401,
        )
        self.requests_mock.register_uri(
            requests_mock.ANY,
            urljoin(API_BASE, "forbidden/url/endpoint"),
            status_code=403,
        )
        self.requests_mock.register_uri(
            requests_mock.ANY,
            urljoin(API_BASE, "notfound/url/endpoint"),
            status_code=404,
        )
        api = MeetechoAPI(API_BASE, CLIENT_ID, CLIENT_SECRET)
        for method in ["POST", "GET"]:
            for code, endpoint in (
                (401, "unauthorized/url/endpoint"),
                (403, "forbidden/url/endpoint"),
                (404, "notfound/url/endpoint"),
            ):
    ...
    def test_request_helper_exception(self):
        self.requests_mock.register_uri(
            requests_mock.ANY,
            urljoin(API_BASE, "exception/url/endpoint"),
    ...
    def test_time_serialization(self):
        ...
        time = (
            timezone.now().astimezone(datetime.timezone.utc).replace(microsecond=0)
        )  # cut off to 0 microseconds
        api = MeetechoAPI(API_BASE, CLIENT_ID, CLIENT_SECRET)
...
class ConferenceManagerTests(TestCase):
            ...
            None,
            {
                "session-1-uuid": {
                    "room": {
                        "id": 1,
                        "start_time": datetime.datetime(
                            2022, 2, 4, 1, 2, 3, tzinfo=datetime.timezone.utc
                        ),
                ...
                "session-2-uuid": {
                    "room": {
                        "id": 2,
                        "start_time": datetime.datetime(
                            2022, 2, 5, 4, 5, 6, tzinfo=datetime.timezone.utc
                        ),
    ...
    def test_delete_conference(self, mock_delete):
        cm = ConferenceManager(settings.MEETECHO_API_CONFIG)
        cm.delete_conference(
            Conference(None, None, None, None, None, None, None, "delete-this")
        )
    ...
    def test_delete_by_url(self, mock_delete):
        cm = ConferenceManager(settings.MEETECHO_API_CONFIG)
        cm.delete_conference(
            Conference(None, None, None, None, None, None, "the-url", "delete-this")
        )
        args, kwargs = mock_delete.call_args
        self.assertEqual(args, ("delete-this",))
...
class SlidesManagerTests(TestCase):
    ...
    def test_delete(self, mock_delete, mock_update, mock_wg_token):
        ...
        slides = SessionPresentationFactory(
            document__type_id="slides", order=1
        ...
        with self.assertRaises(MeetechoAPIError):
            sm.delete(
                session, slides_doc
            )  # can't remove slides still attached to the session
        self.assertFalse(
            any([mock_wg_token.called, mock_delete.called, mock_update.called])
        )
