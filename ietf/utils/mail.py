...
from email.utils import (
    make_msgid,
    formatdate,
    formataddr as simple_formataddr,
    parseaddr as simple_parseaddr,
    getaddresses,
)
...
def decode_header_value(value: str) -> str:
    ...
    return "".join(
        (
            part.decode(charset if charset else "utf-8")
...
def send_smtp(msg, bcc=None):
    ...
    assertion(
        "len(from_tuples)==1",
        note=f"send_smtp received multiple From addresses: {from_tuples}",
    )
    _, frm = from_tuples[0]
    ...
    to = [
        addr
        for name, addr in getaddresses(addrlist)
        if (addr != "" and not addr.startswith("unknown-email-"))
    ]
    if not to:
        log(
            "No addressees for email from '%s', subject '%s'.  Nothing sent."
        ...
        try:
            ...
            if settings.EMAIL_HOST_USER and settings.EMAIL_HOST_PASSWORD:
                ...
                if retval != 220:
                    raise ImproperlyConfigured(
                        "password configured but tls failed: %d %s" % (retval, retmsg)
                    )
            ...
            if unhandled != {}:
                raise SMTPSomeRefusedRecipients(
                    message="%d addresses were refused" % len(unhandled),
                    original_msg=msg,
                    refusals=unhandled,
                )
            ...
            log(
                "Exception while trying to send email from '%s' to %s subject '%s'"
        ...
        tau = time.time() - mark
        log(
            "sent email (%.3fs) from '%s' to %s id %s subject '%s'"
            % (tau, frm, to, msg.get("Message-ID", ""), subj)
        )
...
def copy_email(msg, to, toUser=False, originalBcc=None):
    ...
    if settings.SERVER_MODE == "production":
        ...
        explanation = (
            "The attached message would have been sent, but the tracker is in %s mode.\nIt was not sent to anybody."
...
def send_mail_text(
    ...
    return send_mail_mime(
        request, to, frm, subject, msg, cc, extra, toUser, bcc, copy=copy, save=save
    )
...
def condition_message(to, frm, subject, msg, cc, extra):
    ...
    if isinstance(to, list) or isinstance(to, tuple):
        to = ", ".join(
            [
                isinstance(addr, tuple) and formataddr(addr) or addr
                for addr in to
                if addr
            ]
        )
...
def show_that_mail_was_sent(
    request: HttpRequest, leadline: str, msg: Message, bcc: Optional[str]
):
...
def save_as_message(request, msg, bcc):
    ...
    log(
        "Saved outgoing email from '%s' to %s id %s subject '%s as Message[%s]'"
        % (m.frm, m.to, m.msgid, m.subject, m.pk)
    )
    return m
...
def send_mail_mime(
    ...
    debugging = (
        getattr(settings, "USING_DEBUG_EMAIL_SERVER", False)
        and settings.EMAIL_HOST == "localhost"
        and settings.EMAIL_PORT == 2025
    )
    ...
    if settings.SERVER_MODE == "development":
        show_that_mail_was_sent(
            request, "In production, email would have been sent", msg, bcc
        )
    ...
    try:
        ...
        copy_to = None
    if (
        copy_to and (copy or not production) and not (test_mode or debugging)
    ):  # if we're running automated tests, this copy is just annoying
        if bcc:
    ...
    for key in ["To", "From", "Cc", "Bcc"]:
        ...
        if values:
            ...
            if key == "From":
                assertion(
                    "len(values)<2",
                    note=f"parse_preformatted is constructing a From with multiple values: {values}",
                )
            del msg[key]
...
def send_mail_message(request, message, extra=None):
    ...
    if extra is None:
        extra = {}
    assertion(
        "isinstance(message.to, str) and isinstance(message.cc, str) and isinstance(message.bcc, str)"
    )
...
def build_warning_message(request, e):
    ...
    if request:
        ...
        messages.warning(
            request, warning, extra_tags="preformatted", fail_silently=True
        )
...
def send_error_to_secretariat(msg):
    ...
    try:
        ...
        try:
            ...
            copy_to = None
        if (
            copy_to and not test_mode and not debugging
        ):  # if we're running automated tests, this copy is just annoying
...
def get_email_addresses_from_text(text):
    ...
    def valid(email):
        ...
        try:
            ...
            log(
                "Bad data: get_email_addresses_from_text() got an "
                'invalid email address tuple: {email}, in "{text}".'.format(
