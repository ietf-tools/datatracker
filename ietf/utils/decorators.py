...
def person_required(f):
    ...
    def _wrapper(request, *args, **kwargs):
        if not request.user.is_authenticated:
            raise ValueError(
                "The @person_required decorator should be called after @login_required."
            )
        try:
            request.user.person
...
def require_api_key(f):
    ...
    def _wrapper(request, *args, **kwargs):
        ...
        if not person.user.is_staff:
            time_limit = timezone.now() - datetime.timedelta(
                days=settings.UTILS_APIKEY_GUI_LOGIN_LIMIT_DAYS
            )
        ...
        key.save()
        PersonApiKeyEvent.objects.create(
            person=person,
            type="apikey_login",
            key=key,
            desc="Logged in with key ID %s, endpoint %s" % (key.id, key.endpoint),
        )
        ...
        try:
            ...
            log.log(
                "Bad API call: args: %s, kwargs: %s, exception: %s" % (args, kwargs, e)
            )
