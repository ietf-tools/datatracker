...
for name, level in settings.UTILS_LOGGER_LEVELS.items():
    ...
    if not logger.hasHandlers():
        ...
        for h in handlers:
            ...
            h.setLevel(level)
            logger.addHandler(h)
    debug.say(f" Setting {logger.name} logging level to {level}")
...
def assertion(statement, state=True, note=None):
    ...
    if bool(value) != bool(state):
        if (settings.DEBUG is True) or (settings.SERVER_MODE == 'test') :
            if note:
                raise AssertionError(f"Assertion failed: '{statement}': {repr(value)} != {state} ({note}).")
            else:
                raise AssertionError(f"Assertion failed: '{statement}': {repr(value)} != {state}.")
        else:
...
def unreachable(date="(unknown)"):
    ...
    frame = inspect.currentframe().f_back
    if settings.DEBUG is True or settings.SERVER_MODE == 'test':
        raise AssertionError(f"Arrived at code in {frame.f_code.co_name}() which was marked unreachable on {date}.")
    else:
        # build a simulated traceback object
        tb = build_traceback(inspect.stack()[1:])
        # provide extra info if available
        ...
        for key in [ 'request', 'status_code', ]:
            if key in frame.f_locals:
                extra[key] = frame.f_locals[key]
        logger.error(f"Arrived at code in {frame.f_code.co_name}() which was marked unreachable on {date}.",
