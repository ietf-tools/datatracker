...
class Command(DumpdataCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("filenames", nargs="*", help="One or more files to process")
        parser.add_argument(
            "--pk-name",
            default="id",
            type=str,
            ...
            with gzip.open(fn, "rt") if fn.endswith(".gz") else io.open(fn) as f:
                for line in f:
                    ...
                    if line and line[0] in ["<", ">"]:
                        self.err(
                            "Input file '%s' looks like a diff file.  Please provide just the SQL 'INSERT' statements for the records to be dumped."
                            % (fn,)
                        )
                    if prev:
                        line = prev + line
                    ...
                    if not sql.upper().startswith("INSERT "):
                        ...
                        sql = sql.replace("\\'", "\\x27")
                        match = re.match(
                            r"INSERT( +(LOW_PRIORITY|DELAYED|HIGH_PRIORITY))*( +IGNORE)?( +INTO)?"
                            r" +(?P<table>\S+)"
                            r" +\((?P<fields>([^ ,]+)(, [^ ,]+)*)\)"
                            r" +(VALUES|VALUE)"
                        ...
                        if not match:
                            ...
                            fields = [
                                f.strip("`")
                                for f in re.split(r"(`[^`]+`)", fields)
                                if f and not re.match(r"\s*,\s*", f)
                            ]  # pyflakes:ignore
                            values = match.group("values")
                            values = [
                                v.strip("'")
                                for v in re.split(r"(\d+|'[^']*'|NULL)", values)
    ...
    def get_objects(self, app_list, pks, count_only=False):
        ...
        for model in models:
            ...
            if not model._meta.proxy and router.allow_migrate_model(self.using, model):
                ...
                primary_keys = (
                    pks[model._meta.db_table] if model._meta.db_table in pks else []
                )
                if primary_keys:
