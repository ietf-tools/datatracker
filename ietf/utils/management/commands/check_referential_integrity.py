...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        def check_field(field, through_table=False):
            ...
            if through_table:
                used = set(
                    int(i) if isinstance(i, str) and i.isdigit() else i for i in used
                )
                exists = set(
                    int(i) if isinstance(i, str) and i.isdigit() else i for i in exists
                )
            ...
            if dangling and options.get("delete"):
                if verbosity > 1:
                    self.stdout.write(
                        "Removing dangling values: %s.%s.%s\n"
                        % (
                            model.__module__,
                            model.__name__,
                            field.name,
                ...
                for value in tqdm(dangling):
                    ...
                    for obj in field.model.objects.filter(**kwargs):
                        try:
                            ...
                            self.stderr.write("\n")
                            self.stderr.write(
                                "Tried setting %s[%s].%s to %s, but got:\n"
        ...
        for conf in tqdm(
            [c for c in apps.get_app_configs() if c.name.startswith("ietf")],
            desc="apps  ",
            disable=verbose,
        ):
            if verbosity > 1:
