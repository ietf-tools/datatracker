...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("documents", nargs="*", help="One or more files to process")
    ...
    def check_objects(self, queryset, timeattr, docattr):
        ...
        for obj in queryset:
            ...
            if not obj.rev:
                if doc.type_id != "rfc":
                    self.stdout.write(
                        "Bad revision number: %-52s: '%s'" % (doc.name, obj.rev)
                    )
                continue
            ...
            if doc.name in revs:
                ...
                if rev <= prev.rev:
                    ...
                    if prevd != docd:
                        self.stderr.write(
                            "%s %-50s %02d -> %02d" % (time, doc.name, prev.rev, rev)
                        )
        ...
        for doc, obj, rev in revs.values():
            if not doc.rev == obj.rev:
                self.stderr.write(
                    "%-50s: doc.rev: %s != %s.rev: %s"
                    % (doc.name, doc.rev, obj._meta.model_name, obj.rev)
                )
