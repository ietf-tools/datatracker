...
class Command(BaseCommand):
    ...
    help = (
        "Show coverage info for the latest test run.  By default, the difference in \n"
        ...
        for section in sections:
            ...
            for key in keys:
                ...
                if (
                    mmisslines
                    and lmisslines
                    and set(lmissnum) != set(mmissnum)
                    and options.get("verbosity", 1) > 1
                ):
                    self.stdout.write(
                        "    ------------------------------------------------------------"
                    )
    ...
    def handle(self, *args, **options):
        ...
        for section in sections:
            if not section in self.valid_sections:
                raise CommandError(
                    "Found an unexpected section name, '%s' in the section list. "
                    "Valid names are %s or any combination of them."
                    % (section, ",".join(self.valid_sections))
                )
