...
from ietf.review.policies import (
    AssignmentOrderResolver,
    LeastRecentlyUsedReviewerQueuePolicy,
...
class GetReviewerQueuePolicyTest(TestCase):
    def test_valid_policy(self):
        team = ReviewTeamFactory(
            acronym="rotationteam",
            name="Review Team",
            list_email="rotationteam@ietf.org",
            parent=Group.objects.get(acronym="farfut"),
            settings__reviewer_queue_policy_id="LeastRecentlyUsed",
        )
    ...
    def test_invalid_policy_name(self):
        ...
        team = ReviewTeamFactory(
            ...
            list_email="rotationteam@ietf.org",
            parent=Group.objects.get(acronym="farfut"),
            settings__reviewer_queue_policy_id="invalid",
        )
        with self.assertRaises(ValueError):
            get_reviewer_queue_policy(team)
...
class _Wrapper(TestCase):
    ...
    class ReviewerQueuePolicyTestCase(TestCase):
        ...
        def setUp(self):
            super().setUp()
            self.team = ReviewTeamFactory(
                acronym="rotationteam",
                name="Review Team",
                list_email="rotationteam@ietf.org",
                parent=Group.objects.get(acronym="farfut"),
            )
            self.team.reviewteamsettings.reviewer_queue_policy_id = (
                self.reviewer_queue_policy_id
            )
        ...
        def append_reviewer(self, skip_count=None):
            ...
            assert (
                index < 100
            )  # ordering by label will fail if > 100 reviewers are created
            label = "{:02d}".format(index)
            reviewer = create_person(
                self.team,
                "reviewer",
        ...
        def reviewer_settings_for(self, person):
            return ReviewerSettings.objects.filter(
                team=self.team, person=person
            ).first() or ReviewerSettings(team=self.team, person=person)
        ...
        def test_default_reviewer_rotation_list_ignores_out_of_team_reviewers(self):
            ...
            ReviewAssignmentFactory(
                review_request__team=self.team, reviewer=out_of_team_reviewer.email()
            )
        ...
        def test_assign_reviewer(self):
            ...
            self.assertCountEqual(
                review_req.reviewassignment_set.all().values_list(
                    "reviewer", flat=True
                ),
                [str(reviewer.email())],
            )
        ...
        def test_assign_reviewer_updates_skip_next_minimal(self):
            ...
            self.assertEqual(
                self.reviewer_settings_for(another_reviewer_to_skip).skip_next, 1
            )
            self.assertEqual(
                self.reviewer_settings_for(reviewer_to_assign).skip_next, 0
            )

            self.policy.assign_reviewer(
                review_req, reviewer_to_assign.email(), add_skip=False
            )

            # Check results
            self.assertEqual(
                self.reviewer_settings_for(reviewer_to_skip).skip_next,
                0,
                "skip_next not updated for first skipped reviewer",
            )
            self.assertEqual(
                self.reviewer_settings_for(another_reviewer_to_skip).skip_next,
                0,
        ...
        def test_assign_reviewer_ignores_skip_next_on_out_of_order_assignment(self):
            ...
            self.assertEqual(self.reviewer_settings_for(reviewer_to_skip).skip_next, 1)
            self.assertEqual(
                self.reviewer_settings_for(reviewer_to_ignore).skip_next, 0
            )
            self.assertEqual(
                self.reviewer_settings_for(reviewer_to_assign).skip_next, 0
            )
            self.assertEqual(
                self.reviewer_settings_for(another_reviewer_to_skip).skip_next, 3
            )
            ...
            self.assertEqual(
                self.reviewer_settings_for(reviewer_to_assign).skip_next,
                0,
        ...
        def test_assign_reviewer_updates_skip_next_when_canfinish_other_doc(self):
            ...
            # assign the review
            self.policy.assign_reviewer(
                new_req, reviewer_to_assign.email(), add_skip=False
            )
            ...
            self.assertEqual(
                self.reviewer_settings_for(canfinish_reviewer).skip_next,
                0,
                'skip_next changed unexpectedly for "canfinish" unavailable reviewer',
            )
            self.assertEqual(
                self.reviewer_settings_for(canfinish_reviewer_no_review).skip_next,
                0,
                'skip_next changed unexpectedly for "canfinish" unavailable reviewer with no review',
            )
            self.assertEqual(
                self.reviewer_settings_for(canfinish_reviewer_no_completed).skip_next,
                0,
        ...
        def set_up_default_reviewer_rotation_list_test(self):
            ...
            return (
                [
                    r for r in self.reviewers if r is not unavailable_reviewer
                ],  # available reviewers
                unavailable_reviewer,
            )

        def test_default_reviewer_rotation_list(self):
            available_reviewers, unavailable_reviewer = (
                self.set_up_default_reviewer_rotation_list_test()
            )
            rotation = self.policy.default_reviewer_rotation_list()
        ...
        def test_recommended_assignment_order(self):
            ...
            doc = WgDraftFactory(
                group__acronym="mars", rev="01", authors=[reviewer_low]
            )
            review_req = ReviewRequestFactory(doc=doc, team=self.team, type_id="early")

            order = self.policy.recommended_assignment_order(
                Email.objects.all(), review_req
            )
            ...
            self.assertIn(
                "{}: is author of document; #1".format(reviewer_low.name), order[1][1]
            )

            with self.assertRaises(ValueError):
                review_req_other_team = ReviewRequestFactory(doc=doc, type_id="early")
                self.policy.recommended_assignment_order(
                    Email.objects.all(), review_req_other_team
                )
        ...
        def test_setup_reviewer_field(self):
            ...
            ReviewAssignmentFactory(
                review_request=ReviewRequestFactory(
                    team=self.team, type_id="early", doc=review_req.doc
                ),
            ...
            field = PersonEmailChoiceField(
                label="Assign Reviewer", empty_label="(None)", required=False
            )
            ...
            self.assertNotIn(
                str(no_response_reviewer.email()),
                addresses,
...
class RotateAlphabeticallyReviewerQueuePolicyTest(_Wrapper.ReviewerQueuePolicyTestCase):
    ...
    def test_default_reviewer_rotation_list_with_nextreviewerinteam(self):
        ...
        NextReviewerInTeam.objects.create(
            team=self.team, next_reviewer=available_reviewers[3]
        )
        rotation = self.policy.default_reviewer_rotation_list()
        ...
        NextReviewerInTeam.objects.filter(team=self.team).update(
            next_reviewer=available_reviewers[1]
        )
        rotation = self.policy.default_reviewer_rotation_list()
    ...
    def test_update_policy_state_for_assignment(self):
        ...
        self.policy.update_policy_state_for_assignment(
            review_req, assignee_person=reviewers[0], add_skip=False
        )
        self.assertEqual(
            NextReviewerInTeam.objects.get(team=self.team).next_reviewer, reviewers[1]
        )
        ...
        self.assertFalse(
            self.reviewer_settings_for(reviewers[0]).request_assignment_next
        )

        # In-order assignment with add_skip
        self.policy.update_policy_state_for_assignment(
            review_req, assignee_person=reviewers[1], add_skip=True
        )
        self.assertEqual(
            NextReviewerInTeam.objects.get(team=self.team).next_reviewer, reviewers[2]
        )
        self.assertEqual(get_skip_next(reviewers[0]), 0)
        ...
        self.policy.update_policy_state_for_assignment(
            review_req, assignee_person=reviewers[2], add_skip=False
        )
        self.assertEqual(
            NextReviewerInTeam.objects.get(team=self.team).next_reviewer, reviewers[4]
        )
        self.assertEqual(get_skip_next(reviewers[0]), 0)
        self.assertEqual(get_skip_next(reviewers[1]), 1)  # from previous add_skip=true
        self.assertEqual(get_skip_next(reviewers[2]), 0)
        self.assertEqual(
            get_skip_next(reviewers[3]), 1
        )  # from manually set skip_next - 1
        self.assertEqual(get_skip_next(reviewers[4]), 0)

        # Out of order assignments, nothing should change,
        # except the add_skip=True should still apply
        self.policy.update_policy_state_for_assignment(
            review_req, assignee_person=reviewers[3], add_skip=False
        )
        self.policy.update_policy_state_for_assignment(
            review_req, assignee_person=reviewers[2], add_skip=False
        )
        self.policy.update_policy_state_for_assignment(
            review_req, assignee_person=reviewers[1], add_skip=False
        )
        self.policy.update_policy_state_for_assignment(
            review_req, assignee_person=reviewers[0], add_skip=True
        )
        self.assertEqual(
            NextReviewerInTeam.objects.get(team=self.team).next_reviewer, reviewers[4]
        )
        ...
        self.policy.update_policy_state_for_assignment(
            review_req, assignee_person=reviewers[4], add_skip=False
        )
        self.assertEqual(
            NextReviewerInTeam.objects.get(team=self.team).next_reviewer, reviewers[2]
        )
        self.assertEqual(
            get_skip_next(reviewers[0]), 0
        )  # skipped over with this assignment
        self.assertEqual(
            get_skip_next(reviewers[1]), 0
        )  # skipped over with this assignment
        self.assertEqual(get_skip_next(reviewers[2]), 0)
        self.assertEqual(get_skip_next(reviewers[3]), 1)
        self.assertEqual(get_skip_next(reviewers[4]), 0)
    ...
    def test_assign_reviewer_updates_skip_next_without_add_skip(self):
        ...
        self.assertEqual(
            self.reviewer_settings_for(reviewer_to_skip_later).skip_next,
            0,
    ...
    def test_assign_reviewer_updates_skip_next_with_add_skip(self):
        ...
        self.policy.assign_reviewer(
            review_req, reviewer_to_assign.email(), add_skip=True
        )
...
class LeastRecentlyUsedReviewerQueuePolicyTest(_Wrapper.ReviewerQueuePolicyTestCase):
    ...
    def append_reviewer(self, skip_count=None):
        ...
        """
        reviewer = super(
            LeastRecentlyUsedReviewerQueuePolicyTest, self
        ).append_reviewer(skip_count)
    ...
    def create_reviewer_assignment(self, reviewer):
        ...
        if self.last_assigned_on is None:
            assignment = ReviewAssignmentFactory(
                review_request__team=self.team, reviewer=reviewer.email()
            )
        else:
            assignment = ReviewAssignmentFactory(
    ...
    def create_old_review_assignment(self, reviewer, **kwargs):
        ...
        return super(
            LeastRecentlyUsedReviewerQueuePolicyTest, self
        ).create_old_review_assignment(reviewer, **kwargs)

    def test_default_reviewer_rotation_list_uses_latest_assignment(self):
        available_reviewers, _ = self.set_up_default_reviewer_rotation_list_test()
        assert (
            len(available_reviewers) > 2
        )  # need enough to avoid wrapping around in a way that invalidates tests
        ...
        self.create_reviewer_assignment(
            first_reviewer
        )  # creates a new assignment, later assigned_on than all others

        self.assertEqual(
            self.policy.default_reviewer_rotation_list(),
            available_reviewers[1:] + [first_reviewer],
        )
    ...
    def test_default_reviewer_rotation_list_ignores_rejected(self):
        ...
        first_reviewer = available_reviewers[0]
        rejected_assignment = self.create_reviewer_assignment(
            first_reviewer
        )  # assigned_on later than all others...
        rejected_assignment.state_id = "rejected"  # ... but marked as rejected
        rejected_assignment.save()

        self.assertEqual(
            self.policy.default_reviewer_rotation_list(), available_reviewers
        )  # order unchanged
    ...
    def test_default_review_rotation_list_uses_assigned_on_date(self):
        ...
        completed_assignment = self.create_reviewer_assignment(
            first_reviewer
        )  # moves to the end...
        second_reviewer_assignment = self.create_reviewer_assignment(
            second_reviewer
        )  # moves to the end...
        ...
        completed_assignment.completed_on = (
            second_reviewer_assignment.assigned_on + datetime.timedelta(days=1)
        )
...
class AssignmentOrderResolverTests(TestCase):
    def test_determine_ranking(self):
        ...
        reviewer_high = create_person(
            team, "reviewer", name="Test Reviewer-high", username="testreviewerhigh"
        )
        reviewer_low = create_person(
            team, "reviewer", name="Test Reviewer-low", username="testreviewerlow"
        )
        # This reviewer should be entirely ignored because it is not in the rotation list.
        create_person(
            team,
            "reviewer",
            name="Test Reviewer-out-of-rotation",
            username="testreviewer-out-of-rotation",
        )
        ...
        doc = WgDraftFactory(
            group__acronym="mars",
            rev="01",
            authors=[reviewer_low],
            ad=reviewer_low,
            shepherd=reviewer_low.email(),
            relations=[("replaces", doc_middle_wg)],
        )
        Role.objects.create(
            group=doc.group,
            person=reviewer_low,
            email=reviewer_low.email(),
            name_id="advisor",
        )
        ...
        ReviewAssignmentFactory(
            ...
            review_request__doc=doc_individual,
            reviewer=reviewer_high.email(),
            state_id="completed",
        )
        # Trigger other review stats
        ReviewAssignmentFactory(
            review_request__team=team,
            review_request__doc=doc,
            reviewer=reviewer_high.email(),
            state_id="no-response",
        )
        ReviewAssignmentFactory(
        ...
        self.assertEqual(
            ranking[0]["label"],
            "Test Reviewer-high: unavailable indefinitely (Can do follow-ups); requested to be selected next for assignment; reviewed document before; wishes to review document; #2; 1 no response, 1 partially complete, 1 fully completed",
        )
        self.assertEqual(
            ranking[1]["label"],
            "Test Reviewer-low: rejected review of document before; is author of document; filter regexp matches; max frequency exceeded, ready in 91 days; skip next 2; #1; currently 1 open, 10 pages",
        )
