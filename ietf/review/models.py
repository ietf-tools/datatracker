...
class ReviewerSettings(models.Model):
    ...
    min_interval = models.IntegerField(
        verbose_name="Can review at most", choices=INTERVALS, blank=True, null=True
    )
    filter_re = models.CharField(
        max_length=255,
        verbose_name="Filter regexp",
        blank=True,
        validators=[
            validate_regular_expression_string,
        ],
    ...
    remind_days_before_deadline = models.IntegerField(
        null=True,
        blank=True,
    ...
    remind_days_open_reviews = models.PositiveIntegerField(
        null=True,
        blank=True,
    ...
    request_assignment_next = models.BooleanField(
        default=False,
...
class ReviewSecretarySettings(models.Model):
    ...
    max_items_to_show_in_reviewer_list = models.IntegerField(
        null=True,
        blank=True,
    ...
    days_to_show_in_reviewer_list = models.IntegerField(
        null=True,
        blank=True,
...
class UnavailablePeriod(models.Model):
    ...
    def __str__(self):
        return "{} is unavailable in {} {} - {}".format(
            self.person, self.team.acronym, self.start_date or "", self.end_date or ""
        )
...
class ReviewWish(models.Model):
    ...
    def __str__(self):
        return "{} wishes to review {} in {}".format(
            self.person, self.doc.name, self.team.acronym
        )
...
class ReviewRequest(models.Model):
    ...
    requested_rev = models.CharField(
    ...
    def all_completed_assignments_for_doc(self):
        return ReviewAssignment.objects.filter(
            review_request__doc=self.doc, state__in=["completed", "part-completed"]
        )
...
class ReviewAssignment(models.Model):
    ...
    reviewed_rev = models.CharField(
        verbose_name="reviewed revision", max_length=16, blank=True
    )
    ...
    def __str__(self):
        return "Assignment for %s (%s) : %s %s of %s" % (
            self.reviewer.person,
            self.state,
            self.review_request.team.acronym,
            self.review_request.type,
            self.review_request.doc,
        )
    ...
    def save(self, *args, **kwargs):
        ...
        if self._original_state != self.state_id:
            if self.state_id in ["withdrawn", "rejected", "no-response", "overtaken"]:
                ...
                review_req_has_active_assignments = (
                    self.review_request.reviewassignment_set.filter(
                        state__in=active_req_states
                    )
                )
                if (
                    self.review_request.state_id == "assigned"
                    and not review_req_has_active_assignments
                ):
...
def get_default_review_results():
    return ReviewResultName.objects.filter(
        slug__in=[
            "not-ready",
            "right-track",
            "almost-ready",
...
class ReviewTeamSettings(models.Model):
    ...
    autosuggest = models.BooleanField(
        default=True, verbose_name="Automatically suggest possible review requests"
    )
    allow_reviewer_to_reject_after_deadline = models.BooleanField(
        default=False, verbose_name="Allow reviewer to reject request after deadline."
    )
    reviewer_queue_policy = models.ForeignKey(
        ReviewerQueuePolicyName,
        default="RotateAlphabetically",
        on_delete=models.PROTECT,
    )
    review_types = models.ManyToManyField(
        ReviewTypeName, default=get_default_review_types
    )
    review_results = models.ManyToManyField(
        ReviewResultName,
        default=get_default_review_results,
        related_name="reviewteamsettings_review_results_set",
    )
    notify_ad_when = models.ManyToManyField(
        ReviewResultName, related_name="reviewteamsettings_notify_ad_set", blank=True
    )
    secr_mail_alias = models.CharField(
    ...
    remind_days_unconfirmed_assignments = models.PositiveIntegerField(
        null=True,
        blank=True,
