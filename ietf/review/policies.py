...
def get_reviewer_queue_policy(team):
    try:
        ...
        raise ValueError(
            "Request for a reviewer queue policy for team {} "
    ...
    try:
        ...
        raise ValueError(
            "Team {} has unknown reviewer queue policy: "
...
def persons_with_previous_review(team, review_req, possible_person_ids, state_id):
    ...
    """
    doc_names = {review_req.doc.name}.union(
        *extract_complete_replaces_ancestor_mapping_for_docs(
            [review_req.doc.name]
        ).values()
    )
    ...
    has_reviewed_previous = set(
        has_reviewed_previous.values_list(
            "reviewassignment__reviewer__person", flat=True
        )
    )
...
class AbstractReviewerQueuePolicy:
    ...
    def assign_reviewer(self, review_req, reviewer, add_skip):
        ...
        if assignment:
            assignment.state = ReviewAssignmentStateName.objects.get(
                slug="assigned", used=True
            )
            assignment.assigned_on = timezone.now()
            assignment.save()
            return assignment
        else:
            return review_req.reviewassignment_set.create(
                state_id="assigned", reviewer=reviewer, assigned_on=timezone.now()
            )
    ...
    def default_reviewer_rotation_list(self, include_unavailable=False):
        ...
        rotation_list = list(
            Person.objects.filter(role__name="reviewer", role__group=self.team)
        )
    ...
    def default_reviewer_rotation_list_without_skipped(self):
        ...
        return [
            r
            for r in self.default_reviewer_rotation_list()
            if not self._reviewer_settings_for(r).skip_next
        ]

    def update_policy_state_for_assignment(
        self, review_req, assignee_person, add_skip=False
    ):
        ...
        rotation = self._filter_unavailable_reviewers(
            ...
            ),  # we are going to filter for ourselves
            review_req,
        )
    ...
    def _update_skip_next(self, rotation_pks, assignee_person):
        ...
        skipped_settings = self.team.reviewersettings_set.filter(
            person__in=skipped
        )  # list of PKs is valid here
        changed = []
        for ss in skipped_settings:
            if ss.skip_next > 0:
                ...
                changed.append(ss)
        bulk_update_with_history(changed, ReviewerSettings, ["skip_next"])
    ...
    def _assignment_in_order(self, rotation_pks, assignee_person):
        ...
        skip_next = dict(
            self.team.reviewersettings_set.filter(person__in=rotation_pks).values_list(
                "person_id", "skip_next"
            )
        )
    ...
    def setup_reviewer_field(self, field, review_req):
        ...
        ).values_list("reviewassignment__reviewer__person_id", flat=True)

        # Query the Email objects for reviewers who haven't
        # not responded to this document in the past
        field.queryset = field.queryset.filter(
            role__name="reviewer", role__group=review_req.team
        ...
        if review_req.pk is not None:
            ...
            one_assignment = review_req.reviewassignment_set.exclude(
                state__slug__in=("rejected", "no-response")
            ).first()
        if one_assignment:
    ...
    def recommended_assignment_order(self, email_queryset, review_req):
        ...
        if review_req.team != self.team:
            raise ValueError(
                "Reviewer queue policy was passed a review request belonging to a different team."
            )
    ...
    def _filter_unavailable_reviewers(self, reviewers, review_req=None):
        ...
        available_reviewers = []
        if review_req:
            previous_reviewers = persons_with_previous_review(
                self.team, review_req, [r.pk for r in reviewers], "completed"
            )
        else:
        ...
        for reviewer in reviewers:
            ...
            keep = (current_periods is None) or (
                "canfinish" in [p.availability for p in current_periods]
                and reviewer.pk in previous_reviewers
            )
            if keep:
...
class AssignmentOrderResolver:
    ...
    def _collect_context(self):
        ...
        self.reviewer_settings = self._reviewer_settings_for_person_ids(
            self.possible_person_ids
        )
        self.days_needed_for_reviewers = (
            days_needed_to_fulfill_min_interval_for_reviewers(self.team)
        )
        self.connections = self._connections_with_doc(
            self.doc, self.possible_person_ids
        )
        self.unavailable_periods = current_unavailable_periods_for_reviewers(self.team)
        self.assignment_data_for_reviewers = latest_review_assignments_for_reviewers(
            self.team
        )
        self.unavailable_periods = current_unavailable_periods_for_reviewers(self.team)
        ...
        self.wish_to_review = set(
            ReviewWish.objects.filter(
                team=self.team, person__in=self.possible_person_ids, doc=self.doc
    ...
    def determine_ranking(self):
        ...
        """
        ranking = [
            self._ranking_for_email(e)
            for e in self.possible_emails
            if e.person_id in self.rotation_index
        ]
    ...
    def _ranking_for_email(self, email):
        ...
        add_boolean_score(
            -1,
            email.person_id in self.has_rejected_review_previous,
            "rejected review of document before",
        )
        add_boolean_score(
            +1,
            settings.request_assignment_next,
            "requested to be selected next for assignment",
        )
        add_boolean_score(
            +1,
            email.person_id in self.has_completed_review_previous,
            "reviewed document before",
        )
        add_boolean_score(
            +1, email.person_id in self.wish_to_review, "wishes to review document"
        )
        add_boolean_score(
            -1,
            email.person_id in self.connections,
            self.connections.get(email.person_id),
        )  # reviewer is somehow connected: bad
        add_boolean_score(
            -1,
            settings.filter_re and re.search(settings.filter_re, self.doc.name),
        ...
        if days_needed > 0:
            explanations.append(
                "max frequency exceeded, ready in {} {}".format(
                    days_needed, "day" if days_needed == 1 else "days"
                )
            )
        # skip next value
    ...
    def _collect_reviewer_stats(self, email):
        ...
        currently_open = sum(
            1 for d in assignment_data if d.state in ["assigned", "accepted"]
        )
        pages = sum(
            rd.doc_pages
            for rd in assignment_data
            if rd.state in ["assigned", "accepted"]
        )
        if currently_open > 0:
            stats.append(
                "currently {count} open, {pages} pages".format(
                    count=currently_open, pages=pages
                )
            )
        could_have_completed = [
            d
            for d in assignment_data
            if d.state in ["part-completed", "completed", "no-response"]
        ]
        if could_have_completed:
            ...
            part_completed = len(
                [d for d in assignment_data if d.state == "part-completed"]
            )
            if part_completed:
    ...
    def _connections_with_doc(self, doc, person_ids):
        ...
        for r in Role.objects.filter(
            group=doc.group_id, person__in=person_ids
        ).select_related("name"):
            connections[r.person_id] = "is group {}".format(r.name)
        if doc.shepherd:
            connections[doc.shepherd.person_id] = "is shepherd of document"
        for author in DocumentAuthor.objects.filter(
            document=doc, person__in=person_ids
        ).values_list("person", flat=True):
    ...
    def _reviewer_settings_for_person_ids(self, person_ids):
        reviewer_settings = {
            r.person_id: r
            for r in ReviewerSettings.objects.filter(
                team=self.team, person__in=person_ids
            )
        }
        for p in person_ids:
            if p not in reviewer_settings:
                reviewer_settings[p] = ReviewerSettings(
                    team=self.team, filter_re=get_default_filter_re(p)
                )
...
class RotateAlphabeticallyReviewerQueuePolicy(AbstractReviewerQueuePolicy):
    ...
    def default_reviewer_rotation_list(self, include_unavailable=False):
        ...
        next_reviewer_in_team = (
            NextReviewerInTeam.objects.filter(team=self.team)
            .select_related("next_reviewer")
            .first()
        )
    ...
    def _update_skip_next(self, rotation_pks, assignee_person):
        ...
        super(RotateAlphabeticallyReviewerQueuePolicy, self)._update_skip_next(
            rotation_pks, assignee_person
        )
        ...
        unfolded_rotation_pks = (
            unfolded_rotation_pks[assignee_index:]
            + unfolded_rotation_pks[:assignee_index]
        )
        ...
        rotation_settings = {
            ...
            for settings in self.team.reviewersettings_set.filter(
                person__in=unfolded_rotation_pks
            )
        }
        ...
        log.assertion(
            "next_reviewer_index is not None"
        )  # some entry in the list must have the minimum value
        bulk_update_with_history(changed, ReviewerSettings, ["skip_next"])

        next_reviewer_pk = unfolded_rotation_pks[next_reviewer_index]
        NextReviewerInTeam.objects.update_or_create(
            team=self.team, defaults=dict(next_reviewer_id=next_reviewer_pk)
        )
...
class LeastRecentlyUsedReviewerQueuePolicy(AbstractReviewerQueuePolicy):
    ...
    def default_reviewer_rotation_list(self, include_unavailable=False):
        ...
        assignments = (
            ...
            .values("reviewer__person")
            .annotate(most_recent=Max("assigned_on"))
            .order_by("most_recent")
        )

        reviewers_with_assignment = [
            reviewers_dict[assignment["reviewer__person"]] for assignment in assignments
        ]
