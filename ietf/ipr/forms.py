...
class AddCommentForm(forms.Form):
    ...
    private = forms.BooleanField(
        ...
        required=False,
        help_text="If this box is checked the comment will not appear in the disclosure's public history view.",
    )
...
class AddEmailForm(forms.Form):
    ...
    in_reply_to = MessageModelChoiceField(
        queryset=Message.objects, label="In Reply To", required=False
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.ipr:
            self.fields["in_reply_to"].queryset = Message.objects.filter(
                msgevents__disclosure__id=self.ipr.pk
            )
        ...
        for field in ("to", "from", "subject", "date"):
            if not message[field]:
                raise forms.ValidationError(
                    "Error parsing email: {} field not found.".format(field)
                )
    ...
    def clean(self):
        ...
        if in_reply_to:
            if direction != "incoming":
                raise forms.ValidationError(
                    "Only incoming messages can have In Reply To selected"
                )
            date = utc_from_string(message["date"])
            if date < in_reply_to.time:
                raise forms.ValidationError(
                    "The incoming message must have a date later than the message it is replying to"
                )
...
class DraftForm(forms.ModelForm):
    document = SearchableDocumentField(
        label="I-D name/RFC number", required=True, doc_type="all"
    )
    ...
    def clean(self):
        ...
        if not document:
            self.add_error(
                "document",
                "Identifying the Internet-Draft or RFC for this disclosure is required.",
            )
...
def validate_string(s, letter_min, digit_min, space_min, message):
    ...
    for c in s:
        ...
        if c.isspace():
            space_count += 1
    if not (
        letter_count >= letter_min
        and digit_count >= digit_min
        and space_count >= space_min
    ):
...
class GenericDisclosureForm(forms.Form):
    ...
    compliant = forms.BooleanField(
        label="This disclosure complies with RFC 3979", required=False
    )
    ...
    other_designations = forms.CharField(
        label="Designations for other contributions", max_length=255, required=False
    )
    holder_contact_name = forms.CharField(label="Name", max_length=255)
    holder_contact_email = forms.EmailField(label="Email")
    holder_contact_info = forms.CharField(
        label="Other Info (address, phone, etc.)",
        max_length=255,
        widget=forms.Textarea,
        required=False,
        strip=False,
    ...
    patent_number = forms.CharField(
        max_length=127,
        required=False,
        validators=[validate_patent_number],
        help_text=patent_number_help_text,
    )
    patent_inventor = forms.CharField(
        max_length=63,
        required=False,
        validators=[validate_name],
        help_text="Inventor name",
    )
    patent_title = forms.CharField(
        max_length=255,
        required=False,
        validators=[validate_title],
        help_text="Title of invention",
    )
    patent_date = DatepickerDateField(
        date_format="yyyy-mm-dd",
        required=False,
        help_text="Date granted or applied for",
    )
    patent_notes = forms.CharField(
        max_length=1024, required=False, widget=forms.Textarea
    )

    has_patent_pending = forms.BooleanField(required=False)
    statement = forms.CharField(
        max_length=2000, widget=forms.Textarea, required=False, strip=False
    )
    updates = SearchableIprDisclosuresField(
        required=False,
        help_text="If this disclosure <strong>updates</strong> other disclosures identify here which ones. Leave this field blank if this disclosure does not update any prior disclosures. <strong>Note</strong>: Updates to IPR disclosures must only be made by authorized representatives of the original submitters. Updates will automatically be forwarded to the current Patent Holder's Contact and to the Submitter of the original IPR disclosure.",
    )
    same_as_ii_above = forms.BooleanField(
        label="Same as in section II above", required=False
    )
    ...
    def clean(self):
        ...
        if not self.cleaned_data.get("same_as_ii_above"):
            if not (
                self.cleaned_data.get("submitter_name")
                and self.cleaned_data.get("submitter_email")
            ):
                raise forms.ValidationError(
                    "Submitter information must be provided in section VII"
                )
        ...
        if any(patent_values) and not all(patent_values):
            for k in patent_fields:
                if not cleaned_data.get(k):
                    self.add_error(
                        k,
                        "This field is required if you are filing a patent-specific disclosure.",
                    )
            raise forms.ValidationError(
                "A general IPR disclosure cannot have any patent-specific information, "
        ...
        patent_info = dict(
            [
                (k.replace("patent_", "").capitalize(), cleaned_data.get(k))
                for k in patent_fields
    ...
    def save(self, *args, **kwargs):
        ...
        for k in self.cleaned_data["patent_fields"] + [
            "patent_fields",
        ]:
            del nargs[k]
...
class IprDisclosureFormBase(forms.ModelForm):
    ...
    patent_date = DatepickerDateField(
        date_format="yyyy-mm-dd", required=True, help_text="Date granted or applied for"
    )
    patent_notes = forms.CharField(
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "licensing" in self.fields:
            self.fields["licensing_comments"].label = (
                "Licensing information, comments, notes, or URL for further information"
            )
        if "submitter_claims_all_terms_disclosed" in self.fields:
            self.fields["submitter_claims_all_terms_disclosed"].label = (
    ...
    def clean(self):
        ...
        patent_fields = [
            "patent_" + k for k in ["number", "inventor", "title", "date", "notes"]
        ]
...
class HolderIprDisclosureForm(IprDisclosureFormBase):
    ...
    licensing = CustomModelChoiceField(
        IprLicenseTypeName.objects.all(), widget=forms.RadioSelect, empty_label=None
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.pk:
            ...
            self.fields["licensing"].queryset = IprLicenseTypeName.objects.exclude(
                slug="none-selected"
            )
    ...
    def clean(self):
        ...
        if not self.data.get("iprdocrel_set-0-document") and not cleaned_data.get(
            "other_designations"
        ):
            raise forms.ValidationError(
                "You need to specify a contribution in Section IV"
            )
        return cleaned_data
...
class MessageModelForm(forms.ModelForm):
    response_due = DatepickerDateField(
        date_format="yyyy-mm-dd",
        picker_settings={"autoclose": "1"},
        required=False,
        help_text="The date which a response is due.",
    )
...
class ThirdPartyIprDisclosureForm(IprDisclosureFormBase):
    ...
    def clean(self):
        ...
        if not self.data.get("iprdocrel_set-0-document") and not cleaned_data.get(
            ...
            raise forms.ValidationError(
                "You need to specify a contribution in Section III"
            )
        return cleaned_data
...
class SearchForm(forms.Form):
    ...
    holder = forms.CharField(
        label="Name of patent owner/applicant", max_length=128, required=False
    )
    patent = forms.CharField(
        label="Text in patent information", max_length=128, required=False
    )
    group = GroupModelChoiceField(
        label="Working group",
        queryset=Group.objects.filter(type="wg").order_by("acronym"),
        required=False,
        empty_label="(Select WG)",
    )
    doctitle = forms.CharField(
        label="Words in document title", max_length=128, required=False
    )
    iprtitle = forms.CharField(
        label="Words in IPR disclosure title", max_length=128, required=False
    )

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["state"].choices = [("all", "All States")] + [
            (n.pk, n.name) for n in IprDisclosureStateName.objects.all()
        ]


class StateForm(forms.Form):
    state = forms.ModelChoiceField(
        queryset=IprDisclosureStateName.objects, label="New State", empty_label=None
    )
    comment = forms.CharField(
        required=False,
        widget=forms.Textarea,
        help_text="You may add a comment to be included in the disclosure history.",
        strip=False,
    )
    private = forms.BooleanField(
        label="Private comment",
