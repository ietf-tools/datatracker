...
class ProcessEmailTests(TestCase):
    ...
    def test_process_email(self, process_mock):
        ...
        self.assertEqual(
            process_mock.call_count, 1, "process_response_email should be called once"
        )
    ...
    def test_send_error_to_admin(self, process_mock, send_smtp_mock):
        ...
        self.assertEqual(
            msg["to"], "admin@example.com", "Admins should be emailed on error"
        )
        self.assertIn(
            "error", msg["subject"].lower(), "Error email subject should indicate error"
        )
        ...
        self.assertEqual(
            len(parts),
            3,
            "Error email should contain message, traceback, and original message",
        )
        ...
        self.assertIn(
            "RuntimeError", content, "Error type should be included in error email"
        )
        self.assertIn(
            "mock.py",
            content,
            "File where error occurred should be included in error email",
        )
        self.assertIn(
            "traceback",
            traceback.lower(),
            "Traceback should be attached to error email",
        )
        self.assertEqual(
            original, "contents", "Original message should be attached to error email"
        )
    ...
    def test_invalid_character_encodings(self, process_mock, send_smtp_mock):
        ...
        self.assertEqual(
            msg,
            invalid_characters,
            "Invalid unicode should be passed to process_email()",
        )
    ...
    def test_invalid_character_encodings_via_stdin(
        self, process_mock, send_smtp_mock, stdin_read_mock
    ):
