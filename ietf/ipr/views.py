...
from ietf.ipr.mail import (
    message_from_message,
    get_reply_to,
    get_update_submitter_emails,
)
from ietf.ipr.fields import select2_id_ipr_title_json
from ietf.ipr.forms import (
    HolderIprDisclosureForm,
    GenericDisclosureForm,
    ThirdPartyIprDisclosureForm,
    DraftForm,
    SearchForm,
    MessageModelForm,
    AddCommentForm,
    ...
    NotifyForm,
    StateForm,
    NonDocSpecificIprDisclosureForm,
...
from ietf.ipr.utils import get_genitive, get_ipr_summary, iprs_from_docs, related_docs
...
ipr_form_mapping = {
    "specific": HolderIprDisclosureForm,
...
def get_document_emails(ipr):
    ...
    for rel in ipr.iprdocrel_set.all():
        ...
        addrs = gather_address_lists("ipr_posted_on_doc", doc=doc).as_strings(
            compact=False
        )

        author_names = ", ".join(
            a.person.name for a in doc.documentauthor_set.select_related("person")
        )
...
def get_posted_emails(ipr):
    ...
    addrs = gather_address_lists("ipr_posting_confirmation", ipr=ipr).as_strings(
        compact=False
    )
    context = dict(
        settings=settings,
...
def set_disclosure_title(disclosure):
    ...
    if isinstance(disclosure, HolderIprDisclosure):
        ...
        title = get_genitive(
            disclosure.holder_legal_name
        ) + " Statement about IPR related to {}".format(ipr_summary)
    elif isinstance(disclosure, (GenericIprDisclosure, NonDocSpecificIprDisclosure)):
        title = (
            get_genitive(disclosure.holder_legal_name) + " General License Statement"
        )
    elif isinstance(disclosure, ThirdPartyIprDisclosure):
        ipr_summary = get_ipr_summary(disclosure)
        title = get_genitive(
            disclosure.ietfer_name
        ) + " Statement about IPR related to {} belonging to {}".format(
            ipr_summary, disclosure.holder_legal_name
        )
...
def ipr_rfc_number(disclosureDate, thirdPartyDisclosureFlag):
    ...
    if (
        (thirdPartyDisclosureFlag)
        and (rfcnum == "RFC 3979")
        and (disclosureDate > ipr_rfc_pub_datetime[4879])
    ):
...
def ajax_search(request):
    ...
    return HttpResponse(
        select2_id_ipr_title_json(objs), content_type="application/json"
    )
...
def admin(request, state):
    ...
    states = IprDisclosureStateName.objects.filter(
        slug__in=(
            [state, "rejected", "removed_objfalse"] if state == "removed" else [state]
        )
    )
    if not states:
        raise Http404
...
def edit(request, id, updates=None):
    ...
    DraftFormset = inlineformset_factory(
        IprDisclosureBase, IprDocRel, form=DraftForm, can_delete=True, extra=0
    )
    ...
    if request.method == "POST":
            ...
            if updates:
                for target in updates:
                    RelatedIpr.objects.create(
                        source=disclosure, target=target, relationship_id="updates"
                    )
        ...
        if ipr.updates:
            ...
            form = ipr_form_mapping[ipr.__class__.__name__](
                instance=ipr, initial=initial
            )
        else:
            form = ipr_form_mapping[ipr.__class__.__name__](
                instance=ipr, initial=initial
    ...
    return render(
        ...
        {"form": form, "draft_formset": draft_formset, "type": type},
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            IprEvent.objects.create(
                ...
                by=request.user.person,
                disclosure=ipr,
                response_due=datetime_from_date(
                    form.cleaned_data["response_due"], DEADLINE_TZINFO
                ),
                message=msg,
            )
        ...
        addrs = gather_address_lists("ipr_disclosure_followup", ipr=ipr).as_strings(
            compact=False
        )
        initial = {
            "to": addrs.to,
...
def notify(request, id, type):
    ...
    if request.method == "POST":
        ...
        if formset.is_valid():
            for form in formset.forms:
                ...
                IprEvent.objects.create(
                    ...
                    by=request.user.person,
                    disclosure=ipr,
                    response_due=datetime_today(DEADLINE_TZINFO)
        ...
        if type == "update":
            initial = [
                {"type": "update_notify", "text": escape(m)}
                for m in get_update_submitter_emails(ipr)
            ]
        else:
            initial = [
                {"type": "msgout", "text": escape(m)} for m in get_posted_emails(ipr)
            ]
...
def post(request, id):
    ...
    # create event
    IprEvent.objects.create(
        type_id="posted", by=person, disclosure=ipr, desc="Disclosure Posted"
    )
        ...
        if q or docid:
            ...
            if search_type in ["draft", "rfc"]:
                ...
                if docid:
                    ...
                    if search_type == "draft":
                        ...
                        start = Document.objects.filter(
                            name__icontains=q, name__startswith="draft"
                        )
                            ...
                            [
                                ...
                                for drafts in [
                                    related_docs(
                                        d,
                                        relationship=(),
                                        reverse_relationship=("became_rfc",),
                                    )
                                    for d in docs
                                ]
                                for draft in drafts
            ...
            # IPR list with documents
                ...
                iprs = IprDisclosureBase.objects.filter(
                    holder_legal_name__icontains=q, state_id__in=states
                )
            ...
            # IPR list with documents
                ...
                iprs = iprs.filter(
                    Q(holderiprdisclosure__patent_info__icontains=q)
            ...
            # Document list with IPRs
                ...
                docs = sorted(
                    docs,
                    key=lambda x: max([ipr.disclosure.time for ipr in x.ipr()]),
                    reverse=True,
                )
            ...
            # IPR list with documents
                ...
                iprs = IprDisclosureBase.objects.filter(
                    title__icontains=q, state_id__in=states
                )
...
def show(request, id):
    ...
    if not has_role(request.user, "Secretariat"):
        if ipr.state.slug in ["removed", "removed_objfalse"]:
            return render(request, "ipr/removed.html", {"ipr": ipr})
    ...
    return render(
        ...
        "ipr/details_view.html",
        {
            ...
            "choices_abc": [
                i.desc
                for i in IprLicenseTypeName.objects.filter(
                    slug__in=[
                        "no-license",
                        "royalty-free",
                        "reasonable",
            ...
            "updated_by_iprs": ipr.relatedipr_target_set.filter(
                source__state="posted"
            ).order_by("target__time"),
        },
    )
...
def showlist(request):
    ...
    generic = (
        GenericIprDisclosure.objects.filter(state__in=settings.PUBLISH_IPR_STATES)
        .prefetch_related(
            "relatedipr_source_set__target", "relatedipr_target_set__source"
        )
        .order_by("-time")
    )
    specific = (
        HolderIprDisclosure.objects.filter(state__in=settings.PUBLISH_IPR_STATES)
        .prefetch_related(
            "relatedipr_source_set__target", "relatedipr_target_set__source"
        )
        .order_by("-time")
    ...
    thirdpty = (
        ThirdPartyIprDisclosure.objects.filter(state__in=settings.PUBLISH_IPR_STATES)
        .prefetch_related(
            "relatedipr_source_set__target", "relatedipr_target_set__source"
        )
        .order_by("-time")
    ...
    nondocspecific = (
        NonDocSpecificIprDisclosure.objects.filter(
