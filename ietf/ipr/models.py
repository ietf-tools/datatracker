...
class IprDisclosureBase(models.Model):
    ...
    other_designations = models.CharField(
        "Designations for other contributions", blank=True, max_length=255
    )
    ...
    def get_absolute_url(self):
        return settings.IDTRACKER_BASE_URL + reverse(
            "ietf.ipr.views.show", kwargs={"id": self.id}
        )
    ...
    def latest_event(self, *args, **filter_args):
        ...
        e = (
            model.objects.filter(disclosure=self)
            .filter(**filter_args)
            .order_by("-time", "-id")[:1]
        )
...
class RelatedIpr(models.Model):
    ...
    relationship = ForeignKey(
        DocRelationshipName
    )  # Re-use; change to a dedicated RelName if needed
...
class IprEvent(models.Model):
    ...
    def response_past_due(self):
        ...
        received"""
        qs = IprEvent.objects.filter(
            disclosure=self.disclosure, in_reply_to=self.message
        )
        ...
        if self.type_id in event_type_map:
            for doc in self.disclosure.docs.distinct():
                DocEvent.objects.create(
                    ...
                    doc=doc,
                    rev="",
                    desc="%s related IPR disclosure <b>%s</b>"
                    % (self.type.name, self.disclosure.title),
                )
