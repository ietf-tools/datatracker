...
def render_message_for_history(msg):
    ...
    if isinstance(msg, Message):
        if msg.to.startswith("ietf-ipr+"):
            return format_html(
                "Date: {}<br>From: {}<br>Subject: {}<br>Cc: {}<br><br>{}",
                msg.time,
                msg.frm,
                msg.subject,
                msg.cc,
            ...
            return format_html(
                "Date: {}<br>From: {}<br>To: {}<br>Subject: {}<br>Cc: {}<br><br>{}",
                msg.time,
                msg.frm,
                msg.to,
                msg.subject,
...
def draft_rev_at_time(iprdocrel):
    ...
    if event is None:
        return (
            "",
            "The Internet-Draft's revision at the time this disclosure was posted could not be determined.",
        )
    time = event.time
    ...
    rev_event_before = (
        NewRevisionDocEvent.objects.filter(doc=draft, time__lte=time)
        .order_by("-time")
        .first()
    )
    if rev_event_before is None:
        return (
            "",
            "The Internet-Draft's initial submission was after this disclosure was posted.",
        )
    else:
...
def no_revisions_message(iprdocrel):
    ...
    first_line = (
        "No revisions for this Internet-Draft were specified in this disclosure."
    )
    ...
    if current_rev == "00":
        return (
            f"{first_line} However, there is only one revision of this Internet-Draft."
        )
