...
class SecrMeetingTestCase(TestCase):
    ...
    def test_view(self):
        ...
        self.assertEqual(
            {
                option.get("value")
                for option in q('#id_schedule_selector option:not([value=""])')
            },
            {"base", "test-schedule", "test-unofficial-schedule"},
        )
    ...
    def test_add_meeting(self):
        ...
        post_data = dict(
            ...
            idsubmit_cutoff_time_utc=datetime.timedelta(
    ...
    def test_add_meeting_default_conflict_types(self):
        ...
        meeting = MeetingFactory(
            type_id="ietf", group_conflicts=[]
        )  # start with no conflicts selected
        _run_test(meeting)

        # enable one
        meeting.group_conflict_types.add(
            ConstraintName.objects.filter(is_group_conflict=True).first()
        )
    ...
    def test_edit_meeting(self):
        ...
        post_data = dict(
            ...
            group_conflict_types=[
                cn.slug
                for cn in ConstraintName.objects.filter(is_group_conflict=True).exclude(
                    meeting=meeting,  # replace original set with those not assigned to the meeting
                )
            ],
        )
        self.assertGreater(
            len(post_data["group_conflict_types"]), 0
        )  # test should include at least one
        self.client.login(username="secretary", password="secretary+password")
        response = self.client.post(url, post_data, follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_notifications(self):
        ...
        assert (
            SchedulingEvent.objects.filter(session=ames_stsa.session)
            .order_by("-id")[0]
            .status_id
            == "schedw"
        )
        ...
        SchedulingEvent.objects.create(
            session=mars_stsa.session,
            status=SessionStatusName.objects.get(slug="appr"),
            by=Person.objects.get(name="(System)"),
        )
        url = reverse(
            "ietf.secr.meetings.views.notifications", kwargs={"meeting_id": 72}
        )
        self.client.login(username="secretary", password="secretary+password")
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        ...
        mars_stsa = meeting.schedule.assignments.get(session__group=mars_group)
        self.assertEqual(
            SchedulingEvent.objects.filter(session=mars_stsa.session)
            .order_by("-id")[0]
            .status_id,
            "sched",
        )
    ...
    def test_meetings_rooms(self):
        ...
        url = reverse(
            ...
            kwargs={"meeting_id": 72, "schedule_name": "test-schedule"},
        )
        self.client.login(username="secretary", password="secretary+password")
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        q = PyQuery(response.content)
        self.assertEqual(
            len(q("#id_rooms_table tr input[type='checkbox']")),
            meeting.room_set.count() + 1,
        )
        ...
        SchedTimeSessAssignment.objects.filter(
            schedule__in=[
                meeting.schedule,
                meeting.schedule.base,
                meeting.unofficial_schedule,
            ]
        ).delete()
        self.client.login(username="secretary", password="secretary+password")
        post_dict = {
    ...
    def test_meetings_times_edit(self):
        ...
        url = reverse(
            ...
            kwargs={
                ...
                "time": timeslot.time.astimezone(meeting.tz()).strftime(
                    "%Y:%m:%d:%H:%M"
                ),
            },
        )
        self.client.login(username="secretary", password="secretary+password")
        response = self.client.post(
            url, {"day": "1", "time": "08:00", "duration": "09:00", "name": "Testing"}
        )
        self.assertEqual(response.status_code, 302)
    ...
    def test_meetings_misc_session_edit(self):
        ...
        session = Session.objects.get(
            pk=session.pk
        )  # get a clean instance to avoid cache problems
        timeslot = session.official_timeslotassignment().timeslot
    ...
    def test_meetings_misc_session_delete(self):
        ...
        target = reverse(
            "ietf.secr.meetings.views.misc_sessions",
            kwargs={"meeting_id": meeting.number, "schedule_name": schedule.name},
        )
        self.client.login(username="secretary", password="secretary+password")
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_get_times(self):
        ...
        day = (
            timeslot.time.weekday() + 1
        ) % 7 + 1  # fix up to match django __week_day filter
