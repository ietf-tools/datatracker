...
from ietf.secr.meetings.forms import (
    BaseMeetingRoomFormSet,
    MeetingModelForm,
    MeetingSelectForm,
    MeetingRoomForm,
    MiscSessionForm,
    TimeSlotForm,
    RegularSessionEditForm,
...
def check_misc_sessions(meeting, schedule):
    ...
    slots = TimeSlot.objects.filter(
        meeting=meeting,
        type__in=("break", "reg", "other", "plenary", "lead", "offagenda"),
    )
    ...
    if plenary:
        ...
        if not assignments.filter(schedule=schedule):
            ...
            for ss in SchedTimeSessAssignment.objects.filter(
                schedule=source, timeslot__in=slots
            ):
                SchedTimeSessAssignment.objects.create(
                    schedule=schedule, session=ss.session, timeslot=ss.timeslot
                )
...
def send_notifications(meeting, groups, person):
    ...
    for group in groups:
        ...
        addrs = gather_address_lists(
            "session_scheduled", group=group, session=sessions[0]
        )
        from_email = ('"IETF Secretariat"', "agenda@ietf.org")
        if len(sessions) == 1:
            subject = "%s - Requested session has been scheduled for IETF %s" % (
                group.acronym,
                meeting.number,
            )
        else:
            subject = "%s - Requested sessions have been scheduled for IETF %s" % (
                group.acronym,
                meeting.number,
            )
        ...
        items = [
            {"session": s, "timeslot": get_timeslot(s)}
            for s in sessions
            if get_timeslot(s)
        ]
        ...
        for i, d in enumerate(items):
            ...
            items[i][
                "period"
            ] = f"{t.local_start_time().strftime('%H%M')}-{t.local_end_time().strftime('%H%M')} {t.tz()}"

        # send email
        first_event = (
            SchedulingEvent.objects.filter(session=sessions[0])
            .select_related("by")
            .order_by("time", "id")
            .first()
        )
        ...
        send_mail(None, addrs.to, from_email, subject, template, context, cc=addrs.cc)
        ...
        GroupEvent.objects.create(
            ...
            by=person,
            desc="sent scheduled notification for %s" % meeting,
        )
    ...
    * proceedingform
    ...
    if request.method == "POST":
        ...
        if last_ietf_meeting is not None:
            ...
            initial["group_conflict_types"] = [
                cn.pk for cn in last_ietf_meeting.group_conflict_types.all()
            ]
...
def main(request):
    ...
    if request.method == "POST":
        return redirect(
            "ietf.secr.meetings.views.view", meeting_id=request.POST["meeting"]
        )
...
def misc_sessions(request, meeting_id, schedule_name):
    ...
    assignments = SchedTimeSessAssignment.objects.filter(
        schedule__in=[schedule, schedule.base], timeslot__type__in=misc_session_types
    )
    ...
    no_room = TimeSlot.objects.filter(
        meeting=meeting, type="other", location__isnull=True
    )
    if no_room:
        messages.warning(
            request,
            "There are misc. session timeslots which do not have a room assigned",
        )

    session_statuses = {
        e.session_id: e.status_id
        for e in SchedulingEvent.objects.filter(
            session__in=[a.session_id for a in assignments]
        ).order_by("time", "id")
    }
...
def misc_session_cancel(request, meeting_id, schedule_name, slot_id):
    ...
    if request.method == "POST" and request.POST["post"] == "yes":
        for session in Session.objects.filter(
            timeslotassignments__schedule=schedule, timeslotassignments__timeslot=slot
        ):
            SchedulingEvent.objects.create(
                session=session,
    ...
    if request.method == "POST" and request.POST["post"] == "yes":
        ...
        for session in session_objects:
            if session.materials.exclude(states__slug="deleted"):
                messages.error(
                    request,
                    'Materials have already been uploaded for "%s".  You must delete those before deleting the timeslot.'
                    % slot.name,
                )
                return redirect(
                    "ietf.secr.meetings.views.misc_sessions",
...
def misc_session_edit(request, meeting_id, schedule_name, slot_id):
    ...
    return render(
        request,
        "meetings/misc_session_edit.html",
        {"meeting": meeting, "form": form, "schedule": schedule, "slot": slot},
    )
...
def notifications(request, meeting_id):
    ...
    if request.method == "POST":
        ...
        sessions = add_event_info_to_session_qs(
            Session.objects.filter(timeslotassignments__schedule=meeting.schedule_id)
        ).filter(current_status__in=["schedw", "appr"])
...
def rooms(request, meeting_id, schedule_name):
    ...
    RoomFormset = inlineformset_factory(
        ...
        formset=BaseMeetingRoomFormSet,
        can_delete=True,
        extra=extra,
    )
...
def regular_session_edit(request, meeting_id, schedule_name, session_id):
    ...
    return render(
        request,
        "meetings/regular_session_edit.html",
        {
            "meeting": meeting,
            "schedule": schedule,
            "session": session,
...
def times(request, meeting_id, schedule_name):
    ...
    for t in meeting.timeslot_set.filter(type="regular"):
        if not t.time in time_seen:
            ...
            timeslots.append(t)
    for t in timeslots:
        slots.append({"name": t.name, "time": t.time, "end_time": t.end_time()})
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if time in time_seen:
                messages.error(
                    request,
                    "There is already a timeslot for %s.  To change you must delete the old one first."
                    % time.strftime("%a %H:%M"),
                )
                return redirect(
...
def times_edit(request, meeting_id, schedule_name, time):
    ...
    return render(
        request,
        "meetings/times_edit.html",
        {"meeting": meeting, "schedule": schedule, "form": form},
    )
