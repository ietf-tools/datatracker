...
from ietf.secr.telechat.forms import (
    BallotForm,
    ChangeStateForm,
    DateSelectForm,
    TELECHAT_TAGS,
...
def doc(request, date):
    ...
    if doc:
        ...
        return render(
            request,
            "telechat/doc.html",
            {"agenda": agenda, "date": date, "document": None},
        )
...
def doc_detail(request, date, name):
    ...
    if doc.active_ballot():
        ballots = (
            doc.active_ballot().active_balloter_positions()
        ...
        initial_ballot.append(
            {
                "name": key.name,
                "id": key.id,
                "position": ballots[key].pos.slug if ballots[key] else None,
            }
        )
    ...
    BallotFormset.form.__init__ = partialmethod(
        BallotForm.__init__, ballot_type=ballot_type
    )
    ...
    if request.method == "POST":
            ...
            for form in formset.forms:
                if form.is_valid() and form.changed_data:
                    ...
                    if form.initial["position"] == None:
                        pos.desc = (
                            "[Ballot Position Update] New position, %s, has been recorded for %s by %s"
                            % (pos.pos.name, balloter.name, login.name)
                        )
                    else:
                        pos.desc = (
                            "[Ballot Position Update] Position for %s has been changed to %s by %s"
                            % (balloter.name, pos.pos.name, login.name)
                        )
                    pos.save()
                    has_changed = True
        ...
        # logic from doc/views_draft.py change_state
            ...
            if state_form.is_valid():
                ...
                return redirect(
                    "ietf.secr.telechat.views.doc_detail", date=date, name=name
                )
    else:
        formset = BallotFormset(initial=initial_ballot)
        state_form = ChangeStateForm(initial=initial_state)
...
def main(request):
    ...
    choices = [
        ...
        for d in TelechatDate.objects.all()
    ]
...
def management(request, date):
    ...
    return render(
        ...
        {"agenda": agenda, "date": date, "issues": issues},
    )
...
def minutes(request, date):
    ...
    events = DocEvent.objects.filter(
        type="iesg_approved", time__gte=previous, time__lt=current, doc__type="draft"
    )
    docs = [e.doc for e in events]
    pa_docs = [
        doc for doc in docs if doc.intended_std_level.slug not in ("inf", "exp", "hist")
    ]
    da_docs = [
        doc for doc in docs if doc.intended_std_level.slug in ("inf", "exp", "hist")
    ]
...
def roll_call(request, date):
    ...
    return render(
        ...
        {"agenda": agenda, "date": date, "people": sorted_ads},
    )
