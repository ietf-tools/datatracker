...
class EmailForm(forms.ModelForm):
    ...
    def clean_origin(self):
        validate_email = EmailValidator(
            "Please provide the origin of the new email: A valid user email if provided by email, or 'author: doc' or 'role: role spec'."
        )
        if "origin" in self.changed_data and self.instance.origin:
            raise forms.ValidationError(
                "You may not change existing origin fields, only set the value when empty"
            )
        ...
        if ":" in origin:
            ...
            if not tag in valid_tags:
                raise forms.ValidationError(
                    "Invalid tag.  Valid tags are: %s" % ",".join(valid_tags)
                )
            if tag == "author":
                if not Document.objects.filter(name=value).exists():
                    raise forms.ValidationError(
                        "Invalid document: %s. A valid document is required with 'author:'"
                        % value
                    )
            elif tag == "role":
                if not " " in value:
                    raise forms.ValidationError(
                        "Invalid role spec: %s.  Please indicate 'group role'." % value
                    )
                acronym, slug = value.split(None, 1)
                if not Group.objects.filter(acronym=acronym).exists():
                    raise forms.ValidationError(
                        "Invalid group: %s. A valid 'group role' string is required with 'role:'"
                        % acronym
                    )
                if not RoleName.objects.filter(slug=slug).exists():
                    roles = RoleName.objects.values_list("slug", flat=True)
                    raise forms.ValidationError(
                        "Invalid role: %s. A valid 'group role' string is required with 'role:'.\n  Valid roles are: %s"
                        % (slug, ", ".join(roles))
                    )
        else:
...
class EditPersonForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["user"] = forms.CharField(
            max_length=64,
            required=False,
            help_text="Corresponds to Django User ID (usually email address)",
        )
...
class NameForm(forms.Form):
    ...
    def clean_name(self):
        ...
        if not r1.match(name):
            raise forms.ValidationError(
                "Enter a valid name. (only letters,period,hyphen,paren,numerals 2 and 3 allowed)"
            )
        return name
...
class NewPersonForm(forms.ModelForm):
    ...
    def clean_email(self):
        ...
        try:
            ...
            if user and person:
                raise forms.ValidationError(
                    "This account already exists. [name=%s, id=%s, email=%s]"
                    % (person.name, person.id, email)
                )
