    ...
    for group in items:
        if group == source_group.acronym:
            raise forms.ValidationError(
                "Cannot declare a conflict with the same group: %s" % group
            )
...
class SessionForm(forms.Form):
    ...
    session_time_relation = forms.ChoiceField(
        choices=SESSION_TIME_RELATION_CHOICES, required=False
    )
    ...
    joint_with_groups_selector = forms.ChoiceField(
        choices=[], required=False
    )  # group select widget for prev field
    joint_for_session = forms.ChoiceField(
        choices=JOINT_FOR_SESSION_CHOICES, required=False
    )
    ...
    send_notifications = forms.BooleanField(
        label="Send notification emails?", required=False, initial=False
    )
    ...
    def __init__(self, group, meeting, data=None, *args, **kwargs):
        ...
        formset_class = sessiondetailsformset_factory(
            max_num=3 if group.features.acts_like_wg else 50
        )
        ...
        self.fields["adjacent_with_wg"].choices = [
            ("", "--No preference")
        ] + other_groups
        ...
        for constraintname in meeting.group_conflict_types.all():
            ...
            constraint_field.widget.attrs["data-constraint-name"] = str(
                constraintname
            ).title()
            self._add_widget_class(constraint_field.widget, "wg_constraint")

            selector_field = forms.ChoiceField(
                choices=group_acronym_choices, required=False
            )
            selector_field.widget.attrs["data-slug"] = (
                constraintname.slug
            )  # used by onchange handler
        ...
        inactive_cnames = (
            ...
            .exclude(meeting=meeting)  # ...that are not enabled for this meeting...
            .filter(
        ...
        for inactive_constraint_name in inactive_cnames:
            ...
            self.fields[field_id] = forms.BooleanField(
                required=False,
                label="Delete this conflict",
                help_text="Delete this inactive conflict?",
            )
            constraints = group.constraint_source_set.filter(
                meeting=meeting, name=inactive_constraint_name
            )
        ...
        self.fields["joint_with_groups_selector"].widget.attrs[
            "onchange"
        ] = "document.form_post.joint_with_groups.value=document.form_post.joint_with_groups.value + ' ' + this.options[this.selectedIndex].value; return 1;"
        self.fields["resources"].choices = [
            (x.pk, x.desc)
            for x in ResourceAssociation.objects.filter(name__used=True).order_by(
                "name__order"
            )
        ]
    ...
    def _validate_duplicate_conflicts(self, cleaned_data):
        ...
        all_conflicts = self._join_conflicts(
            cleaned_data, ["conflict", "conflic2", "conflic3"]
        )
        seen = []
        duplicated = []
        errors = []
        for c in all_conflicts:
            if c not in seen:
                ...
                duplicated.append(c)
                errors.append(
                    forms.ValidationError("%s appears in conflicts more than once" % c)
                )
        return errors
    ...
    def clean_send_notifications(self):
        return (
            True
            if not self.notifications_optional
            else self.cleaned_data["send_notifications"]
        )
            ...
            if data.get("session_time_relation"):
                self.add_error(
                    ...
                    forms.ValidationError(
                        "Time between sessions can only be used when two sessions are requested."
                    ),
                )
    ...
    def media(self):
        ...
        return (
            super().media
            + self.session_forms.media
            + forms.Media(js=("secr/js/session_form.js",))
        )
...
class ToolStatusForm(forms.Form):
    message = forms.CharField(
        widget=forms.Textarea(attrs={"rows": "3", "cols": "80"}), strip=False
    )
