...
from ietf.secr.sreq.forms import (
    SessionForm,
    ToolStatusForm,
...
def get_initial_session(sessions, prune_conflicts=False):
    ...
    constraints = group.constraint_source_set.filter(
        meeting=meeting
    )  # all constraints with this group as source
    conflicts = constraints.filter(
        name__is_group_conflict=True
    )  # only the group conflict constraints
    ...
    def valid_conflict(conflict):
        return (
            conflict.target != sessions[0].group
            and allowed_conflicting_groups().filter(pk=conflict.target_id).exists()
        )
    ...
    initial["session_time_relation"] = (
        time_relation[0].time_relation if time_relation else None
    )
    initial["session_time_relation_display"] = (
        time_relation[0].get_time_relation_display if time_relation else None
    )
    ...
    for idx, session in enumerate(sessions):
        if session.joint_with_groups.count():
            initial["joint_with_groups"] = " ".join(
                session.joint_with_groups_acronyms()
            )
            initial["joint_for_session"] = str(idx + 1)
            initial["joint_for_session_display"] = dict(JOINT_FOR_SESSION_CHOICES)[
                initial["joint_for_session"]
            ]
    return initial
...
def save_conflicts(group, meeting, conflicts, name):
    ...
    for acronym in acronyms:
        ...
        constraint = Constraint(
            source=group, target=target, meeting=meeting, name=constraint_name
        )
        constraint.save()
...
def send_notification(group, meeting, login, sreq_data, session_data, action):
    ...
    (to_email, cc_list) = gather_address_lists(
        "session_requested", group=group, person=login
    )
    from_email = settings.SESSION_REQUEST_FROM_EMAIL
    subject = "%s - New Meeting Session Request for IETF %s" % (
        group.acronym,
        meeting.number,
    )
    ...
    # update overrides
    if action == "update":
        subject = "%s - Update to a Meeting Session Request for IETF %s" % (
            group.acronym,
            meeting.number,
        )
    ...
    if len(session_data) > 2:
        (to_email, cc_list) = gather_address_lists(
            "session_requested_long", group=group, person=login
        )
        subject = "%s - Request for meeting session approval for IETF %s" % (
            group.acronym,
            meeting.number,
        )
    ...
    send_mail(None, to_email, from_email, subject, template, context, cc=cc_list)
...
def inbound_session_conflicts_as_string(group, meeting):
    ...
    """
    constraints = group.constraint_target_set.filter(
        meeting=meeting, name__is_group_conflict=True
    )
    group_set = set(
        constraints.values_list("source__acronym", flat=True)
    )  # set to de-dupe
...
def approve(request, acronym):
    ...
    session = (
        add_event_info_to_session_qs(
            Session.objects.filter(meeting=meeting, group=group)
        )
    ...
    if has_role(request.user, "Secretariat") or group.parent.role_set.filter(
        name="ad", person=request.user.person
    ):
...
def cancel(request, acronym):
    ...
    # send notifitcation
    (to_email, cc_list) = gather_address_lists(
        "session_request_cancelled", group=group, person=login
    )
    from_email = settings.SESSION_REQUEST_FROM_EMAIL
    subject = "%s - Cancelling a meeting request for IETF %s" % (
        group.acronym,
        meeting.number,
    )
    send_mail(
        ...
        from_email,
        subject,
        "sreq/session_cancel_notification.txt",
        {"requester": get_requester_text(login, group), "meeting": meeting},
        cc=cc_list,
    )

    messages.success(
        request, "The %s Session Request has been cancelled" % group.acronym
    )
...
def confirm(request, acronym):
    ...
    form = SessionForm(
        ...
        request.POST,
        hidden=True,
        notifications_optional=has_role(request.user, "Secretariat"),
    )
    form.is_valid()
    ...
    if add_event_info_to_session_qs(
        Session.objects.filter(group=group, meeting=meeting)
    ).filter(
        Q(current_status__isnull=True) | ~Q(current_status__in=["deleted", "notmeet"])
    ):
        messages.warning(
            request,
            "Sessions for working group %s have already been requested once."
            % group.acronym,
        )
        return redirect("ietf.secr.sreq.views.main")
    ...
    session_data["bethere"] = (
        form.cleaned_data["bethere"] if "bethere" in form.cleaned_data else []
    )
    if session_data.get("session_time_relation"):
        session_data["session_time_relation_display"] = dict(
            Constraint.TIME_RELATION_CHOICES
        )[session_data["session_time_relation"]]
    if session_data.get("joint_for_session"):
        session_data["joint_for_session_display"] = dict(JOINT_FOR_SESSION_CHOICES)[
            session_data["joint_for_session"]
        ]
    if form.cleaned_data.get("timeranges"):
        session_data["timeranges_display"] = [
            t.desc for t in form.cleaned_data["timeranges"]
        ]
    session_data["resources"] = [
        ResourceAssociation.objects.get(pk=pk)
        for pk in request.POST.getlist("resources")
    ]
    ...
    if request.method == "POST" and button_text == "Submit":
        ...
        add_event_info_to_session_qs(
            Session.objects.filter(group=group, meeting=meeting)
        ).filter(current_status__in=["canceled", "notmeet"]).delete()
        num_sessions = int(form.cleaned_data["num_session"]) + (
            1 if form.cleaned_data["third_session"] else 0
        )
        ...
        for count, sess_form in enumerate(form.session_forms[:num_sessions]):
            ...
            SchedulingEvent.objects.create(
                session=new_session,
                status=SessionStatusName.objects.get(
                    slug=status_slug_for_new_session(new_session, count)
                ),
                by=login,
            )
            ...
            if int(jfs) == count + 1:  # count is zero-indexed
                groups_split = (
                    form.cleaned_data.get("joint_with_groups").replace(",", " ").split()
                )
        ...
        for conflictname, cfield_id in form.wg_constraint_field_ids():
            save_conflicts(
                group, meeting, form.data.get(cfield_id, ""), conflictname.slug
            )
        save_conflicts(
            group, meeting, form.data.get("adjacent_with_wg", ""), "wg_adjacent"
        )
        ...
        if form.cleaned_data.get("timeranges"):
            ...
            constraint = Constraint.objects.create(
                source=group, meeting=meeting, name=cn
            )
        ...
        if "bethere" in form.data:
            ...
            for p in session_data["bethere"]:
                Constraint.objects.create(
                    name=bethere_cn, source=group, person=p, meeting=new_session.meeting
                )

        # clear not meeting
        add_event_info_to_session_qs(
            Session.objects.filter(group=group, meeting=meeting)
        ).filter(current_status="notmeet").delete()

        # send notification
        if form.cleaned_data.get("send_notifications"):
            session_data["outbound_conflicts"] = [
                f"{d['name']}: {d['groups']}" for d in outbound_conflicts
            ]
            send_notification(
                group,
                meeting,
...
def session_changed(session):
    ...
    if (
        latest_event
        and latest_event.status_id == "schedw"
        and session.meeting.schedule != None
    ):
...
def edit(request, acronym, num=None):
    ...
    sessions = (
        ...
        .filter(
            ...
            | ~Q(current_status__in=["canceled", "notmeet", "deleted"])
        )
        .order_by("id")
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            if form.has_changed():
                changed_session_forms = [
                    sf for sf in form.session_forms.forms_to_keep if sf.has_changed()
                ]
                form.session_forms.save()
                ...
                sessions = (
                    ...
                    .filter(
                        Q(current_status__isnull=True)
                        | ~Q(current_status__in=["canceled", "notmeet"])
                    )
                    .order_by("id")
                )

                if (
                    "joint_with_groups" in form.changed_data
                    or "joint_for_session" in form.changed_data
                ):
                    joint_with_groups_list = (
                        form.cleaned_data.get("joint_with_groups")
                        .replace(",", " ")
                        .split()
                    )
                    new_joint_with_groups = Group.objects.filter(
                        acronym__in=joint_with_groups_list
                    )
                    new_joint_for_session_idx = (
                        int(form.data.get("joint_for_session", "-1")) - 1
                    )
                    ...
                    if (
                        current_joint_with_groups != new_joint_with_groups
                        or current_joint_for_session_idx != new_joint_for_session_idx
                    ):
                        if current_joint_for_session_idx is not None:
                            sessions[
                                current_joint_for_session_idx
                            ].joint_with_groups.clear()
                            session_changed(sessions[current_joint_for_session_idx])
                        sessions[new_joint_for_session_idx].joint_with_groups.set(
                            new_joint_with_groups
                        )
                ...
                for cname, cfield_id in form.wg_constraint_field_ids():
                    if cfield_id in form.changed_data:
                        Constraint.objects.filter(
                            meeting=meeting, source=group, name=cname.slug
                        ).delete()
                        save_conflicts(
                            group, meeting, form.cleaned_data[cfield_id], cname.slug
                        )
                ...
                if "adjacent_with_wg" in form.changed_data:
                    Constraint.objects.filter(
                        meeting=meeting, source=group, name="wg_adjacent"
                    ).delete()
                    save_conflicts(
                        group,
                        meeting,
                        form.cleaned_data["adjacent_with_wg"],
                        "wg_adjacent",
                    )
                ...
                if "resources" in form.changed_data:
                    ...
                    new_resources = [
                        ResourceAssociation.objects.get(pk=a) for a in new_resource_ids
                    ]
                    first_session.resources = new_resources

                if "bethere" in form.changed_data and set(
                    form.cleaned_data["bethere"]
                ) != set(initial["bethere"]):
                    ...
                    for p in form.cleaned_data["bethere"]:
                        Constraint.objects.create(
                            ...
                            source=group,
                            person=p,
                            meeting=first_session.meeting,
                        )

                if "session_time_relation" in form.changed_data:
                    Constraint.objects.filter(
                        meeting=meeting, source=group, name="time_relation"
                    ).delete()
                ...
                if "timeranges" in form.changed_data:
                    Constraint.objects.filter(
                        meeting=meeting, source=group, name="timerange"
                    ).delete()
                ...
                if form.cleaned_data.get("send_notifications"):
                    ...
                    )  # do not add things to the original cleaned_data
                    session_data["outbound_conflicts"] = [
                        f"{d['name']}: {d['groups']}" for d in outbound_conflicts
                    ]
                    send_notification(
        ...
        for obc in group.constraint_source_set.filter(
            meeting=meeting, name__is_group_conflict=True
        ):
...
def main(request):
    ...
    group_types = GroupFeatures.objects.filter(has_meetings=True).values_list(
        "type", flat=True
    )

    my_groups = [
        g
        for g in get_my_groups(request.user, conclude=True)
        if g.type_id in group_types
    ]

    sessions_by_group = defaultdict(list)
    for s in add_event_info_to_session_qs(
        Session.objects.filter(meeting=meeting, group__in=my_groups)
    ).filter(current_status__in=["schedw", "apprw", "appr", "sched"]):
    ...
    if not scheduled_groups and not unscheduled_groups:
        messages.warning(
            request,
            "The account %s is not associated with any groups.  If you have multiple Datatracker accounts you may try another or report a problem to %s"
            % (request.user, settings.SECRETARIAT_ACTION_EMAIL),
        )
    ...
    for group in scheduled_groups:
        if not group.features.acts_like_wg or (len(group.meeting_sessions) < 3):
            ...
            group.status_message = "First two sessions: %s, Third session: %s" % (
                group.meeting_sessions[0].current_status,
                group.meeting_sessions[2].current_status,
            )
...
def new(request, acronym):
    ...
    # pre-populated with data from last meeeting's session request
        ...
        latest_session = (
            add_event_info_to_session_qs(
                Session.objects.filter(meeting__type_id="ietf", group=group)
            )
            .exclude(
                current_status__in=[
                    "notmeet",
        ...
        if latest_session:
            ...
            previous_sessions = (
                add_event_info_to_session_qs(
                    Session.objects.filter(meeting=previous_meeting, group=group)
                )
                .exclude(current_status__in=["notmeet", "deleted"])
                .order_by("id")
            )
            if not previous_sessions:
                messages.warning(
                    request, "This group did not meet at %s" % previous_meeting
                )
                return redirect("ietf.secr.sreq.views.new", acronym=acronym)
            else:
                messages.info(
                    request, "Fetched session info from %s" % previous_meeting
                )
        else:
...
def no_session(request, acronym):
    ...
    add_event_info_to_session_qs(
        Session.objects.filter(group=group, meeting=meeting)
    ).filter(current_status="canceled").delete()

    # skip if state is already notmeet
    if add_event_info_to_session_qs(
        Session.objects.filter(group=group, meeting=meeting)
    ).filter(current_status="notmeet"):
        messages.info(
            request, "The group %s is already marked as not meeting" % group.acronym
        )
        return redirect("ietf.secr.sreq.views.main")
    ...
    # send notification
    (to_email, cc_list) = gather_address_lists(
        "session_request_not_meeting", group=group, person=login
    )
    from_email = settings.SESSION_REQUEST_FROM_EMAIL
    ...
    send_mail(
        ...
        from_email,
        subject,
        "sreq/not_meeting_notification.txt",
        {"login": login, "group": group, "meeting": meeting},
        cc=cc_list,
    )
    ...
    messages.success(
        request,
        "A message was sent to notify not having a session at IETF %s" % meeting.number,
    )
    return redirect("ietf.secr.sreq.views.main")
...
def tool_status(request):
    ...
    return render(
        request,
        "sreq/tool_status.html",
        {"is_locked": is_locked, "form": form},
    )
...
def view(request, acronym, num=None):
    ...
    session_conflicts = dict(
        inbound=inbound_session_conflicts_as_string(group, meeting),
        outbound=[
            dict(name=ConstraintName.objects.get(slug=slug), groups=" ".join(groups))
