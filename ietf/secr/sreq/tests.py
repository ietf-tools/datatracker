...
class SessionRequestTestCase(TestCase):
    ...
    def test_approve(self):
        ...
        self.assertRedirects(
            r, reverse("ietf.secr.sreq.views.view", kwargs={"acronym": "mars"})
        )
        self.assertEqual(
    ...
    def test_cancel_notification_msg(self):
        ...
        self.assertEqual(
            requester.name, "James O'Rourke"
        )  # note ' (single quote) in the name
        self.assertIn(
    ...
    def test_edit(self):
        ...
        sessions = Session.objects.filter(meeting=meeting, group=mars).order_by(
            "id"
        )  # order to match edit() view
        self.assertEqual(len(sessions), 2)
        session = sessions[0]

        self.assertEqual(
            session.constraints().get(name="chair_conflict").target.acronym,
            iabprog.acronym,
        )
        self.assertEqual(
            session.constraints().get(name="time_relation").time_relation,
            "subsequent-days",
        )
        self.assertEqual(
            session.constraints().get(name="wg_adjacent").target.acronym, group2.acronym
        )
        self.assertEqual(
            list(
                session.constraints()
            ...
            list(
                TimerangeName.objects.filter(
                    name__in=["thursday-afternoon-early", "thursday-afternoon-late"]
                ).values("name")
            ),
        )
        ...
        self.assertRegex(
            r.content.decode(),
            r"Second session with: ({} {}|{} {})".format(
                group3.acronym, group4.acronym, group4.acronym, group3.acronym
            ),
        )
    ...
    def test_edit_inactive_conflicts(self):
        ...
        meeting = MeetingFactory(
            type_id="ietf", date=date_today(), group_conflicts=["chair_conflict"]
        )
        mars = RoleFactory(
            name_id="chair",
            person__user__username="marschairman",
        ...
        self.assertIn(
            other_group.acronym, delete_checkbox.find('../input[@type="text"]').value
        )
...
class SubmitRequestCase(TestCase):
    ...
    def test_submit_request(self):
        ...
        session_count_before = Session.objects.filter(
            meeting=meeting, group=group
        ).count()
        url = reverse("ietf.secr.sreq.views.new", kwargs={"acronym": group.acronym})
        confirm_url = reverse(
            "ietf.secr.sreq.views.confirm", kwargs={"acronym": group.acronym}
        )
        ...
        self.assertContains(r, group2.acronym)
        self.assertContains(
            r, "First session with: {} {}".format(group3.acronym, group4.acronym)
        )
        ...
        session_count_after = Session.objects.filter(
            meeting=meeting, group=group, type="regular"
        ).count()
        ...
        self.assertEqual(
            set(list(session.joint_with_groups.all())), set([group3, group4])
        )
    ...
    def test_submit_request_check_constraints(self):
        m1 = MeetingFactory(
            type_id="ietf", date=date_today() - datetime.timedelta(days=100)
        )
        MeetingFactory(
            type_id="ietf",
            date=date_today(),
    ...
    def test_request_notification(self):
        ...
        self.assertTrue(
            "Schedule the sessions on subsequent days" in notification_payload
        )
        self.assertTrue(group2.acronym in notification_payload)
        self.assertTrue(
            "Can't meet: Thursday early afternoon, Thursday late"
            in notification_payload
        )
        self.assertTrue(
            "Second session joint with: {}".format(group3.acronym)
            in notification_payload
        )
        self.assertTrue(ad.ascii_name() in notification_payload)
        self.assertIn(
            ConstraintName.objects.get(slug="chair_conflict").name, notification_payload
        )
        self.assertIn(group.acronym, notification_payload)
        self.assertIn("1 Hour, 1 Hour", notification_payload)
        self.assertNotIn("1 Hour, 1 Hour, 1 Hour", notification_payload)
        self.assertNotIn(
            "The third session requires your approval", notification_payload
        )
...
class LockAppTestCase(TestCase):
    def setUp(self):
        ...
        self.meeting = MeetingFactory(
            type_id="ietf", date=date_today(), session_request_lock_message="locked"
        )
        self.group = GroupFactory(acronym="mars")
        RoleFactory(
            name_id="chair", group=self.group, person__user__username="marschairman"
        )
        SessionFactory(group=self.group, meeting=self.meeting)

    def test_edit_request(self):
        url = reverse(
            "ietf.secr.sreq.views.edit", kwargs={"acronym": self.group.acronym}
        )
        self.client.login(username="secretary", password="secretary+password")
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
    ...
    def test_view_request(self):
        url = reverse(
            "ietf.secr.sreq.views.view", kwargs={"acronym": self.group.acronym}
        )
        self.client.login(username="secretary", password="secretary+password")
    ...
    def test_new_request(self):
        url = reverse(
            "ietf.secr.sreq.views.new", kwargs={"acronym": self.group.acronym}
        )
...
class NotMeetingCase(TestCase):
    def test_not_meeting(self):
        ...
        url = reverse(
            "ietf.secr.sreq.views.no_session", kwargs={"acronym": group.acronym}
        )
        self.client.login(username="secretary", password="secretary+password")
...
class SessionFormTest(TestCase):
    ...
    def test_valid(self):
        ...
        form = SessionForm(
            data=self.valid_form_data, group=self.group1, meeting=self.meeting
        )
    ...
    def test_invalid_group_appears_in_multiple_conflicts(self):
        ...
        self.meeting.group_conflict_types.add(
            ConstraintName.objects.get(slug="conflict")
        )
        self.meeting.group_conflict_types.add(
            ConstraintName.objects.get(slug="conflic2")
        )
        new_form_data = {
        ...
        self.assertEqual(
            ...
            ["%s appears in conflicts more than once" % self.group2.acronym],
        )
    ...
    def test_invalid_session_time_relation(self):
        ...
        self.assertEqual(
            ...
            {
                "session_time_relation": [
                    "Time between sessions can only be used when two "
