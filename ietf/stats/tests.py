...
class StatisticsTests(TestCase):
    ...
    def test_review_stats(self):
        ...
        completion_url = urlreverse(
            ietf.stats.views.review_stats, kwargs={"stats_type": "completion"}
        )
        ...
        for stats_type in ["completion", "results", "states"]:
            url = urlreverse(
                ietf.stats.views.review_stats, kwargs={"stats_type": stats_type}
            )
            r = self.client.get(url)
            self.assertEqual(r.status_code, 200)
            q = PyQuery(r.content)
        ...
        self.assertEqual(r.status_code, 200)
        self.assertEqual(
            json.loads(r.context["data"]),
            [
                {
                    "label": "in time",
                    "color": "#3d22b3",
                    "data": [[expected_js_timestamp, 0]],
                },
                {
                    "label": "late",
                    "color": "#b42222",
        ...
        self.assertEqual(
            json.loads(r.context["data"]),
            [{"color": "#3d22b3", "data": [[expected_js_timestamp, 0]]}],
        )
        q = PyQuery(r.content)
        self.assertTrue(q("#stats-time-graph"))
    ...
    def test_get_meeting_registration_data(self, mock_get):
        ...
        get_meeting_registration_data(meeting)
        query = MeetingRegistration.objects.filter(
            meeting=meeting, email=person.email()
        )
