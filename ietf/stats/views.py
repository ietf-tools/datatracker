...
from ietf.review.utils import (
    extract_review_assignment_data,
...
def add_labeled_top_series_from_bins(chart_data, bins, limit):
    ...
    for label in top:
        ...
        chart_data.append({"data": series_data, "name": label})
...
def known_countries_list(request, stats_type=None, acronym=None):
    ...
    return render(
        request,
        "stats/known_countries_list.html",
        {
            "countries": countries,
        },
    )
...
def review_stats(request, stats_type=None, acronym=None):
    ...
    def build_review_stats_url(
        stats_type_override=Ellipsis, acronym_override=Ellipsis, get_overrides=None
    ):
        if get_overrides is None:
            get_overrides = {}
        kwargs = {
            "stats_type": (
                stats_type if stats_type_override is Ellipsis else stats_type_override
            ),
        }
        ...
        return urlreverse(review_stats, kwargs=kwargs) + generate_query_string(
            request.GET, get_overrides
        )
    ...
    if not stats_type:
        return HttpResponseRedirect(
            build_review_stats_url(stats_type_override=possible_stats_types[0][0])
        )
    ...
    ) or today - dateutil.relativedelta.relativedelta(years=1)
    to_date = parse_date(request.GET.get("to")) or today

    from_time = datetime.datetime.combine(
        from_date, datetime.time.min, tzinfo=DEADLINE_TZINFO
    )
    to_time = datetime.datetime.combine(
        to_date, datetime.time.max, tzinfo=DEADLINE_TZINFO
    )
    ...
    if has_role(request.user, ["Secretariat", "Area Director"]):
        ...
        for r in Role.objects.filter(
            person__user=request.user, name__in=["secr", "reviewer"], group__in=teams
        ).distinct():
            if r.name_id == "secr":
        ...
        teams = [
            t for t in teams if t.pk in secr_access or t.pk in reviewer_only_access
        ]
    ...
    possible_teams = possible_completion_types = possible_results = possible_states = (
        None
    )
    ...
    if stats_type == "time":
        ...
        possible_teams = add_url_to_choices(
            possible_teams,
            lambda slug: build_review_stats_url(
                get_overrides={
        ...
        extracted_data = extract_review_assignment_data(
            query_teams, query_reviewers, from_time, to_time
        )
        ...
        for d, request_data_items in itertools.groupby(extracted_data, key=time_key_fn):
            raw_aggr = aggregate_raw_period_review_assignment_stats(
                request_data_items, count=count
            )
        ...
        possible_completion_types = add_url_to_choices(
            ...
            lambda slug: build_review_stats_url(
                get_overrides={"completion": slug, "result": None, "state": None}
            ),
        )

        selected_completion_type = get_choice(
            request, "completion", possible_completion_types
        )

        possible_results = add_url_to_choices(
            [(r.slug, r.name) for r in results],
            lambda slug: build_review_stats_url(
                get_overrides={"completion": None, "result": slug, "state": None}
            ),
        )
        ...
        possible_states = add_url_to_choices(
            ...
            lambda slug: build_review_stats_url(
                get_overrides={"completion": None, "result": None, "state": slug}
            ),
        )
        ...
        extracted_data = extract_review_assignment_data(
            query_teams, query_reviewers, from_time, to_time, ordering=[level]
        )
        ...
        found_states = set()
        raw_aggrs = []
        for group_pk, request_data_items in itertools.groupby(
            extracted_data, key=lambda t: t[group_by_index]
        ):
            raw_aggr = aggregate_raw_period_review_assignment_stats(
                request_data_items, count=count
            )
            ...
            if (
                aggr["open"] == 0
                and aggr["completed"] == 0
                and aggr["not_completed"] == 0
            ):
                continue
        ...
        # add totals row
        if len(raw_aggrs) > 1:
            totals = sum_period_review_assignment_stats(
                sum_raw_review_assignment_aggregations(raw_aggrs)
            )
            totals["obj"] = "Totals"
            data.append(totals)
