...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if sent.exists() and not resend and not unsent:
            self.stderr.write(
                "Error: Asked to send one or more already sent messages, and --resend not given"
            )
            for m in sent:
            ...
            for m in messages:
                ...
                try:
                    ...
                    self.stdout.write(
                        '%s  %s -> %s  "%s"' % (m.pk, m.frm, to, m.subject.strip())
                    )
                except smtplib.SMTPException as e:
                    self.stdout.write(
                        'Failure %s:  %s  %s -> %s  "%s"'
                        % (e, m.pk, m.frm, to, m.subject.strip())
                    )
