...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options["stop"]:
            ...
            selection_str = "%s messages between %s and %s" % (
                options["state"],
                options["start"],
                options["stop"],
            )
            ...
            selection_str = "%s messages since %s" % (
                options["state"],
                options["start"],
            )
        self.stdout.write("\nShowimg %s:\n\n" % selection_str)
        ...
        if options["pk"]:
            self.stdout.write(
                ",".join([str(pk) for pk in messages.values_list("pk", flat=True)])
            )
        else:
            for m in messages:
                ...
                self.stdout.write(
                    ...
                    % (
                        ...
                        m.sent and m.sent.strftime("%Y-%m-%d %H:%M") or "",
                        m.msgid.strip("<>"),
                        addr(m.frm),
                        to,
                        m.subject.strip(),
