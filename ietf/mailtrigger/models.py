...
class MailTrigger(models.Model):
    ...
    to = models.ManyToManyField(
        "mailtrigger.Recipient", blank=True, related_name="used_in_to"
    )
    cc = models.ManyToManyField(
        "mailtrigger.Recipient", blank=True, related_name="used_in_cc"
    )
...
class Recipient(models.Model):
    ...
    def gather(self, **kwargs):
        ...
        if self.template:
            rendering = Template(
                "{%% autoescape off %%}%s{%% endautoescape %%}" % self.template
    ...
    def gather_doc_group_delegates(self, **kwargs):
        ...
        if "doc" in kwargs:
            ...
            if doc.group and doc.group.features.acts_like_wg:
                addrs.extend(
                    doc.group.role_set.filter(name="delegate").values_list(
    ...
    def gather_doc_affecteddoc_authors(self, **kwargs):
        addrs = []
        if "doc" in kwargs:
            for reldoc in kwargs["doc"].related_that_doc(
                ("conflrev", "tohist", "tois", "tops")
            ):
                addrs.extend(
                    Recipient.objects.get(slug="doc_authors").gather(**{"doc": reldoc})
                )
        return addrs
    ...
    def gather_doc_affecteddoc_group_chairs(self, **kwargs):
        ...
        if "doc" in kwargs:
            for reldoc in kwargs["doc"].related_that_doc(
                ...
                addrs.extend(
                    Recipient.objects.get(slug="doc_group_chairs").gather(
                        **{"doc": reldoc}
                    )
                )
        return addrs
    ...
    def gather_doc_affecteddoc_notify(self, **kwargs):
        ...
        if "doc" in kwargs:
            for reldoc in kwargs["doc"].related_that_doc(
                ...
                addrs.extend(
                    Recipient.objects.get(slug="doc_notify").gather(**{"doc": reldoc})
                )
        return addrs
    ...
    def gather_conflict_review_stream_manager(self, **kwargs):
        ...
        if "doc" in kwargs:
            for reldoc in kwargs["doc"].related_that_doc(("conflrev",)):
                addrs.extend(
                    Recipient.objects.get(slug="doc_stream_manager").gather(
                        **{"doc": reldoc}
                    )
                )
        return addrs
    ...
    def gather_group_steering_group(self, **kwargs):
        addrs = []
        sg_map = dict(
            wg='"The IESG" <iesg@ietf.org>',
            rg='"Internet Research Steering Group" <irsg@irtf.org>',
        )
    ...
    def gather_stream_managers(self, **kwargs):
        ...
        manager_map = dict(
            ...
            editorial=Role.objects.filter(
                group__acronym="rsab", name_id="chair"
    ...
    def gather_doc_stream_manager(self, **kwargs):
        ...
        if "doc" in kwargs:
            addrs.extend(
                Recipient.objects.get(slug="stream_managers").gather(
                    **{"streams": [kwargs["doc"].stream_id]}
                )
            )
        return addrs
    ...
    def gather_group_responsible_directors(self, **kwargs):
        ...
        if "group" in kwargs:
            ...
            if not group.acronym == "none":
                addrs.extend(
                    group.role_set.filter(name="ad").values_list(
                        "email__address", flat=True
                    )
                )
            if group.type_id == "rg":
    ...
    def gather_doc_group_responsible_directors(self, **kwargs):
        ...
        if "doc" in kwargs:
            ...
            if group and not group.acronym == "none":
                addrs.extend(
                    Recipient.objects.get(slug="group_responsible_directors").gather(
                        **{"group": group}
                    )
                )
        return addrs
    ...
    def gather_submission_group_chairs(self, **kwargs):
        ...
        if "submission" in kwargs:
            ...
            if submission.group:
                addrs.extend(
                    Recipient.objects.get(slug="group_chairs").gather(
    ...
    def gather_submission_confirmers(self, **kwargs):
        ...
        if "submission" in kwargs:
            ...
            if doc:
                old_authors = [
                    author for author in doc.documentauthor_set.all() if author.email
                ]

                addrs.extend([author.formatted_email() for author in old_authors])

                old_author_email_set = set(
                    author.email.address for author in old_authors
                )
                new_author_email_set = set(
                    author["email"]
                    for author in submission.authors
                    if author.get("email")
                )
                    ...
                    if doc.group.features.acts_like_wg:
                        ...
                        addrs.extend(
                            Recipient.objects.get(
                                slug="group_responsible_directors"
                            ).gather(**{"group": doc.group})
                        )
                    else:
                        pass
                ...
                emails = [
                    Email.objects.filter(address=author["email"]).first() or author
                    for author in submission.authors
                    if author.get("email")
                ]
                addrs.extend(
                    [
                        (
                            ...
                            else formataddr((e["name"], e["email"]))
                        )
                        for e in emails
                    ]
                )
                submitter_email = submission.submitter_parsed()["email"]
                if submitter_email and not submitter_email in [
                    parseaddr(a)[1] for a in addrs
                ]:
    ...
    def gather_submission_group_mail_list(self, **kwargs):
        ...
        if "submission" in kwargs:
            ...
            if submission.group:
                addrs.extend(
                    Recipient.objects.get(slug="group_mail_list").gather(
                        **{"group": submission.group}
                    )
                )
        return addrs
    ...
    def gather_doc_ipr_group_or_ad(self, **kwargs):
        ...
        if "doc" in kwargs:
            ...
            if doc.group and doc.group.acronym == "none":
                ...
                addrs.extend(
                    Recipient.objects.get(slug="doc_group_mail_list").gather(**kwargs)
                )
        return addrs
    ...
    def gather_liaison_manager(self, **kwargs):
        ...
        if "group" in kwargs:
            group = kwargs["group"]
            addrs.extend(
                group.role_set.filter(name="liaiman").values_list(
                    "email__address", flat=True
                )
            )
        return addrs
    ...
    def gather_yang_doctors_secretaries(self, **kwargs):
        return self.gather_group_secretaries(
            group=Group.objects.get(acronym="yangdoctors")
        )
