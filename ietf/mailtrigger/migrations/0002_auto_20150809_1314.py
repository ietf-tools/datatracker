...
def make_recipients(apps):
    ...
    rc(slug='iesg', desc='The IESG', template='The IESG <iesg@ietf.org>')

    rc(slug='iab', desc='The IAB', template='The IAB <iab@iab.org>')
    ...
    rc(slug='doc_authors',
       ...
       template=
       '{% if doc.type_id == "draft" %}<{{doc.name}}@ietf.org>{% endif %}')
    ...
    rc(slug='doc_group_chairs',
       desc=
       "The document's group chairs (if the document is assigned to a working or research group)",
       template=None)

    rc(slug='doc_group_delegates',
       desc=
       "The document's group delegates (if the document is assigned to a working or research group)",
       template=None)

    rc(slug='doc_affecteddoc_authors',
       desc=
       "The authors of the subject documents of a conflict-review or status-change",
       template=None)

    rc(slug='doc_affecteddoc_group_chairs',
       desc=
       "The chairs of groups of the subject documents of a conflict-review or status-change",
       template=None)

    rc(slug='doc_affecteddoc_notify',
       desc=
       "The notify field of the subject documents of a conflict-review or status-change",
       template=None)
    ...
    rc(slug='conflict_review_stream_manager',
       desc=
       "The stream manager of a document being reviewed for IETF stream conflicts",
       template=None)

    rc(slug='conflict_review_steering_group',
       desc=
       "The steering group (e.g. IRSG) of a document being reviewed for IETF stream conflicts",
       template=None)
    ...
    rc(slug='iana', desc="IANA", template='<iana@iana.org>')
    ...
    rc(slug='group_chairs',
       ...
       template=
       "{% if group and group.acronym %}<{{group.acronym}}-chairs@ietf.org>{% endif %}"
       )
    ...
    rc(slug='doc_non_ietf_stream_manager',
       desc=
       "The document's stream manager if the document is not in the IETF stream",
       template=None)
    ...
    rc(slug='group_changed_personnel',
       desc=
       "Any personnel who were added or deleted when a group's personnel changes",
    ...
    rc(slug='logged_in_person',
       desc=
       "The person currently logged into the datatracker who initiated a given action",
       template=
       '{% if person and person.email_address %}<{{ person.email_address }}>{% endif %}'
       )
    ...
    rc(slug='ipr_submitter',
       ...
       template=
       '{% if ipr.submitter_email %}{{ ipr.submitter_email }}{% endif %}')

    rc(slug='ipr_updatedipr_contacts',
       desc=
       "The submitter (or ietf participant if the submitter is not available) "
    ...
    rc(slug='ipr_updatedipr_holders',
       desc=
       "The holders of all IPR disclosures updated by disclosure and disclosures updated by those and so on.",
       template=None)
    ...
    rc(slug='liaison_to_contacts',
       desc=
       "The addresses captured in the To field of the liaison statement form",
       template='{{liaison.to_contacts}}')

    rc(slug='liaison_cc',
       desc=
       "The addresses captured in the Cc field of the liaison statement form",
       template='{{liaison.cc_contacts}}')

    rc(slug='liaison_technical_contacts',
       desc=
       "The addresses captured in the technical contact field of the liaison statement form",
       template='{{liaison.technical_contacts}}')

    rc(slug='liaison_response_contacts',
       desc=
       "The addresses captured in the response contact field of the liaison statement form",
...
def make_mailtriggers(apps):
    ...
    mt_factory(
        slug='ballot_approved_ietf_stream',
    ...
    mt_factory(
        slug='ballot_approved_ietf_stream_iana',
        desc=
        "Recipients for IANA message when an IETF stream document ballot is approved",
        to_slugs=['iana_approve'])

    mt_factory(
        slug='ballot_approved_conflrev',
    ...
    mt_factory(
        slug='ballot_approved_status_change',
    ...
    mt_factory(
        slug='charter_external_review_new_work',
    ...
    mt_factory(
        slug='conflrev_requested',
        desc=
        "Recipients for a stream manager's request for an IETF conflict review",
    ...
    mt_factory(
        slug='doc_stream_state_edited',
        desc=
        "Recipients when the stream state of a document is manually edited",
        to_slugs=[
            'doc_group_chairs',
            'doc_group_delegates',
            'doc_shepherd',
    ...
    mt_factory(
        slug='group_approved_milestones_edited',
        desc=
        "Recipients when the set of approved milestones for a group are edited",
        to_slugs=[
            'group_mail_list',
        ])

    mt_factory(
        slug='doc_state_edited',
    ...
    mt_factory(
        slug='doc_telechat_details_changed',
    ...
    mt_factory(
        slug='doc_pulled_from_rfc_queue',
        desc=
        "Recipients when a document is taken out of the RFC's editor queue "
        "before publication",
        to_slugs=[
            'iana',
            'rfc_editor',
    ...
    mt_factory(
        slug='doc_replacement_changed',
        desc=
        "Recipients when what a document replaces or is replaced by changes",
        to_slugs=[
            'doc_authors',
            'doc_notify',
            'doc_shepherd',
    ...
    mt_factory(
        slug='charter_state_edit_admin_needed',
        desc=
        "Recipients for message to adminstrators when a charter state edit "
        "needs followon administrative action",
        to_slugs=['iesg_secretary'])

    mt_factory(
        slug='group_closure_requested',
        desc="Recipients for message requesting closure of a group",
        to_slugs=['iesg_secretary'])

    mt_factory(
        slug='doc_expires_soon',
        desc=
        "Recipients for notification of impending expiration of a document",
    ...
    mt_factory(
        slug='resurrection_requested',
        desc=
        "Recipients of a request to change the state of a draft away from 'Dead'",
        to_slugs=[
            'internet_draft_requests',
        ])

    mt_factory(
        slug='resurrection_completed',
    ...
    mt_factory(
        slug='sub_manual_post_requested',
    ...
    mt_factory(
        slug='sub_chair_approval_requested',
    ...
    mt_factory(
        slug='sub_confirmation_requested',
        desc=
        "Recipients for a message requesting confirmation of a draft submission",
        to_slugs=[
            'submission_confirmers',
        ])

    mt_factory(
        slug='sub_management_url_requested',
        desc=
        "Recipients for a message with the full URL for managing a draft submission",
        to_slugs=[
            'submission_confirmers',
        ])
    ...
    mt_factory(
        slug='sub_announced',
        desc=
        "Recipients for the announcement of a successfully submitted draft",
        to_slugs=[
            'ietf_announce',
        ],
        cc_slugs=[
    ...
    mt_factory(
        slug='sub_new_version',
        desc=
        "Recipients for notification of a new version of an existing document",
        to_slugs=[
            'doc_notify',
            'doc_ad',
    ...
    mt_factory(
        slug='session_requested_long',
        desc=
        "Recipients for a meeting session request for more than 2 sessions",
        to_slugs=[
            'group_responsible_directors',
        ],
        cc_slugs=[
    ...
    mt_factory(
        slug='session_request_cancelled',
    ...
    mt_factory(
        slug='session_request_not_meeting',
    ...
    mt_factory(
        slug='ipr_disclosure_followup',
        desc=
        "Recipients when the secretary follows up on an IPR disclosure submission",
        to_slugs=[
            'ipr_submitter',
        ], )

    mt_factory(
        slug='ipr_posting_confirmation',
        desc=
        "Recipients for a message confirming that a disclosure has been posted",
        to_slugs=[
            'ipr_submitter',
        ],
        cc_slugs=[
    ...
    mt_factory(
        slug='liaison_approval_requested',
        desc=
        "Recipients for a message that a pending liaison statement needs approval",
        to_slugs=[
            'liaison_approvers',
        ])

    mt_factory(
        slug='liaison_deadline_soon',
        desc=
        "Recipients for a message about a liaison statement deadline that is "
        "approaching.",
        to_slugs=[
            'liaison_to_contacts',
        ],
    ...
    mt_factory(
        slug='liaison_manager_update_request',
        desc=
        "Recipients for a message requesting an updated list of authorized individuals",
        to_slugs=[
            'liaison_manager',
        ])

    mt_factory(
        slug='nomination_received',
        desc=
        "Recipients for a message noting a new nomination has been received",
        to_slugs=[
            'nomcom_chair',
        ])

    mt_factory(
        slug='nomination_receipt_requested',
    ...
    mt_factory(
        slug='nomcom_comment_receipt_requested',
    ...
    mt_factory(
        slug='nomination_created_person',
    ...
    mt_factory(
        slug='nomination_accept_reminder',
        desc=
        "Recipeints of message reminding a nominee to accept or decline a nomination",
        to_slugs=[
            'nominee',
        ])

    mt_factory(
        slug='nomcom_questionnaire',
    ...
    mt_factory(
        slug='nomcom_questionnaire_reminder',
    ...
    mt_factory(
        slug='doc_replacement_suggested',
        desc=
        "Recipients for suggestions that this doc replaces or is replace by "
        "some other document",
        to_slugs=[
            'doc_group_chairs',
            'doc_group_responsible_directors',
    ...
    mt_factory(
        slug='doc_adopted_by_group',
        desc=
        "Recipients for notification that a document has been adopted by a group",
        to_slugs=[
            'doc_authors',
            'doc_group_chairs',
            'doc_group_mail_list',
    ...
    mt_factory(
        slug='doc_added_comment',
        desc=
        "Recipients for a message when a new comment is manually entered into the document's history",
        to_slugs=[
            'doc_authors',
            'doc_group_chairs',
            'doc_shepherd',
    ...
    mt_factory(
        slug='doc_intended_status_changed',
    ...
    mt_factory(
        slug='doc_iesg_processing_started',
        desc=
        "Recipients for a message when the IESG begins processing a document ",
        to_slugs=[
            'doc_authors',
            'doc_ad',
            'doc_shepherd',
