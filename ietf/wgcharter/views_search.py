...
def search_query(query_original, sort_by=None):
    ...
    s_obj = Q()
    if query["inprocess"]:
        s_obj |= Q(charter__charter_state__slug__in=("infrev", "intrev", "extrev", "iesgrev")) & Q(state__slug__in=("proposed", "active"))
    if query["active"]:
        s_obj |= Q(charter__charter_state__slug="approved") & Q(state__slug="active")
    if query["concluded"]:
        s_obj |= Q(state__slug="conclude") | Q(charter__charter_state__slug="notrev")

    results = Group.objects.filter(q_obj & s_obj)
