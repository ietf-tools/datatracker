...
def wg_main(request, name, rev, tab):
    ...
    if ch:
        ...
        seen = []
        latest_positions = []
        for p in GroupBallotPositionDocEvent.objects.filter(doc=wg.charter, type="changed_ballot_position", time__gte=started_process.time).order_by("-time", '-id').select_related('ad'):
            if p.ad not in seen:
