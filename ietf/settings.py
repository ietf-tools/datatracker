...
warnings.filterwarnings(
    "ignore", "Log out via GET requests is deprecated"
)  # happens in oidc_provider
warnings.filterwarnings(
    "ignore",
    module="tastypie",
    message="The django.utils.datetime_safe module is deprecated.",
)
warnings.filterwarnings(
    "ignore",
    module="oidc_provider",
    message="The django.utils.timezone.utc alias is deprecated.",
)
warnings.filterwarnings(
    "ignore", message="The USE_DEPRECATED_PYTZ setting,"
)  # https://github.com/ietf-tools/datatracker/issues/5635
warnings.filterwarnings(
    "ignore", message="The USE_L10N setting is deprecated."
)  # https://github.com/ietf-tools/datatracker/issues/5648
warnings.filterwarnings(
    "ignore", message="django.contrib.auth.hashers.CryptPasswordHasher is deprecated."
)  # https://github.com/ietf-tools/datatracker/issues/5663
...
warnings.filterwarnings(
    "ignore",
    message="Report.file_reporters will no longer be available in Coverage.py 4.2",
...
CSP_DEFAULT_SRC = (
    "'self'",
    "'unsafe-inline'",
    f"data: {IDTRACKER_BASE_URL} http://ietf.org/ https://www.ietf.org/ https://analytics.ietf.org/ https://static.ietf.org",
)
...
GLOBAL_TEST_FIXTURES = [
    "names",
    "ietf.utils.test_data.make_immutable_base_data",
    "nomcom_templates",
...
if SERVER_MODE != "production":
    ...
    TEST_CODE_COVERAGE_CHECKER = coverage.Coverage(
        source=[BASE_DIR], cover_pylib=False, omit=TEST_CODE_COVERAGE_EXCLUDE_FILES
    )

TEST_CODE_COVERAGE_REPORT_PATH = "coverage/"
TEST_CODE_COVERAGE_REPORT_URL = os.path.join(
    STATIC_URL, TEST_CODE_COVERAGE_REPORT_PATH, "index.html"
)
TEST_CODE_COVERAGE_REPORT_DIR = os.path.join(
    BASE_DIR, "static", TEST_CODE_COVERAGE_REPORT_PATH
)
TEST_CODE_COVERAGE_REPORT_FILE = os.path.join(
    TEST_CODE_COVERAGE_REPORT_DIR, "index.html"
)
...
# document state:
GROUP_STATES_WITH_EXTRA_PROCESSING = [
    "sub-pub",
    "rfc-edit",
]
...
MEETINGHOST_LOGO_PATH = (
    AGENDA_PATH  # put these in the same place as other proceedings files
)
...
MAILING_LIST_INFO_URL = (
    "https://mailman3.%(domain)s/mailman3/lists/%(list_addr)s.%(domain)s"
)
MAILING_LIST_ARCHIVE_URL = "https://mailarchive.ietf.org"

# Liaison Statement Tool settings (one is used in DOC_HREFS below)
LIAISON_UNIVERSAL_FROM = (
    "Liaison Statement Management Tool <statements@" + IETF_DOMAIN + ">"
)
LIAISON_ATTACH_PATH = (
    "/a/www/ietf-datatracker/documents/LIAISON/"  # should end in a slash
)
...
DOC_TEXT_FILE_VALID_UPLOAD_MIME_TYPES = (
    "text/plain",
    "text/markdown",
    "text/x-rst",
    "text/x-markdown",
...
SESSION_REQUEST_FROM_EMAIL = (
    "IETF Meeting Session Request Tool <session-request@ietf.org>"
)
...
RFC_EDITOR_INLINE_ERRATA_URL = (
    "https://www.rfc-editor.org/rfc/inline-errata/rfc{rfc_number}.html"
)
...
INTERIM_ANNOUNCE_FROM_EMAIL_PROGRAM = (
...
SUBMIT_YANG_CATALOG_MODULE_URL = (
    "https://www.yangcatalog.org/yang-search/module_details.php?module={module}"
)
SUBMIT_YANG_CATALOG_MODULE_DESC = "Yang catalog entry for {module}"

SUBMIT_YANG_CATALOG_CHECKER_URL = (
    "https://yangcatalog.org/yangvalidator/api/v1/datatracker/{type}"
)
...
MEETING_LEGACY_OFFICE_HOURS_END = (
    112  # last meeting to use legacy office hours representation
)
...
PPT2PDF_COMMAND = [
    ...
    "--convert-to",
    "pdf:writer_globaldocument_pdf_Export",
    "--outdir",
]

STATS_REGISTRATION_ATTENDEES_JSON_URL = (
    "https://registration.ietf.org/{number}/attendees/"
)
...
UTILS_MEETING_CONFERENCE_DOMAINS = [
    "webex.com",
    "zoom.us",
    "jitsi.org",
    "meetecho.com",
...
CELERY_BROKER_CONNECTION_RETRY_ON_STARTUP = (
    True  # the default, but setting it squelches a warning
)
...
CELERY_RESULT_EXPIRES = datetime.timedelta(
    minutes=5
)  # how long are results valid? (Default is 1 day)
CELERY_TASK_IGNORE_RESULT = (
    True  # ignore results unless specifically enabled for a task
)
...
MEETECHO_AUDIO_STREAM_URL = (
    "https://mp3.conf.meetecho.com/ietf{session.meeting.number}/{session.pk}.m3u"
)
MEETECHO_SESSION_RECORDING_URL = (
    "https://meetecho-player.ietf.org/playout/?session={session_label}"
)
...
for app in INSTALLED_APPS:
    if app.startswith("ietf"):
        app_settings_file = os.path.join(
            BASE_DIR, "../", app.replace(".", os.sep), "settings.py"
        )
...
if SERVER_MODE != "production":
    ...
    loaders = tuple(
        l
        for e in loaders
        for l in (e[1] if isinstance(e, tuple) and "cached.Loader" in e[0] else (e,))
    )
