...
def update_rfc_log_from_protocol_page(rfc_names, rfc_must_published_later_than):
    ...
    docs = (
        Document.objects.filter(name__in=rfc_names)
        .exclude(docevent__type="rfc_in_iana_registry")
        .filter(
            ...
            docevent__time__gte=rfc_must_published_later_than,
        )
        .distinct()
    )
...
def fetch_changes_json(url, start, end):
    url += "?start=%s&end=%s" % (
        urlquote(start.astimezone(datetime.timezone.utc).strftime("%Y-%m-%d %H:%M:%S")),
    ...
    try:
        response = requests.get(
            url, headers=headers, timeout=settings.DEFAULT_REQUESTS_TIMEOUT
        )
...
def parse_changes_json(text):
    ...
    for i in changes:
        for f in ["doc", "type", "time"]:
            if f not in i:
                raise Exception(
                    "Error in response: Field %s missing in input: %s - %s"
                    % (f, json.dumps(i), json.dumps(changes))
                )
...
def update_history_with_changes(changes, send_email=True):
    ...
    slookup = dict(
        ...
        for s in State.objects.filter(
            used=True, type=StateType.objects.get(slug="draft-iana-action")
        )
    )
    states["action"] = {
        "": slookup["newdoc"],
    ...
    slookup = dict(
        (s.slug, s)
        for s in State.objects.filter(
            used=True, type=StateType.objects.get(slug="draft-iana-review")
        )
    )
    states["review"] = {
    ...
    wrong_action_states = (
        ...
        "With Reviewer",
        "IESG Notification Received",
        "Watiing on Last Call",
        "IANA Comments Submitted",
        "Waiting on Last Call",
    )
        ...
        if c["type"] in ("iana_state", "iana_review"):
            if c["type"] == "iana_state":
                ...
                if c["state"] in wrong_action_states:
                    warnings.append(
                        "Wrong action state '%s' encountered in changes from IANA"
                        % c["state"]
                    )
                    continue
            else:
            ...
            if c["state"] not in states[kind]:
                warnings.append(
                    "Unknown IANA %s state %s (%s)" % (kind, c["state"], timestamp)
                )
                continue
            ...
            e = StateDocEvent.objects.filter(
                type="changed_state", time=timestamp, state_type=state_type, state=state
            )
            if not e:
                try:
                ...
                e = add_state_change_event(
                    doc, system, prev_state, state, timestamp=timestamp
                )
                ...
                if not StateDocEvent.objects.filter(
                    doc=doc, time__gt=timestamp, state_type=state_type
                ):
                    doc.set_state(state)
...
def parse_review_email(text):
    ...
    if "Date" in msg:
        ...
        if (
            review_time.tzinfo is None
            or review_time.tzinfo.utcoffset(review_time) is None
        ):
    ...
    begin_search = re.search(
        r"\(BEGIN\s+IANA\s+(LAST\s+CALL\s+)?COMMENTS?(\s*:\s*[a-zA-Z0-9-\.]*)?\s*\)",
        body,
    )
...
def add_review_comment(doc_name, review_time, by, comment):
    if comment:
        try:
            e = DocEvent.objects.get(
                doc__name=doc_name, time=review_time, type="iana_review"
            )
...
def ingest_review_email(message: bytes):
    ...
    try:
        ...
        raise EmailIngestionError(
            "Unable to parse message as IANA review email"
        ) from err
