...
class IetfAuthTests(TestCase):
    ...
    def test_index(self):
        self.assertEqual(
            self.client.get(urlreverse("ietf.ietfauth.views.index")).status_code, 200
        )
    ...
    def test_login_and_logout(self):
        ...
        r = self.client.post(
            ...
            {"username": "plain", "password": "plain+password"},
        )
        self.assertEqual(r.status_code, 302)
        self.assertEqual(
            urlsplit(r["Location"])[2], urlreverse("ietf.ietfauth.views.profile")
        )
        ...
        self.assertEqual(r.status_code, 302)
        self.assertEqual(
            urlsplit(r["Location"])[2], urlreverse("ietf.ietfauth.views.login")
        )

        # try logging in with a next
        r = self.client.post(
            urlreverse("ietf.ietfauth.views.login") + "?next=/foobar",
            {"username": "plain", "password": "plain+password"},
        )
        self.assertEqual(r.status_code, 302)
    ...
    def test_login_button(self):
        ...
        def _test_login(url):
            ...
            r = self.client.post(
                login_url, {"username": "plain", "password": "plain+password"}
            )
            self.assertEqual(r.status_code, 302)
    ...
    def test_create_account_failure_template(self):
        r = render_to_string(
            "registration/manual.html",
            {"account_request_email": settings.ACCOUNT_REQUEST_EMAIL},
        )
    ...
    def register_and_verify(self, email):
        ...
        # password mismatch
        r = self.client.post(
            confirm_url, {"password": "secret", "password_confirmation": "nosecret"}
        )
        self.assertEqual(r.status_code, 200)
    ...
    def test_ietfauth_profile(self):
        ...
        username = "plain"
        email_address = (
            Email.objects.filter(person__user__username=username).first().address
        )
        ...
        self.assertEqual(
            len(q('[name="active_emails"][value="%s"][checked]' % email_address)), 1
        )
        ...
        q = PyQuery(r.content)
        self.assertTrue(
            len(q("form div.invalid-feedback")) == 1
        )  # we get a warning about reconstructed name
        ...
        self.assertEqual(
            Person.objects.filter(
                alias__name="Test Name", user__username=username
            ).count(),
            1,
        )
        self.assertEqual(
            Person.objects.filter(
                alias__name="Test Nãme", user__username=username
            ).count(),
            1,
        )
        self.assertEqual(
            Email.objects.filter(
                address=email_address, person__user__username=username, active=True
            ).count(),
            1,
        )
        ...
        without_email_address = {
            k: v for k, v in base_data.items() if k != "active_emails"
        }
        ...
        self.assertEqual(
            Email.objects.filter(
                address=email_address, person__user__username="plain", active=True
            ).count(),
            0,
        )
        ...
        self.assertEqual(
            Email.objects.filter(
                address=new_email_address, person__user__username=username, active=1
            ).count(),
            1,
        )
        ...
        self.assertEqual(r.status_code, 200)
        updated_roles = Role.objects.filter(
            person=role.person, name=role.name, group=role.group
        )
    ...
    def test_nomcom_dressing_on_profile(self):
        ...
        nomcom = NomComFactory(
            group__acronym=f"nomcom{year}", is_accepting_volunteers=True
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
    ...
    def test_reset_password(self):
        ...
        # confirm
        r = self.client.post(
            confirm_url, {"password": "secret", "password_confirmation": "secret"}
        )
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
    ...
    def test_reset_password_without_person(self):
        ...
        r = self.client.post(url, {"username": user.username})
        self.assertContains(
            r, "We have sent you an email with instructions", status_code=200
        )
        q = PyQuery(r.content)
        self.assertTrue(len(q("form .is-invalid")) == 0)
    ...
    def test_reset_password_without_user(self):
        ...
        self.assertTrue(
            lastReceivedEmail.get("Subject").startswith("Confirm password reset")
        )
        self.assertContains(
            r,
    ...
    def test_review_overview(self):
        ...
        assignment = ReviewAssignmentFactory(
            ...
            reviewer=EmailFactory(person__user__username="reviewer"),
        )
        RoleFactory(
        ...
        self.assertEqual(r.status_code, 302)
        self.assertEqual(
            ReviewWish.objects.filter(doc=doc, team=review_req.team).count(), 1
        )
        ...
        self.assertEqual(r.status_code, 302)
        self.assertEqual(
            ReviewWish.objects.filter(doc=doc, team=review_req.team).count(), 0
        )
    ...
    def test_change_password(self):
        ...
        # log in
        r = self.client.post(
            redir_url, {"username": user.username, "password": "password"}
        )
    ...
    def test_apikey_errors(self):
        ...
        for key in person.apikeys.all()[:3]:
            ...
            unauthorized_url = urlreverse(
                "ietf.api.views.app_auth", kwargs={"app": "authortools"}
            )
            invalidated_apikey = PersonalApiKeyFactory(
                endpoint=unauthorized_url, person=person, valid=False
            )
            r = self.client.post(
                unauthorized_url, {"apikey": invalidated_apikey.hash()}
            )
            self.assertContains(r, "Invalid apikey", status_code=403)

            # too long since regular login
            person.user.last_login = timezone.now() - datetime.timedelta(
                days=settings.UTILS_APIKEY_GUI_LOGIN_LIMIT_DAYS + 1
            )
            person.user.save()
            r = self.client.post(
                key.endpoint,
    ...
    def test_edit_person_extresources(self):
        ...
        self.assertEqual(
            person.personextresource_set.get(name__slug="github_repo").display_name,
            "Some display text",
        )
        self.assertIn(
            person.personextresource_set.first().name.slug,
            str(person.personextresource_set.first()),
        )
    ...
    def test_confirm_new_email(self):
        ...
        r = self.client.get(
            urlreverse(
                "ietf.ietfauth.views.confirm_new_email", kwargs={"auth": invalid_auth}
            )
        )
        self.assertEqual(r.status_code, 404)
        r = self.client.post(
        ...
        r = self.client.post(
            urlreverse(
                "ietf.ietfauth.views.confirm_new_email", kwargs={"auth": other_auth}
            ),
            data={"action": "confirm"},
        )
        ...
        self.assertContains(r, "in use by another user", status_code=200)
        self.assertFalse(
            other_person.email_set.filter(
                address__icontains="new_email@example.com"
            ).exists()
        )
        self.assertTrue(
            person.email_set.filter(address__icontains="new_email@example.com").exists()
...
class OpenIDConnectTests(TestCase):
    ...
    def test_oidc_code_auth(self):
        ...
        oid_client_record = OidClientRecordFactory(
            _redirect_uris="\n".join(redirect_uris),
        )
        ...
        with requests_mock.Mocker() as mock:
            ...
            client_reg = RegistrationResponse(
                client_id=oid_client_record.client_id,
            ...
            closed_group = RoleFactory(
                name_id="chair", person=person, group__state_id="conclude"
            ).group
            # an additional email
            ...
            self.assertTrue(
                login_url.startswith(urlreverse("ietf.ietfauth.views.login"))
            )
            ...
            r = self.client.post(
                login_url,
                {"username": username, "password": "%s+password" % username},
                follow=True,
            )
            ...
            params = client.parse_response(
                AuthorizationResponse,
                info=urllib.parse.urlsplit(location).query,
                sformat="urlencoded",
            )

            # Use grant code to get access token
            access_token_info = client.do_access_token_request(
                state=params["state"], authn_method="client_secret_basic"
            )
            ...
            self.assertEqual(
                set(userinfo["reg_type"].split()), set(["remote", "hackathon"])
            )

            # Check that ending a session works
            r = client.do_end_session_request(
                state=params["state"], scope=args["scope"]
            )
            self.assertEqual(r.status_code, 302)
            self.assertEqual(
                r.headers["Location"], urlreverse("ietf.ietfauth.views.login")
            )
...
class UtilsTests(TestCase):
    def test_has_role_empty_role_names(self):
        ...
        self.assertFalse(
            has_role(role.person.user, []),
            "has_role() should return False when role_name is empty",
        )
