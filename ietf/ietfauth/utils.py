...
def is_authorized_in_doc_stream(user, doc):
    ...
    return Role.objects.filter(
        Q(name__in=docman_roles, person__user=user) & group_req
    ).exists()
...
def is_authorized_in_group(user, group):
    ...
    if group.parent:
        if group.parent.type_id == "area" and has_role(
            user,
            [
                "Area Director",
            ],
            ...
            return True
        if group.parent.acronym == "irtf" and has_role(
            user,
            [
                "IRTF Chair",
            ],
            ...
            return True
        if group.parent.acronym == "iab" and has_role(
            user,
            [
                "IAB",
    ...
    return Role.objects.filter(
        name__in=group.features.groupman_roles, person__user=user, group=group
    ).exists()
...
class OidcExtraScopeClaims(oidc_provider.lib.claims.ScopeClaims):
    ...
    def scope_roles(self):
        roles = self.user.person.role_set.filter(
            group__state_id__in=("active", "bof", "proposed")
        ).values_list("name__slug", "group__acronym")
    ...
    def scope_registration(self):
        ...
        if regs:
            ...
            info = {
                ...
                "affiliation": (
                    [reg.affiliation for reg in regs if reg.affiliation] or [""]
                )[0],
            }
