...
from ietf.doc.mails import (
    email_bofreq_title_changed,
    email_bofreq_editors_changed,
...
def bof_requests(request):
    ...
    sorted_reqs = sorted(
        sorted(reqs, key=lambda doc: doc.latest_revision_event.time, reverse=True),
        key=lambda doc: doc.get_state().order,
    )
...
class BofreqUploadForm(forms.Form):
    ...
    bofreq_file = forms.FileField(
        label="Markdown source file to upload", required=False
    )
    bofreq_content = forms.CharField(
            ...
            if require_field("bofreq_content"):
                ...
                if content == default_content:
                    raise forms.ValidationError(
                        "The example content may not be saved. Edit it as instructed to document this BOF request."
                    )
        elif submission_method == "upload":
            if require_field("bofreq_file"):
                content = get_cleaned_text_file_content(
                    self.cleaned_data["bofreq_file"]
                )
        try:
...
def submit(request, name):
    ...
    if not (
        has_role(request.user, ("Secretariat", "Area Director", "IAB"))
        or (state_id == "proposed" and request.user.person in previous_editors)
    ):
        permission_denied(
            request,
            "You do not have permission to upload a new revision of this BOF Request",
        )
    ...
    if request.method == "POST":
        ...
        init = {
            "bofreq_content": bofreq.text_or_error(),
            "bofreq_submission": "enter",
        }
        form = BofreqUploadForm(initial=init)
    return render(
        request, "doc/bofreq/upload_content.html", {"form": form, "doc": bofreq}
    )
...
class NewBofreqForm(BofreqUploadForm):
    ...
    def clean_title(self):
        ...
        if Document.objects.filter(name=name).exists():
            raise forms.ValidationError(
                "This title produces a filename already used by an existing BOF request"
            )
        return title
...
def new_bof_request(request):
    ...
    return render(request, "doc/bofreq/new_bofreq.html", {"form": form})
...
def change_editors(request, name):
    ...
    if not (
        ...
        permission_denied(
            request, "You do not have permission to change this document's editors"
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if set(new_editors) != set(previous_editors):
                ...
                e.desc = (
                    f'Editors changed to {", ".join([p.name for p in new_editors])}'
                )
                e.save()
    ...
    return render(
        request,
        "doc/bofreq/change_editors.html",
        {
            "form": form,
            "doc": bofreq,
            "titletext": titletext,
...
class ChangeResponsibleForm(forms.Form):
    ...
    def clean_responsible(self):
        ...
        if not_leadership:
            raise forms.ValidationError(
                "Only current IAB and IESG members are allowed. Please remove: "
                + ", ".join([person.plain_name() for person in not_leadership])
            )
        return responsible
...
def change_responsible(request, name):
    if not has_role(request.user, ("Secretariat", "Area Director", "IAB")):
        permission_denied(
            request,
            "You do not have permission to change this document's responsible leadership",
        )
    bofreq = get_object_or_404(Document, type="bofreq", name=name)
    ...
    return render(
        request,
        "doc/bofreq/change_responsible.html",
        {
            "form": form,
            "doc": bofreq,
            "titletext": titletext,
...
def edit_title(request, name):
    ...
    if not (
        has_role(request.user, ("Secretariat", "Area Director", "IAB"))
        or (state_id == "proposed" and request.user.person in editors)
    ):
        permission_denied(
            request, "You do not have permission to edit this document's title"
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            c = DocEvent(
                type="added_comment", doc=bofreq, rev=bofreq.rev, by=request.user.person
            )
...
class ChangeStateForm(forms.Form):
    new_state = forms.ModelChoiceField(
        State.objects.filter(type="bofreq", used=True),
    ...
    comment = forms.CharField(
        ...
        help_text="Optional comment for the state change history entry.",
        required=False,
        strip=False,
    )
...
def change_state(request, name, option=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if new_state != prev_state:
                ...
                events = []
                events.append(
                    add_state_change_event(bofreq, login, prev_state, new_state)
                )
    ...
    return render(
        ...
        dict(
            ...
            login=login,
            help_url=urlreverse(
                "ietf.doc.views_help.state_help", kwargs=dict(type="bofreq")
            ),
        ),
    )
