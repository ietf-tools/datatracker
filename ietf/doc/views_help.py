...
def state_index(request):
    ...
    for type in types:
        if "-" in type.slug and type.slug.split("-", 1)[0] in names:
            ...
            type.stategroups = [
                g.slug[len(type.slug) + 1 :] for g in groups if not g == type
            ] or ""
...
def state_help(request, type=None):
    slug, title = {
        ...
        "draft-iana-action": (
            "draft-iana-action",
            "IANA Action States for Internet-Drafts",
        ),
        "draft-iana-review": (
            "draft-iana-review",
            "IANA Review States for Internet-Drafts",
        ),
        "draft-iana-experts": (
            "draft-iana-experts",
            "IANA Expert Review States for Internet-Drafts",
        ),
        "draft-stream-ietf": (
            "draft-stream-ietf",
            "IETF Stream States for Internet-Drafts",
        ),
        "draft-stream-irtf": (
            "draft-stream-irtf",
            "IRTF Stream States for Internet-Drafts",
        ),
        "draft-stream-ise": (
            "draft-stream-ise",
            "ISE Stream States for Internet-Drafts",
        ),
        "draft-stream-iab": (
            "draft-stream-iab",
            "IAB Stream States for Internet-Drafts",
        ),
        "draft-stream-editorial": (
            "draft-stream-editorial",
            "Editorial Stream States for Internet-Drafts",
        ),
...
def relationship_help(request, subset=None):
    subsets = {
        "reference": ["refnorm", "refinfo", "refunk", "refold"],
