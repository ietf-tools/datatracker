...
class StatusChangeTests(TestCase):
    def test_start_review(self):
        ...
        r = self.client.post(
            url,
            dict(
                document_name="imaginary-mid-review",
                title="Bogus Title",
                ad=ad_strpk,
                create_in_state=state_strpk,
                notify="ipu@ietf.org",
        ...
        r = self.client.post(
            url,
            dict(
                document_name="imaginary-new",
                title="A new imaginary status change",
                ad=ad_strpk,
                create_in_state=state_strpk,
                notify="ipu@ietf.org",
                new_relation_row_blah="rfc9999",
        ...
        self.assertTrue(
            status_change.relateddocument_set.filter(
                relationship__slug="tois", target__name="rfc9999"
            )
        )
    ...
    def test_change_state(self):
        ...
        self.assertTrue(
            doc.latest_event(DocEvent, type="added_comment").desc.startswith("RDNK84ZD")
        )
        ...
        self.assertTrue(
            doc.latest_event(DocEvent, type="added_comment").desc.startswith("TGmZtEjt")
        )
        self.assertTrue(doc.active_ballot())
        self.assertEqual(
            doc.latest_event(
                BallotPositionDocEvent, type="changed_ballot_position"
            ).pos_id,
            "yes",
        )
        ...
        appr_sent_pk = str(
            State.objects.get(used=True, slug="appr-sent", type__slug="statchg").pk
        )
    ...
    def test_edit_notices(self):
        doc = Document.objects.get(name="status-change-imaginary-mid-review")
        url = urlreverse(
            "ietf.doc.views_doc.edit_notify;status-change", kwargs=dict(name=doc.name)
        )
    ...
    def test_edit_title(self):
        doc = Document.objects.get(name="status-change-imaginary-mid-review")
        url = urlreverse(
            "ietf.doc.views_status_change.edit_title", kwargs=dict(name=doc.name)
        )
    ...
    def test_edit_ad(self):
        doc = Document.objects.get(name="status-change-imaginary-mid-review")
        url = urlreverse(
            "ietf.doc.views_status_change.edit_ad", kwargs=dict(name=doc.name)
        )
    ...
    def test_edit_telechat_date(self):
        doc = Document.objects.get(name="status-change-imaginary-mid-review")
        url = urlreverse(
            "ietf.doc.views_doc.telechat_date;status-change", kwargs=dict(name=doc.name)
        )
        ...
        r = self.client.post(
            url, dict(telechat_date=telechat_date.isoformat(), returning_item="on")
        )
        self.assertEqual(r.status_code, 302)
        doc = Document.objects.get(name="status-change-imaginary-mid-review")
    ...
    def test_edit_lc(self):
        doc = Document.objects.get(name="status-change-imaginary-mid-review")
        url = urlreverse(
            "ietf.doc.views_status_change.last_call", kwargs=dict(name=doc.name)
        )
        ...
        # save
        r = self.client.post(
            url, dict(last_call_text="Bogus last call text", save_last_call_text="1")
        )
        self.assertEqual(r.status_code, 200)

        last_call_event = doc.latest_event(
        ...
        self.assertTrue("Last Call:" in outbox[-1]["Subject"])
        self.assertTrue(
            "Last Call Request has been submitted"
            in "".join(wrap(outbox[-1].as_string(), width=2**16))
        )

    def test_approve(self):
        doc = Document.objects.get(name="status-change-imaginary-mid-review")
        url = urlreverse(
            "ietf.doc.views_status_change.approve", kwargs=dict(name=doc.name)
        )
        ...
        create_ballot_if_not_open(
            None, doc, Person.objects.get(user__username="secretary"), "statchg"
        )
        ...
        self.assertTrue(
            "(rfc9998) to Historic"
            in "".join(wrap(outbox[-1].as_string() + outbox[-2].as_string(), 2**16))
        )
        self.assertTrue(
            "(rfc9999) to Internet Standard"
            in "".join(wrap(outbox[-1].as_string() + outbox[-2].as_string(), 2**16))
        )
    ...
    def approval_pend_notice_test_helper(self, role):
        ...
        doc.relateddocument_set.create(
            target=Document.objects.get(name="rfc14"), relationship_id="updates"
        )
        ...
        # Issue the request
        appr_pend_pk = str(
            State.objects.get(used=True, slug="appr-pend", type__slug="statchg").pk
        )
        r = self.client.post(
            url, dict(new_state=appr_pend_pk, comment="some comment or other")
        )
        ...
        if role == "ad":
            ...
            self.assertNotIn(
                "No value found for", notification_text
            )  # make sure all interpolation values were set
        else:
    ...
    def test_edit_relations(self):
        doc = Document.objects.get(name="status-change-imaginary-mid-review")
        url = urlreverse(
            "ietf.doc.views_status_change.edit_relations", kwargs=dict(name=doc.name)
        )
        ...
        r = self.client.post(
            url, dict(new_relation_row_blah="rfc9997", statchg_relation_row_blah="tois")
        )
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
        ...
        r = self.client.post(
            url, dict(new_relation_row_blah="rfc9999", statchg_relation_row_blah="")
        )
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
        self.assertTrue(len(q("form ul.errorlist")) > 0)
        ...
        r = self.client.post(
            url,
            dict(new_relation_row_blah="rfc9999", statchg_relation_row_blah="badslug"),
        )
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
        self.assertTrue(len(q("form ul.errorlist")) > 0)
    ...
    def test_clear_ballot(self):
        ...
        url = urlreverse(
            ...
            kwargs=dict(name=doc.name, ballot_type_slug="statchg"),
        )
        login_testing_unauthorized(self, "secretary", url)
    ...
    def setUp(self):
        super().setUp()
        IndividualRfcFactory(
            rfc_number=14, std_level_id="unkn"
...
class StatusChangeSubmitTests(TestCase):
    ...
    def test_initial_submission(self):
        doc = Document.objects.get(name="status-change-imaginary-mid-review")
        url = urlreverse(
            "ietf.doc.views_status_change.submit", kwargs=dict(name=doc.name)
        )
        login_testing_unauthorized(self, "ad", url)
        ...
        q = PyQuery(r.content)
        self.assertTrue(
            q("textarea")[0].text.strip().startswith("Provide a description")
        )
        ...
        with ftp_filepath.open() as f:
            self.assertEqual(f.read(), "Some initial review text\n")
        self.assertEqual(
            retrieve_str("statchg", basename), "Some initial review text\n"
        )
    ...
    def test_subsequent_submission(self):
        ...
        self.assertEqual(doc.rev, "00")
        path = os.path.join(
            settings.STATUS_CHANGE_PATH, "%s-%s.txt" % (doc.name, doc.rev)
        )
        ...
        q = PyQuery(r.content)
        self.assertTrue(
            q("textarea")[0].text.strip().startswith("This is the old proposal.")
        )
        ...
        with io.open(path) as f:
            ...
            f.close()
        self.assertEqual(
            retrieve_str("statchg", f"{doc.name}-{doc.rev}.txt"), test_content
        )
        ...
        url = urlreverse(
            "ietf.doc.views_doc.document_main", kwargs=dict(name=doc.name, rev="00")
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
