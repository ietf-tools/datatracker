...
class ChangeStateForm(forms.Form):
    new_state = forms.ModelChoiceField(
        State.objects.filter(type="statchg", used=True),
    ...
    comment = forms.CharField(
        ...
        help_text="Optional comment for the review history.",
        required=False,
        strip=False,
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not has_role(user, "Secretariat"):
            self.fields["new_state"].queryset = self.fields[
                "new_state"
            ].queryset.exclude(slug="appr-sent")
...
def change_state(request, name, option=None):
        ...
        if form.is_valid():
            ...
            if new_state != prev_state:
                ...
                if new_state.slug == "iesgeval":
                    ...
                    ballot = status_change.latest_event(
                        BallotDocEvent, type="created_ballot"
                    )
                    if has_role(
                        ...
                        pos = BallotPositionDocEvent(
                            doc=status_change, rev=status_change.rev, by=login
                        )
                        pos.ballot = ballot
                ...
                elif new_state.slug == "appr-pend" and has_role(
                    request.user, "Area Director"
                ):
    ...
    return render(
        ...
        dict(
            ...
            login=login,
            help_url=reverse(
                "ietf.doc.views_help.state_help", kwargs=dict(type="status-change")
            ),
        ),
    )
...
class UploadForm(forms.Form):
    content = forms.CharField(
        widget=forms.Textarea,
        label="Status change text",
        help_text="Edit the status change text.",
        required=False,
        strip=False,
    )
    txt = forms.FileField(
        ...
        if "submit_response" in request.POST:
            ...
            init = {
                "content": render_to_string(
                    "doc/status_change/initial_template.txt", dict()
                )
            }
...
def default_approval_text(status_change, relateddoc):
    ...
    if relateddoc.target.std_level_id in (
        "std",
        "ps",
        "ds",
        "bcp",
    ...
    addrs = gather_address_lists(
        "ballot_approved_status_change", doc=status_change
    ...
    text = render_to_string(
        ...
        dict(
            ...
            + relateddoc.target.get_absolute_url(),
...
class AnnouncementForm(forms.Form):
    announcement_text = forms.CharField(
        widget=forms.Textarea,
        label="Status Change Announcement",
        help_text="Edit the announcement message.",
        required=True,
        strip=False,
    )
    label = None
...
def approve(request, name):
        ...
        if formset.is_valid():
            ...
            for form in formset.forms:
                ...
                send_mail_preformatted(
                    request, form.cleaned_data["announcement_text"], extra={}
                )
            ...
            for rel in status_change.relateddocument_set.filter(
                ...
                c = DocEvent(
                    type="added_comment", doc=rel.target, rev=rel.target.rev, by=login
                )
            ...
            init.append(
                {
                    ...
                    "label": "Announcement text for %s to %s"
                    % (rel.target.name, newstatus(rel)),
                }
            )
    ...
    return render(
        request,
        "doc/status_change/approve.html",
        dict(
            doc=status_change,
            formset=formset,
        ),
...
class StartStatusChangeForm(forms.Form):
    document_name = forms.CharField(
        max_length=255,
        label="Document name",
        help_text="A descriptive name such as status-change-md2-to-historic is better than status-change-rfc1319.",
        required=True,
    )
    title = forms.CharField(max_length=255, label="Title", required=True)
    ...
    create_in_state = forms.ModelChoiceField(
        State.objects.filter(type="statchg", slug__in=("needshep", "adrev")),
        empty_label=None,
        required=False,
    )
    ...
    def clean_document_name(self):
        ...
        if re.search("[^a-z0-9-]", name):
            errors.append(
                "The name of the document may only contain digits, lowercase letters and dashes"
            )
        if re.search("--", name):
            errors.append(
        ...
        if name.startswith("-"):
            errors.append(
                "status-change- will be added automatically as a prefix, starting with a - will result in status-change-%s"
                % name
            )
        ...
        if name.endswith("CHANGETHIS"):
            errors.append(
                "Please change CHANGETHIS to reflect the intent of this status change"
            )
        if errors:
            raise forms.ValidationError(errors)
        return name
...
def rfc_status_changes(request):
    ...
    return render(
        request,
        "doc/status_change/status_changes.html",
        {
            "docs": doclist,
        },
    )
        ...
        if form.is_valid():
            ...
            for key in form.cleaned_data["relations"]:
                status_change.relateddocument_set.create(
                    target=Document.objects.get(name=key),
...
def edit_relations(request, name):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            c = DocEvent(
                type="added_comment", doc=status_change, rev=status_change.rev, by=login
            )
            c.desc = "Affected RFC list changed.\nOLD:"
            for relname, relslug in set(old_relations.items()) - set(
                new_relations.items()
            ):
                c.desc += (
                    "\n  "
                    + relname
                    + ": "
                    + DocRelationshipName.objects.get(slug=relslug).name
                )
            c.desc += "\nNEW:"
            for relname, relslug in set(new_relations.items()) - set(
                old_relations.items()
            ):
                c.desc += (
                    "\n  "
    ...
    return render(
        request,
        "doc/status_change/edit_relations.html",
        {
            "doc": status_change,
            "form": form,
            "relation_slugs": relation_slugs,
    ...
    new_text = render_to_string(
        ...
        dict(
            ...
            changes=[
                "%s from %s to %s\n    (%s)"
                % (
                    rel.target.name.upper(),
                    rel.target.std_level.name,
                    newstatus(rel),
                    rel.target.title,
                )
...
def last_call(request, name):
    ...
    last_call_event = status_change.latest_event(
    ...
    if request.method == "POST":
        if "save_last_call_text" in request.POST or (
            "send_last_call_request" in request.POST and status_change.ad is not None
        ):
            ...
            if form.is_valid():
                ...
                if "send_last_call_request" in request.POST:
                    ...
                    e = add_state_change_event(
                        status_change, login, prev_state, new_state
                    )
                    if e:
                        events.append(e)
