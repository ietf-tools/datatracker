...
class IssueIRSGBallotTests(TestCase):
    ...
    def test_issue_ballot_button(self):
        ...
        url = urlreverse(
            "ietf.doc.views_doc.document_main", kwargs=dict(name=individual_draft.name)
        )
        ...
        url = urlreverse(
            "ietf.doc.views_doc.document_main", kwargs=dict(name=wg_draft.name)
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        self.assertNotIn("Issue IRSG ballot", unicontent(r))
        ...
        url = urlreverse(
            "ietf.doc.views_doc.document_main", kwargs=dict(name=rg_draft.name)
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        ...
        url = urlreverse(
            "ietf.doc.views_doc.document_main", kwargs=dict(name=rg_rfc.name)
        )
    ...
    def test_close_ballot_button(self):
        ...
        url = urlreverse(
            "ietf.doc.views_ballot.issue_irsg_ballot", kwargs=dict(name=rg_draft1.name)
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        ...
        # Get the page with the Issue IRSG Ballot Yes/No buttons
        url = urlreverse(
            "ietf.doc.views_ballot.issue_irsg_ballot", kwargs=dict(name=rg_draft2.name)
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        ...
        self.client.logout()
        url = urlreverse(
            "ietf.doc.views_doc.document_main", kwargs=dict(name=rg_draft1.name)
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        ...
        url = urlreverse(
            "ietf.doc.views_ballot.close_irsg_ballot", kwargs=dict(name=rg_draft1.name)
        )
        r = self.client.get(url)
        ...
        # The close button should now be available
        url = urlreverse(
            "ietf.doc.views_doc.document_main", kwargs=dict(name=rg_draft2.name)
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        self.assertIn("Close IRSG ballot", unicontent(r))
        ...
        # Get the page with the Close IRSG Ballot Yes/No buttons
        url = urlreverse(
            "ietf.doc.views_ballot.close_irsg_ballot", kwargs=dict(name=rg_draft2.name)
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
    ...
    def test_issue_ballot(self):
        ...
        self.assertEqual(r.status_code, 302)
        ballot_created = list(
            BallotDocEvent.objects.filter(doc=rg_draft1, type="created_ballot")
        )
        ...
        self.assertNotIn(
            "IRSG Evaluation Ballot has not been created yet", unicontent(r)
        )

        # We should find an IRSG member's name on the IRSG evaluation tab regardless of any positions taken or not
        url = urlreverse(
            "ietf.doc.views_doc.document_irsg_ballot", kwargs=dict(name=rg_draft1.name)
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
    ...
    def test_edit_ballot_position_permissions(self):
        ...
        rg_ballot = create_ballot_if_not_open(
            None, rg_draft, secr.person, "irsg-approve", due
        )

        url = urlreverse(
            "ietf.doc.views_ballot.edit_position",
            kwargs=dict(name=wg_draft.name, ballot_id=wg_ballot.pk),
        )
        ...
        url = urlreverse(
            "ietf.doc.views_ballot.edit_position",
            kwargs=dict(name=rg_draft.name, ballot_id=rg_ballot.pk),
        )
...
class BaseManipulationTests:
    ...
    def test_issue_ballot(self):
        ...
        url = urlreverse(
            "ietf.doc.views_ballot.issue_irsg_ballot", kwargs=dict(name=draft.name)
        )
    ...
    def test_close_ballot(self):
        ...
        url = urlreverse(
            "ietf.doc.views_ballot.close_irsg_ballot", kwargs=dict(name=draft.name)
        )
        empty_outbox()
    ...
    def test_view_outstanding_ballots(self):
        ...
        close_ballot(
            draft, Person.objects.get(user__username=self.username), "irsg-approve"
        )
        r = self.client.get(url)
...
class SecretariatTests(BaseManipulationTests, TestCase):
    ...
    def setUp(self):
        ...
        self.balloter = "?balloter={}".format(
            Person.objects.get(user__username="irtf-chair").pk
        )
...
class NobodyTests(TestCase):
    ...
    def can_see_IRSG_tab(self):
        ...
        ballot = IRSGBallotDocEventFactory(doc=draft)
        BallotPositionDocEventFactory(
            ballot=ballot, by=get_active_irsg()[0], pos_id="yes", comment="b2390sn3"
        )

        url = urlreverse(
            "ietf.doc.views_doc.document_irsg_ballot", kwargs=dict(name=draft.name)
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
