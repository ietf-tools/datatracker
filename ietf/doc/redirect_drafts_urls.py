...
urlpatterns = [
    ...
    url(
        r"^(?P<object_id>\d+)/(related/)?$",
        RedirectView.as_view(url="/doc/", permanent=True),
    ),
    url(
        r"^(?P<name>[^/]+)/(related/)?$",
    ...
    url(
        r"^wgid/(?P<id>\d+)/$",
        lambda request, id: HttpResponsePermanentRedirect(
            "/wg/%s/" % get_object_or_404(Group, id=id).acronym
        ),
    ),
    url(
        r"^wg/(?P<acronym>[^/]+)/$",
        RedirectView.as_view(url="/wg/%(acronym)s/", permanent=True),
    ),
    url(
        r"^all_id(?:_txt)?.html$",
        RedirectView.as_view(
            url="%s/all_id.txt" % settings.IETF_ID_ARCHIVE_URL, permanent=True
        ),
    ),
]
