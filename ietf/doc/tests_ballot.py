...
class EditPositionTests(TestCase):
    ...
    def test_api_set_position(self):
        ...
        mailbox_before = len(outbox)
        r = self.client.post(
            url, dict(apikey=apikey.hash(), doc=draft.name, position="noobj")
        )
        self.assertEqual(r.status_code, 200)
        ...
        mailbox_before = len(outbox)
        r = self.client.post(
            url, dict(apikey=apikey.hash(), doc=draft.name, position="norecord")
        )
        self.assertEqual(r.status_code, 200)
    ...
    def test_send_ballot_comment(self):
        ...
        BallotPositionDocEvent.objects.create(
            ...
            balloter=ad,
            ballot=ballot,
            pos=BallotPositionName.objects.get(slug="discuss"),
        ...
        r = self.client.post(
            url,
            dict(
                extra_cc="test298347@example.com",
...
class BallotWriteupsTests(TestCase):
    def test_edit_last_call_text(self):
        ...
        # save
        r = self.client.post(
            url, dict(last_call_text="This is a simple test.", save_last_call_text="1")
        )
        self.assertEqual(r.status_code, 200)
        draft = Document.objects.get(name=draft.name)
        self.assertTrue(
            "This is a simple test"
            in draft.latest_event(WriteupDocEvent, type="changed_last_call_text").text
        )
        ...
        r = self.client.post(
            url,
            dict(
                last_call_text="This is a simple test.", regenerate_last_call_text="1"
            ),
        )
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
    ...
    def test_request_last_call(self):
        ...
        # send
        r = self.client.post(url, dict(last_call_text=text, send_last_call_request="1"))
        draft = Document.objects.get(name=draft.name)
    ...
    def test_edit_ballot_writeup(self):
        draft = IndividualDraftFactory(
            states=[("draft", "active"), ("draft-iesg", "iesg-eva")], stream_id="ietf"
        )
        url = urlreverse(
            "ietf.doc.views_ballot.ballot_writeupnotes", kwargs=dict(name=draft.name)
        )
        login_testing_unauthorized(self, "secretary", url)

        # add a IANA review note
        draft.set_state(
            State.objects.get(used=True, type="draft-iana-review", slug="not-ok")
        )
        DocEvent.objects.create(
            type="iana_review",
            doc=draft,
        ...
        # save
        r = self.client.post(
            url, dict(ballot_writeup="This is a simple test.", save_ballot_writeup="1")
        )
        self.assertEqual(r.status_code, 200)
        d = Document.objects.get(name=draft.name)
        self.assertTrue(
            "This is a simple test"
            in d.latest_event(WriteupDocEvent, type="changed_ballot_writeup_text").text
        )
    ...
    def test_edit_ballot_writeup_already_approved(self):
        draft = IndividualDraftFactory(
            states=[("draft", "active"), ("draft-iesg", "approved")], stream_id="ietf"
        )
        url = urlreverse(
            "ietf.doc.views_ballot.ballot_writeupnotes", kwargs=dict(name=draft.name)
        )
    ...
    def test_edit_ballot_rfceditornote(self):
        draft = IndividualDraftFactory()
        url = urlreverse(
            "ietf.doc.views_ballot.ballot_rfceditornote", kwargs=dict(name=draft.name)
        )
        login_testing_unauthorized(self, "secretary", url)
        ...
        r = self.client.post(
            url,
            dict(
                rfc_editor_note="This is a simple test.", save_ballot_rfceditornote="1"
            ),
        )
        self.assertEqual(r.status_code, 302)
        draft = Document.objects.get(name=draft.name)
        ...
        r = self.client.post(
            url, dict(rfc_editor_note=" ", clear_ballot_rfceditornote="1")
        )
        self.assertEqual(r.status_code, 200)
        draft = Document.objects.get(name=draft.name)
        ...
        r = self.client.post(
            url,
            dict(rfc_editor_note="This is a new note.", save_ballot_rfceditornote="1"),
        )
        self.assertEqual(r.status_code, 302)
        ...
        for case in ("none", "past", "future"):
            ...
            if case in ("past", "future"):
                LastCallDocEvent.objects.create(
                    ...
                    + datetime.timedelta(days=1 if case == "future" else -1),
                )
            url = urlreverse(
                "ietf.doc.views_ballot.ballot_writeupnotes",
    ...
    def test_issue_ballot_auto_state_change(self):
        ...
        draft = IndividualDraftFactory(
            ad=ad,
            states=[("draft", "active"), ("draft-iesg", "writeupw")],
            stream_id="ietf",
        )
        url = urlreverse(
            "ietf.doc.views_ballot.ballot_writeupnotes", kwargs=dict(name=draft.name)
        ...
        self.assertEqual(len(q("textarea[name=ballot_writeup]")), 1)
        self.assertFalse(
            q('[class=form-text]:contains("not completed IETF Last Call")')
        )
        self.assertTrue(q('[type=submit]:contains("Save")'))
        ...
        # save
        r = self.client.post(
            url, dict(ballot_writeup="This is a simple test.", issue_ballot="1")
        )
        self.assertEqual(r.status_code, 200)
        d = Document.objects.get(name=draft.name)
        self.assertTrue("iesg-eva" == d.get_state_slug("draft-iesg"))
    ...
    def test_issue_ballot_warn_if_early(self):
        ad = Person.objects.get(user__username="ad")
        draft = IndividualDraftFactory(
            ad=ad, states=[("draft", "active"), ("draft-iesg", "lc")], stream_id="ietf"
        )
        url = urlreverse(
            "ietf.doc.views_ballot.ballot_writeupnotes", kwargs=dict(name=draft.name)
        )
        ...
        self.assertEqual(len(q("textarea[name=ballot_writeup]")), 1)
        self.assertTrue(
            q('[class=text-danger]:contains("not completed IETF Last Call")')
        )
        self.assertTrue(q('[type=submit]:contains("Save")'))
        ...
        LastCallDocEvent.objects.filter(doc=draft).update(
            expires=datetime_today() - datetime.timedelta(days=2)
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
        ...
        for state_slug in ["lc", "ad-eval"]:
            ...
            self.assertTrue(
                q(
                    '[class=text-danger]:contains("It would be unexpected to issue a ballot while in this state.")'
                )
            )
    ...
    def test_edit_approval_text(self):
        ...
        url = urlreverse(
            "ietf.doc.views_ballot.ballot_approvaltext", kwargs=dict(name=draft.name)
        )
        login_testing_unauthorized(self, "secretary", url)
        ...
        # save
        r = self.client.post(
            url, dict(approval_text="This is a simple test.", save_approval_text="1")
        )
        self.assertEqual(r.status_code, 200)
        draft = Document.objects.get(name=draft.name)
        self.assertTrue(
    ...
    def test_edit_verify_permissions(self):
        ...
        ad = Person.objects.get(user__username="ad")
        draft = WgDraftFactory(
            group=mars, ad=ad, states=[("draft", "active"), ("draft-iesg", "ad-eval")]
        )
...
class ApproveBallotTests(TestCase):
    ...
    def test_approve_ballot(self, mock_urlopen):
        ...
        draft.set_state(
            State.objects.get(used=True, type="draft-iesg", slug="iesg-eva")
        )  # make sure it's approvable

        url = urlreverse(
            "ietf.doc.views_ballot.approve_ballot", kwargs=dict(name=draft.name)
        )
        login_testing_unauthorized(self, "secretary", url)
        ...
        self.assertEqual(
            len(q('form pre:contains("This is a note for the RFC Editor")')), 0
        )
        ...
        self.assertEqual(len(q('form pre:contains("Subject: Protocol Action")')), 1)
        self.assertEqual(
            len(q('form pre:contains("This is a note for the RFC Editor")')), 1
        )
        ...
        self.assertTrue(
            "Protocol Action" in draft.message_set.order_by("-time")[0].subject
        )
    ...
    def test_clear_ballot(self):
        ...
        url = urlreverse(
            ...
            kwargs=dict(name=draft.name, ballot_type_slug="approve"),
        )
        login_testing_unauthorized(self, "secretary", url)
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        ...
        url = urlreverse(
            "ietf.doc.views_ballot.clear_ballot",
            kwargs=dict(name=draft.name, ballot_type_slug="statchg"),
        )
        r = self.client.post(url, {})
        self.assertEqual(r.status_code, 404)
    ...
    def test_ballot_downref_approve(self):
        ...
        url = urlreverse(
            "ietf.doc.views_ballot.approve_downrefs", kwargs=dict(name=draft.name)
        )
        ...
        d = [
            rdoc
            for rdoc in draft.relateddocument_set.all()
            if rel.is_approved_downref()
        ]
        original_len = len(d)
        r = self.client.get(url)
...
class MakeLastCallTests(TestCase):
    def test_make_last_call(self):
        ...
        url = urlreverse(
            "ietf.doc.views_ballot.make_last_call", kwargs=dict(name=draft.name)
        )
        login_testing_unauthorized(self, "secretary", url)
        ...
        self.assertEqual(
            draft.latest_event(LastCallDocEvent, "sent_last_call").expires.strftime(
                "%Y-%m-%d"
            ),
            expire_date,
        )
    ...
    def test_make_last_call_yang_document(self):
        ...
        r = self.client.post(
            url,
            dict(
                last_call_sent_date=last_call_sent_date,
                last_call_expiration_date=expire_date,
            ),
        )
        self.assertEqual(r.status_code, 302)
...
class DeferUndeferTestCase(TestCase):
    def helper_test_defer(self, name):
        ...
        url = urlreverse(
            "ietf.doc.views_ballot.defer_ballot", kwargs=dict(name=doc.name)
        )
        ...
        defer_states = dict(
            draft=["draft-iesg", "defer"],
            conflrev=["conflrev", "defer"],
            statchg=["statchg", "defer"],
        )
        if doc.type_id in defer_states:
            self.assertEqual(
                doc.get_state(defer_states[doc.type_id][0]).slug,
                defer_states[doc.type_id][1],
            )
        ...
        if doc.type_id == "draft":
            ...
            self.assertIn(
                "Changed action holders",
                doc.latest_event(type="changed_action_holders").desc,
            )
        else:
    ...
    def helper_test_undefer(self, name):
        ...
        doc = Document.objects.get(name=name)
        url = urlreverse(
            "ietf.doc.views_ballot.undefer_ballot", kwargs=dict(name=doc.name)
        )
        ...
        if doc.type_id in defer_states:
            doc.set_state(
                State.objects.get(
                    used=True,
                    type=defer_states[doc.type_id][0],
                    slug=defer_states[doc.type_id][1],
                )
            )
        ...
        undefer_states = dict(
            draft=["draft-iesg", "iesg-eva"],
            conflrev=["conflrev", "iesgeval"],
            statchg=["statchg", "iesgeval"],
        )
        if doc.type_id in undefer_states:
            self.assertEqual(
                doc.get_state(undefer_states[doc.type_id][0]).slug,
                undefer_states[doc.type_id][1],
            )
    ...
    def setUp(self):
        ...
        DocumentFactory(
            ...
            states=[("statchg", "iesgeval")],
        )
        DocumentFactory(
            type_id="conflrev",
            name="conflict-review-imaginary-irtf-submission",
            states=[("conflrev", "iesgeval")],
        )
...
class IetfFiltersTests(TestCase):
    def test_can_defer(self):
        ...
        rsab_member = RoleFactory(
            group=Group.objects.get(acronym="rsab"), name_id="member"
        ).person.user
        nobody = PersonFactory().user

        users = set(
            [secretariat, ad, irtf_chair, rsab_chair, irsg_member, rsab_member, nobody]
        )
        ...
        rsab_ballot = BallotDocEventFactory(
            ballot_type__slug="rsab-approve", doc__stream_id="editorial"
        )
        for user in users:
...
class BallotContentTests(TestCase):
    ...
    def _assertBallotMessage(self, q, balloter, expected):
        ...
        next = heading.next()
        self.assertEqual(
            len(next.find(f'*[title="{expected}"]')),
            1,
        )
    ...
    def test_document_ballot_content_email_sent(self):
        ...
        q = PyQuery(content)
        self._assertBallotMessage(
            q, balloters[0], "Email requested to be sent for this discuss"
        )
        self._assertBallotMessage(
            q, balloters[1], "Email requested to be sent for this comment"
        )
        self._assertBallotMessage(
            q, balloters[2], "No email send requests for this discuss"
        )
        self._assertBallotMessage(
            q, balloters[3], "No email send requests for this comment"
        )
        self._assertBallotMessage(
            q, balloters[4], "Email requested to be sent for earlier discuss"
        )
        self._assertBallotMessage(
            q, balloters[5], "Email requested to be sent for earlier comment"
        )
        self._assertBallotMessage(
            q, no_email_balloter, "No email send requests for this ballot position"
        )
        self._assertBallotMessage(
            q,
            send_email_balloter,
            "Email requested to be sent for this ballot position",
        )
        self._assertBallotMessage(
            q,
            prev_send_email_balloter,
            "Email requested to be sent for earlier ballot position",
        )
    ...
    def test_document_ballot_content_without_send_email_values(self):
        ...
        self._assertBallotMessage(
            q, old_balloter, "No ballot position send log available"
        )
...
class ReturnToUrlTests(TestCase):
    def test_invalid_return_to_url(self):
        with self.assertRaises(ValueError):
            parse_ballot_edit_return_point(
                "/", "draft-ietf-opsawg-ipfix-tcpo-v6eh", "998718"
            )
    ...
    def test_valid_default_return_to_url(self):
        self.assertEqual(
            parse_ballot_edit_return_point(
                None, "draft-ietf-opsawg-ipfix-tcpo-v6eh", "998718"
            ),
