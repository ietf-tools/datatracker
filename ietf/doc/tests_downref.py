...
class Downref(TestCase):
    ...
    def test_downref_last_call(self):
        ...
        doc9999 = WgDraftFactory(name='draft-ietf-mars-another-approved-document',states=[('draft-iesg','rfcqueue')])
        ...
        self.assertTrue('The document contains these normative downward references' in text)
        ...
        q = PyQuery(r.content)
        text = q("[name=last_call_text]").text()
        self.assertFalse('The document contains these normative downward references' in text)
