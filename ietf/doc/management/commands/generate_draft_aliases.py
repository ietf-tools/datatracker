...
def get_draft_notify_emails(doc):
    ...
    ad_email_alias_regex = fr"^{doc.name}.ad@({settings.DRAFT_ALIAS_DOMAIN}|{settings.TOOLS_SERVER})$"
    all_email_alias_regex = fr"^{doc.name}.all@({settings.DRAFT_ALIAS_DOMAIN}|{settings.TOOLS_SERVER})$"
    author_email_alias_regex = fr"^{doc.name}@({settings.DRAFT_ALIAS_DOMAIN}|{settings.TOOLS_SERVER})$"
    notify_email_alias_regex = fr"^{doc.name}.notify@({settings.DRAFT_ALIAS_DOMAIN}|{settings.TOOLS_SERVER})$"
    shepherd_email_alias_regex = fr"^{doc.name}.shepherd@({settings.DRAFT_ALIAS_DOMAIN}|{settings.TOOLS_SERVER})$"
    notify_emails = set()
    if doc.notify:
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        signature = f'# Generated by {__file__} at {date}\n'
