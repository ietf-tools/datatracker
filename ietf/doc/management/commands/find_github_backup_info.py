...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            dest="verbose",
            action="store_true",
            help="Show counts of types of repositories",
        )

    def handle(self, *args, **options):

        if not (
            hasattr(settings, "GITHUB_BACKUP_API_KEY")
            and settings.GITHUB_BACKUP_API_KEY
        ):
            raise CommandError(
                "ERROR: can't find GITHUB_BACKUP_API_KEY"
            )  # TODO: at >= py3.1, use returncode
