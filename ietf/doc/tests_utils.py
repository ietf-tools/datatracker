...
class ActionHoldersTests(TestCase):
            ...
            for next_state in initial_state.next_states.all():
                ...
                if next_state.slug in DocumentActionHolder.CLEAR_ACTION_HOLDERS_STATES:
                    ...
                    self.assertCountEqual(
                        doc.action_holders.all(),
                        [doc.ad],
                        "AD should be only action holder after transition to %s"
                        % next_state.slug,
                    )
    ...
    def test_update_action_holders_resets_age(self):
        ...
        dah.time_added = datetime.datetime(
        ...
        self.assertLess(
            doc.documentactionholder_set.get(person=self.ad).time_added, right_now
        )
    ...
    def test_update_action_holders_add_tag_need_rev(self):
        ...
        self.update_doc_state(
            doc, doc.get_state("draft-iesg"), add_tags=["need-rev"], remove_tags=None
        )
    ...
    def test_update_action_holders_remove_tag_need_rev(self):
        ...
        self.assertEqual(doc.action_holders.count(), 0)
        self.update_doc_state(
            doc, doc.get_state("draft-iesg"), add_tags=None, remove_tags=["need-rev"]
        )
        self.assertEqual(doc.action_holders.count(), 0)
...
class MiscTests(TestCase):
    def test_update_documentauthors_with_nulls(self):
        ...
        doc.documentauthor_set.update(affiliation="Some Affiliation", country="USA")
        try:
    ...
    def do_fuzzy_find_documents_rfc_test(self, name):
        ...
        self.assertCountEqual(
            found.documents, [], "Should not find document that does not match"
        )
        found = fuzzy_find_documents(draft.name + "-incorrect", "02")
        self.assertCountEqual(
            found.documents, [], "Still should not find document, even with a version"
        )
        found = fuzzy_find_documents(draft.name, "22")
        self.assertCountEqual(
            found.documents, [draft], "Should find document even if rev does not exist"
        )

        # by rfc name mistakenly trying to provide a revision
        found = fuzzy_find_documents(rfc.name + "-22")
        self.assertCountEqual(
            found.documents, [rfc], "Should ignore versions when fuzzyfinding RFCs"
        )
        found = fuzzy_find_documents(rfc.name, "22")
        self.assertCountEqual(
            found.documents, [rfc], "Should ignore versions when fuzzyfinding RFCs"
...
class RebuildReferenceRelationsTests(TestCase):
    def setUp(self):
        ...
        self.normative, self.informative, self.unknown = WgRfcFactory.create_batch(
            3
        )  # AMHERE - these need to have rfc names.
        ...
        self.doc.relateddocument_set.create(
            target=self.updated, relationship_id="updates"
        )
        self.assertCountEqual(
            self.doc.relateddocument_set.values_list("relationship__slug", flat=True),
        ...
        for other_doc in [self.normative, self.informative, self.unknown]:
            self.assertEqual(
                self.doc.relateddocument_set.filter(
                    target__name=other_doc.name
                ).count(),
                0,
    ...
    def test_requires_txt_or_xml(self):
        ...
        self.assertIn(
            "No Internet-Draft text available",
            result["errors"][0],
    ...
    def test_xml(self, mock_init, mock_get_refs):
        ...
        self.assertEqual(
            xmldraft_init_args,
            ("file.xml",),
            "XMLDraft initialized with unexpected arguments",
        )
        self.assertEqual(
            result,
            {
        ...
        self.assertCountEqual(
            self.doc.relateddocument_set.values_list(
                "target__name", "relationship__slug"
            ),
            [
    ...
    def test_plaintext(self, mock_init, mock_get_refs):
        ...
        self.assertEqual(
            mock_init_kwargs,
            {"text": "contents", "source": temp_file_name},
    ...
    def test_xml_and_plaintext(self, mock_init, mock_get_refs, mock_plaintext_init):
        ...
        result = rebuild_reference_relations(
            self.doc, {"txt": "file.txt", "xml": "file.xml"}
        )

        self.assertFalse(
            mock_plaintext_init.called,
            "PlaintextDraft should not be used when XML is available",
        )
...
class DraftBibxmlTests(TestCase):
    ...
    def test_create_draft_bibxml_file(self, mock):
        ...
        doc = DocumentFactory()
        ref_path = (
            bibxml_path / f"reference.I-D.{doc.name}-26.xml"
        )  # we're pretending it's rev 26
