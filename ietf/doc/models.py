...
class DocumentInfo(models.Model):
    ...
    def render_txt_and_html(self):
        """Generate TXT and HTML formats from XML"""

        if self.get_state_slug() == "rfc":
            xml_path = Path(
                os.path.join(settings.RFC_PATH, self.canonical_name() + ".xml")
            )
        else:
            xml_path = Path(
                os.path.join(
                    settings.INTERNET_ALL_DRAFTS_ARCHIVE_DIR,
                    self.name + "-" + self.rev + ".xml",
                )
            )

        # check if we have cached versions and if so, return them
        cache = caches["htmlized"]
        cache_key = xml_path.stem
        try:
            txt = cache.get("txt-" + cache_key)
        except EOFError:
            txt = None
        try:
            html = cache.get("html-" + cache_key)
        except EOFError:
            html = None
        if txt and html:
            return (txt, html)

        # No cached versions, process them
        xml2rfc.log.write_out = io.StringIO()
        xml2rfc.log.write_err = io.StringIO()

        # Parse the XML
        parser = xml2rfc.XmlRfcParser(str(xml_path), quiet=True)
        xmltree = parser.parse(remove_comments=False)

        # If we have v2, run it through v2v3
        ...
        cache.set("txt-" + cache_key, txt, settings.HTMLIZER_CACHE_TIME)
        cache.set("html-" + cache_key, html, settings.HTMLIZER_CACHE_TIME)
        return (txt, html)

    def text(self):
        text = self.render_txt_and_html()[0]
        if (text):
            return text

        # use the existing TXT instead of freshly rendered one
    ...
    def html_body(self, classes=""):
        html = self.render_txt_and_html()[1]

        if not html:
            # use existing HTML instead of freshly rendered one
            if self.get_state_slug() == "rfc":
                try:
                    html = Path(
