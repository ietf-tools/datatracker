        ...
        if create and extracted:
            ...
            for person in extracted:
                DocumentAuthor.objects.create(
                    document=obj, person=person, email=person.email(), order=order
                )
                order += 1
...
class StatusChangeFactory(BaseDocumentFactory):
    ...
    group = factory.SubFactory(
        "ietf.group.factories.GroupFactory", acronym="iesg", type_id="ietf"
    )
    ...
    def changes_status_of(obj, create, extracted, **kwargs):
        ...
        if extracted:
            ...
            obj.relateddocument_set.create(
                relationship_id="tobcp", target=WgRfcFactory()
            )
...
class ConflictReviewFactory(BaseDocumentFactory):
    ...
    @factory.lazy_attribute_sequence
    def name(self, n):
        return draft_name_generator(self.type_id, self.group, n).replace(
        ...
        if extracted:
            ...
            obj.relateddocument_set.create(
                ...
                target=DocumentFactory(
                    name=obj.name.replace("conflict-review-", "draft-"),
                    type_id="draft",
                    group=Group.objects.get(type_id="individ"),
                ),
            )
...
class ReviewFactory(BaseDocumentFactory):
    ...
    name = factory.LazyAttribute(
        lambda o: "review-doesnotexist-00-%s-%s"
        % (o.group.acronym, date_today().isoformat())
    )
...
class DocEventFactory(factory.django.DjangoModelFactory):
    ...
    doc: Any = factory.SubFactory(
        DocumentFactory
    )  # `Any` to appease mypy when a subclass overrides doc
...
class BofreqFactory(BaseDocumentFactory):
    ...
    name = factory.LazyAttribute(
        lambda o: "bofreq-%s-%s" % (xslugify(o.requester_lastname), xslugify(o.title))
    )

    bofreqeditordocevent = factory.RelatedFactory(
        "ietf.doc.factories.BofreqEditorDocEventFactory", "doc"
    )
    bofreqresponsibledocevent = factory.RelatedFactory(
        "ietf.doc.factories.BofreqResponsibleDocEventFactory", "doc"
    )
...
class EditorialDraftFactory(BaseDocumentFactory):
    ...
    type_id = "draft"
    group = factory.SubFactory(
        "ietf.group.factories.GroupFactory", acronym="rswg", type_id="edwg"
    )
    stream_id = "editorial"
    ...
    def states(obj, create, extracted, **kwargs):
        ...
        if extracted:
            ...
            obj.set_state(State.objects.get(type_id="draft", slug="active"))
            obj.set_state(
                State.objects.get(type_id="draft-stream-editorial", slug="active")
            )
            obj.set_state(State.objects.get(type_id="draft-iesg", slug="idexists"))
...
class SubseriesFactory(factory.django.DjangoModelFactory):
    ...
    def contains(obj, create, extracted, **kwargs):
        ...
        if extracted:
            ...
            obj.relateddocument_set.create(
                relationship_id="contains", target=RfcFactory()
            )
