...
class EditAuthorsTests(IetfSeleniumTestCase):
    ...
    def test_add_author_forms(self):
        def _fill_in_author_form(form_elt, name, email, affiliation, country):
            ...
            email_select = form_elt.find_element(
                By.CSS_SELECTOR, 'select[name$="email"]'
            )
            email_option = self.wait.until(
                presence_of_element_child_by_css_selector(
                    email_select, 'option[value="{}"]'.format(email)
                )
            )
            email_option.click()  # select the email

            # Fill in the affiliation and country. Finally, simple text inputs!
            affil_input = form_elt.find_element(
                By.CSS_SELECTOR, 'input[name$="affiliation"]'
            )
            affil_input.send_keys(affiliation)
            country_input = form_elt.find_element(
        ...
        def _read_author_form(form_elt):
            ...
            """
            hidden_person_input = form_elt.find_element(
                By.CSS_SELECTOR, 'select[name$="person"]'
            )
            email_select = form_elt.find_element(
                By.CSS_SELECTOR, 'select[name$="email"]'
            )
        ...
        for index, auth in enumerate(authors):
            self.scroll_and_click(
                (By.ID, "add-author-button")
            )  # Create new form. Automatically scrolls to it.
            author_forms = authors_list.find_elements(By.CLASS_NAME, "author-panel")
            authors_added = index + 1
            self.assertEqual(
                len(author_forms), authors_added + 1
            )  # Started with 1 author, hence +1
            _fill_in_author_form(
                author_forms[index + 1],
                auth.name,
                str(auth.email()),
                orgs[index],
                countries[index],
        ...
        self.assertEqual(
            ...
            (
                first_auth.person,
                str(first_auth.email),
                first_auth.affiliation,
                first_auth.country,
            ),
        )
        for index, auth in enumerate(authors):
