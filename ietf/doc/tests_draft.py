...
class ChangeStateTests(TestCase):
    def test_ad_approved(self):
        ...
        draft = WgDraftFactory(
            ad=ad, states=[("draft", "active"), ("draft-iesg", "iesg-eva")]
        )
        DocEventFactory(
        ...
        url = urlreverse(
            "ietf.doc.views_draft.change_state", kwargs=dict(name=draft.name)
        )
        ...
        r = self.client.post(
            ...
            dict(
                state=State.objects.get(
                    used=True, type="draft-iesg", slug="approved"
                ).pk,
                substate="",
        ...
        self.assertTrue(
            "Removed all action holders" in draft.docevent_set.all()[1].desc
        )
    ...
    def test_change_state(self):
        ...
        if next_states:
            self.assertEqual(
                len(q('[type=submit]:contains("%s")' % next_states[0].name)), 1
            )

        # faulty post
        r = self.client.post(
            url,
            dict(state=State.objects.get(used=True, type="draft", slug="active").pk),
        )
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
        ...
        r = self.client.post(
            ...
            dict(
                state=State.objects.get(
                    used=True, type="draft-iesg", slug="review-e"
                ).pk,
                substate="need-rev",
                comment="Test comment",
            ),
        ...
        q = PyQuery(r.content)
        self.assertEqual(
            len(q('form [type=submit]:contains("%s")' % first_state.name)), 1
        )
        ...
        r = self.client.post(
            url,
            dict(
                state=State.objects.get(used=True, type="draft-iesg", slug="ann").pk,
                comment="Test comment",
            ),
        )
        self.assertEqual(r.status_code, 200)
    ...
    def test_change_iana_state(self):
        ...
        first_state = State.objects.get(
            used=True, type="draft-iana-review", slug="need-rev"
        )
        next_state = State.objects.get(
            used=True, type="draft-iana-review", slug="ok-noact"
        )
        draft.set_state(first_state)

        url = urlreverse(
            "ietf.doc.views_draft.change_iana_state",
            kwargs=dict(name=draft.name, state_type="iana-review"),
        )
    ...
    def test_change_iana_expert_review_state(self):
        ...
        first_state = State.objects.get(
            used=True, type="draft-iana-experts", slug="reviews-assigned"
        )
        next_state = State.objects.get(
            used=True, type="draft-iana-experts", slug="reviewers-ok"
        )
        ...
        url = urlreverse(
            "ietf.doc.views_draft.change_iana_state",
            kwargs=dict(name=draft.name, state_type="iana-experts"),
        )
        login_testing_unauthorized(self, "iana", url)
    ...
    def test_add_expert_review_comment(self):
        ...
        self.assertEqual(r.status_code, 302)
        self.assertEqual(
            draft.latest_event(IanaExpertDocEvent, type="comment").desc, "!2ab3x#1"
        )
    ...
    def test_request_last_call(self):
        ...
        r = self.client.post(
            url,
            dict(
                state=State.objects.get(used=True, type="draft-iesg", slug="lc-req").pk
            ),
        )
        self.assertEqual(r.status_code, 200)
    ...
    def test_iesg_state_edit_button(self):
        ad = Person.objects.get(user__username="ad")
        draft = WgDraftFactory(
            ad=ad, states=[("draft", "active"), ("draft-iesg", "ad-eval")]
        )
...
class EditInfoTests(TestCase):
    def test_edit_info(self):
        ...
        r = self.client.post(
            url,
            dict(
                intended_std_level=str(draft.intended_std_level.pk),
    ...
    def test_edit_telechat_date(self):
        ...
        draft = Document.objects.get(name=draft.name)
        self.assertTrue(
            draft.latest_event(TelechatDocEvent, type="scheduled_for_telechat")
        )
        self.assertEqual(
            draft.latest_event(
                TelechatDocEvent, type="scheduled_for_telechat"
            ).telechat_date,
            TelechatDate.objects.active()[0].date,
        )
        self.assertEqual(len(outbox), mailbox_before + 1)
        ...
        draft = Document.objects.get(name=draft.name)
        telechat_event = draft.latest_event(
            TelechatDocEvent, type="scheduled_for_telechat"
        )
        self.assertEqual(
            telechat_event.telechat_date, TelechatDate.objects.active()[1].date
        )
    ...
    def test_start_iesg_process_on_draft(self):
        ...
        self.assertCountEqual(
            draft.docevent_set.exclude(id__in=events_before).values_list(
                "type", flat=True
            ),
            [
    ...
    def test_edit_consensus(self):
        ...
        url = urlreverse(
            "ietf.doc.views_draft.edit_consensus", kwargs=dict(name=draft.name)
        )
        login_testing_unauthorized(self, "secretary", url)
        ...
        # post
        self.assertTrue(
            not draft.latest_event(ConsensusDocEvent, type="changed_consensus")
        )
        r = self.client.post(url, dict(consensus="Yes"))
        self.assertEqual(r.status_code, 302)
        draft = Document.objects.get(name=draft.name)
        self.assertEqual(
            draft.latest_event(ConsensusDocEvent, type="changed_consensus").consensus,
            True,
        )
        ...
        e.desc = "Intended Status changed to <b>%s</b> from %s" % (
...
class DraftFileMixin:
    ...
    def setUp(self):
        ...
        (
            Path(settings.INTERNET_DRAFT_ARCHIVE_DIR) / "expired_without_tombstone"
        ).mkdir()
...
class ResurrectTests(DraftFileMixin, TestCase):
    def test_request_resurrect(self):
        ...
        url = urlreverse(
            "ietf.doc.views_draft.request_resurrect", kwargs=dict(name=draft.name)
        )
    ...
    def test_resurrect(self):
        ...
        self.assertTrue(
            not os.path.exists(os.path.join(settings.INTERNET_DRAFT_ARCHIVE_DIR, txt))
        )
...
class ExpireIDsTests(DraftFileMixin, TestCase):
    def test_in_draft_expire_freeze(self):
        ...
        self.assertFalse(
            in_draft_expire_freeze(
                (second_cut_off - datetime.timedelta(days=7)).replace(
                    hour=0, minute=0, second=0
                )
            )
        )
        self.assertFalse(
            in_draft_expire_freeze(second_cut_off.replace(hour=0, minute=0, second=0))
        )
        self.assertTrue(
            in_draft_expire_freeze(
                (second_cut_off + datetime.timedelta(days=7)).replace(
        ...
        self.assertFalse(
            in_draft_expire_freeze(
                datetime.datetime.combine(
                    ietf_monday, datetime.time(0, 0, 0), tzinfo=datetime.timezone.utc
                )
            )
        )
    ...
    def test_warn_expirable_drafts(self):
        ...
        RoleFactory(
            group=mars, name_id="ad", person=Person.objects.get(user__username="ad")
        )
        ...
        send_expire_warning_for_draft(draft)
        self.assertEqual(
            len(outbox), mailbox_before, "Sent expiration warning for dead draft"
        )
        ...
        send_expire_warning_for_draft(draft)
        self.assertEqual(
            len(outbox), mailbox_before, "Sent expiration warning for replaced draft"
        )
    ...
    def test_expire_drafts(self):
        ...
        self.assertFalse(
            expirable_drafts(Document.objects.filter(pk=draft.pk)).exists()
        )
        draft.set_state(
            State.objects.get(used=True, type="draft-iesg", slug="idexists")
        )
        ...
        self.assertIn(
            ...
            draft.latest_event(type="changed_action_holders").desc,
        )
        self.assertTrue(
            not os.path.exists(os.path.join(settings.INTERNET_DRAFT_PATH, txt))
        )
        self.assertFalse(exists_in_storage("active-draft", f"txt/{txt}"))
        self.assertTrue(
            os.path.exists(os.path.join(settings.INTERNET_DRAFT_ARCHIVE_DIR, txt))
        )
    ...
    def test_clean_up_draft_files(self):
        ...
        self.assertTrue(
            not os.path.exists(os.path.join(settings.INTERNET_DRAFT_PATH, unknown))
        )
        self.assertFalse(exists_in_storage("active-draft", f"txt/{unknown}"))
        self.assertTrue(
            os.path.exists(
                os.path.join(
                    settings.INTERNET_DRAFT_ARCHIVE_DIR, "unknown_ids", unknown
                )
            )
        )
        ...
        self.assertTrue(
            not os.path.exists(os.path.join(settings.INTERNET_DRAFT_PATH, malformed))
        )
        self.assertFalse(exists_in_storage("active-draft", f"txt/{malformed}"))
        self.assertTrue(
            os.path.exists(
                os.path.join(
                    settings.INTERNET_DRAFT_ARCHIVE_DIR, "unknown_ids", malformed
                )
            )
        )
        ...
        self.assertTrue(
            not os.path.exists(os.path.join(settings.INTERNET_DRAFT_PATH, pdf))
        )
        self.assertFalse(exists_in_storage("active-draft", f"pdf/{pdf}"))
        self.assertTrue(
            os.path.exists(os.path.join(settings.INTERNET_DRAFT_ARCHIVE_DIR, pdf))
        )
...
class ExpireLastCallTests(TestCase):
    def test_expire_last_call(self):
        ...
        e = LastCallDocEvent(
            doc=draft, rev=draft.rev, type="sent_last_call", by=secretary
        )
    ...
    def test_expire_last_call_with_downref(self):
        ...
        self.assertEqual(len(outbox), mailbox_before + 2)
        self.assertTrue(
            "Review Downrefs From Expired Last Call" in outbox[-1]["Subject"]
        )
        self.assertTrue(d.ad.email().address in outbox[-1]["To"])
        self.assertCountEqual(d.action_holders.all(), [ad])
        self.assertIn(
            "Changed action holders", d.latest_event(type="changed_action_holders").desc
        )
...
class IndividualInfoFormsTests(TestCase):
    ...
    def setUp(self):
        ...
        doc = WgDraftFactory(
            ...
            shepherd=PersonFactory(
                user__username="plain", name="Plain Man"
            ).email_set.first(),
        )
    ...
    def test_doc_change_stream(self):
        url = urlreverse(
            "ietf.doc.views_draft.change_stream", kwargs=dict(name=self.docname)
        )
        login_testing_unauthorized(self, "secretary", url)
        ...
        self.assertTrue(
            "7gRMTjBM" in doc.latest_event(DocEvent, type="added_comment").desc
        )
    ...
    def test_doc_change_notify(self):
        url = urlreverse(
            "ietf.doc.views_doc.edit_notify", kwargs=dict(name=self.docname)
        )
        login_testing_unauthorized(self, "secretary", url)
    ...
    def test_doc_change_intended_status(self):
        url = urlreverse(
            "ietf.doc.views_draft.change_intention", kwargs=dict(name=self.docname)
        )
        login_testing_unauthorized(self, "secretary", url)
        ...
        self.assertTrue(
            "ZpyQFGmA" in doc.latest_event(DocEvent, type="added_comment").desc
        )

    def test_doc_change_telechat_date(self):
        url = urlreverse(
            "ietf.doc.views_doc.telechat_date", kwargs=dict(name=self.docname)
        )
        login_testing_unauthorized(self, "secretary", url)
        ...
        doc = Document.objects.get(name=self.docname)
        self.assertEqual(
            doc.latest_event(TelechatDocEvent, "scheduled_for_telechat").telechat_date,
            telechat_date,
        )
        self.assertEqual(len(outbox), 1)
    ...
    def test_doc_change_ad_allows_pre_ad(self):
        ...
        q = PyQuery(r.content)
        self.assertEqual(
            len(q(f'form select[name=ad] option[value="{pre_ad.pk}"]')),
            1,
    ...
    def test_doc_change_shepherd(self):
        ...
        url = urlreverse(
            "ietf.doc.views_draft.edit_shepherd", kwargs=dict(name=self.docname)
        )
        ...
        self.assertEqual(
            set(comment_events),
            set(doc.docevent_set.filter(time=doc.time, type="added_comment")),
        )
        r = self.client.get(url)
        self.assertTrue(
            any(
                [
                    "no changes have been made" in m.message
        ...
        self.assertTrue(
            any(
                [
                    "Document shepherd changed to (None)" in x.desc
                    for x in doc.docevent_set.filter(
                        time=doc.time, type="added_comment"
                    )
                ]
            )
        )
    ...
    def test_doc_change_shepherd_email(self):
        ...
        url = urlreverse(
            "ietf.doc.views_draft.change_shepherd_email", kwargs=dict(name=self.docname)
        )
        r = self.client.get(url)
        ...
        new_email = Email.objects.create(
            address="anotheremail@example.com",
            person=doc.shepherd.person,
            origin=doc.shepherd.person.user.username,
        )
        ...
        self.assertTrue(
            comment_event.desc.startswith("Document shepherd email changed")
        )
    ...
    def test_doc_change_shepherd_writeup(self):
        url = urlreverse(
            "ietf.doc.views_draft.edit_shepherd_writeup", kwargs=dict(name=self.docname)
        )
        ...
        # direct edit
        r = self.client.post(
            url, dict(content="here is a new writeup", submit_response="1")
        )
        self.assertEqual(r.status_code, 302)
        doc = Document.objects.get(name=self.docname)
        self.assertTrue(
            ...
            ).text.startswith("here is a new writeup")
        )
        ...
        self.assertTrue(
            doc.latest_event(
                WriteupDocEvent, type="changed_protocol_writeup"
            ).text.startswith("This is a different writeup.")
        )
        ...
        q = PyQuery(r.content)
        self.assertTrue(
            q("textarea")[0].text.strip().startswith("# Document Shepherd Write-Up")
        )  # TODO: This is a poor test of whether the reset did anything

    def test_edit_doc_extresources(self):
        url = urlreverse(
            "ietf.doc.views_draft.edit_doc_extresources", kwargs=dict(name=self.docname)
        )
        ...
        doc = Document.objects.get(name=self.docname)
        self.assertEqual(
            doc.latest_event(DocEvent, type="changed_document").desc[:35],
            "Changed document external resources",
        )
        self.assertIn(
            "github_username githubuser",
            doc.latest_event(DocEvent, type="changed_document").desc,
        )
        self.assertEqual(doc.docextresource_set.count(), 4)
        self.assertEqual(
            doc.docextresource_set.get(name__slug="github_repo").display_name,
            "Some display text",
        )
        self.assertIn(
            doc.docextresource_set.first().name.slug,
            str(doc.docextresource_set.first()),
        )
    ...
    def do_doc_change_action_holders_test(self, username):
        ...
        url = urlreverse(
            "ietf.doc.views_doc.edit_action_holders", kwargs=dict(name=doc.name)
        )
        login_testing_unauthorized(
            self, some_other_chair.user.username, url
        )  # other chair can't edit action holders
        ...
        for role_name in [
            ...
            self.assertEqual(
                len(q('button:contains("Add %s")' % role_name)),
                1,
                'Expected "Add %s" button' % role_name,
            )
            self.assertEqual(
                len(q('button:contains("Remove %s")' % role_name)),
                1,
    ...
    def do_doc_remind_action_holders_test(self, username):
        ...
        url = urlreverse(
            "ietf.doc.views_doc.remind_action_holders", kwargs=dict(name=doc.name)
        )

        login_testing_unauthorized(
            self, some_other_chair.user.username, url
        )  # other chair can't send reminder
        login_testing_unauthorized(self, username, url)
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
        ...
        empty_outbox()
        r = self.client.post(
            url, dict(note="this is my note")
        )  # note should be < 78 chars to avoid wrapping
        self.assertEqual(r.status_code, 302)
...
class RequestPublicationTests(TestCase):
    ...
    def test_request_publication(self, mockobj):
        ...
        draft = IndividualDraftFactory(
            stream_id="iab",
            group__acronym="iab",
            intended_std_level_id="inf",
            states=[("draft-stream-iab", "approved")],
        )

        url = urlreverse(
            "ietf.doc.views_draft.request_publication", kwargs=dict(name=draft.name)
        )
        ...
        self.assertTrue(
            "Document Action" in draft.message_set.order_by("-time")[0].subject
        )
...
class ReleaseDraftTests(TestCase):
    def test_release_wg_draft(self):
        ...
        url = urlreverse(
            "ietf.doc.views_draft.release_draft", kwargs=dict(name=draft.name)
        )
        ...
        self.client.login(
            username=other_chair_role.person.user.username,
            password=other_chair_role.person.user.username + "+password",
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 403)
        ...
        descs = sorted(
            [
                event.desc
                for event in set(list(draft.docevent_set.all())) - set(events_before)
            ]
        )
        ...
        self.assertEqual(
            "Tags Awaiting Merge with Other Document, Document Shepherd Followup cleared.",
            descs[3],
        )
    ...
    def test_release_ise_draft(self):
        ...
        self.client.login(
            username=ise.person.user.username,
            password=ise.person.user.username + "+password",
        )
        ...
        self.assertEqual(
            "Tag Waiting for Dependency on Other Document cleared.", descs[3]
        )
...
class AdoptDraftTests(TestCase):
    def test_adopt_document(self):
        ...
        for type_id in ("wg", "ag", "rg", "rag", "edwg"):
            ...
            # call for adoption
            group_type_can_call_for_adoption = State.objects.filter(
                type_id=stream_state_type_slug[type_id], slug="c-adopt"
            ).exists()
            if group_type_can_call_for_adoption:
                empty_outbox()
                events_before = draft.docevent_set.count()
                call_issued = State.objects.get(
                    type=stream_state_type_slug[type_id], slug="c-adopt"
                )
                r = self.client.post(
                    url,
                    dict(
                ...
                draft = Document.objects.get(pk=draft.pk)
                self.assertEqual(
                    draft.get_state_slug(stream_state_type_slug[type_id]), "c-adopt"
                )
                self.assertEqual(draft.group, chair_role.group)
                self.assertEqual(
                    draft.stream_id, stream_state_type_slug[type_id][13:]
                )  # trim off "draft-stream-"
                self.assertEqual(draft.docevent_set.count() - events_before, 5)
                self.assertEqual(len(outbox), 1)
                self.assertTrue("Call For Adoption" in outbox[-1]["Subject"])
                self.assertTrue(
                    f"{chair_role.group.acronym}-chairs@" in outbox[-1]["To"]
                )
            ...
            stream_adopt_state = State.objects.get(
                type=stream_state_type_slug[type_id], slug=stream_adopt_state_slug
            )
            r = self.client.post(
                url,
                dict(
            ...
            draft = Document.objects.get(pk=draft.pk)
            self.assertEqual(
                draft.get_state_slug(stream_state_type_slug[type_id]),
                stream_adopt_state_slug,
            )
            self.assertEqual(draft.group, chair_role.group)
            self.assertEqual(
                draft.stream_id, stream_state_type_slug[type_id][13:]
            ...
            if type_id in ("wg", "ag"):
                self.assertEqual(
                    Counter(
                        list(draft.docevent_set.values_list("type", flat=True))[
                            events_before:
                        ]
                    ),
                    Counter(
                        {"changed_group": 1, "changed_stream": 1, "new_revision": 1}
                    ),
                )
            else:
                self.assertEqual(
...
class ChangeStreamStateTests(TestCase):
    def test_set_tags(self):
        ...
        draft = WgDraftFactory(
            group=role.group,
            shepherd=PersonFactory(
                user__username="plain", user__email="plain@example.com"
            ).email_set.first(),
        )
    ...
    def test_set_initial_state(self):
        ...
        RoleFactory(
            name_id="delegate",
            group=role.group,
            person__user__email="marsdelegate@ietf.org",
        )
        ...
        new_state = State.objects.get(
            used=True, type="draft-stream-%s" % draft.stream_id, slug="parked"
        )
        empty_outbox()
        events_before = draft.docevent_set.count()
        ...
        draft = Document.objects.get(pk=draft.pk)
        self.assertEqual(
            draft.get_state("draft-stream-%s" % draft.stream_id), new_state
        )
        self.assertEqual(draft.docevent_set.count() - events_before, 2)
    ...
    def test_pubreq_validation(self):
        ...
        old_state = draft.get_state("draft-stream-%s" % draft.stream_id)
        new_state = State.objects.get(
            used=True, type="draft-stream-%s" % draft.stream_id, slug="sub-pub"
        )
        self.assertNotEqual(old_state, new_state)
...
class ChangeReplacesTests(TestCase):
    def setUp(self):
        ...
        e = Email.objects.create(
            address="basea_author@example.com", person=p, origin=p.user.username
        )
        ...
        self.baseb = WgDraftFactory(
            ...
            - datetime.timedelta(days=365 - settings.INTERNET_DRAFT_DAYS_TO_EXPIRE),
        )
        p = PersonFactory(name="baseb_author")
        e = Email.objects.create(
            address="baseb_author@example.com", person=p, origin=p.user.username
        )
        ...
        e = Email.objects.create(
            address="replacea_author@example.com", person=p, origin=p.user.username
        )
        ...
        e = Email.objects.create(
            address="replaceboth_author@example.com", person=p, origin=p.user.username
        )
        ...
        self.replacea.set_state(
            State.objects.get(used=True, type="draft", slug="active")
        )
        self.replaceboth.set_state(
            State.objects.get(used=True, type="draft", slug="active")
        )

    def test_change_replaces(self):
        url = urlreverse(
            "ietf.doc.views_draft.replaces", kwargs=dict(name=self.replacea.name)
        )
        login_testing_unauthorized(self, "secretary", url)
        ...
        self.assertEqual(
            RelatedDocument.objects.filter(
                relationship__slug="replaces", source=self.replacea
            ).count(),
            1,
        )
        self.assertEqual(
            Document.objects.get(name="draft-test-base-a").get_state().slug, "repl"
        )
        self.assertTrue(
            not RelatedDocument.objects.filter(
                relationship="possibly-replaces", source=self.replacea
            )
        )
        self.assertEqual(len(outbox), 1)
        ...
        url = urlreverse(
            "ietf.doc.views_draft.replaces", kwargs=dict(name=self.replaceboth.name)
        )
        ...
        self.assertEqual(
            Document.objects.get(name="draft-test-base-b").get_state().slug, "repl"
        )
        self.assertEqual(len(outbox), 1)
        self.assertTrue("basea_author@" in outbox[-1]["To"])
        ...
        self.assertEqual(
            Document.objects.get(name="draft-test-base-a").get_state().slug, "repl"
        )  # Because A is still also replaced by replacea
        self.assertEqual(
            Document.objects.get(name="draft-test-base-b").get_state().slug, "expired"
        )
        self.assertEqual(len(outbox), 1)
        self.assertTrue("basea_author@" in outbox[-1]["To"])
        self.assertTrue("baseb_author@" in outbox[-1]["To"])
        ...
        self.assertEqual(r.status_code, 302)
        self.assertEqual(
            Document.objects.get(name="draft-test-base-a").get_state().slug, "active"
        )
        self.assertTrue("basea_author@" in outbox[-1]["To"])
        self.assertTrue("replacea_author@" in outbox[-1]["To"])
    ...
    def test_review_possibly_replaces(self):
        ...
        url = urlreverse(
            ...
            kwargs=dict(name=self.replacea.name),
        )
        login_testing_unauthorized(self, "secretary", url)
        ...
        self.assertEqual(
            Document.objects.get(pk=self.basea.pk).get_state().slug, "repl"
        )
...
class MoreReplacesTests(TestCase):
    ...
    def test_stream_state_changes_when_replaced(self):
        ...
        for stream in ("iab", "irtf", "ise"):
            ...
            old_doc.set_state(
                State.objects.get(
                    type_id="draft-stream-%s" % stream,
                    slug="ise-rev" if stream == "ise" else "active",
                )
            )
            new_doc = IndividualDraftFactory(stream_id=stream)

            url = urlreverse(
                "ietf.doc.views_draft.replaces", kwargs=dict(name=new_doc.name)
            )
...
class ShepherdWriteupTests(TestCase):
    ...
    def test_shepherd_writeup_generation(self):
        ...
        url = urlreverse(
            ...
            kwargs=dict(name=ind_draft.name),
        )
        login_testing_unauthorized(self, "secretary", url)
        r = self.client.get(url)
        ...
        url = urlreverse(
            "ietf.doc.views_draft.edit_shepherd_writeup",
            kwargs=dict(name=wg_draft.name),
        )
        r = self.client.get(url)
