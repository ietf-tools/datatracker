...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        orm.StateType.objects.filter(slug='statchg').update(label='RFC Status Change state')
        for event in orm.StateDocEvent.objects.filter(desc__startswith='RFC Status Change changed to'):
            event.desc = 'RFC Status Change state '+event.desc[len('RFC Status Change '):]
            event.save()

    def backwards(self, orm):
        orm.StateType.objects.filter(slug='statchg').update(label='RFC Status Change')
        for event in orm.StateDocEvent.objects.filter(desc__startswith='RFC Status Change state changed to'):
            event.desc = 'RFC Status Change '+event.desc[len('RFC Status Change state '):]
            event.save()
