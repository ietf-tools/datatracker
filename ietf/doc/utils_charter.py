...
def historic_milestones_for_charter(charter, rev):
    ...
    e = charter.docevent_set.filter(
        time__gt=revision_event.time, type="new_revision"
    ).order_by("time")
    if not chartering:
...
def default_action_text(group, charter, by):
    ...
    addrs = gather_address_lists(
        "ballot_approved_charter", doc=charter, group=group
    ...
    e.text = render_to_string(
        ...
        dict(
            ...
            + urlreverse(
                "ietf.group.views.group_home", kwargs=dict(acronym=group.acronym)
            ),
...
def derive_new_work_text(review_text, group):
    addrs = gather_address_lists(
        "charter_external_review_new_work", group=group
    ).as_strings()
    (m, _, _) = parse_preformatted(
        review_text,
        override={
            "To": addrs.to,
            "Cc": addrs.cc,
...
def default_review_text(group, charter, by):
    now = timezone.now()
    addrs = gather_address_lists("charter_external_review", group=group).as_strings(
        compact=False
    )
...
def generate_issue_ballot_mail(request, doc, ballot):
    ...
    return render_to_string(
        "doc/charter/issue_ballot_mail.txt",
        dict(
            doc=doc,
...
def replace_charter_of_replaced_group(group, by):
    ...
    if charter:
        ...
        state_change_event = StateDocEvent.objects.create(
            state_type_id="charter",
            state=replaced_state,
            doc=charter,
            rev=charter.rev,
            by=by,
            type="changed_state",
            desc="Charter's group has been replaced",
        )
