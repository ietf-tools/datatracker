...
def email_state_changed(request, doc, text, mailtrigger_id=None):
    ...
    send_mail(
        ...
        dict(text=text, url=settings.IDTRACKER_BASE_URL + doc.get_absolute_url()),
        cc=cc,
    )
...
def email_ad_approved_conflict_review(request, review, ok_to_publish):
    ...
    send_mail(
        ...
        dict(ok_to_publish=ok_to_publish, review=review, conflictdoc=conflictdoc),
        cc=cc,
    )
...
def email_ad_approved_status_change(request, status_change, related_doc_info):
    ...
    send_mail(
        ...
        dict(related_doc_info=related_doc_info),
        cc=cc,
    )
...
def email_remind_action_holders(request, doc, note=None):
    ...
    log.assertion(
        ...
        note="All action holders should receive a reminder email. Failed for %s."
        % doc.name,
    )
    send_mail(
        request,
...
def generate_ballot_writeup(request, doc):
    ...
    e.text = force_str(
        render_to_string("doc/mail/ballot_writeup.txt", {"iana": iana, "doc": doc})
    )
...
def generate_last_call_announcement(request, doc):
    ...
    if iprs:
        ipr_links = [
            urlreverse("ietf.ipr.views.show", kwargs=dict(id=i.id)) for i in iprs
        ]
        ipr_links = [
...
def generate_approval_mail(request, doc):
    if doc.get_state_slug(
        "draft-iesg"
    ) in DO_NOT_PUBLISH_IESG_STATES or doc.stream_id in ("ise", "irtf"):
...
def generate_approval_mail_approved(request, doc):
    ...
    if doc.group.type_id not in (
        ...
        "rg",
        "rag",
    ) and not doc.group.name.endswith("Working Group"):
    ...
    if doc.group.type_id not in ("individ", "area"):
        responsible_directors.update(
            [x.person for x in Role.objects.filter(group=doc.group.parent, name="ad")]
        )
...
def email_resurrect_requested(request, doc, by):
    ...
    send_mail(
        ...
        dict(doc=doc, by=frm, url=settings.IDTRACKER_BASE_URL + doc.get_absolute_url()),
        cc=cc,
    )
...
def generate_issue_ballot_mail(request, doc, ballot):
    ...
    return render_to_string(
        "doc/mail/issue_iesg_ballot_mail.txt",
        dict(
            doc=doc,
            doc_url=settings.IDTRACKER_BASE_URL + doc.get_absolute_url(),
...
def extra_automation_headers(doc):
    extra = {}
    extra["X-IETF-Draft-string"] = [
        doc.name,
    ]
    extra["X-IETF-Draft-revision"] = [
        doc.rev,
    ]
...
def email_last_call_expired(doc):
    ...
    text = (
        ...
        % doc.get_state("draft-iesg" if doc.type_id == "draft" else "statchg").name
    )
    ...
    send_mail(
        ...
        dict(
            text=text, doc=doc, url=settings.IDTRACKER_BASE_URL + doc.get_absolute_url()
        ),
        cc=addrs.cc,
    )
...
def email_intended_status_changed(request, doc, text):
    ...
    send_mail(
        ...
        to,
        None,
        "Intended Status for %s changed to %s"
        % (doc.file_tag(), doc.intended_std_level),
...
def send_review_possibly_replaces_request(request, doc, submitter_info):
    ...
    possibly_replaces = Document.objects.filter(
        name__in=[related.name for related in doc.related_that_doc("possibly-replaces")]
    )
...
def email_charter_internal_review(request, charter):
    ...
    send_mail(
        ...
        extra={
            "Reply-To": [
                "irsg@irtf.org" if charter.group.type_id == "rg" else "iesg@ietf.org",
            ]
        },
    )
...
def email_iana_expert_review_state_changed(request, events):
    ...
    send_mail(
        ...
        dict(
            event=events[0],
            url=settings.IDTRACKER_BASE_URL + events[0].doc.get_absolute_url(),
        ),
        cc=addrs.cc,
    )


def send_external_resource_change_request(
    request, doc, submitter_info, requested_resources
):
...
def email_bofreq_editors_changed(request, bofreq, previous_editors):
    ...
    addrs = gather_address_lists(
        "bofreq_editors_changed", doc=bofreq, previous_editors=previous_editors
    )
    ...
    send_mail(
        ...
        dict(
            bofreq=bofreq,
            request=request,
            editors=plain_names(editors),
            previous_editors=plain_names(previous_editors),
        ),
        cc=addrs.cc,
    )
...
def email_bofreq_responsible_changed(request, bofreq, previous_responsible):
    ...
    send_mail(
        ...
        dict(
            bofreq=bofreq,
            request=request,
            responsible=plain_names(responsible),
            previous_responsible=plain_names(previous_responsible),
        ),
        cc=addrs.cc,
    )
