...
class ViewCharterTests(TestCase):
    def test_view_revisions(self):
        ...
        url = urlreverse(
            "ietf.doc.views_doc.document_main", kwargs={"name": charter.name}
        )
        r = self.client.get(url)
        q = PyQuery(r.content)
        self.assertIn(
            "The information below is for a proposed recharter. The current approved charter is",
            q(".alert").text(),
        )

        url = urlreverse(
            "ietf.doc.views_doc.document_main",
            kwargs={"name": charter.name, "rev": "02-00"},
        )
        r = self.client.get(url)
        q = PyQuery(r.content)
        self.assertIn(
        ...
        url = urlreverse(
            "ietf.doc.views_doc.document_main",
            kwargs={"name": charter.name, "rev": "02"},
        )
        r = self.client.get(url)
        q = PyQuery(r.content)
        self.assertIn(
        ...
        url = urlreverse(
            "ietf.doc.views_doc.document_main",
            kwargs={"name": charter.name, "rev": "01-00"},
        )
        r = self.client.get(url)
        q = PyQuery(r.content)
        self.assertIn(
            "The information below is for an older proposed", q(".alert").text()
        )

        url = urlreverse(
            "ietf.doc.views_doc.document_main",
            kwargs={"name": charter.name, "rev": "01"},
        )
        r = self.client.get(url)
        q = PyQuery(r.content)
        self.assertIn(
            "The information below is for an older approved", q(".alert").text()
        )
...
class EditCharterTests(TestCase):
    ...
    def write_charter_file(self, charter):
        (Path(settings.CHARTER_PATH) / f"{charter.name}-{charter.rev}.txt").write_text(
            "This is a charter."
        )
    ...
    def test_startstop_process(self):
        ...
        for option in ("recharter", "abandon"):
            ...
            url = urlreverse(
                ...
                kwargs=dict(name=charter.name, option=option),
            )
            ...
            if option == "abandon":
                self.assertTrue(
                    "abandoned"
                    in charter.latest_event(type="changed_document").desc.lower()
                )
                ...
                self.assertTrue(
                    "state changed"
                    in charter.latest_event(type="changed_state").desc.lower()
                )
    ...
    def test_change_state(self):
        ...
        RoleFactory(
            name_id="ad", group=area, person=Person.objects.get(user__username="ad")
        )
        ...
        RoleFactory(
            name_id="ad", group=ames, person=Person.objects.get(user__username="ad")
        )
        RoleFactory(
            name_id="chair",
            group=ames,
            person__name="Ames Man",
            person__user__email="ameschairman@example.org",
        )
        RoleFactory(
            name_id="secr",
            group=ames,
            person__name="Secretary",
            person__user__email="amessecretary@example.org",
        )
        ...
        url = urlreverse(
            "ietf.doc.views_charter.change_state", kwargs=dict(name=charter.name)
        )
        login_testing_unauthorized(self, "secretary", url)
        ...
        for slug in ("intrev", "extrev", "iesgrev"):
            ...
            r = self.client.post(
                url, dict(charter_state=str(s.pk), message="test message")
            )
            self.assertEqual(r.status_code, 302)
            ...
            def find_event(t):
                return [
                    e
                    for e in charter.docevent_set.all()[: events_now - events_before]
                    if e.type == t
                ]

            self.assertTrue(
                "state changed" in find_event("changed_state")[0].desc.lower()
            )
            ...
            if slug == "intrev":
                ...
                for word in [
                    "A new IETF WG",
                    "Chairs",
                    "Ames Man <ameschairman@example.org>",
                    "Secretaries",
                    "Secretary <amessecretary@example.org>",
                    "Assigned Area Director",
                    "Areað Irector <aread@example.org>",
                    "Mailing list",
                    "ames-wg@ietf.org",
                    "Charter",
                    "Milestones",
        ...
        for slug in ("extrev", "iesgrev"):
            ...
            r = self.client.post(
                ...
                dict(
                    charter_state=str(
                        State.objects.get(used=True, type="charter", slug=slug).pk
                    )
                ),
            )
        ...
        r = self.client.post(
            ...
            dict(
                charter_state=str(
                    State.objects.get(used=True, type="charter", slug="intrev").pk
                ),
                message="test",
            ),
        )
    ...
    def test_abandon_bof(self):
        ...
        url = urlreverse(
            "ietf.doc.views_charter.change_state",
            kwargs={"name": charter.name, "option": "abandon"},
        )
        login_testing_unauthorized(self, "secretary", url)
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        response = self.client.post(
            url, {"comment": "Testing Abandoning a BOF Charter"}
        )
        ...
        self.assertTrue(
            "Testing Abandoning"
            in charter.docevent_set.filter(type="added_comment").first().desc
        )

    def test_change_title(self):
        charter = CharterFactory(group__type_id="wg")
        url = urlreverse(
            "ietf.doc.views_charter.change_title", kwargs={"name": charter.name}
        )
        login_testing_unauthorized(self, "secretary", url)
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_edit_telechat_date(self):
        ...
        url = urlreverse(
            "ietf.doc.views_doc.telechat_date;charter", kwargs=dict(name=charter.name)
        )
        login_testing_unauthorized(self, "secretary", url)
        ...
        # add to telechat
        self.assertTrue(
            not charter.latest_event(TelechatDocEvent, "scheduled_for_telechat")
        )
        ...
        r = self.client.post(
            ...
            dict(
                ...
                telechat_date=telechat_date.isoformat(),
            ),
        )
        self.assertEqual(r.status_code, 302)
        ...
        self.assertTrue(
            charter.latest_event(TelechatDocEvent, "scheduled_for_telechat")
        )
        self.assertEqual(
            charter.latest_event(
        ...
        r = self.client.post(
            url,
            dict(name=group.name, acronym=group.acronym, telechat_date=telechat_date),
        )
        self.assertEqual(r.status_code, 302)

        charter = Document.objects.get(name=charter.name)
        self.assertTrue(
            not charter.latest_event(
                TelechatDocEvent, "scheduled_for_telechat"
            ).telechat_date
        )
    ...
    def test_no_returning_item_for_different_ballot(self):
        ...
        charter = Document.objects.get(name=charter.name)
        telechat_event = charter.latest_event(
            TelechatDocEvent, "scheduled_for_telechat"
        )
    ...
    def test_edit_notify(self):
        ...
        url = urlreverse(
            "ietf.doc.views_doc.edit_notify;charter", kwargs=dict(name=charter.name)
        )
        login_testing_unauthorized(self, "secretary", url)
    ...
    def test_edit_ad(self):
        ...
        url = urlreverse(
            "ietf.doc.views_charter.edit_ad", kwargs=dict(name=charter.name)
        )
        login_testing_unauthorized(self, "secretary", url)
    ...
    def test_submit_charter(self):
        ...
        url = urlreverse(
            "ietf.doc.views_charter.submit", kwargs=dict(name=charter.name)
        )
        login_testing_unauthorized(self, "secretary", url)
    ...
    def test_submit_initial_charter(self):
        group = GroupFactory(
            type_id="wg", acronym="mars", list_email="mars-wg@ietf.org"
        )

        url = urlreverse(
            "ietf.doc.views_charter.submit",
            kwargs=dict(name=charter_name_for_group(group)),
        )
        login_testing_unauthorized(self, "secretary", url)
    ...
    def test_submit_charter_with_invalid_name(self):
        ...
        for bad_name in (
            "charter-irtf-{}",
            "charter-randomjunk-{}",
            "charter-ietf-thisisnotagroup",
        ):
            url = urlreverse(
                "ietf.doc.views_charter.submit",
                kwargs={"name": bad_name.format(ietf_group.acronym)},
            )
            r = self.client.get(url)
            self.assertEqual(
                r.status_code, 404, f"GET of charter named {bad_name} should 404"
            )
            r = self.client.post(url, {})
            self.assertEqual(
                r.status_code, 404, f"POST of charter named {bad_name} should 404"
            )
        ...
        for bad_name in (
            "charter-ietf-{}",
            "charter-whatisthis-{}",
            "charter-irtf-thisisnotagroup",
        ):
            url = urlreverse(
                "ietf.doc.views_charter.submit",
                kwargs={"name": bad_name.format(irtf_group.acronym)},
            )
            r = self.client.get(url)
            self.assertEqual(
                r.status_code, 404, f"GET of charter named {bad_name} should 404"
    ...
    def test_edit_review_announcement_text(self):
        ...
        charter = CharterFactory(
            group__parent=area, group__list_email="mars-wg@ietf.org"
        )
        group = charter.group
        ...
        self.assertTrue(
            ...
            in charter.latest_event(
                WriteupDocEvent, type="changed_review_announcement"
            ).text
        )
        self.assertTrue(
            "New work gets something different."
            in charter.latest_event(WriteupDocEvent, type="changed_new_work_text").text
        )
    ...
    def test_edit_action_announcement_text(self):
        ...
        # save
        r = self.client.post(
            url, dict(announcement_text="This is a simple test.", save_text="1")
        )
        self.assertEqual(r.status_code, 302)
        charter = Document.objects.get(name=charter.name)
        self.assertTrue(
            "This is a simple test"
            in charter.latest_event(
                WriteupDocEvent, type="changed_action_announcement"
            ).text
        )

        # test regenerate
        r = self.client.post(
            url, dict(announcement_text="This is a simple test.", regenerate_text="1")
        )
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
        charter = Document.objects.get(name=charter.name)
    ...
    def test_edit_ballot_writeupnotes(self):
        ...
        url = urlreverse(
            "ietf.doc.views_charter.ballot_writeupnotes", kwargs=dict(name=charter.name)
        )
        login_testing_unauthorized(self, "secretary", url)
        ...
        empty_outbox()
        r = self.client.post(
            url, dict(ballot_writeup="This is a simple test.", send_ballot="1")
        )
        self.assertEqual(len(outbox), 1)
    ...
    def test_approve(self):
        ...
        charter = CharterFactory(
            group__acronym="ames",
            group__list_email="ames-wg@ietf.org",
            group__parent=area,
            group__state_id="bof",
        )
        group = charter.group
        ...
        url = urlreverse(
            "ietf.doc.views_charter.approve", kwargs=dict(name=charter.name)
        )
        login_testing_unauthorized(self, "secretary", url)
        ...
        m4 = GroupMilestone.objects.create(
            group=group,
            state_id="charter",
        ...
        charter_path = Path(settings.CHARTER_PATH) / (
            "charter-ietf-%s-%s.txt" % (group.acronym, charter.rev)
        )
        ...
        self.assertEqual(
            group.groupmilestone_set.filter(state="active", desc=m1.desc).count(), 1
        )
        self.assertEqual(
            group.groupmilestone_set.filter(state="active", desc=m4.desc).count(), 1
        )
    ...
    def test_charter_with_milestones(self):
        ...
        url = urlreverse(
            ...
            kwargs=dict(name=charter.name, rev=charter.rev),
        )
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
    ...
    def test_chartering_from_bof(self):
        ...
        charter = CharterFactory(
            group__type_id="wg", group__state_id="bof", group__parent=ad_role.group
        )
        e1, _ = default_review_text(
            charter.group, charter, Person.objects.get(name="(System)")
        )
