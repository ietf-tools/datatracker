...
def downref_registry(request):
    ...
    add_button = has_role(request.user, "Area Director") or has_role(
        request.user, "Secretariat"
    )

    downref_doc_pairs = []
    downref_relations = RelatedDocument.objects.filter(
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for d in drafts:
                ...
                c = DocEvent(type="downref_approved", doc=d, rev=d.rev, by=login)
                ...
                c.save()
                c = DocEvent(type="downref_approved", doc=rfc, rev=rfc.rev, by=login)
                c.desc = "Downref to RFC %s approved by Last Call for %s-%s" % (
                    rfc.rfc_number,
                    d.name,
                    d.rev,
