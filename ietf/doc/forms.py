...
from ietf.doc.models import Document, RelatedDocument, DocAlias, State
...
class AddDownrefForm(forms.Form):
    ...
    drafts = SearchableDocAliasesField(
                ...
                help_text="The drafts that approve the downref in thier Last Call",
                required=True)

    def clean_rfc(self):
        if 'rfc' not in self.cleaned_data:
            raise forms.ValidationError("Must provide a referenced RFC and a referencing Internet-Draft")
    ...
    def clean_drafts(self):
        ...
        for da in drafts:
            ...
            if not state or state.slug not in IESG_APPROVED_STATE_LIST:
                if v_err_names:
                    v_err_names = v_err_names + ", " + da.name
                else:
                    v_err_names = da.name
        if v_err_names:
            raise forms.ValidationError("Draft is not yet approved: " + v_err_names)
        return drafts
    ...
    def clean(self):
        ...
        drafts = self.cleaned_data['drafts']
        for da in drafts:
            if RelatedDocument.objects.filter(source=da.document, target=rfc, relationship_id='downrefappr'):
                if v_err_pairs:
                    v_err_pairs = v_err_pairs + ", " + da.name + " --> RFC " + rfc.document.rfc_number()
                else:
                    v_err_pairs = da.name + " --> RFC " + rfc.document.rfc_number()
        if v_err_pairs:
            raise forms.ValidationError("Downref is already in the registry: " + v_err_pairs)
