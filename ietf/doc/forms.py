    ...
    def clean(self):
        ...
        for da in drafts:
            if RelatedDocument.objects.filter(source=da.document, target=rfc, relationship_id='downref-approval'):
                v_err_pairs.append(da.name + " --> RFC " + rfc.document.deprecated_rfc_number())
        if v_err_pairs:
        ...
        # this check is skipped if the save_downref_anyway button is used
            ...
            if v_err_refnorm:
                v_err_refnorm_prefix = "There does not seem to be a normative reference to RFC " + rfc.document.deprecated_rfc_number() + " by "
