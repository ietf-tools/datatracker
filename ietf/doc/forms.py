...
class TelechatForm(forms.Form):
    telechat_date = forms.TypedChoiceField(
        ...
        empty_value=None,
        required=False,
        help_text="Page counts are the current page counts for the telechat, before this telechat date edit is made.",
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["telechat_date"].choices = [("", "(not on agenda)")] + [
            (d, choice_display[d]) for d in dates
        ]
    ...
    def clean_notify(self):
        ...
        if len(duplicate_addrspecs) != 0:
            error_messages.append(
                f'Duplicate addresses: {", ".join(duplicate_addrspecs)}'
            )
...
class AddDownrefForm(forms.Form):
    ...
    def clean_rfc(self):
        if "rfc" not in self.cleaned_data:
            raise forms.ValidationError(
                "Please provide a referenced RFC and a referencing Internet-Draft"
            )
    ...
    def clean_drafts(self):
        ...
        if v_err_names:
            raise forms.ValidationError(
                "Internet-Draft is not yet approved: " + ", ".join(v_err_names)
            )
        return drafts
    ...
    def clean(self):
        ...
        for d in drafts:
            if RelatedDocument.objects.filter(
                source=d, target=rfc, relationship_id="downref-approval"
            ):
                v_err_pairs.append(f"{d.name} --> RFC {rfc.rfc_number}")
        if v_err_pairs:
            raise forms.ValidationError(
                "Downref is already in the registry: " + ", ".join(v_err_pairs)
            )
        ...
        if "save_downref_anyway" not in self.data:
            ...
            for d in drafts:
                if not RelatedDocument.objects.filter(
                    source=d, target=rfc, relationship_id="refnorm"
                ):
                    if v_err_refnorm:
...
class ExtResourceForm(forms.Form):
    ...
    def clean(self):
        ...
        for crs in cleaned_data.get("resources", []):
            cleaned_resources.append(cls.from_form_entry_str(crs))
        cleaned_data["resources"] = cleaned_resources

    @staticmethod
    def valid_resource_tags():
        return (
            ExtResourceName.objects.all()
            .order_by("slug")
    ...
    def clean_name_fragment(self):
        ...
        if any(c in name_fragment for c in disallowed_characters):
            raise ValidationError(
                f"The following characters are disallowed: {', '.join(disallowed_characters)}"
            )
