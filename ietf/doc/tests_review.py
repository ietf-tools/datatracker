...
class ReviewTests(TestCase):
    ...
    def verify_review_files_were_written(
        self, assignment, expected_content="This is a review\nwith two lines"
    ):
        review_file = Path(self.review_subdir) / f"{assignment.review.name}.txt"
        content = review_file.read_text()
        self.assertEqual(content, expected_content)
        self.assertEqual(retrieve_str("review", review_file.name), expected_content)
    ...
    def test_request_review(self):
        ...
        RoleFactory(
            name_id="chair", person__user__username="marschairman", group=doc.group
        )
        ...
        review_team3 = ReviewTeamFactory(
            acronym="reviewteam3",
            name="Review Team3",
            type_id="review",
            list_email="reviewteam3@ietf.org",
            parent=Group.objects.get(acronym="farfut"),
        )
        rev_role = RoleFactory(
            group=review_team,
        ...
        RoleFactory(
            group=review_team,
            person__user__username="reviewsecretary",
            person__user__email="reviewsecretary@example.com",
            name_id="secr",
        )
        RoleFactory(
            group=review_team3,
            person__user__username="reviewsecretary3",
            person__user__email="reviewsecretary3@example.com",
            name_id="secr",
        )
        ...
        url = urlreverse(
            "ietf.doc.views_review.request_review", kwargs={"name": doc.name}
        )
        ...
        self.assertEqual(
            set(qs.values_list("team__acronym", flat=True)),
            set(["reviewteam", "reviewteam3"]),
        )
        for req in qs:
    ...
    def test_doc_page(self):
        ...
        RelatedDocument.objects.create(
            source=old_doc, target=older_doc, relationship_id="replaces"
        )
        RelatedDocument.objects.create(
            source=doc, target=old_doc, relationship_id="replaces"
        )
    ...
    def test_review_request(self):
        ...
        url = urlreverse(
            ...
            kwargs={"name": doc.name, "request_id": review_req.pk},
        )
    ...
    def test_close_request(self):
        ...
        RoleFactory(
            group=review_team,
            person__user__username="reviewsecretary2",
            person__user__email="reviewsecretary2@example.com",
            name_id="secr",
        )
        review_req = ReviewRequestFactory(
            doc=doc,
        ...
        self.client.login(
            username="reviewsecretary", password="reviewsecretary+password"
        )
    ...
    def test_assign_reviewer(self):
        ...
        ReviewerSettings.objects.create(
            team=review_team, person=rev_role.person, min_interval=14, skip_next=0
        )
        queue_policy = get_reviewer_queue_policy(review_team)

        # review to assign to
        review_req = ReviewRequestFactory(
            team=review_team, doc=doc, state_id="requested"
        )

        # set up some reviewer-suitability factors
        reviewer_email = Email.objects.get(person__user__username="reviewer")
        DocumentAuthor.objects.create(
            person=reviewer_email.person, email=reviewer_email, document=doc
        )
        doc.rev = "10"
        ...
        reviewer_settings = ReviewerSettings.objects.get(
            person__email=reviewer_email, team=review_req.team
        )
        ...
        another_reviewer = PersonFactory.create(
            name="Extra TestReviewer"
        )  # needs to be lexically greater than the existing one
        another_reviewer.role_set.create(
            name_id="reviewer", email=another_reviewer.email(), group=review_req.team
        )

        ReviewWish.objects.create(
            person=reviewer_email.person, team=review_req.team, doc=doc
        )
        ...
        q = PyQuery(r.content)
        reviewer_label = (
            q('option[value="{}"]'.format(reviewer_email.address)).text().lower()
        )
        ...
        reviewer = Email.objects.filter(
            role__name="reviewer", role__group=review_req.team, person=rotation_list[0]
        ).first()
        ...
        self.assertIn(
            "{} has assigned {}".format(secretary.person.ascii, reviewer.person.ascii),
            message,
        )
        self.assertIn("This team has completed other reviews", message)
        self.assertIn(
            "{} -01 Serious Issues".format(reviewer_email.person.ascii), message
        )
    ...
    def test_assign_reviewer_after_reject(self):
        ...
        reviewer_email = Email.objects.get(person__user__username="reviewer")
        RoleFactory(
            group=review_team, person__user__username="reviewsecretary", name_id="secr"
        )
    ...
    def test_assign_reviewer_after_withdraw(self):
        ...
        reviewer = rev_role.person.email_set.first()
        ReviewAssignmentFactory(
            review_request=review_req, state_id="withdrawn", reviewer=reviewer
        )
        req_url = urlreverse(
            "ietf.doc.views_review.review_request",
            kwargs={"name": doc.name, "request_id": review_req.pk},
        ...
        self.assertEqual(
            assignment.state, ReviewAssignmentStateName.objects.get(slug="assigned")
        )
        self.assertEqual(
    ...
    def test_previously_reviewed_replaced_doc(self):
        ...
        ReviewAssignmentFactory(
            review_request__team=review_team,
            review_request__doc=old_wg_doc,
            reviewer=rev_role.email,
            state_id="completed",
        )
        ...
        assign_url = urlreverse(
            "ietf.doc.views_review.assign_reviewer",
            kwargs={"name": new_wg_doc.name, "request_id": review_req.pk},
        )
        ...
        q = PyQuery(r.content)
        reviewer_label = (
            q('option[value="{}"]'.format(rev_role.email.address)).text().lower()
        )
        self.assertIn("reviewed document before", reviewer_label)
    ...
    def test_reject_reviewer_assignment(self):
        ...
        reject_url = urlreverse(
            "ietf.doc.views_review.reject_reviewer_assignment",
            kwargs={"name": doc.name, "assignment_id": assignment.pk},
        )

        # follow link
        req_url = urlreverse(
            "ietf.doc.views_review.review_request",
            kwargs={"name": doc.name, "request_id": assignment.review_request.pk},
        )
        self.client.login(
            username="reviewsecretary", password="reviewsecretary+password"
        )
        ...
        self.client.logout()
        r = self.client.post(
            reject_url, {"action": "reject", "message_to_secretary": "Test message"}
        )
        ...
        login_testing_unauthorized(
            self, assignment.reviewer.person.user.username, reject_url
        )
    ...
    def make_test_mbox_tarball(self, review_req):
        ...
        with tarfile.open(mbox_path, "w:gz") as tar:
            with tempfile.NamedTemporaryFile(
                dir=self.review_dir, suffix=".mbox"
            ) as tmp:
                ...
                msg.attach(
                    email.mime.text.MIMEText("Hi!,\r\nLooks OK!\r\n-John", "plain")
                )
                msg.attach(
                    email.mime.text.MIMEText(
                        "<html><body><p>Hi!,</p><p>Looks OK!</p><p>-John</p></body></html>",
                        "html",
                    )
                )
                mbox.add(msg)
    ...
    def test_search_mail_archive(self):
        ...
        try:
            ...
            ietf.review.mailarch.construct_query_urls = lambda doc, team, query=None: {
                "query_data_url": "file://" + os.path.abspath(mbox_path)
            }
            url = urlreverse(
            ...
            self.assertEqual(
                messages[0]["subject"], "Review of {}-01".format(review_req.doc.name)
            )
            self.assertEqual(messages[0]["revision_guess"], "01")
            self.assertEqual(
                messages[0]["splitfrom"], ["John Doe", "johndoe@example.com"]
            )
            ...
            self.assertEqual(
                messages[1]["subject"], "Review of {}".format(review_req.doc.name)
            )
            self.assertFalse("revision_guess" in messages[1])
            self.assertEqual(
                messages[1]["splitfrom"], ["John Doe II", "johndoe2@example.com"]
            )
            ...
            ietf.review.mailarch.construct_query_urls = lambda doc, team, query=None: {
                "query_data_url": "file://" + os.path.abspath(no_result_path)
            }
    ...
    def test_complete_review_upload_content(self):
        ...
        r = self.client.post(
            url,
            data={
                "result": ReviewResultName.objects.get(
                    reviewteamsettings_review_results_set__group=assignment.review_request.team,
                    slug="ready",
                ).pk,
        ...
        self.assertTrue(
            assignment.review_request.team.acronym.lower() in assignment.review.name
        )
        ...
        self.assertIn(
            assignment.reviewer.role_set.filter(
                group=assignment.review_request.team, name="reviewer"
            )
            .first()
            .person.plain_name(),
            parseaddr(outbox[0]["From"])[0],
        )
        ...
        self.assertEqual(
            assignment.reviewer, assignment.review.documentauthor_set.first().email
        )
        ...
        self.assertEqual(
            parseaddr(outbox[0]["From"]), parseaddr(on_behalf_of(message.frm))
        )
        ...
        url = urlreverse(
            "ietf.doc.views_doc.document_main", kwargs={"name": assignment.review.name}
        )
        r = self.client.get(url)
    ...
    def test_complete_review_enter_content_by_secretary(self):
        ...
        self.assertEqual(
            ...
            datetime.datetime(2012, 12, 24, 12, 13, 14, tzinfo=DEADLINE_TZINFO),
        )
        ...
        events = ReviewAssignmentDocEvent.objects.filter(
            doc=assignment.review_request.doc
        ).order_by("-time")
    ...
    def test_complete_notify_ad_because_team_settings(self):
        assignment, url = self.setup_complete_review_test()
        assignment.review_request.team.reviewteamsettings.notify_ad_when.add(
            ReviewResultName.objects.get(slug="issues")
        )
        ...
        assignment.review_request.doc.save_with_history(
            [
                DocEvent.objects.create(
                    doc=assignment.review_request.doc,
                    rev=assignment.review_request.doc.rev,
                    by=assignment.reviewer.person,
                    type="changed_document",
                    desc="added an AD",
                )
            ]
    ...
    def test_double_submit_review(self):
        ...
        ReviewFactory(
            name=review_name, type_id="review", group=assignment.review_request.team
        )
        ...
        self.assertIn(
            "Attempt to save review failed", list(r2.context["messages"])[0].message
        )
    ...
    def test_partially_complete_review(self):
        ...
        r = self.client.post(
            ...
            data={
                ...
                "state": ReviewAssignmentStateName.objects.get(
                    slug="part-completed"
                ).pk,
                "reviewed_rev": assignment.review_request.doc.rev,
                "review_submission": "enter",
    ...
    def test_mark_no_response(self):
        assignment = ReviewAssignmentFactory()
        secr = RoleFactory(
            group=assignment.review_request.team,
            person__user__username="reviewsecretary",
            person__user__email="reviewsecretary@example.com",
            name_id="secr",
        ).person
    ...
    def test_review_wish_add(self):
        ...
        url = urlreverse(
            "ietf.doc.views_review.review_wish_add", kwargs={"name": doc.name}
        )
    ...
    def test_review_wishes_remove(self):
        ...
        url = urlreverse(
            "ietf.doc.views_review.review_wishes_remove", kwargs={"name": doc.name}
        )
