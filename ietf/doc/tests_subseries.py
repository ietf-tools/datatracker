...
class SubseriesTests(TestCase):
    ...
    def test_index_and_view(self):
        ...
        for type_id in types:
            ...
            # Index
            url = urlreverse(
                "ietf.doc.views_search.index_subseries", kwargs=dict(type_id=type_id)
            )
            ...
            q = PyQuery(r.content)
            self.assertIn(
                f"{doc.type_id.upper()} {doc.name[3:]} consists of:", q("h2").text()
            )
            ...
            q = PyQuery(r.content)
            self.assertIn(
                f"RFC {rfc.name[3:]} also known as {type_id.upper()} {doc.name[3:]}",
                q("h1").text(),
            )
        ...
        for rfc in bcp.contains():
            ...
            q = PyQuery(r.content)
            self.assertIn(
                f"RFC {rfc.name[3:]} part of BCP {bcp.name[3:]}", q("h1").text()
            )
