...
"""
    ...
    def test_show_bof_requests(self):
        ...
        for i in range(3 * len(states)):
            BofreqFactory(
                states=[("bofreq", states[i % len(states)].slug)],
                newrevisiondocevent__time=timezone.now()
                - datetime.timedelta(days=randint(0, 20)),
            )
        r = self.client.get(url)
        ...
        for state in states:
            self.assertEqual(
                len(q(f"#bofreqs-{state.slug}")), 1 if state.slug != "spam" else 0
            )
            self.assertEqual(
                len(q(f"#bofreqs-{state.slug} tbody tr")),
                3 if state.slug != "spam" else 0,
            )
    ...
    def test_bofreq_main_page(self):
        ...
        self.client.logout()
        url = urlreverse(
            "ietf.doc.views_doc.document_main", kwargs=dict(name=doc, rev="00")
        )
        r = self.client.get(url)
    ...
    def test_edit_state(self):
        ...
        editor = bofreq_editors(doc).first()
        url = urlreverse(
            "ietf.doc.views_bofreq.change_state", kwargs=dict(name=doc.name)
        )
        state = doc.get_state("bofreq")
        r = self.client.post(
            url, dict(new_state=self.state_pk_as_str("bofreq", "approved"))
        )
        self.assertEqual(r.status_code, 302)
        doc = reload_db_objects(doc)
        self.assertEqual(state, doc.get_state("bofreq"))
        self.client.login(
            username=editor.user.username, password=editor.user.username + "+password"
        )
        r = self.client.post(
            url, dict(new_state=self.state_pk_as_str("bofreq", "approved"))
        )
        ...
        for username in ("secretary", "ad", "iab-member"):
            ...
            r = self.client.post(
                url,
                dict(
                    new_state=self.state_pk_as_str(
                        "bofreq", "approved" if username == "secretary" else "declined"
                    ),
                    comment=f"{username}-2309hnf",
                ),
            )
            self.assertEqual(r.status_code, 302)
            doc = reload_db_objects(doc)
            self.assertEqual(
                "approved" if username == "secretary" else "declined",
                doc.get_state_slug("bofreq"),
            )
            self.assertEqual(docevent_count + 2, doc.docevent_set.count())
            self.assertIn(
                f"{username}-2309hnf", doc.latest_event(type="added_comment").desc
            )
            self.client.logout()
    ...
    def test_change_editors(self):
        ...
        url = urlreverse(
            "ietf.doc.views_bofreq.change_editors", kwargs=dict(name=doc.name)
        )
        ...
        self.client.logout()
        for username in (
            previous_editors[0].user.username,
            "secretary",
            "ad",
            "iab-member",
        ):
            ...
            unescaped = unescape(
                unescape(unicontent(r).encode("utf-8").decode("unicode-escape"))
            )
    ...
    def test_change_responsible(self):
        ...
        url = urlreverse(
            "ietf.doc.views_bofreq.change_responsible", kwargs=dict(name=doc.name)
        )
        ...
        for username in ("secretary", "ad", "iab-member"):
            ...
            self.assertEqual(len(outbox), 1)
            self.assertIn(
                "BOF Request responsible leadership changed", outbox[0]["Subject"]
            )
    ...
    def test_change_responsible_options(self):
        ...
        doc = BofreqFactory()
        url = urlreverse(
            "ietf.doc.views_bofreq.change_responsible", kwargs={"name": doc.name}
        )
        self.client.login(username="secretary", password="secretary+password")
        r = self.client.get(url)
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
        option_ids = [
            opt["id"] for opt in json.loads(q("#id_responsible").attr("data-pre"))
        ]
        ad_people = [
            p for p in Person.objects.all() if has_role(p.user, "Area Director")
        ]
    ...
    def test_submit(self):
        ...
        rev = doc.rev
        r = self.client.post(
            url, {"bofreq_submission": "enter", "bofreq_content": "# oiwefrase"}
        )
        self.assertEqual(r.status_code, 302)
        doc = reload_db_objects(doc)
            ...
            try:
                with open(file.name, "rb") as bofreq_fd:
                    for postdict in [
                        ...
                        self.assertEqual(
                            f"# {username}", retrieve_str("bofreq", doc.get_base_name())
                        )
                        self.assertEqual(docevent_count + 1, doc.docevent_set.count())
                        self.assertEqual(1, len(outbox))
                        rev = doc.rev
    ...
    def test_start_new_bofreq(self):
        ...
        r = self.client.post(
            ...
            dict(
                title="default",
                bofreq_submission="enter",
                bofreq_content=render_to_string(
                    "doc/bofreq/bofreq_template.md", {"settings": settings}
                ),
            ),
        )
            ...
            with open(file.name, "rb") as bofreq_fd:
                for postdict in [
                    dict(
                        title="title one",
                        bofreq_submission="enter",
                        bofreq_content="some stuff",
                    ),
                    dict(
                        title="title two",
                        bofreq_submission="upload",
                        bofreq_file=bofreq_fd,
                    ),
                ]:
                    empty_outbox()
                    r = self.client.post(url, postdict)
                    ...
                    bofreq = Document.objects.filter(
                        name=name, type_id="bofreq"
                    ).first()
                    ...
                    self.assertEqual(
                        retrieve_str("bofreq", bofreq.get_base_name()), "some stuff"
                    )
                    self.assertEqual(len(outbox), 1)
        finally:
            os.unlink(file.name)
    ...
    def test_post_proposed_restrictions(self):
        ...
        for view in ("submit", "change_editors", "edit_title"):
            url = urlreverse(
                f"ietf.doc.views_bofreq.{view}", kwargs=dict(name=bofreq.name)
            )
            for state in states:
                ...
                self.client.login(
                    ...
                    password=editor.user.username + "+password",
                )
                r = self.client.get(url)
                self.assertEqual(
                    r.status_code,
                    403,
                    f"editor should not be able to use {view} in state {state.slug}",
                )
                self.client.logout()

        url = urlreverse(
            "ietf.doc.views_doc.document_main", kwargs=dict(name=bofreq.name)
        )
        self.client.login(
            username=editor.user.username, password=editor.user.username + "+password"
        )
