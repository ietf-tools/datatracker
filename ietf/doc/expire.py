...
def expirable_drafts(queryset=None):
    ...
    if nonexpirable_states is None:
        ...
        nonexpirable_states += list(
            State.objects.filter(
                used=True,
                type__in=("draft-stream-iab", "draft-stream-irtf", "draft-stream-ise"),
    ...
    return (
        ...
        .exclude(tags="rfc-rev")  # under review by the RFC Editor blocks expiry
        .distinct()
    )
...
def get_expired_drafts():
    return expirable_drafts().filter(
        expires__lt=datetime_today(DEADLINE_TZINFO) + datetime.timedelta(1)
    )
...
def in_draft_expire_freeze(when=None):
    ...
    meeting = (
        Meeting.objects.filter(type="ietf", date__gte=when - datetime.timedelta(days=7))
        .order_by("date")
        .first()
    )
...
def send_expire_warning_for_draft(doc):
    ...
    log.assertion("s")
    state = (
        s.name if s else "I-D Exists"
    )  # TODO remove the if clause after some runtime shows no assertions
    ...
    if to or cc:
        send_mail(
            ...
            dict(doc=doc, state=state, expiration=expiration),
            cc=cc,
        )
...
def send_expire_notice_for_draft(doc):
    ...
    state = (
        s.name if s else "I-D Exists"
    )  # TODO remove the if clause after some rintime shows no assertions
...
def expire_draft(doc):
    ...
    events.append(
        update_action_holders(doc, prev_draft_state, doc.get_state("draft"), [], [])
    )
    ...
    for path in glob.glob(pattern):
        ...
        try:
            ...
            if state in ("rfc", "repl"):
                ...
                state in ("expired", "auth-rm", "ietf-rm")
                and doc.expires
