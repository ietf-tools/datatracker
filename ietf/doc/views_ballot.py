...
def do_undefer_ballot(request, doc):
    ...
    e = add_state_change_event(
        doc, by, prev_state, new_state, prev_tags=prev_tags, new_tags=new_tags
    )
    if e:
        events.append(e)
    e = update_action_holders(
        doc, prev_state, new_state, prev_tags=prev_tags, new_tags=new_tags
    )
    if e:
        events.append(e)
...
class EditPositionForm(forms.Form):
    ...
    def clean_discuss(self):
        ...
        entered_pos = self.cleaned_data.get(
            "position", BallotPositionName.objects.get(slug="norecord")
        )
...
def save_position(form, doc, ballot, balloter, login=None, send_email=False):
    ...
    if not old_pos and pos.pos.slug != "norecord":
        pos.desc = (
            "[Ballot Position Update] New position, %s, has been recorded for %s"
            % (pos.pos.name, pos.balloter.plain_name())
        )
    elif old_pos and pos.pos != old_pos.pos:
        pos.desc = (
            "[Ballot Position Update] Position for %s has been changed to %s from %s"
            % (pos.balloter.plain_name(), pos.pos.name, old_pos.pos.name)
        )

    if not pos.desc and changes:
        pos.desc = "Ballot %s text updated for %s" % (
            " and ".join(changes),
            balloter.plain_name(),
        )
...
def edit_position(request, name, ballot_id):
    ...
    ballot = get_object_or_404(
        BallotDocEvent, type="created_ballot", pk=ballot_id, doc=doc
    )
    ...
    try:
        return_to_url = parse_ballot_edit_return_point(
            request.GET.get("ballot_edit_return_point"), doc.name, ballot_id
        )
    except ValueError:
    ...
    if request.method == "POST":
        old_pos = None
        if not has_role(request.user, "Secretariat") and not can_ballot(
            request.user, doc
        ):
            # prevent pre-ADs from taking a position
            permission_denied(
                request,
                "Must be an active member (not a pre-AD for example) of the balloting body to take a position",
            )
        ...
        if form.is_valid():
            ...
            if send_mail:
                ...
                if request.GET.get("ballot_edit_return_point"):
                    query["ballot_edit_return_point"] = request.GET.get(
                        "ballot_edit_return_point"
                    )
                qstr = ""
                if len(query) > 0:
    ...
    blocking_positions = dict(
        (p.pk, p.name) for p in form.fields["position"].queryset.all() if p.blocking
    )
...
def api_set_position(request):
    ...
    if request.method == "POST":
        ...
        if not position:
            return err(
                400,
                "Missing parameter: position, one of: %s " % ",".join(position_names),
            )
        if not position in position_names:
            return err(
                400, "Bad position name, must be one of: %s " % ",".join(position_names)
            )
...
def build_position_email(balloter, doc, pos):
    ...
    balloter_name_genitive = (
        balloter.plain_name() + "'"
        if balloter.plain_name().endswith("s")
        else balloter.plain_name() + "'s"
    )
    subject = "%s %s on %s" % (
        balloter_name_genitive,
        pos.pos.name if pos.pos else "No Position",
        doc.name + "-" + doc.rev,
    )
    if subj:
...
def send_ballot_comment(request, name, ballot_id):
    ...
    if not has_role(request.user, "Secretariat"):
        if any(
            [
                ...
                and not has_role(request.user, "RSAB Member"),
            ]
        ):
            raise Http404
    ...
    if request.method == "POST":
        cc = []
        cc_select_form = CcSelectForm(
            data=request.POST,
            mailtrigger_slug=mailtrigger_slug,
            mailtrigger_context={"doc": doc},
        )
        if cc_select_form.is_valid():
            cc.extend(cc_select_form.get_selected_addresses())
        extra_cc = [
            x.strip() for x in request.POST.get("extra_cc", "").split(",") if x.strip()
        ]
        if extra_cc:
            cc.extend(extra_cc)
        ...
        cc_select_form = CcSelectForm(
            mailtrigger_slug=mailtrigger_slug, mailtrigger_context={"doc": doc}
        )

        return render(
            request,
            "doc/ballot/send_ballot_comment.html",
            dict(
                doc=doc,
                subject=subject,
                body=body,
...
def defer_ballot(request, name):
    ...
    if request.method == "POST":
        ...
        e = add_state_change_event(
            doc, login, prev_state, new_state, prev_tags=prev_tags, new_tags=new_tags
        )
        if e:
            events.append(e)
        e = update_action_holders(
    ...
    return render(
        request,
        "doc/ballot/defer_ballot.html",
        dict(doc=doc, telechat_date=telechat_date, back_url=doc.get_absolute_url()),
    )
...
def undefer_ballot(request, name):
    ...
    return render(
        request,
        "doc/ballot/undefer_ballot.html",
        dict(doc=doc, telechat_date=telechat_date, back_url=doc.get_absolute_url()),
    )
        ...
        for l, next in zip(lines, lines[1:]):
            if l.startswith("Subject:") and next.strip():
                raise forms.ValidationError(
                    "Subject line appears to have a line break, please make sure there is no line breaks in the subject line and that it is followed by an empty line."
                )
...
def lastcalltext(request, name):
    ...
    if request.method == "POST":
        if (
            "save_last_call_text" in request.POST
            or "send_last_call_request" in request.POST
        ):
            ...
            if form.is_valid():
                ...
                if "send_last_call_request" in request.POST:
                    ...
                    new_state = State.objects.get(
                        used=True, type="draft-iesg", slug="lc-req"
                    )
                    ...
                    return render(
                        request, "doc/draft/last_call_requested.html", dict(doc=doc)
                    )
...
def ballot_writeupnotes(request, name):
    ...
    if (
        request.method == "POST"
        and "save_ballot_writeup" in request.POST
        or "issue_ballot" in request.POST
    ):
        ...
        if form.is_valid():
            ...
            if "issue_ballot" in request.POST and not ballot_already_approved:
                ...
                if not ballot_already_approved:
                    e = create_ballot_if_not_open(
                        request, doc, login, "approve"
                    )  # pyflakes:ignore
                    ballot = doc.latest_event(BallotDocEvent, type="created_ballot")
                    if has_role(request.user, "Area Director") and not doc.latest_event(
                        BallotPositionDocEvent, balloter=login, ballot=ballot
                    ):
    ...
    return render(
        request,
        "doc/ballot/writeupnotes.html",
        dict(
            doc=doc,
            back_url=doc.get_absolute_url(),
            ballot_issued=bool(doc.latest_event(type="sent_ballot_announcement")),
            warn_lc=not doc.docevent_set.filter(
                lastcalldocevent__expires__date__lt=date_today(DEADLINE_TZINFO)
            ).exists(),
            warn_unexpected_state=(
                prev_state if bool(prev_state.slug in ["ad-eval", "lc"]) else None
            ),
...
class BallotRfcEditorNoteForm(forms.Form):
    rfc_editor_note = forms.CharField(
        widget=forms.Textarea, label="RFC Editor Note", required=True, strip=False
    )
...
def ballot_rfceditornote(request, name):
    ...
    form = BallotRfcEditorNoteForm(
        auto_id=False, initial=dict(rfc_editor_note=escape(existing.text))
    )
...
def approve_ballot(request, name):
    ...
    approval_mail_event = doc.latest_event(
        WriteupDocEvent, type="changed_ballot_approval_text"
    )
    ...
    ballot_writeup_event = doc.latest_event(
        WriteupDocEvent, type="changed_ballot_writeup_text"
    )
    ...
    if error_duplicate_rfc_editor_note:
        return render(
            request, "doc/draft/rfceditor_note_duplicate_error.html", {"doc": doc}
        )
    ...
    if request.method == "POST":
        ...
        e = add_state_change_event(
            doc, login, prev_state, new_state, prev_tags=prev_tags, new_tags=[]
        )
        if e:
            events.append(e)
        e = update_action_holders(
        ...
        if action == "to_announcement_list":
            addrs = gather_address_lists("ballot_approved_ietf_stream_iana").as_strings(
                compact=False
            )
            send_mail_preformatted(
                request,
        ...
        if not downrefs:
            ...
            return HttpResponseRedirect(
                doc.get_absolute_url() + "edit/approvedownrefs/"
            )

    return render(
        request,
        "doc/ballot/approve_ballot.html",
...
def approve_downrefs(request, name):
    ...
    downrefs_to_rfc_qs = RelatedDocument.objects.filter(
        pk__in=[r.pk for r in downrefs_to_rfc]
    )

    last_call_text = doc.latest_event(
        WriteupDocEvent, type="changed_last_call_text"
    ).text.strip()
    ...
    return render(
        request,
        "doc/ballot/approve_downrefs.html",
        dict(
            doc=doc,
...
def make_last_call(request, name):
    ...
    announcement_event = doc.latest_event(
        WriteupDocEvent, type="changed_last_call_text"
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if doc.type.slug in ("draft", "statchg"):
                addrs = gather_address_lists(
                    "last_call_issued_iana", doc=doc
                ).as_strings(compact=False)
                send_mail_preformatted(
                    request,
                    announcement,
                    extra=extra_automation_headers(doc),
            ...
            e.desc = (
                "The following Last Call announcement was sent out (ends %s):<br><br>"
                % expiration_date
            )
            e.desc += announcement
            ...
            if form.cleaned_data["last_call_sent_date"] != e_production_time.date():
                ...
                e.time = e_production_time.replace(
                    year=lcsd.year, month=lcsd.month, day=lcsd.day
                )  # preserves tzinfo
        ...
        initial["last_call_expiration_date"] = date_today() + datetime.timedelta(
            days=expire_days
        )
...
def issue_irsg_ballot(request, name):
    ...
    if request.method == "POST":
        ...
        if button == "Yes":
            ...
            e.duedate = datetime_from_date(
                datetime.datetime.strptime(duedate, "%Y-%m-%d"), DEADLINE_TZINFO
            )
            ...
            if doc.type_id == "draft":
                new_state = State.objects.get(
                    used=True, type="draft-stream-irtf", slug="irsgpoll"
                )
        ...
        return render(
            request, templ, dict(doc=doc, question=question, fillerdate=fillerdate)
        )
...
def close_irsg_ballot(request, name):
    ...
    if request.method == "POST":
        ...
        if button == "Yes":
            ...
            email_irsg_ballot_closed(
                request, doc=doc, ballot=IRSGBallotDocEvent.objects.get(pk=ballot.pk)
            )
...
def irsg_ballot_status(request):
    possible_docs = Document.objects.filter(
        docevent__ballotdocevent__irsgballotdocevent__isnull=False
    )
    docs = []
...
def issue_rsab_ballot(request, name):
    doc = get_object_or_404(Document, name=name)
    if doc.stream.slug != "editorial" or doc.type != DocTypeName.objects.get(
        slug="draft"
    ):
        raise Http404
    ...
    if request.method == "POST":
        button = request.POST.get(
            "rsab_button"
        )  # TODO: Really? There's an irsg button? The templates should be generalized.
        if button == "Yes":
            ...
            if doc.type_id == "draft":
                new_state = State.objects.get(
                    used=True, type="draft-stream-editorial", slug="rsabpoll"
                )
...
def close_rsab_ballot(request, name):
    ...
    if request.method == "POST":
        ...
        if button == "Yes":
            ...
            email_rsab_ballot_closed(
                request, doc=doc, ballot=BallotDocEvent.objects.get(pk=ballot.pk)
            )
        return HttpResponseRedirect(doc.get_absolute_url())
...
def parse_ballot_edit_return_point(path, doc_name, ballot_id):
    get_default_path = lambda: urlreverse(
        "ietf.doc.views_doc.document_ballot",
        kwargs=dict(name=doc_name, ballot_id=ballot_id),
    )
