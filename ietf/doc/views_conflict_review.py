...
class ChangeStateForm(forms.Form):
    review_state = forms.ModelChoiceField(
        State.objects.filter(used=True, type="conflrev"),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not has_role(user, "Secretariat"):
            self.fields["review_state"].queryset = self.fields[
                "review_state"
            ].queryset.exclude(slug__in=("appr-reqnopub-sent", "appr-noprob-sent"))
...
def change_state(request, name, option=None):
                ...
                if new_state.slug == "iesgeval":
                    ...
                    if has_role(
                        ...
                        pos = BallotPositionDocEvent(
                            doc=review, rev=review.rev, by=login
                        )
                        pos.ballot = ballot
                    ...
                    if new_state.slug == "appr-noprob-pend":
                        ...
                        ok_to_publish = False
                    email_ad_approved_conflict_review(request, review, ok_to_publish)

                if new_state.slug in [
                    "appr-reqnopub-sent",
                    "appr-noprob-sent",
                    "withdraw",
                    "dead",
                    ...
                    update_stream_state(
                        doc,
                        login,
                        "chair-w" if doc.stream_id == "irtf" else "ise-rev",
                        "iesg-com",
                    )
    ...
    return render(
        ...
        dict(
            ...
            login=login,
            help_url=reverse(
                "ietf.doc.views_help.state_help", kwargs=dict(type="conflict-review")
            ),
        ),
    )
...
def send_conflict_review_ad_changed_email(request, review, event):
    addrs = gather_address_lists("conflrev_ad_changed", doc=review).as_strings(
        compact=False
    )
    msg = render_to_string(
        "doc/conflict_review/changed_ad.txt",
        dict(
            ...
            event=event,
            review=review,
            reviewed_doc=review.relateddocument_set.get(
...
def send_conflict_review_started_email(request, review):
    addrs = gather_address_lists("conflrev_requested", doc=review).as_strings(
        compact=False
    )
    msg = render_to_string(
        "doc/conflict_review/review_started.txt",
        dict(
            frm=settings.DEFAULT_FROM_EMAIL,
            to=addrs.to,
            cc=addrs.cc,
    ...
    addrs = gather_address_lists("conflrev_requested_iana", doc=review).as_strings(
        compact=False
    )
    email_iana(
        request,
...
class UploadForm(forms.Form):
    content = forms.CharField(
        widget=forms.Textarea,
        label="Conflict review response",
        help_text="Edit the conflict review response.",
        required=False,
        strip=False,
    )
    txt = forms.FileField(
    ...
    def save(self, review):
        ...
        try:
            ...
            )  # Path.hardlink_to is not available until 3.10
        except IOError as e:
            log.log(
...
def submit(request, name):
    ...
    path = os.path.join(
        settings.CONFLICT_REVIEW_PATH, "%s-%s.txt" % (review.name, review.rev)
    )
    ...
    if request.method == "POST":
        if "submit_response" in request.POST:
            ...
            init = {
                "content": escape(
                    render_to_string("doc/conflict_review/review_choices.txt", dict())
                )
            }
    ...
    return render(
        request,
        "doc/conflict_review/submit.html",
        {
            "form": form,
            "next_rev": next_rev,
            "review": review,
...
def edit_ad(request, name):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            c = DocEvent(
                type="added_comment", doc=review, rev=review.rev, by=request.user.person
            )
    ...
    return render(
        request,
        "doc/change_ad.html",
        {"form": form, "doc": review, "titletext": titletext},
    )
...
def default_approval_text(review):
    ...
    if conflictdoc.stream_id == "ise":
        ...
        receiver = "recipient"
    addrs = gather_address_lists("ballot_approved_conflrev", doc=review).as_strings(
        compact=False
    )
...
class AnnouncementForm(forms.Form):
    announcement_text = forms.CharField(
        widget=forms.Textarea,
        label="IETF Conflict Review Announcement",
...
def approve_conflict_review(request, name):
    ...
    if review.get_state("conflrev").slug not in (
        "appr-reqnopub-pend",
        "appr-noprob-pend",
    ):
        raise Http404
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            new_state_slug = (
                "appr-reqnopub-sent"
                if prev_state.slug == "appr-reqnopub-pend"
                else "appr-noprob-sent"
            )
            new_state = State.objects.get(
                used=True, type="conflrev", slug=new_state_slug
            )
...
class StartReviewForm(forms.Form):
    ...
    create_in_state = forms.ModelChoiceField(
        State.objects.filter(
            used=True, type="conflrev", slug__in=("needshep", "adrev")
        ),
        empty_label=None,
        required=False,
    )
...
def start_review_sanity_check(request, name):
    ...
    if (not doc_to_review.stream_id in ("ise", "irtf")) or (
        not is_authorized_in_doc_stream(request.user, doc_to_review)
    ):
        raise Http404
    ...
    if [
        rel.source
        for rel in doc_to_review.targets_related.filter(relationship="conflrev")
    ]:
        raise Http404
...
def build_conflict_review_document(login, doc_to_review, ad, notify, create_in_state):
    ...
    conflict_review.relateddocument_set.create(
        target=doc_to_review, relationship_id="conflrev"
    )

    c = DocEvent(
        type="added_comment", doc=conflict_review, rev=conflict_review.rev, by=login
    )
    c.desc = "IETF conflict review requested"
    c.save()

    c = DocEvent(
        type="added_comment", doc=doc_to_review, rev=doc_to_review.rev, by=login
    )
    # Is it really OK to put html tags into comment text?
    c.desc = 'IETF conflict review initiated - see <a href="%s">%s</a>' % (
        reverse(
            "ietf.doc.views_doc.document_main", kwargs={"name": conflict_review.name}
        ),
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            conflict_review = build_conflict_review_document(
                ...
                notify=form.cleaned_data["notify"],
                create_in_state=State.objects.get(
                    used=True, type="conflrev", slug="needshep"
                ),
            )
...
def update_stream_state(doc, by, state, tag=None):
    ...
    if new_state != prev_state:
        ...
        if tag:
            ...
            events.append(
                add_state_change_event(
                    doc, by, prev_state, new_state, prev_tags, new_tags
                )
            )
        else:
