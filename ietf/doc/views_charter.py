...
from ietf.doc.utils_charter import (
    historic_milestones_for_charter,
    approved_revision,
    default_review_text,
    default_action_text,
    ...
    fix_charter_revision_after_approval,
    split_charter_name,
...
class ChangeStateForm(forms.Form):
    charter_state = forms.ModelChoiceField(
        State.objects.filter(used=True, type="charter"),
        label="Charter state",
        empty_label=None,
        required=False,
    )
    initial_time = forms.IntegerField(
        initial=0, label="Review time", help_text="(in weeks)", required=False
    )
    message = forms.CharField(
        widget=forms.Textarea,
        help_text="Leave blank to change state without notifying the Secretariat.",
        required=False,
        label=mark_safe("Message to the Secretariat"),
        strip=False,
    )
    comment = forms.CharField(
        widget=forms.Textarea,
        help_text="Optional comment for the charter history.",
        required=False,
        strip=False,
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if group.type_id == "wg":
            state_field.queryset = state_field.queryset.filter(
                slug__in=("infrev", "intrev", "extrev", "iesgrev")
            )
        else:
            state_field.queryset = state_field.queryset.filter(
                slug__in=("intrev", "extrev", "approved")
            )
...
def change_state(request, name, option=None):
    ...
    if (
        charter.get_state_slug() != "infrev"
        or (initial_review and initial_review.expires < timezone.now())
        or chartering_type == "rechartering"
    ):
        initial_review = None
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if option in ("initcharter", "recharter"):
                if group.type_id == "wg":
                    charter_state = State.objects.get(
                        used=True, type="charter", slug="infrev"
                    )
                else:
                ...
                if oldstate.slug in ("proposed", "bof", "unknown"):
                    charter_state = State.objects.get(
                        used=True, type="charter", slug="notrev"
                    )
                    ...
                    charter_state = State.objects.get(
                        used=True, type="charter", slug="approved"
                    )
            ...
            if charter_state != charter.get_state():
                ...
                if option != "abandon":
                    ...
                    telechat_doc_event = charter.latest_event(
                    ...
                    if telechat_doc_event and telechat_doc_event.telechat_date:
                        e = TelechatDocEvent.objects.create(
                            ...
                            telechat_date=None,
                            desc="Removed from agenda for telechat",
                        )
                        events.append(e)

                    # Special log for abandoned efforts
                    e = DocEvent(
                        type="changed_document", doc=charter, rev=charter.rev, by=by
                    )
                ...
                if (
                    message
                    or charter_state.slug == "intrev"
                    or charter_state.slug == "extrev"
                ):
            ...
            if (
                charter_state.slug == "infrev"
                and clean["initial_time"]
                and clean["initial_time"] != 0
            ):
                e = InitialReviewDocEvent(
                    type="initial_review", by=by, doc=charter, rev=charter.rev
                )
                e.expires = timezone.now() + datetime.timedelta(
        ...
        if group.type_id == "wg":
            if option == "recharter":
                ...
                init = dict(
                    initial_time=1,
                    message='%s has initiated chartering of the proposed %s:\n "%s" (%s).'
                    % (by.plain_name(), group.type.name, group.name, group.acronym),
                )
            elif option == "abandon":
                hide = ["initial_time", "charter_state"]
                init = dict(
                    message='%s has abandoned the chartering effort on the %s:\n "%s" (%s).'
                    % (by.plain_name(), group.type.name, group.name, group.acronym)
                )
    ...
    charter_hists = (
        DocHistory.objects.filter(doc=charter)
        .exclude(states__type="charter", states__slug=charter.get_state_slug())
        .order_by("-time")[:1]
    )
    if charter_hists:
    ...
    title = {
        "initcharter": "Initiate chartering of %s %s"
    ...
    if group.type_id == "wg":
        states_for_ballot_wo_extern = State.objects.filter(
            used=True, type="charter", slug="intrev"
        ).values_list("pk", flat=True)

    return render(
        request,
        "doc/charter/change_state.html",
        dict(
            form=form,
            doc=group.charter,
            option=option,
...
class ChangeTitleForm(forms.Form):
    charter_title = forms.CharField(
        widget=forms.TextInput,
        label="Charter title",
        help_text="Enter new charter title.",
        required=True,
    )
    message = forms.CharField(
        widget=forms.Textarea,
        help_text="Leave blank to change the title without notifying the Secretariat.",
        required=False,
        label=mark_safe("Message to Secretariat"),
        strip=False,
    )
    comment = forms.CharField(
        widget=forms.Textarea,
            ...
            if new_title != prev_title:
                ...
                if message:
                    email_admin_re_charter(
                        request,
                        group,
                        "Charter title changed to %s" % new_title,
                        message,
                        "charter_state_edit_admin_needed",
                    )
                email_state_changed(
...
class UploadForm(forms.Form):
    content = forms.CharField(
        widget=forms.Textarea,
        label="Charter text",
        help_text="Edit the charter text.",
        required=False,
        strip=False,
    )
    txt = forms.FileField(
...
def submit(request, name, option=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                os.link(
                    charter_filename, ftp_filename
                )  # os.link until we are on python>=3.10
            except IOError:
                log(
...
class ActionAnnouncementTextForm(forms.Form):
    announcement_text = forms.CharField(
        widget=forms.Textarea, required=True, strip=False
    )
    ...
    if request.method == "POST":
        ...
        if "save_text" in request.POST and form.is_valid():
            ...
            if request.GET.get("next", "") == "approve":
                return redirect("ietf.doc.views_charter.approve", name=charter.name)
...
def ballot_writeupnotes(request, name):
    ...
    if request.method == "POST" and (
        "save_ballot_writeup" in request.POST or "send_ballot" in request.POST
    ):
        ...
        if form.is_valid():
            ...
            if "send_ballot" in request.POST and approval:
                if has_role(request.user, "Area Director") and not charter.latest_event(
                    BallotPositionDocEvent,
                ...
                return render(
                    request,
                    "doc/charter/ballot_issued.html",
                    dict(
                        doc=charter,
                    ),
                )
...
def approve(request, name):
    ...
    if request.method == "POST":
        new_charter_state = State.objects.get(
            used=True, type="charter", slug="approved"
        )
        ...
        if group_state_change_event:
            change_description += (
                " and group state has been changed to %s" % group.state.name
            )
        ...
        for m in group.groupmilestone_set.filter(state="charter"):
            ...
            for i, o in enumerate(milestones_to_delete):
                if (
                    o.desc == m.desc
                    and o.due == m.due
                    and set(o.docs.all()) == set(m.docs.all())
                ):
                    found = True
                    break
            ...
            if found:
                ...
                if not o.state_id == "active":
                    ...
                    MilestoneGroupEvent.objects.create(
                        ...
                        by=by,
                        desc='Changed milestone "%s", set state to active from review'
                        % o.desc,
                        milestone=o,
                    )
    ...
    return render(
        request,
        "doc/charter/approve.html",
        dict(charter=charter, announcement=escape(announcement)),
    )
