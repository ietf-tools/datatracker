...
urlpatterns = [
    ...
    url(
        r"^(?P<request_id>[0-9]+)/addrequestcomment/$", views_review.add_request_comment
    ),
    url(r"^(?P<request_id>[0-9]+)/assignreviewer/$", views_review.assign_reviewer),
    url(
        r"^(?P<assignment_id>[0-9]+)/rejectreviewerassignment/$",
        views_review.reject_reviewer_assignment,
    ),
    url(r"^(?P<assignment_id>[0-9]+)/complete/$", views_review.complete_review),
    url(
        r"^%(acronym)s/submitunsolicitedreview/$" % settings.URL_REGEXPS,
        views_review.complete_review,
    ),
    url(
        r"^submitunsolicitedreview/$",
        views_review.submit_unsolicited_review_choose_team,
    ),
    url(
        r"^(?P<assignment_id>[0-9]+)/withdraw/$",
        views_review.withdraw_reviewer_assignment,
    ),
    url(
        r"^(?P<assignment_id>[0-9]+)/noresponse/$",
        views_review.mark_reviewer_assignment_no_response,
    ),
    url(
        r"^assignment/(?P<assignment_id>[0-9]+)/searchmailarchive/$",
        views_review.search_mail_archive,
    ),
    url(
        r"^team/%(acronym)s/searchmailarchive/$" % settings.URL_REGEXPS,
        views_review.search_mail_archive,
    ),
