...
class GroupMaterialTests(TestCase):
    ...
    def create_slides(self):
        ...
        doc = Document.objects.create(
            name="slides-testteam-test-file", rev="01", type_id="slides", group=group
        )
    ...
    def test_choose_material_type(self):
        ...
        url = urlreverse(
            "ietf.doc.views_material.choose_material_type",
        ...
        url = urlreverse(
            "ietf.doc.views_material.choose_material_type", kwargs=dict(acronym="mars")
        )
        r = self.client.get(url)
    ...
    def test_upload_slides(self):
        ...
        r = self.client.post(
            url,
            dict(
                title="Test File - with fancy title",
    ...
    def test_change_state(self):
        ...
        url = urlreverse(
            "ietf.doc.views_material.edit_material",
            kwargs=dict(name=doc.name, action="state"),
        )
        login_testing_unauthorized(self, "secretary", url)

        # post
        r = self.client.post(
            url, dict(state=State.objects.get(type="slides", slug="deleted").pk)
        )
        self.assertEqual(r.status_code, 302)
    ...
    def test_edit_title(self, mock_slides_manager_cls):
        ...
        url = urlreverse(
            "ietf.doc.views_material.edit_material",
            kwargs=dict(name=doc.name, action="title"),
        )
        login_testing_unauthorized(self, "secretary", url)
        ...
        session = SessionPresentationFactory(
            session__group=doc.group, document=doc
        ).session

        # Grab the title on the slides when the API call was made (to be sure it's not before it was updated)
        titles_sent = []
        mock_slides_manager_cls.return_value.send_update.side_effect = (
            lambda sess: titles_sent.extend(
    ...
    def test_revise(self, mock_slides_manager_cls):
        ...
        url = urlreverse(
            "ietf.doc.views_material.edit_material",
            kwargs=dict(name=doc.name, action="revise"),
        )
        login_testing_unauthorized(self, "secretary", url)
        self.assertFalse(mock_slides_manager_cls.called)
