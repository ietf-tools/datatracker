...
class ConflictReviewTests(TestCase):
    def test_start_review_as_secretary(self):
        ...
        url = urlreverse(
            "ietf.doc.views_conflict_review.start_review", kwargs=dict(name=doc.name)
        )
        ...
        self.assertEqual(
            Document.objects.filter(
                name="conflict-review-imaginary-independent-submission"
            ).count(),
            0,
        )
        ...
        state_strpk = str(
            State.objects.get(used=True, slug="needshep", type__slug="conflrev").pk
        )
        r = self.client.post(
            url, dict(ad=ad_strpk, create_in_state=state_strpk, notify="ipu@ietf.org")
        )
        self.assertEqual(r.status_code, 302)
        review_doc = Document.objects.get(
            name="conflict-review-imaginary-independent-submission"
        )
        ...
        self.assertTrue(
            ...
            in [
                x.target
                for x in review_doc.relateddocument_set.filter(
        ...
        self.assertTrue(
            review_doc.latest_event(DocEvent, type="added_comment").desc.startswith(
                "IETF conflict review requested"
            )
        )
        self.assertTrue(
            doc.latest_event(DocEvent, type="added_comment").desc.startswith(
                "IETF conflict review initiated"
            )
        )
        ...
        r = self.client.post(
            ...
            dict(
                ad="Areað Irector",
                create_in_state="Needs Shepherd",
                notify="ipu@ietf.org",
            ),
        )
        self.assertEqual(r.status_code, 404)
    ...
    def test_change_state(self):
        ...
        url = urlreverse(
            "ietf.doc.views_conflict_review.change_state", kwargs=dict(name=doc.name)
        )
        ...
        adrev_pk = str(
            State.objects.get(used=True, slug="adrev", type__slug="conflrev").pk
        )
        ...
        iesgeval_pk = str(
            State.objects.get(used=True, slug="iesgeval", type__slug="conflrev").pk
        )
        ...
        appr_noprob_sent_pk = str(
            State.objects.get(
                used=True, slug="appr-noprob-sent", type__slug="conflrev"
            ).pk
        )
        r = self.client.post(
            url, dict(review_state=appr_noprob_sent_pk, comment="xyzzy")
        )
        self.assertEqual(r.status_code, 200)
        q = PyQuery(r.content)
    ...
    def test_edit_notices(self):
        doc = Document.objects.get(name="conflict-review-imaginary-irtf-submission")
        url = urlreverse(
            "ietf.doc.views_doc.edit_notify;conflict-review", kwargs=dict(name=doc.name)
        )
    ...
    def test_edit_ad(self):
        doc = Document.objects.get(name="conflict-review-imaginary-irtf-submission")
        url = urlreverse(
            "ietf.doc.views_conflict_review.edit_ad", kwargs=dict(name=doc.name)
        )
    ...
    def test_edit_telechat_date(self):
        doc = Document.objects.get(name="conflict-review-imaginary-irtf-submission")
        url = urlreverse(
            "ietf.doc.views_doc.telechat_date;conflict-review",
    ...
    def approve_test_helper(self, approve_type):
        ...
        create_ballot_if_not_open(
            None, doc, Person.objects.get(name="Sec Retary"), "conflrev"
        )
        doc.set_state(
            State.objects.get(used=True, slug=approve_type + "-pend", type="conflrev")
        )
        ...
        if approve_type == "appr-noprob":
            self.assertIn(
                "IESG has no problem", "".join(wrap(get_payload_text(outbox[0]), 2**16))
            )
        else:
            self.assertIn(
                "NOT be published", "".join(wrap(get_payload_text(outbox[0]), 2**16))
            )
    ...
    def approval_pend_notice_test_helper(self, approve_type, role):
        ...
        pending_pk = str(
            State.objects.get(
                used=True, slug=approve_type + "-pend", type__slug="conflrev"
            ).pk
        )
        r = self.client.post(
            url, dict(review_state=pending_pk, comment="some comment or other")
        )
...
class ConflictReviewSubmitTests(TestCase):
    ...
    def test_initial_submission(self):
        doc = Document.objects.get(name="conflict-review-imaginary-irtf-submission")
        url = urlreverse(
            "ietf.doc.views_conflict_review.submit", kwargs=dict(name=doc.name)
        )
        login_testing_unauthorized(self, "ad", url)
        ...
        q = PyQuery(r.content)
        self.assertTrue(
            q('textarea[name="content"]')[0].text.strip().startswith("[Edit this page")
        )
    ...
    def test_subsequent_submission(self):
        ...
        self.assertEqual(doc.rev, "00")
        path = os.path.join(
            settings.CONFLICT_REVIEW_PATH, "%s-%s.txt" % (doc.name, doc.rev)
        )
...
class ConflictReviewStreamStateTests(TestCase):
    ...
    def start_review(self, stream, role, kwargs=None):
        doc = (
            RgDraftFactory()
            if stream == "irtf"
            else IndividualDraftFactory(stream=StreamName.objects.get(slug="ise"))
        )
        url = urlreverse(
            "ietf.doc.views_conflict_review.start_review", kwargs=dict(name=doc.name)
        )
    ...
    def test_start_irtf_review_as_secretary(self):
        ...
        self.start_review(
            "irtf", "secretary", kwargs=dict(ad=ad_strpk, create_in_state=state_strpk)
        )
    ...
    def test_start_ise_review_as_secretary(self):
        ...
        self.start_review(
            "ise", "secretary", kwargs=dict(ad=ad_strpk, create_in_state=state_strpk)
        )
    ...
    def close_review(self, close_type, stream, role):
        ...
        url = urlreverse(
            "ietf.doc.views_conflict_review.change_state", kwargs=dict(name=review.name)
        )
        login_testing_unauthorized(self, role, url)
        strpk = str(
            State.objects.get(used=True, slug=close_type, type__slug="conflrev").pk
        )
        r = self.client.post(url, dict(review_state=strpk))
        self.assertEqual(r.status_code, 302)
        self.assertEqual(
            doc.get_state("draft-stream-" + stream).slug,
            "chair-w" if stream == "irtf" else "ise-rev",
        )
    ...
    def test_approve_review(self):
        ...
        review = ConflictReviewFactory(review_of=doc)
        review.set_state(
            State.objects.get(used=True, slug="appr-noprob-pend", type="conflrev")
        )

        url = urlreverse(
            "ietf.doc.views_conflict_review.approve_conflict_review",
            kwargs=dict(name=review.name),
        )
        login_testing_unauthorized(self, "secretary", url)
